...
\date{SBML Level 2, Version 1\\[5pt]\fbox{Working Draft Revision 4}\\[5pt]
  \today{}}
...
\emph{Cellerator}~\citep{shapiro:2000,shapiro:2001,shapiro:2003}, \emph{COPASI}~\citep{mendes:2000},
...
\emph{StochSim}~\citep{bray:2001,morton-firth:1998}, and \emph{Virtual
  Cell}~\citep{schaff:2000,schaff:2001}. 
SBML was developed with the help
...
\item A new field called \attrib{id} replaces the \attrib{name} field
  ...
  Section~\ref{sec:idnameattribs}.  The \attrib{name} field has become
  optional and has been redefined to allow any Unicode characters defined
  by the \class{string} type of XML Schema~\citep{biron:2000}.  The
  combination of \attrib{id} and \attrib{name} allows software tools to use
...
\item The namespace for identifiers in a model does not contain any
  ...
  \class{FunctionDefinition} mechanism.  SBML Level~2 uses a feature of
  MathML to allow models to refer to two built-in entities, a symbol
  representing time and a delay function which represents a variable's value
  at some point in the past.  However even these entities are not in the
  namespace of a model's identifiers.  See Section~\ref{sec:mathmltokens}.
...
\item All structures are derived from \class{SBase} including \class{Sbml} and
  \class{listOf}\rule{0.5in}{0.5pt} elements.
...
\item \emph{Event}: A statement describing a discrete instantaneous
  discontinuous transformation of a set of variables of any type
  (species concentration, compartment volume or parameter) when the
  model makes a defined transition from one state to another.
\end{description}
...
list of anticipated enhancements that will be made in SBML Level 3; a review
of other similar efforts; and discusses the availability of documentation and
a XML schema for this standard.
...
abstract type called \class{SBase}.  This inheritance hierarchy is depicted in
Figure~\vref{fig:top-level}.  The definition of \class{SBase} is presented
...
\attrib{metaid}, \attrib{notes} and \changed{\attrib{annotation}}.  The
field \attrib{notes} is a container for XHTML content.  It is intended for
recording optional user-visible information.  Every data object derived
directly or indirectly from type \class{SBase} can have a separate value
for \attrib{notes}, allowing users considerable freedom for annotating
their models.  The second field, \attrib{annotation}, is provided for
software-generated annotations.  It is a container for arbitrary data (XML
type \class{any}) and is intended to store information not meant for human
viewing.  As with the user-visible \attrib{notes} field, every data object
can have its own \attrib{annotation} value.

Metadata expressed using RDF can be placed anywhere within an \class{sbml}
element and its subelements, \emph{except} within MathML elements.  To
support RDF, \class{SBase} has an optional field named \attrib{metaid} of
type \class{ID} (see Figure~\vref{fig:sbase}).  RDF \class{description}
elements can be created in which the RDF \attrib{describes} attributes
contain the values of the \attrib{metaid} fields of SBML elements.  The
...
hiding the attributes derived from a parent type such as \class{SBase}. It
...
\begin{table}[t]
  ...
  \begin{tabular}{ll}
    \toprule
    \texttt{http://www.sbml.org/2001/ns/biocharon}  & \texttt{http://www.sbml.org/2001/ns/jarnac}\\
    \texttt{http://www.sbml.org/2001/ns/bioreactor} & \texttt{http://www.sbml.org/2001/ns/jdesigner}\\
    \texttt{http://www.sbml.org/2001/ns/biosketchpad}   & \texttt{http://www.sbml.org/2001/ns/jigcell}\\
    ...
    \texttt{http://www.sbml.org/2001/ns/isys}\\
    \bottomrule
  \end{tabular}
...
characters~\citep{biron:2000}.  The purpose of the \attrib{name} field is
...
composition.  Further, the use of \class{ID} type for SBML identifiers
would have limited utility because MathML \class{ci} elements are not of
the type \class{IDREF} (see Section~\ref{sec:formulas}).
...
function definition has a single MathML \class{lambda} subelement as well.
...
the component identifier namespace.  The \attrib{encoding} field of
...
In SBML Level 2, there are 2 URLs in the set of predefined SBML symbols:
\begin{itemize}

\item \texttt{http://www.sbml.org/symbols/time}, which represents the current
simulation time.  The units of the current time entity is
determined from the built-in \texttt{time} of Table~\vref{tab:builtin}.

\item \texttt{http://www.sbml.org/symbols/delay}, for
representing a delay function.  
The delay function has the form $delay(x, d)$, taking two
arguments.  Its value is the value of argument $x$ at $d$ time units
before the current time.  The units of the $d$ parameter are
determined from the built-in \texttt{time}.  The \texttt{delay} function is
useful for representing biological processes having a delayed response, but
where the detail of the processes and delay mechanism is not relevant to the
...
Section~\ref{sec:delayeg} contains a complete model which uses the built-in delay function.
...
An SBML Level 2 model definition in XML consists of a single \class{Sbml}
element enclosing a single \class{Model} element (see next Section).  The
structure of the \class{Sbml} element is shown in Figure~\ref{fig:sbml}.

\begin{figure}[htb]
  \centering
  \includegraphics{sbml}
  \caption{The definition of \class{Sbml}.  Additional fields are
    inherited from \class{SBase} but are not shown here.}
  \label{fig:sbml}
\end{figure}

The namespace URI for SBML Level~2 is
\texttt{http://www.sbml.org/sbml/level2}.  The SBML element has two
attributes: \attrib{version} and \attrib{level}.  For the SBML described in
this document these attributes should be set to ``\texttt{1}'' and
...
The following is an example of the outermost content of an SBML model
definition in XML:
...
\class{Parameter}, \class{Reaction}, and \class{Rule} components.  All of
...
\attrib{name} and \attrib{lambda}.  The \attrib{id} and \attrib{name}
fields operate in the manner described in section~\ref{sec:idnameattribs}.
\attrib{id} is a field of type \class{SId} and \attrib{name} is an optional
string field.  MathML elements can refer to the function contained in a
\class{FunctionDefinition} using the value of its \attrib{id} field.  The
\attrib{lambda} field is a container for MathML content that defines the
...
The following is an example \class{FunctionDefinition} structure defining
...
\begin{figure}[htb]
  \vspace*{12pt}                        % ADJUSTMENT FOR SPECIFIC PAGE
  \centering
...
default value for the \attrib{constant} field is \attribvalue{true} because
in typical modeling scenarios, compartment volumes do not change.  When
\attrib{constant} field id is \attribvalue{false} then \attrib{volume}
contains the initial volume of the compartment.
...
implies that the value is either either unknown, not required for analysis
...
\attrib{constant} which indicates whether the concentration of the species
...
section~\ref{sec:constantspecieseg} contains all4 possible states of the
...
rules (see Section~\ref{sec:rules}) and that the \attrib{value} contains
...
is to define an abstract \class{Rule} structure that contains only one
field, \attrib{math}, to hold the right-hand side expression, then to
derive subtypes of \class{Rule} that add fields to cover the various cases
above.  Figure~\vref{fig:rules} gives the definitions of \class{Rule} and
the subtypes derived from it.  The figure shows there are two subtypes,
\class{AlgebraicRule} and \class{AssignmentRule}, defined directly from
\class{Rule}.

\begin{figure}[htb]
  \centering
  \includegraphics[scale = 0.68]{new-rules}
...
\item \emph{In the case of a species}, the effect of the rule depends on the
  value of \attrib{type}: if the value is ``\class{scalar}'', the rule sets
  the referenced species's concentration to the value determined by the
  formula in \attrib{math}; if the value is ``\class{rate}'', the rule sets
  the rate of change of the species's concentration to the value determined
  by the formula.  The units are in terms of $substance/volume$, where the
  $substance$ units are those that are declared on the referenced
  \class{Species} element, and the $volume$ units are those declared on the
  \class{compartment} element that contains the \class{Species}.
  
  A rule and a \class{SpeciesReference} structure (see
...
\item \emph{In the case of a compartment}, the effect of the rule depends
  on the value of \attrib{type}: if the type is ``\class{scalar}'', the
  rule sets the referenced compartment's volume to the volume determined by
  the formula in \attrib{math}; if the type is ``\class{rate}'', the rule
  sets the rate of change of the compartment's volume to the volume
  determined by the formula.  No more than one
  \class{AssignmentRule} can refer to a given compartment in an SBML
  model definition.
  
\item \emph{In the case of a parameter}, the effect of this \class{AssignmentRule} depends on
  the value of the \attrib{type} field: if the
  type is ``\texttt{scalar}'', the rule sets the referenced parameter's
  value to that determined by the formula in \attrib{math}; if the type is
  ``\texttt{rate}'', the rule sets the rate of change of the parameter's
  value to that determined by the formula.  No more than one
  \class{AssignmentRule} can refer to a given parameter.
...
\attrib{product}, \attrib{modifier} and \attrib{kineticLaw} in the
\class{Reaction} structure.  The fields \attrib{reactant} and
\attrib{product} are both references to species implemented using lists of
\class{SpeciesReference} structures (defined in
Section~\ref{subsec:speciesreference} below).  The \class{modifier} field
is also a list of references to species, but implemented using
\class{ModifierSpeciesReference} structures (defined in
Section~\ref{subsec:modifierreference}).  
All species involved in a reaction including modifiers must be listed in the
\attrib{reactant}, \attrib{product} and \attrib{modifier} fields.
The \attrib{kineticLaw} is an
...
In addition to these, the \class{Reaction} structure also has two optional
boolean fields.  The first field, \attrib{reversible}, indicates whether
the reaction is reversible.  The field is optional, and if left unspecified
in a model, it defaults to a value of ``\attribvalue{true}''.  Although the
...
places (i.e., the rate law and the flag \attrib{reversible}) leaves open
...
All species in a model are listed once, in the the \attrib{listOfSpecies}
field of the top-level \class{Model} data structure (see
Section~\ref{sec:model}).  Lists of products, reactants and modifiers in
\class{Reaction} type structures do not introduce new species, but rather
refer to those listed in the \class{Model}.  The connection between the species listed
in the model and the reactions in which they are involved is established using
\class{SimpleSpeciesReference} structures.

\class{SimpleSpeciesReference} has only one field, \attrib{species}, of type \class{SId}; its value must
be the identifier of a species defined in the enclosing \class{Model}.
The \class{SimpleSpecieReference} structure is the base definition for the
\class{SpeciesReference} and \class{ModifierSpeciesReference} structures.

\subsubsection{\class{SpeciesReference}}
\label{subsec:speciesreference}

The reactants and products of a reaction are defined using a \class{SpeciesReference} type data
structure defined in Figure~\vref{fig:reaction} which is derived from \class{SimpleSpeciesReference}.
Note that whether a species is allowed to
appear as a reactant or product or not is dictated by certain flags on the
structure describing the species in the \class{Model}; see
Table~\ref{tab:specieattrib} for more information.

Stoichiometric numbers for the products and reactants can be specified
using two optional attributes on the \attrib{speciesReference} element:
\attrib{stoichiometry} and \attrib{denominator}.  Both attributes take
positive integers as values, and both have default values of ``\class{1}''
(one).  The absolute value of the stoichiometric number is the value of
\attrib{stoichiometry} divided by \attrib{denominator}, and the sign is
...
The following example shows how a stoichiometry of a half is expressed:
\begin{example}
<speciesReference species="X1" stoichiometry="1" denominator="2"/>
\end{example}
...
is derived from \class{SimpleSpeciesReference}.
...
The field \attrib{math} of \class{KineticLaw} is a MathML element and contains an expression that
...
The only species
identifiers that can be used in \attrib{math} are those in the \texttt{species} fields contained in the
\class{Reactant}, \class{Product} and \class{Modifier} fields of the
\class{Reaction} structure. In the XML encoding this means that any
species identifier occurring within the MathML \texttt{math} element must also
occur as a \texttt{species} attribute value of a \texttt{speciesReference} or
\texttt{modifierSpeciesReference} element
contained within the same \texttt{reaction} element.
...
A complete example model that uses events is shown in Section~\ref{sec:eventeg}
...
by a kinetic law therefore they are both on the boundary of the
reaction system and in SBML have \attrib{boundaryCondition} 
attributes of \texttt{true}. The value of $S_1$ varies over time
so in SBML $S_1$ has a \attrib{constant} attribute default value
of \texttt{false}. The values of $S_2$ and $S_3$ are fixed so in
SBML they have a \attrib{constant} attribute values of 
\texttt{true}. $S_3$ is only occurs as a modifier so the value of
...
therefore in the SBML representation has false values for both
\attrib{boundaryCondition} and \attrib{constant} attribute values.
