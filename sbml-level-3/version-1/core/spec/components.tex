...
that are affected by the event, how the variables are
affected, and its relationship to other events.  The effect of the event can optionally be delayed after
...
\Trigger, \Delay, \Priority, and \EventAssignment are derived from \SBase{}
...
event can include an optional delay and an optional priority.  These features of \Event are
described below.
...
Sometimes in the interval between when
the event \emph{fires} and the time at which assignments are
\emph{executed}, the model has changed sufficiently that the \Event should no longer \emph{execute}.  This is particularly true if the modeler is using optional \Delay within \Event, but this interval may be significant even in models without \Delay if two or more Events are scheduled to be \emph{executed} simultaneously.

The \token{persistent} boolean attribute on the \class{Trigger} distinguishes between two types of triggers:  those which, once \emph{fired}, must \emph{execute} (\token{persistent}=false), and those whose \class{Trigger}, once \emph{fired}, must remain continually true until the time comes to \emph{execute} the Event, or the Event is not \emph{executed} at all (\token{persistent}=true).

For delayed events, the interpretation of \token{persistent}=true \class{Trigger}s is straightforward:  The condition defined in the MathML \token{math} element of the \class{Trigger} must switch to `true' when the event \emph{fires}, and then must remain true for the time interval between \emph{firing} and \emph{execution}.  A more subtle condition arises when two or more Events are scheduled to \emph{execute} simultaneously (whether or not those Events had a \Delay).  In this case, the \emph{execution} of a first Event may change the \class{Trigger} state of a second \Event in the queue from true to false.  If this happens, and the second \Event is set `\token{persistent}=true', that \Event is taken off the queue, and does not \emph{execute}.

\paragraph{The \token{initialValue} attribute on \class{Trigger}}
\label{sec:trigger-initialvalue}

Events \emph{fire} when their value changes from false to true.  In order to determine whether an event may fire at the the initial simulation time, \ie $t = 0$, it is necessary to know what value the \class{Trigger} had before the simulation began.  This is defined by this boolean attribute.  Triggers with `\token{initialValue}=false' may fire at $t = 0$ should their condition evaluate to true at the beginning of the simulation.  Triggers with `\token{initialValue}=true' may not fire at $t = 0$, as it would be impossible to transition from false to true at that moment in time.  It probably need not be said that Events may not fire before the simulation begins, or $t < 0$.

In SBML Level 2, only the latter case was (essentially) allowed, so all SBML Level 2 models can be read as defining `\token{initialValue}=true' for all \Event \class{Trigger}s.
...
\quantity{time} as set by the \Model attribute \val{timeUnits}
...
As shown in Figure~\ref{fig:event}, an \Event object can contain
an optional \Priority object.  The \Priority class, like \Delay, is derived from
\SBase and contains a mathematical formula stored in \token{math}.
The formula is used to compute a value that defines exactly how the simulator is to behave when two Events are \emph{executed} simultaneously.
If no Priority is present on a given \Event, no priority is defined for that event.

`Simultaneous event execution' is defined as a situation where multiple events are to be executed `at the same time':  the sum of the time at which the \class{Trigger} was \emph{fired} plus the \Delay for two Events are equal.  (Note that as far as the specification of the model is concerned, 'equal' means 'mathematically equal'.  Simulators attempting to adhere to this specification may have to rely on numerical equality instead of mathematical equality; robust models will ensure that this discrepancy will not cause significant diversions from expected behavior.)  In this case, if no \Priority objects are defined, the order in which the two events are to fire is undefined:  a given simulator may choose any algorithm it wishes to define for itself the order in which to \emph{execute} the Events, as long as any and all such simultaneous Events are indeed \emph{executed} exactly once, checking to see if other event \class{Trigger}s are enabled or disabled after each \Event's \emph{execution}.

If instead a \Priority is defined for two or more simultaneously-firing events, the order in whice those Events must fire is defined by these Priorities:  if the calculated values for two Priorities are different, the \Event with the higher \Priority must \emph{execute} before the \Event with the lower \Priority.  If the two Priorities are equal, the two Events must fire in a randomly-determined order.  Note that a randomly-determined order is not the same as an undefined order:  an `undefined order' algorithm may place Event A before Event B every time those two Events \emph{execute} simultaneously, or it ay always place it second, or it may be random.  A randomly-determined order must have an equal chance of \emph{executing} Event A first or Event B first, every time those two Events \emph{execute} simultaneously.

Should a combination of simultaneous Events arise in which some have defined Priorities and others do not, the ones with defined Priorities must fire in \Priority order, and the ones without defined Priorities must fire as normal, in an undefined order with respect to each other and with respect to those events with defined Priorities.  (Again, `undefined order' does not mean `random', but a random order is one valid implementation of the `undefined order' algorithm.)

It follows that if there are four simultaneously-\emph{executing} Events, two with one \Priority and two with a different \Priority, the two events with the higher \Priority must fire first, in a random order with respect to each other, and the remaining two events must fire next, again in a random order, for a total of four possible and equally-likely \Event \emph{executions}: A-B-C-D, A-B-D-C, B-A-C-D, and B-A-D-C.  If these four Events had all had the same \Priority, there would be $4!$ or 24 possible orderings, each of which must be equally likely.  If none of those four Events had had a defined \Priority, or even if exactly one of those four Events had had a defined \Priority, there would still be 24 possible orderings, but likelihood of choosing any particular order would be undefined: the simulator would choose between them as it wished.  (Since Priorities only matter with respect to other Events with Priorities, a single defined \Priority can not change the defined order of a set of simultaneous Events.)

The expression in the \Priority object's \token{math} element must be evaluated at the time the \Event is to be \emph{executed}, but before any other \Event is \emph{executed} at the same moment in time in the simulation.  This is so that the Priorities of these Events may be usefully compared with one another, and the Events ordered appropriately.  If one \Event's \emph{execution} causes another \Event with a defined \Priority and no \Delay to \emph{fire}, its \Priority must be calculated before the \emph{execution} of any other \Event currently queued to be \emph{executed} at this time, and inserted into that list appropriately.  Simulators should note that it is possible for one \Event's \emph{execution} to cause the \Priority of a different simultaneous \Event to change.  As such, the simplest algorithm to comply with this specification may be to \emph{execute} the highest-priority \Event, check to see if new Events should be added or removed from the list, then re-calculate all pending \Event Priorities.  The expression may evaluate to a positive or negative number, with positive numbers having a higher priority than negative numbers.

Further discussion can be found in Section~\ref{sec:events-semantics}.

\paragraph{Units of priority expressions}

The unit associated with the value of a \Priority object's
\token{math} expression must be \token{dimensionless} (if defined), as the expression only has meaning with respect to other Priority expressions, and not with respect to any other element of the model.


\paragraph{The optional \token{sboTerm} attribute on \class{Priority}}
\label{sec:priority-sboterm}

\Priority  inherits an optional \token{sboTerm}
attribute of type \primtype{SBOTerm} from its parent
class \SBase (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this
attribute in a  \Priority instance, it should be an
SBO identifier belonging to the branch for type  \Priority
indicated in Table~\ref{tab:sboterm-availability}.  The relationship is
of the form ``the priority \emph{is-a} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
one that captures the role of the priority in the model.
...
simulation may \emph{fire} at the start of the simulation ($t =
0$) if their \token{Trigger}'s \token{initialValue} attribute is set to be `false'.  Events \emph{fire} only when the trigger \emph{becomes}
...
\item A software package should retest all event triggers after
  ...
  transition from \val{false} to \val{true}.  This check should be made after each individual \Event's \emph{execution}, even when several Events are \emph{executed} simultaneously.

\item Similarly, a software package should retest all event triggers for Events that have \emph{fired} but have not \emph{executed}, and which are set `\token{persistent}=true', to see if they should be removed from the list of Events pending \emph{execution}.  If this happens, the \Event is immedately removed from the `pending events' queue, and thereafter behaves like any other \Event whose \Trigger is `false' (and may be re-added to the queue should some other \Event \emph{fire} it again.  (see Section~\ref{sec:trigger-persistent})
...
\item It is assumed that, although the precise time at which these
  ...
  events that are pending \emph{execution}.  Events such as $Y$ 
  may have a special priority if the \Priority object is defined.  Events $X$ and $Y$
...
\item Events with no \Priority are \emph{executed} in an undefined order.  This does not mean that the behavior of the simulation is completely undefined; merely that the \emph{order} is undefined.  A given simulator may choose any algorithm it wishes to determine that order, from some form of a deterministic algorithm to some form of a random algorithm, as long as every \Event in the queue is \emph{exectuted} exactly once (see Section~\ref{sec:event-priority}).


\item Events with a \Priority are \emph{executed} in \Priority order, with those Events with the highest \Priority \emph{executing} first, and those Events with identical Priorities \emph{executing} in a random order with respect to one another (as determined at run-time by some random algorithm akin to a coin flip).  As such, newly-\emph{fired} Events that are to \emph{execute} immediately when there are already pending Events to be \emph{executed} at the current time should be inserted into the list according to their \Priority:  Events with a defined \Priority must be inserted before pending Events with a lower \Priority, after pending Events with a higher \Priority, and inserted randomly among pending Events with the same \Priority.  Events with no \Priority must be inserted into the list in some fashion, but the algorithm used to place it in the list is up to the simulator.  Similarly, there is no restriction on the order of a newly-inserted \Event with a defined \Priority with respect to any other pending \Event with no defined \Priority.  (see Section~\ref{sec:event-priority})

\item Any \Event that has \emph{executed} is part of the history of the model, and not part of the current state of the model.  If the \emph{execution} of an \Event with a \Priority of 3 causes a different \Event with a \Priority of 10 to \emph{fire} and immedately \emph{execute}, it is not a problem that a `Priority 3' \Event \emph{executed} before a `Priority 10' Event.
