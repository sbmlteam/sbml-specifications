...
Section~3.1 of the \sbmlthreecore specification defines a number of primitive data types and also uses a number of XML Schema 1.0 data types~\citep{biron:2000}. More specifically we make use of \primtype{integer}, \primtype{double}, \primtype{string}, \primtype{SId} and \primtype{SIdRef}. In addition we make use of two new primitives: the enumerations \primtype{FbcType} and \primtype{FbcOperation}, see \ref{fig:fbc_uml} for the interrelation between these entities.

The \primtype{SId} type is used as the data type for the identifiers of \FluxBound (\ref{fluxbound-class}), \FluxObjective (\ref{fluxobjective-class}) and \Objective (\ref{objective-class}) classes. In the FBC package the \ListOfObjectives has an attribute of type \primtype{SIdRef} that is used to refer to an `active' \Objective.
...
The \SBML \Model class is extended by adding an optional \token{listOfFluxBounds} as well as a \token{listOfObjectives}.  A \Model may contain at most one of each of these lists.
...
is obviously inconsistent since it defines \token{R >= 3} and \token{R = 2}, which cannot be correct. However

\exampleFile{examples/ex_fb_fbc2.txt}

would be fine since, whilst the information is essentially repeated, \token{R >= 3} and \token{R = 3} produce the consistent result that \token{R = 3}.
