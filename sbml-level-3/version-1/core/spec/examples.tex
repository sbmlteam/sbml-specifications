...
this model, the global values for \token{substance}, \token{volume}
and \token{time} are declared as \val{substance}, \val{litre} and
\val{second}. The units of \token{substance} are declared to 
...
\begin{linenomath}\vspace*{-0.5ex}
\begin{reaction}\centering\notag
  E + S \eqbm^{\cit k1\sub{on}}_{\cit k1\sub{off}} ES \yields^{\cit k2} E + P
\end{reaction}
\end{linenomath}\vspace*{-1ex}%
