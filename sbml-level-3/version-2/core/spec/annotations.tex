...
elements described in \sec{sec:annotation-use}.  We refer
readers to \sec*{sec:annotation-use} for important
...
(\sec{sec:sbase-sboterm}).  However in certain situations, for
...
\relchange{Annotations only add additional qualifying information and never change existing information.  They can be ignored without changing the (broader) meaning of the model.  The same is true of nested annotations (described below), which qualify their parent annotation but never change the meaning of that annotation.}


\subsection{XML namespaces in the standard annotation}

This format uses a restricted form of Dublin
Core~\citep{DCMI:2003} and BioModels.net qualifier elements \relchange{(see
\url{http://co.mbine.org/standards/qualifiers})} embedded in the XML form
of RDF~\citep{w3c:2004}.  The scheme defined here uses a number of
external XML standards and associated XML namespaces.
\tab*{tab:namespaces-for-standard-annotation} lists these
...
\begin{table}[bh]
  ...
  \begin{edtable}{tabular}{lll}
    ...
    \token{\relchange{dcterms}} & \uri{http://purl.org/dc/terms/} & \cite{kokkelink:2002},\\
                    &                                 & \cite{DCMIUB:2005} \\[1pt]
    \reltwo{\token{vcard}   & \uri{http://www.w3.org/2001/vcard-rdf/3.0\#} & \cite{iannella:2001} \\[1pt]}
    \token{\relchange{vcard4}}  & \relchange{\uri{http://www.w3.org/2006/vcard/ns\#}} & \relchange{\cite{perreault:2011}} \\[1pt]
    \token{bqbiol}  & \uri{http://biomodels.net/biology-qualifiers/} & \url{http://co.mbine.org/standards/qualifiers} \\[1pt]
    \token{bqmodel} & \uri{http://biomodels.net/model-qualifiers/} & \url{http://co.mbine.org/standards/qualifiers} \\[1pt]
    \bottomrule
  \end{edtable}
  \vspace*{-0.95ex}
...
\relverdiff{\changed{Note that while \token{vCard} 3 is defined above, its use is discouraged in favor of \token{vCard} 4.  \token{vCard} 3 will be deprecated in future versions of SBML.}}{Note that \token{vCard} 3 has been deprecated in favor of \token{vCard} 4.  Models using \token{vCard} 3 will continue to be legal, but tools may issue a warning that \token{vCard} 4 is preferred.  Future versions of SBML will drop \token{vCard} 3 as part of the officially supported annotation `standard format'.}
...
order as described in \sec{sec:annotation-use}.  The
...
<\sbmlElement \otherContentTall metaid="\sbmlMetaNS" \otherContentTall>
  ...
  <annotation>
    ...
    <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns\#"
             \relchange{xmlns:dcterms="http://purl.org/dc/terms/"}
             \relchange{xmlns:vcard4="http://www.w3.org/2006/vcard/ns\#"}
...
(see \sec*{sec:qualified-dc-annotation}); \resourceURI is a
URI identifying a resource (see
\sec*{sec:uri-in-annotation}); and \history refers to
optional content described in
\sec*{sec:model-history-annotation}.
...
history section (see \sec{sec:model-history-annotation})
...
or resource.  Although \sec{sec:qualified-dc-annotation}
...
contain one or more \token{rdf:li} elements, \relchange{and may additionally contain nested content that provides additional annotations about the contents of the \token{rdf:Bag}}.
The \token{rdf:li}
elements must only have a \token{rdf:resource} attribute
containing a URI referring to an information resource (see
\sec{sec:uri-in-annotation}).

\relchange{Note that the various namespaces (\token{xmlns:rdf}, \token{xmlns:dcterms}, etc.) may be declared in any order, and that only the namespaces that are actually used need be declared.  If no \token{vcard4} terms are used in a particular annotation, for example, the line \token{xmlns:vcard4="http://www.w3.org/2006/vcard/ns\#"} is optional.}
...
\sec{sec:qualified-dc-annotation}) simply define the
...
this might correspond to the URI \relchange{\uri{http://identifiers.org/uniprot/P12999}} \citep{juty:2012}.
...
identifier \relchange{\uri{http://identifiers.org/uniprot/P12999}}, thereby identifying
...
\relchange{\uri{http://identifiers.org/go/GO:0007268}} can be translated into any
of the following:
...
Similarly, \relchange{\uri{http://identifiers.org/ec-code/3.5.4.4}} can refer to
...
at the address \relchange{\url{http://www.ebi.ac.uk/miriam/main/collections}.  Each
...
\begin{table}[b]
  ...
  \begin{edtable}{tabular}{lp{4.84in}}
    ...
    \midrule
    \relchange{\token{bqmodel:hasInstance}}
    & \relchange{The subject of the referenced resource is 
    ...
    \\[6pt]
    \relchange{\token{bqmodel:isInstanceOf}}
    & \relchange{The modeling object represented by the model element is an
    ...
    the \relchange{ChEBI} or Reactome databases.
    \\[6pt]
    ...
    & The biological entity represented by
    the \relchange{SBML} component takes place in the subject of the reference
    resource.
    \\[6pt]
    \token{\relchange{bqbiol:hasTaxon}}
    & \relchange{The biological entity represented by the SBML component is
...
\sec{sec:general-syntax} can include additional elements
...
\otherContent
<vCard4:fn>
  <vCard4:text>\fullNameNS</vCard4:text>
</vCard4:fn>
\otherContent
[<vCard4:hasEmail>
  <vCard4:text>\emailAddressNS</vCard4:text>
</vCard4:hasEmail>]
\otherContent
[<vCard4:organization-name>
  <vCard4:text>\orgNameNS</vCard4:text>
</vCard4:organization-name>]
\otherContent
...
of the format contained in the light gray box \relchange{(\token{vCard4:fn} or \token{vCard4:hasName}, plus \token{vCard4:hasEmail}, and \token{vCard4:organization-name})} can occur in any
order.  The elements of the format contained between \texttt{[}
and \texttt{]} \relchange{(\token{vCard4:organization-name}, and \token{vCard4:hasEmail})} are optional, \relchange{but everything else is required}.  The precise form of the whitespace,
and the specific XML namespace prefixes used \relchange{(\val{dcterms}, \val{rdf}, and \val{vcard4})} are not constrained.

The \token{\relchange{dcterms}:creator} element describes the person(s) who created
...
those elements can be the following: \reltwo{\token{vcard:N},
\token{vcard:EMAIL} and \token{vcard:ORG} (if using \token{vCard} version 3), or} \token{vCard4:fn} or \token{vCard4:hasName}, plus \token{vCard4:hasEmail}, and \token{vCard4:organization-name}\reltwo{ (if using \token{vCard} version 4)}.
The \token{dcterms:created}, and \token{dcterms:modified} elements
must have the attribute \token{rdf:parseType} set to the literal
value \val{Resource}.

\begin{blockRelChange}
Note that \token{dcterms:creator} has been added to \token{http://purl.org/dc/terms/} relatively recently, but the same term (with the same meaning) once only lived in the \token{http://purl.org/dc/elements/1.1/} namespace.  It is legal to continue using the old namespace (called \val{dc} in previous versions of the SBML specifications), but as all the terms once defined there are now also defined in \token{http://purl.org/dc/terms/}, we now recommend using the latter.
\end{blockRelChange}
...
a person who created the model; \relchange{when using \token{vCard4}, \fullName stands for the full name of that person.}  \emailAddress is the email address
...
shown in \sec{sec:general-syntax}, `\otherContent' is
...
\sec{sec:annotation-examples} below provides an example of
...
<model metaid="_180340" id="GMO" name="Goldbeter1991_MinMitOscil">
  <annotation>
    <rdf:RDF
            ...
            \relchange{xmlns:dcterms="http://purl.org/dc/terms/"}
            \relchange{xmlns:vcard4="http://www.w3.org/2001/vcard-rdf/3.0\#"}
            xmlns:bqbiol="http://biomodels.net/biology-qualifiers/"
            xmlns:bqmodel="http://biomodels.net/model-qualifiers/"
    >
      <rdf:Description rdf:about="#_180340">
        <\relchange{dcterms}:creator>
          <rdf:Bag>
            <rdf:li rdf:parseType="Resource">\begin{blockRelChange}
              <vcard4:fn><vcard4:text>Bruce Shapiro</vcard4:text></vcard4:fn>
              <vcard4:email><vcard4:text>bshapiro@jpl.nasa.gov</vcard4:text></vcard4:email>
              <vcard4:organization-name>
                  <vcard4:text>NASA Jet Propulsion Laboratory</vcard4:text>
              </vcard4:organization-name>\end{blockRelChange}
            </rdf:li>
          </rdf:Bag>
        </\relchange{dcterms}:creator>
        ...
        <bqmodel:is>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/biomodels.db/BIOMD0000000003}\!"/>
          </rdf:Bag>
        </bqmodel:is>
        <bqmodel:isDescribedBy>
           <rdf:Bag>
             <rdf:li rdf:resource="\relchange{http://identifiers.org/pubmed/1833774}\!"/>
           </rdf:Bag>
        </bqmodel:isDescribedBy>
        <bqbiol:isVersionOf>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/wikipathways/WP179}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/reactome/REACT_152}\!"/>
          </rdf:Bag>
        </bqbiol:isVersionOf>
      </rdf:Description>
    </rdf:RDF>
...
\begin{example}
<reaction id="cdc2Phospho" metaid="jb007" reversible="true"\reltwo{ fast="false"}>
  <annotation>
    <rdf:RDF
      xmlns:bqbiol="http://biomodels.net/biology-qualifiers/"
      xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns\#"
    >
      <rdf:Description rdf:about="#jb007">
        <bqbiol:is>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/reactome/REACT_6327}\!"/>
          </rdf:Bag>
        </bqbiol:is>
      </rdf:Description>
    </rdf:RDF>
  ...
  <annotation>
    ...
    >
      <rdf:Description rdf:about="\#cacam">
        <bqbiol:hasPart>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/uniprot/P62158}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/chebi/CHEBI:29108}\!"/>
          </rdf:Bag>
        </bqbiol:hasPart>
      </rdf:Description>
    </rdf:RDF>
  ...
  <annotation>
    ...
    >
      <rdf:Description rdf:about="\#cacam">
        <bqbiol:hasVersion>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/uniprot/Q9UQM7}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/uniprot/Q13554}\!"/>
          </rdf:Bag>
        </bqbiol:hasVersion>
      </rdf:Description>
    </rdf:RDF>
...
<reaction id="NMDAR_phosphorylation" metaid="thx1138"
          reversible="true"\reltwo{ fast="false"}>
  <annotation>
    <rdf:RDF
      xmlns:bqbiol="http://biomodels.net/biology-qualifiers/"
      xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns\#"
    >
      <rdf:Description rdf:about="#thx1138">
        <bqbiol:isVersionOf>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/ec-code/2.7.1.17}\!"/>
          </rdf:Bag>
        </bqbiol:isVersionOf>
      </rdf:Description>
    </rdf:RDF>
  ...
  <annotation>
    ...
    >
      <rdf:Description rdf:about="\#C8H10N4O2">
        <bqbiol:isVersionOf>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/go/GO:0005954}\!"/>
          </rdf:Bag>
        </bqbiol:isVersionOf>
      </rdf:Description>
    </rdf:RDF>
  ...
  <annotation>
    ...
    >
      <rdf:Description rdf:about="\#GXP">
        <bqbiol:hasVersion>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/chebi/CHEBI:17345}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/chebi/CHEBI:17552}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/chebi/CHEBI:17627}\!"/>
          </rdf:Bag>
        </bqbiol:hasVersion>
        <bqbiol:hasVersion>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/kegg.compound/C00035}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/kegg.compound/C00044}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/kegg.compound/C00144}\!"/>
          </rdf:Bag>
        </bqbiol:hasVersion>
      </rdf:Description>
    </rdf:RDF>
...
\begin{example}
<reaction id="adenineProd" metaid="adeprod" reversible="true"\reltwo{ fast="false"}>
  <annotation>
    <rdf:RDF
      xmlns:bqbiol="http://biomodels.net/biology-qualifiers/"
      xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns\#"
    >
      <rdf:Description rdf:about="\#adeprod">
        <bqbiol:hasPart>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/ec-code/2.5.1.22}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/ec-code/3.2.2.16}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/ec-code/4.1.1.50}\!"/>
          </rdf:Bag>
        </bqbiol:hasPart>
        <bqbiol:hasPart>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/kegg.reaction/R00178}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/kegg.reaction/R01401}\!"/>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/kegg.reaction/R02869}\!"/>
          </rdf:Bag>
        </bqbiol:hasPart>
      </rdf:Description>
    </rdf:RDF>
  ...
  <annotation>
    ...
    >
     <rdf:Description rdf:about="\#heme">
       <bqbiol:hasPart>
         <rdf:Bag>
           <rdf:li rdf:resource="\relchange{http://identifiers.org/uniprot/P69905}\!"/>
           <rdf:li rdf:resource="\relchange{http://identifiers.org/uniprot/P68871}\!"/>
           <rdf:li rdf:resource="\relchange{http://identifiers.org/chebi/CHEBI:17627}\!"/>
         </rdf:Bag>
       </bqbiol:hasPart>
       <bqbiol:hasPart>
         <rdf:Bag>
          <rdf:li rdf:resource="\relchange{http://identifiers.org/uniprot/P69905}\!"/>
           <rdf:li rdf:resource="\relchange{http://identifiers.org/uniprot/P68871}\!"/>
           <rdf:li rdf:resource="\relchange{http://identifiers.org/kegg.compound/C00032}\!"/>
         </rdf:Bag>
       </bqbiol:hasPart>
     </rdf:Description>
   </rdf:RDF>
  ...
  <annotation>
    ...
    >
      <rdf:Description rdf:about="\#phosphorylation">
        <bqbiol:is>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/kegg.reaction/R03313}\!"/>
          </rdf:Bag>
        </bqbiol:is>
        <bqbiol:isVersionOf>
          <rdf:Bag>
            <rdf:li rdf:resource="\relchange{http://identifiers.org/go/GO:0016310}\!"/>
          </rdf:Bag>
        </bqbiol:isVersionOf>
      </rdf:Description>
    </rdf:RDF>
