...
Section~\ref{sec:notation-uml} for defining clasess of objects.
...
\ListOfEvents.  These ``list'' classes are also defined in
...
\token{math}.  Ascertaining these units requires performing
...
called \token{offset}.  This \changed{attribute} has been removed entirely from
...
\begin{table}[h]
  ...
  \begin{edtable}{tabular}{cllll}
    \toprule
    \textbf{Value of \changed{attribute}}    & \textbf{\Changed{Attribute}} \texttt{size} & \textbf{\Changed{Attribute}} \texttt{units} &  & \textbf{Default value}\\
...
The units of the value in the \token{initialAmount} \changed{attribute} are set
by the \token{substanceUnits} \changed{attribute} of the species structure.  The
units of the value in the \token{initialConcentration} \changed{attribute} are
...
given in the \token{spatialSizeUnits} \changed{attribute} as described in
...
\changed{attributes} \token{substanceUnits}, \token{spatialSizeUnits} and
...
\token{substanceUnits} \changed{attribute}, and the \quantity{size} units are
those defined by the \token{spatialSizeUnits} \changed{attribute} of a \Species
instance.
...
units derived from these. The \token{substanceUnits} \changed{attribute}
defaults to the the built-in unit ``\token{substance}'' shown in
Table~\vref{tab:builtin}.

The type of units assigned to the \token{spatialSizeUnits} \changed{attribute}
...
\token{spatialSizeUnits} \changed{attribute} must not have a value if
\token{spatialDimensions} on the compartment has a value of
``\token{0}'', or if the species' \token{hasOnlySubstanceUnits}
\changed{attribute} has a value of \val{true}. The default value of the
\token{spatialSizeUnits} is the value of the \token{units} \changed{attribute}
...
\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.79]{figs/reaction-uml}
  \caption{\changed{The definitions of \Reaction, \KineticLaw,
      \SpeciesReference, \ModifierSpeciesReference, and the
...
\SimpleSpeciesReference inherits from \SBase the optional
...
removes these \changed{attributes} for several reasons.  First, the introduction
...
assignment affects, must be explicitly established.  (See also
Section~\ref{???}.)
