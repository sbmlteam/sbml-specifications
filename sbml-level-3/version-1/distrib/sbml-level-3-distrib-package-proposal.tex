...
\packageVersion{Version \changed{0.17} (Draft)}
...
\author{%
  ...
  University of Newcastle\\ Babraham, UK & Newcastle, UK\\ \\ Maciej
  Swat & Sarah Keating\\ \mailto{mjswat@ebi.ac.uk} &
  \mailto{skeating@ebi.ac.uk}\\ EMBL-EBI & EMBL-EBI\\ Hinxton, UK &
  Hinxton, UK\\
\\
...
0.17 (Draft) & May 2017 & Lucian Smith & Extensive update to reflect demise of UncertML 3.0, and appearance of ProbOnto.\\
%\midrule
\bottomrule
\end{edtable}
...
\begin{center}
\uri{http://www.sbml.org/sbml/level3/distrib/version1}
\end{center}
...
\changed{Note that the \distrib package may be used with both \threeone and \threetwo documents, with little semantic change between the two.  When there is a difference, the difference will be noted.}
...
<sbml xmlns="http://www.sbml.org/sbml/level3/version2/core" level="3" version="2"
      xmlns:distrib="http://www.sbml.org/sbml/level3/distrib/version1"
...
The \distrib package uses \changed{data types} described in Section~3.1 of the \sbmlthreecore specification, and adds the additional primitive types described below.

\begin{blockChanged}
\subsubsection{Type \fixttspace\primtypeNC{ExternalRef}}
\label{sec:primtype-externalref}

The type \primtype{ExternalRef} is derived from \primtype{string} with the additional requirement that it be a valid URI.  An \primtype{ExternalRef} is used in \distrib to point to ontologies such as ProbOnto which contain defined distributions and parameters. 
\end{blockChanged}
...
\changed{Both purposes are achieved by using the \Distribution and \UncertStatistics classes, modeled after \uncertml.  Several distributions and statistics are defined explicitly in this specification, but more can be defined by referencing the an external ontology such as the ProbOnto ontology through the \ExternalDistribution and \ExternalParameter classes.}

\changed{It is hoped that with this approach, modelers may use the \Distribution classes defined in this specification with a reasonable expectation of various software packages recognizing and replicating those distributions.  However, if another distribution is required, those distributions may still be encoded, even if this makes the model less exchangeable.}
...
As outlined above, the \FunctionDefinition class is extended to contain a \DrawFromDistribution child, \changed{which overrides any \LambdaClass child present in the \FunctionDefinition.  Software} that does not support \distribshort could potentially invoke the function found in the \LambdaClass element (see \ref{sec:fallbackfunc}) \changed{instead}.

In the \Model, an extended \FunctionDefinition may be used in any \mathml \changed{elsewhere in the document} to perform a draw from a distribution.  This draw will be unique for every use of the \FunctionDefinition, whether or not the draw is performed at the same simulation time as a different draw (for example, if used in two different \InitialAssignment elements).
...
A \DistribInput always inherits the optional \token{metaid} and \token{sboTerm} attributes, and inherits the \token{id} and \token{name} attributes as described in \sec{sec:idname}.  The \token{id} of a \DistribInput has no external mathematical meaning, is required, and is of type \primtype{UncertId}, as described above.
\end{blockChanged}
...
\changed{In this draft of the \distrib specification, no mixed distributions and no multivariate distributions are presented, as the author has not seen any call for these distributions specifically, and believes that the generic \ExternalDistribution distribution could cover those cases on an as-needed basis.  If this turns out to not be the case, those distributions will be added to a subsequent version of this specification.  The use of the Arrays package would be required for any multivariate distribution.}
...
\changed{The \distrib package defines optional \token{id} and \token{name} attributes which shadow the \token{id} and \token{name} attributes defined on \SBase in \threetwo.  They are provided to allow the classes defined in this package (\Category, \DistribInput, \Distribution, \DrawFromDistribution, \ListOfCategories, \ListOfDistribInputs, \ListOfExternalParameters, \UncertStatisticSpan, \UncertStatistics, \UncertValue, and \Uncertainty, and only those classes and their derivatives) to have an \token{id} or a \token{name} in a \threeone document, where those attributes are not provided on \SBase.  (Elements defined in \sbmlthreecore or in another package may not use the \token{id} nor the \token{name} attributes defined in this package.)  In a \threetwo document, these attributes have identical meaning to the ones provided on a \threetwo \SBase object, and any object may \emph{only} have an \token{id} or \token{name} in the \distrib namespace or the \sbmlthreecore namespace, but not both.  The \token{id} attribute is of type \primtype{SId}, and must be unique among other IDs in the \primtype{SId} namespace in the parent \Model, unless stated otherwise in the definition of that object.  Similarly, the \token{id} has no mathematical meaning unless stated otherwise in the definition of that element.  The \token{name} attribute is of type \primtype{string}, and is provided to allow the user to define a human-readable label for the object, and has no uniqueness restrictions.}
...
The \Uncertainty class has two optional children: an \UncertStatistics child and a \Distribution child.  Either or both may be used, depending on the information about the parent that the modeler wishes to store in this object.
...
\item \token{centredMoment}:  [definition]
\item \token{coefficientOfVariation}:  [definition]
\item \token{correlation}:  [definition]
\item \token{decile}:  [definition]
\item \token{kurtosis}:  [definition]
\item \token{mean}:  [definition]
\item \token{median}:  [definition]
\item \token{mode}:  [definition]
\item \token{moment}:  [definition]
\item \token{percentile}:  [definition]
\item \token{probability}:  [definition]
\item \token{quantile}:  [definition]
\item \token{quartile}:  [definition]
\item \token{skewness}:  [definition]
\item \token{standardDeviation}:  [definition]
\item \token{variance}:  [definition]
...
\item \token{confidenceInterval}:  [definition]
\item \token{credibleInterval}:  [definition]
\item \token{interquartileRange}:  [definition]
\item \token{range}:  [definition]
...
The \BetaDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{alpha} and \token{beta}.
...
The \CauchyDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{location} and \token{scale}.
...
The \ChiSquareDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue child \token{degreesOfFreedom}.
...
The \ExponentialDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue child \token{rate}.
...
The \FDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{numerator} and \token{denominator}.
...
The \GammaDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{shape} and \token{scale}.
...
The \LaPlaceDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{location} and \token{scale}.
...
The \LogNormalDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{shape} and \token{logScale}.

A random variable x is Log Normal distributed if the probability density function (pdf) is of the form:

\begin{center}
$\frac{1}{x\sqrt{2\pi\sigma^2}}exp(-\frac{(ln(x)-\mu)^2}{2\mu^2})$
\end{center}
...
The \LogisticDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{location} and \token{scale}.
...
The \NormalDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{mean}, \token{stddev}, and \token{variance}.  The distribution must either define a \token{stddev} or a \token{variance}, but not both.
...
The \ParetoDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{scale} and \token{shape}.
...
The \StudentTDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children  \token{location}, \token{scale} and \token{degreesOfFreedom}.
...
The \UniformDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{minimum}, \token{maximum} and the optional \token{numberOfClasses}.
...
The \WeibullDistribution is a \ContinuousUnivariateDistribution that defines the \UncertValue children \token{shape} and \token{scale}.
...
The \BinomialDistribution is a \DiscreteUnivariateDistribution that defines the \UncertValue children \token{numberOfTrials} and \token{probabilityOfSuccess}.
...
The \GeometricDistribution is a \DiscreteUnivariateDistribution that defines the \UncertValue child \token{probability}.
...
The \HypergeometricDistribution is a \DiscreteUnivariateDistribution that defines the \UncertValue children \token{numberOfSuccesses}. \token{numberOfTrials}, and \token{populationSize}.
...
\subsection{The \class{NegativeBinomialyDistribution} class}
\label{NegativeBinomialDistribution-class}
\label{negativebinomialdistribution-class}

The \NegativeBinomialDistribution is a \DiscreteUnivariateDistribution that defines the \UncertValue children \token{numberOfFailures} and \token{probability}.
...
The distribution describes the probability of getting $ x $ successes in trials of independent experiments that have the same probability of success, and are run until we observe $ r $ failures.

\subsection{The \class{PoissonDistribution} class}
\label{PoissonDistribution-class}
\label{poissondistribution-class}

The \PoissonDistribution is a \DiscreteUnivariateDistribution that defines the \UncertValue child \token{rate}.
...
The \BernoulliDistribution is a \CategoricalUnivariateDistribution that defines the \UncertValue child \token{prob}.
