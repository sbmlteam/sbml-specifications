...
The SBML Level~2 Version~2 specification has designed to be
backward compatible with SBML Level~2 Version~1.  A SBML Level~2
Version~1 document which has a transformed \class{sbml} element
with modified XML Namespace and \attrib{version} attribute values,
as described in Section~\ref{sec:sbml}, can become a valid SBML
Level~2 Version~2 document.  However in some cases the following
aspects of SBML Level~2 Version~2 may make the resulting document
an invalid SBML document.

\begin{itemize}

\item SBML Level~2 Version~2 restricts the form of elements in
\class{annotation} elements.  See
section~\ref{sec:annotation-use}.

\item SBML Level~2 Version~2 restricts the use of \attrib{offset}
...
Errata result in new \emph{issues} of the SBML specification. Each
issue is numbered. The first issue of the specification is
numbered 1.  Subsequent issues of the specification will contain a
section listing the errata created since the first issue. Each
errata will be numbered with the issue in which the errata is
introduced. Errata do not change the fundamental semantics or
syntax of SBML, but rather clarify and disambiguate the
specification and correct errors.  (New syntax and semantics are
only introduced in SBML Versions and Levels.) A complete list of
the errata for SBML Level~2 Version~2 created since Issue~1 is
...
\changed{\class{SBase} does not contain an \attrib{id} field
because there is no universal rule concerning the scope of
identifiers declared using such a field (see
Section~\ref{sec:namespaces} for more details).  If \class{SBase}
where to have an \attrib{id} field then the specification of
\class{SBase} would have to have a default scoping rule which
would then have to be overloaded on derived classes that used a
different rule. This would make this specification unnecessarily
complex. \class{SBase} does not have a \attrib{name} simply
...
XML identifier type), and thus each \attrib{metaid} value is
unique within an SBML file.  The \attrib{metaid} value serves to
identify the element from a metadata structure. Metadata expressed
using RDF can be placed anywhere within an \class{sbml} element
and its subelements, \emph{except} within MathML elements. The
metadata elements can include RDF \class{description} elements in
which the RDF \attrib{describes} attributes contain the values of
the \attrib{metaid} fields of SBML elements in the model.
\end{blockChanged}
...
provide a container for software-generated annotations that are
\emph{not} intended to be seen by humans. This field is a
container for essentially arbitrary data (XML type \class{any}).
There are restrictions on the use XML Namespaces~\citep{bray:1999}
within an \class{annotation} element which are described below. As
with the user-visible \attrib{notes} field, every data object
derived from \class{SBase} can have its own value for
\attrib{annotation}.

Applications must use XML Namespaces~\citep{bray:1999} to specify
the intended vocabulary of a particular annotation. Here is an
example. Suppose a particular application needs to annotate data
structures in an SBML model definition with screen layout
information.  The application's developers must choose a URI
(\emph{Universal Resource
  Identifier}; \citealt{harold:2001,w3c:2000}) reference that uniquely
identifies the vocabulary the application will use for such
annotations, and a prefix string for the annotations.  For
illustration purposes, let us say the URI reference is
``\texttt{http://www.mysim.org/ns}'' and the chosen prefix is
\texttt{mysim}.  An example of an annotation might then be as
follows:
...
<annotation>
    <mysim:nodecolors
        xmlns:mysim="http://www.mysim.org/ns"
        mysim:bgcolor="green" mysim:fgcolor="white"/>
</annotation>
\end{example}

The namespace prefix \texttt{mysim} is used to qualify the XML
element \texttt{mysim:nodecolors} presumably these symbols have
meaning to the application.

In the following text the term \emph{top level element} of an
\class{annotation} element refers to an element immediately
contained within the \class{annotation} element. The order of top
level elements within an \class{annotation} element is \emph{not}
significant. An \class{annotation} element must not contain: (a)
top level elements in the SBML XML namespace; and (b) two or more
top level elements in the same XML namespace.  For example the
following is not valid because it contains a top level element in
...
elements in the same XML Namespace:
...
<annotation>
    <mysim:geometry
        ...
        <graph:node
            graph:x="4"
            graph:y="5"
            xmlns:graph="http://www.graph.org/ns"/>
    </mysim:geometry>
    <funky:foo xmlns:funky="http://www.funky.org/ns">
         Wham
    </funky:foo>
</annotation>
\end{example}
...
annotations on XML elements of a model without risking
interference or element name collisions. Annotations stored by
different simulation packages can thus coexist in the same model
definition.  The rules governing the content of \class{annotation}
elements are designed to enable applications to easily add, change
and remove their annotations from SBML elements whilst at the same
time preserving annotations inserted by other applications when
mapping SBML from input to output.

Although XML Namespace names must be URI references, an XML
Namespace name is \emph{not required} to be directly usable in the
sense of identifying an actual, retrieval document or resource on
the Internet~\citep{bray:1999}. ``\texttt{http://www.mysim.org/}''
is a namespace name or URI in the example above.  The name is
simply intended to enable unique identification of constructs, and
using URIs is a common and simple way of creating a unique name
string. Finally, note that the namespaces being referred to here
are XML Namespaces specifically in the context of the
\attrib{annotation} field on \class{SBase}.  The namespace issue
here is unrelated to the namespaces discussed in
Section~\ref{sec:namespaces} in the context of \class{SId} and
symbols in SBML.

The \attrib{annotation} field in the definition of \class{SBase}
exists in order that software developers may attach any
information they need to the structures in an SBML model. However,
it is important that this facility not be misused.  In particular,
it is critical that information essential to a model definition
\changed{or that can be easily encoded in existing SBML
structures} is \emph{not} stored in \attrib{annotation}. Parameter
values, functional dependencies between model structures, etc.,
should not be recorded as annotations.
...
instructions that do not change the essence of a model;
\changed{(c) identification information for cross-referencing
components in a model with items in a data resource.  For case (c)
SBML Level 2 Version 2 recommends a standard form for use within
an \attrib{annotation} field.  This form should be used in
preference to proprietary syntaxes, see
...
%=============================================================================
\subsubsection{General Syntax for Dubin Core annotation}
%=============================================================================
...
\item \changed{The apostrophe (\texttt{'}) and  single-quote
character (\texttt{"}) must be
...
(Section~\ref{sec:events}). The \class{KineticLaw},
...
subelement. A function definition has a single \class{lambda}
subelement. The \class{Event} structure has two math fields,
\attrib{trigger} and \attrib{delay} each containing a single
MathML \class{math} element.
...
\class{and}, \class{or} and \class{xor}.  The MathML
specification~\citep{w3c:2000b} appendix C.2.3 describes the
semantics for these operators with zero or one arguments.}
...
\changed{The content of a \class{ci} element must be an identifier
...
that reaction's lists of reactants, products or modifiers.  In an
SBML model, all species that participate in any reaction are
...
back to those listed in the model's \attrib{listOfSpecies}.  For
reactants and products, the connection is made using the
\class{SpeciesReference} data structure defined in
Figure~\vref{fig:reaction}.

\changed{\class{SimpleSpeciesReference} is an abstract structure
type that is used to represent the fields that are common between
\class{SpeciesReference} and \class{ModifierSpeciesReference}.  An
SBML structure cannot contain an \class{SimpleSpeciesReference}
structure only \class{SpeciesReference} and
\class{ModifierSpeciesReference} structures.}
