...
\hl{\consistencyRule{qual-10202}{The MathML \token{math} element in a \FunctionTerm object should not use the \token{csymbol} elements \val{time} and \val{delay} as these expliciltly introduce time into the model. As yet time is not considered within the Qualitative Models package specification.
  (References: SBML Level~3 Package Specification for Qualitative Models, Version~1, \sec{sec:function-term}.) }
}
