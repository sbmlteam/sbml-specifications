...
\Species, \SpeciesReference{}s and \Parameter{}s. Wherever the \token{constant} attribute 
appears with a \val{true} it has the implication that its SBML element cannot be 
changed by other constructs  in SBML (with the exception of an \InitialAssignment ). 
A value of \val{false} indicates that the element is likely to be changed as a 
...
\todo{someone}{this needs work}

Here are examples of the kinds of data that may be appropriately
stored in \token{annotation}: (a) application-specific processing
...
In order to maximize the likelihood that multiple software
...
\item  The modular nature of SBML \thisLV means that information
  previously stored as \token{annotation} is likely to be developed
  into an L3 package.  Users should check the status of packages Ref??
  \todo {someone} {add reference to l3 packages}
  
\item  A list of already defined syntaxes used in existing 
  \token{annotation} is available here.  \todo {someone}{add reference to wiki page on existing annotations} 
...
- use SBO:0000410 for compartments that are actually default
compartments added to a model only because SBML requires a compartment
