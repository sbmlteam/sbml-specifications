...
\newcommand{\sboref}{\url{http://www.biomodels.net/SBO}\xspace}
...
model, the value must be an SBO identifier referring to a modeling
framework defined in SBO (\ie terms derived from \sboframework).
The SBO term chosen should be the most precise (narrow) term that
defines the mathematical framework assumed by the model.  An
example framework might be ``continuous deterministic'', which
would mean the model was created under the assumption it will be
simulated in a tool that turns the set of reactions into a system
of coupled differential equations.  Other frameworks are possible.

The value given to \token{sboTerm} on a \Model interacts with the
SBO labels on the model's \Reaction elements.  The usage rule is
simple: there should \emph{only} be a label on \Model if it means
that \emph{all} reactions in the model assume the same framework.
In that case, the \token{sboTerm} values on \Reaction elements may
be omitted, but only the \Reaction SBO labels can be thus omitted;
the SBO labels on \KineticLaw within \Reaction must still be
supplied, as must the terms on other components of the model, in
order to characterize the model completely.  Conversely, if a
model uses a mixture of frameworks (some reactions are intended to
be simulated using one framework, other reactions are intended to
be simulated using a different framework), then a value should
\emph{not} be given on the \Model's \token{sboTerm}, and instead,
each reaction individually should be given an appropriate
\token{sboTerm} value.

In summary, the presence of an \token{sboTerm} value on a \Model
object implies that all reactions in the model are intended to be
represented using the same simulation framework (for example,
continuous deterministic, or discrete stochastic).  The absence of
a \token{sboTerm} value means that either the model has not been
labeled with SBO terms at all, or else that a mixture of
frameworks is being assumed and software applications reading the
model should observe the \token{sboTerm} labels on the individual
reactions.
...
referring to a mathematical formula (\ie terms derived from
...
the SBO labels on the \Model element.  The usage rule is simple:
there should \emph{only} be a label on \Model if it means that
\emph{all} reactions in the model assume the same framework.  If
that is the case, the \token{sboTerm} values on \Reaction elements
may be omitted, but only the \Reaction SBO labels can be thus
omitted; the SBO labels on \KineticLaw within \Reaction must still
be supplied, as must the terms on other components of the model,
in order to characterize the model completely.  Conversely, if a
model uses a mixture of frameworks (some reactions are intended to
be simulated using one framework, other reactions are intended to
be simulated using a different framework), then a value should
\emph{not} be given on the \Model's \token{sboTerm}, and instead,
...
then it should be a term in the \sboreactant hierarchy; and if a
...
\begin{figure}[htb]
  ...
  \begin{classbox}{Event}
    ...
    delay: (math : Math \{ namespace="http://www.w3.org/1998/Math/MathML" \})   \\    
...
An \Event definition has four required parts: an identifier, a
trigger condition, a delay, and at least one \EventAssignment.
...
value is given to this field, it must be a valid SBO referring to
a mathematical expression (\ie terms derived from
\sbomathformula).  The \EventAssignment should have an ``is a''
relationship with the SBO term, and the term should be the most
precise (narrow) term that captures the form of the assignment
...
SBO (\sboref) should be consulted for the current version of the
ontology.
...
are included in order to be able to characterize other
mathematical components of a model, namely initial assignments,
assignment rules, rate rules, algebraic rules, constraints, and
event triggers and assignments.

\begin{figure}[tbh]
  \centering
  \color{\changedColor}
  \includegraphics{figs/sbo-math-expression}
...
\item \emph{Children}: The term(s), if any, which are children of
  a given term.  The children have an ``is a'' relationship to the
  parent.
...
Some explanation is warranted about recording the children of a
given term.  To navigate the hierarchy and find alternative
definitions, a software tool needs to be able to find not only the
parents of a term, but also the children of parent terms.  Both
are available in the database and will be exposed using the query
interfaces (be they web services, library APIs, or other).
However, in reality, the child connections are not stored
explicitly in the CV, but are computed by the underlying SBO
database system based on the parent relationships.
...
\begin{table}[bht]
  ...
  \begin{tabular}{lll}
    ...
    \SpeciesReference (when reactant)	& participant role	  & \sboreactantID \\
    \SpeciesReference (when product)	& participant role	  & \sboproductID \\
    \ModifierSpeciesReference		& participant role	  & \sbomodifierID \\
...
In Version 2, the \sboelements structures have an addition field
named \token{sboTerm}.  This field is of type \primtype{SBOTerm}.
Its value is permitted to be only valid Systems
Biology Ontology (SBO) identifiers.
