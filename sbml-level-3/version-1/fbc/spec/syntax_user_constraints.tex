...
%
\newtxt{The \UserDefinedConstraint contains a \ListOfUserDefinedConstraintComponents representing a linear combination of \UserDefinedConstraintComponent{s}. Similar to a \FluxObjective each \UserDefinedConstraintComponent contains a coefficient and model variable(s). In the case of \UserDefinedConstraintComponent the \token{coefficient} refers to a constant \Parameter while the \token{variable} and optional \token{variable2} attributes may either refer to a \Reaction or to a non-constant \Parameter. This allows for the creation of non-reaction based variables. Quadratic constraint components are also supported, see the attribute descriptions for more detail.}
...
\Annotation and \Notes. The \UserDefinedConstraintComponent class is a relatively simple container for a signed coefficient, model variable(s), parameter(s) and a variable type specifier.}
...
\paragraph{The \token{variable} and \token{variable2} attributes}
\newtxt{The required \token{variable} attribute and optional \token{variable2}, contain an \primtype{SIdRef} that is restricted to reference the \primtype{SId} of either a \Reaction or non-constant \Parameter. In \textbf{strict} mode such a non-constant \Parameter whose \primtype{SId} is referenced by a \UserDefinedConstraintComponent \token{variable} or \token{variable2} attribute may not be referenced by any \UserDefinedConstraintComponent \token{coefficient}, \UserDefinedConstraint \token{lowerBound} \& \token{upperBound} or \Reaction \token{lowerFluxBound} \& \token{upperFluxBound} attribute.}

\paragraph{The \token{variableType} attribute}
\newtxt{The required \token{variableType} attribute contains a \primtype{FbcVariableType} that indicates whether a variable should be considered as \val{linear} or \val{quadratic}. Quadratic variables can be defined as either single or mixed variable terms. For example, where $J$ represents a steady-state flux in the \token{variable} attribute, the \primtype{FbcVariableType} allows the definition of either a \val{linear}, $J^1$ or \val{quadratic}, $J^2$ variable. In addition, a mixed \val{quadratic} component may be defined using the optional attribute \token{variable2}. For example, with \token{variable2} represented by $J_{R2}$ and \token{coefficient}, $C$, it is possible to define a mixed, two variable \val{quadratic} \UserDefinedConstraintComponent of the form $C \times J^{1} \times J_{R2}^{1}$.}
...
\newtxt{The optional \ListOfKeyValuePairs forms the basis of a controlled annotation defined by the \FBCPackage, see \ref{fig:fbc_v3_uml_keyvalue} for details. This element defines a `descriptive list' of data keys and associated values which can be used to store metadata that is not encodable using a typical \SBML annotation.}
%
\exampleFile{examples/ex-v3-kvp1.txt}
%
\newtxt{It is analogous and supplemental to the official \SBML RDF annotation used to support MIRIAM compliant annotations. The \ListOfKeyValuePairs is an optional element of an \Annotation that declares the \token{xmlns} attribute to be \val{http://sbml.org/fbc/keyvaluepair} and then strictly applies the \ListOfKeyValuePairs format. As it is defined as an \Annotation, software may choose to support reading and interpreting the \ListOfKeyValuePairs content and optionally ignore the annotation and merely read and write it with any other third party \Annotation{s}. As is the case with the RDF/MIRIAM \Annotation{s}, read and write support for the \ListOfKeyValuePairs annotation is included in the \SBML support libraries and easily accessible to tool developers.}
%
\begin{figure}[ht]
  \centering
  % Requires \usepackage{graphicx}
  ...
  \caption{A UML representation of the \FBCPackage \protect{\ListOfKeyValuePairs} and derived
...
\newtxt{The \FBC \KeyValuePair class defines a typical key--value pair with a single, mandatory attribute, the \token{key}, as well as four optional attributes: \token{value}, \token{uri}, \token{id} and \token{name}.}
...
\newtxt{The optional \token{value} attribute is of type \primtype{string} and contains the matching \token{value} associated with a particular \token{key}. If not present, the \KeyValuePair is defined as having no value.}
