...
\item \class{Class}: Names of ordinary classes begin with a
...
\changed{The class definitions are mapped to XML Schema~1.0
...
\item \changed{ \emph{Species type}: A type of quantified entity
...
namespace; this matches the MathML~2.0 definition of integers.
...
fields in the SBML XML namespace; this matches the MathML~2.0
...
because MathML \token{ci} elements are not of the type
...
``\token{SBO:0000014}'' and ``\token{SBO:0003204}''.  These
...
\val{http://www.w3.org/1999/xhtml}.  There are multiple ways in
...
The subset of MathML elements used in SBML Level~2 is similar to
...
blank is replaced by the name of the component type.  If the word
already ends in ``s'', such as ``species'', then the extra letter
is omitted from the end, leading to \token{listOfSpecies}.}  The
...
\begin{figure}[htb]
  ...
  \begin{classbox}{Unit}  
    ...
    offset: double \{ use="optional" default="0" \}     \\
  \end{classbox}
...
definitions from more complex ones is that they be expressed
without the use of an additive offset from a zero point.  The use
of offsets complicates all unit definition system, yet the
real-life cases requiring offsets are few (and in fact, to the
best of our knowledge, only involve temperature).  Consequently,
the SBML unit system has been consciously designed in a way that
attempts to simplify implementation of unit support for the most
common cases in systems biology, at the cost of an asymmetry in
unit calculations introduced when offsets are used.  The two cases
(without offsets, and with) are described separately below.
...
chosen base unit, whereas the four fields \token{exponent},
\token{scale}, \token{multiplier}, and \token{offset} define how
the base unit is being transformed.
...
exponent on the unit.  Its default value is ``\token{1}'' (one).
A \Unit structure also has an optional \token{scale} field; its
...
\token{kind} value of ``\token{gram}'' and a \token{scale} value
of ``\token{-3}'' signifies $10^{-3} * gram$, or milligrams.  The
default value of \token{scale} is ``\token{0}'' (zero), because
$10^0 = 1$.  Lastly, the optional \token{multiplier} field can be
used to multiply the \token{kind} unit by a real-numbered factor;
this enables the definition of units that are not power-of-ten
multiples of SI units.  For instance, a \token{multiplier} of
0.3048 could be used to define ``foot'' as a measure of length in
terms of a metre.  The \token{multiplier} field has a default
value of ``\token{1}'' (one).

In the following explanation of the use of these fields, we limit
discussion to the case where offsets are not required (\ie when
\token{offset} in a \Unit definition is not set, or is set to
\val{0}).

\newcommand{\xnew}{x}
\newcommand{\xold}{x_\emph{base}}
\newcommand{\unew}{u}
\newcommand{\uold}{u_\emph{base}}
...
quantities back to the base units.  The relationship between a
quantity $\xnew$ in units $\unew$ to a quantity $\xold$ in base
units $\uold$ (when the relationship does not require the use of
zero offsets) is
\begin{equation}
  \xold \, \uold = \xnew \, \unew \cdot \left( w \frac{\uold}{\unew} \right)
\label{eq:new-to-old}
\end{equation}
where the dot ($\cdot$) represents simple scalar multiplication
and $w$ is a numerical factor.  The $\unew$ terms on the
right-hand side cancel, leaving a value in units of $\uold$.
Numerically, the value is equal to $\xnew \cdot w$.  The factor
$w$ is determined as follows:
\begin{equation}
  \unew = (\token{multiplier} \cdot 10^\token{scale} \cdot \uold)^\token{exponent}
\label{eq:single-unit-no-offset}
\end{equation}
...
Figure~\ref{fig:unitdefinition}.
Equation~\ref{eq:single-unit-no-offset} asserts a mathematical
relationship between a given unit $\uold$ and another unit
$\unew$.  The exponent may make it more difficult to grasp the
relationship immediately; suppose for the moment than
\token{exponent}=\val{1}, then it is easy to see that
\begin{equation*}
  \unew = \token{multiplier} \cdot 10^\token{scale} \cdot \uold 
\end{equation*}
which means that $w = \token{multiplier} \cdot 10^\token{scale}$
in Equation~\ref{eq:new-to-old}.  To take a concrete example, one
foot expressed in terms of the metre (a base unit) requires
\token{multiplier}=\val{0.3048}, \token{exponent}=\val{1}, and
\token{scale}=\val{0} (the default):
\begin{align*}
  \text{foot} = 0.3048 \cdot 10^0 \cdot \text{metre} \\
\intertext{leading to a conversion between quantities of}
  \xold \, \text{metres} = 0.3048 \cdot \xnew \, \text{feet} 
\end{align*}

The case above is the simplest possible situation, involving the
transformation of quantities from a single defined unit $\unew$
into a quantity expressed in a single base unit $\uold$.  If,
instead, multiple base units $u_1, u_2, \ldots, u_n$ are involved,
the following equation holds (where the $m_i$ terms are the
\token{multiplier} values, the $s_i$ terms are the \token{scale}
values, and the $x_i$ terms are the \token{exponent} values):
\begin{align}
  u_\emph{new} &= (m_1 \cdot 10^{s_1} \cdot u_1)^{x_1} \cdot 
  (m_2 \cdot 10^{s_2} \cdot u_2)^{x_2} \cdot \ldots \cdot (m_n \cdot
  10^{s_n} \cdot u_n)^{x_n} \notag
  \\[2pt]
               &= m_1^{x_1} \cdot m_2^{x_2} \cdot \ldots \cdot m_n^{x_n}
  \cdot 10^{s_1 x_1} \cdot 10^{s_2 x_2} \cdot \ldots \cdot 10^{s_n x_n}
  \cdot u_1^{x_1} \cdot u_2^{x_2} \cdot \ldots \cdot u_n^{x_n} \notag
  \\[2pt]
               &= m_1^{x_1} \cdot m_2^{x_2} \cdot \ldots \cdot m_n^{x_n}
  \cdot 10^{(s_1 x_1 + s_2 x_2 + \ldots + s_n x_n)} 
  \cdot u_1^{x_1} \cdot u_2^{x_2} \cdot \ldots \cdot u_n^{x_n}
\label{eq:multip-units-no-offset}
\end{align}
The use of this unit transformation expression is analogous to
Equation~\ref{eq:single-unit-usage}:
\begin{align}
  \xold \uold &= \xnew \, \unew
  \cdot \left( m_1^{x_1} \cdot m_2^{x_2} \cdot \ldots \cdot m_n^{x_n}
  \cdot 10^{(s_1 x_1 + s_2 x_2 + \ldots + s_n x_n)} 
  \cdot u_1^{x_1} \cdot u_2^{x_2} \cdot \ldots \cdot u_n^{x_n}
  \right)
\label{eq:multip-unit-usage}
\end{align}

Some additional points are worth discussing about the unit scheme
introduced so far.  First, the equations above are formulated with
the assumption that the base units do not require an additive
offset as part of their definition.  This is true for all the base
units, except the temperature units (Celsius and kelvin) under one
condition.  This condition is when a temperature value rather than
a temperature \emph{change} is being considered.  \emph{When
  temperature values are being considered, then a different
  interpretation must be made}, as discussed below.  In all other
cases, including when temperature degrees or intervals are being
discussed, the straightforward unit transformation scheme
described so far applies.
...
think that defining Fahrenheit degrees in terms of Celsius degrees
...
be defined and the unit definition must be interpreted in a
different way, described below.


\paragraph{The \token{offset} Field}

The \token{offset} field is used to represent the addition of a
constant in the transformation of the \token{kind} unit.  For
example, an \token{offset} value of ``\token{32.0}'' would be
needed to define Fahrenheit in terms of degrees Celsius.  The
\token{offset} field has a default value of ``\token{0}'' (zero).

When an offset is used in an SBML unit definition, the definition
must be interpreted in a different way.  

\emph{INCOMPLETE PENDING RESOLUTION OF OFFSETS DEBATE.}


\paragraph{Examples}

\begin{blockChanged}
The following example defines minutes in terms of seconds:
\begin{example}
<listOfUnitDefinitions>
...
\token{scale}=\val{0} and \token{offset}=\val{0}.  
...
\begin{figure}[htb]
  ...
  \begin{classbox}{Compartment}
    id: SId \\
    name: string \{ use="optional" \} \\
    \changed{compartmentType: SId \{ use="optional" \}} \\
...
\begin{figure}[htb]
  ...
  \begin{classbox}{Species}
    id: SId \\
    name: string \{ use="optional" \} \\
    \changed{speciesType: SId \{ use="optional" \}} \\
    compartment: SId \\
    ...
    charge: integer \{ use="optional" \} \changed{\emph{deprecated}}\\
...
\val{1}.}. If these fields are not set in a given
