...
\item There are different degrees of rule strictness.  Formally,
  ...
  \item[\hspace*{7.5pt}\mSymbol] A \mSymbolName indicates a strong
    ...
    choice of symbol: ``You're a star if you follow this.'')

  \end{description}

\item Most rules have existed in previous Levels/Versions of SBML.
  Rules introduced in \emph{this} \sbmlthree specification are
  indicated by an underlined rule number (e.g., \underline{10101}
  instead of 10101).  Note that, because each SBML specification
  is independent, the precise details of a given rule in this
  specification may differ slightly from its formulation in other
  Levels/Versions, to reflect changes in \sbmlthree; however, the
  essential purpose of the rule will be the same.
...
\validationRuleNew{20216}\question{20216 added namespace clause}
