\section{Package syntax and semantics} \label{syntax} 
...
this package} \label{xml-namespace} 
...
\BoundingBox consists of an optional attribute \token{id} as well as 
required elements \token{position} and \token{dimensions}. 
...
the two elements \token{start} and \token{end}, that specify 
the starting point and the endpoint of the cubic bezier curve. The two 
elements \token{basePoint1} and \token{basePoint2} specify two 
...
Hereby \token{basePoint1} is the base point closer to the \token{start} 
point and \token{basePoint2} closer to \token{end}. This allows tools not able to render bezier curves to approximate them by directly connecting the four points. 
...
\paragraph{The \token{id} attribute}
The \GraphicalObject has an mandatory \token{id} attribute of type 
...
allows to add a reference to the \token{id} of the corresponding 
...
The \token{reaction} attribute of type \primtype{SIdRef} allows to 
...
\paragraph{The \token{role} attribute}
The \token{role} attribute is of \primtype{string} and is used to 
...
\token{role} attribute is defined the role to be assumed is taken from 
the role that the \SpeciesReference referenced via the attribute 
\token{speciesReference} has, otherwise it is \token{undefined}. 
