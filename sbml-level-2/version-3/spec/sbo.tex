...
\section{The Systems Biology Ontology and the \token{sboTerm} \changed{attribute}}
\label{sec:sboTerm}
\label{sec:sbo}

The values of \token{id} \changed{attributes} on SBML components
allow the components to be cross-referenced within a model. The
values of \token{name} \changed{attributes} on SBML components
\changed{provide the opportunity to assign them meaningful labels}
...
\changed{object classes} defined in other sections of this document.  For
...
to be in some given model).  \changed{However, one may need to convert a
model between different representations (\eg}
\changed{Henri-Michaelis-Menten vs. elementary steps), or to use it with
different modelling approaches (discrete} \changed{or continuous).  One may
also need to relate the model components with other description
format, using} \changed{a deeper semantics.  Although} an advanced software
tool \emph{might} be able to deduce the semantics of some model
components \changed{through} detailed analysis of the kinetic rate expressions
...
\token{sboTerm} \changed{attribute} provided on \changed{the SBML}
\changed{class \SBase}.  The \token{sboTerm} \changed{attribute} always refers to
terms belonging to the Systems Biology Ontology (SBO, \sboref). In
this section, we discuss the \token{sboTerm} \changed{attribute},
...
expressions, parameters, and a few others.  \changed{The use of
\token{sboTerm} attributes is optional, and} \changed{the presence of
\token{sboTerm} on an element does not change the way the model is
\emph{interpreted}.  Annotating SBML} \changed{elements with SBO terms
adds additional semantic information that may be used to
\emph{convert} the model into} \changed{another model, or another format.
Athough SBO support provides an important source of information to}
\changed{understand the meaning of a model, software does not need to
support \token{sboTerm} to be considered SBML-}\changed{compliant.}
...
tools \changed{with editing} interfaces frequently provide these names
...
names or identifiers of reactions \changed{used} in models written
by other tools.
...
\changed{The Systems Biology Ontology (SBO) provides vocabularies for
identifying common reaction rate expressions,} \changed{common participant
types and roles in reactions, common parameter types and their
roles in rate expressions}, \changed{common modeling frameworks (e.g.,
``continuous'', ``discrete'', etc.), and common types of
species and} \changed{reactions.}  The relationship implied by an
...
the \token{sboTerm} \changed{attribute} value.  By adding SBO term
...
\changed{attribute} with this value, then regardless of the
...
\changed{certain types of} \changed{reaction formulas} as being ones it
...
for any SBML \changed{object} labeled with that term.  Two important
...
to alter the mathematical meaning of a model would \changed{allow
  too much} leeway to shoehorn inconsistent concepts into SBML
\changed{objects}, ultimately reducing the interoperability of the
models.

\subsection{Using SBO and \token{sboTerm}}

The \token{sboTerm} \changed{attribute} data type is always
\primtype{SBOTerm}, defined in Section~\ref{sec:sboterm-type}.
When present in a given model object instance, the
\changed{attribute}'s value must be an identifier taken from the
...
extent possible the nature of \changed{each element in} \changed{a model}.  The
approach taken in SBO begins with a hierarchically-structured set
of controlled vocabularies with \changed{five} main divisions: \changed{(1) modeling
framework, (2) participant type, (3) quantitative parameter, (4)}
\changed{mathematical expression, and (5) event}.
Figure~\vref{fig:sbo-top-level} illustrates the highest level of
SBO.

Each of the \changed{five} branches of Figure~\ref{fig:sbo-top-level} have a
...
\begin{figure}[tbh]
  ...
  \vspace*{1ex}
  \caption{\changed{The five controlled vocabularies (CVs) that
...
\changed{expressing} the parameter using other SBO parameters.  The
...
\changed{The \emph{mathematical expression} vocabulary} encompasses the
...
are likely to be included \changed{in the future} in order to be
...
precise statement about the rate law in question.  \changed{Some of the
non-leaf terms also contain rate} \changed{laws encoded using \mathmltwo.
However, those are generic rate-laws, containing MathML constructs
not} \changed{yet supported by SBML and need to be expanded into
the MathML subset allowed in SBML before they can} \changed{be
...
<math xmlns="http://www.w3.org/1998/Math/MathML">
    \changed{<semantics definitionURL="http://biomodels.net/SBO/#SBO:0000062">}
        <lambda>
...
\begin{figure}[tbh]
  ...
  \caption{\changed{Partial expansion of some of the terms in the \emph{event} CV.}}
  \label{fig:sbo-event}
\end{figure}
...
\changed{The \token{sboTerm} attribute is defined on the abstract
  class \SBase and can be used in all
derived elements.  However,} \changed{not all SBO terms can} \changed{be used to
annotate all SBML elements.}  Table~\ref{tab:sboterm-availability}
summarizes the relationship between SBML components and the
branches within SBO that apply to that component.  \changed{(There are
currently no} \changed{specific SBO term that correspond to the \Sbml,
\UnitDefinition and \Unit classes).}
...
\begin{table}[bht]
  ...
  \begin{edtable}{tabular}{lll}
    ...
    \midrule
    \changed{\Model}              & \changed{modeling framework}        & \changed{\sboframeworkID} \\
    \FunctionDefinition           & mathematical expression             & \sbomathformulaID \\
    \changed{\CompartmentType}    & \changed{participant physical type} & \changed{\sboparticipantphysicalID} \\
    \changed{\SpeciesType}        & \changed{participant physical type} & \changed{\sboparticipantphysicalID} \\
    \changed{\Compartment}        & \changed{participant physical type} & \changed{\sboparticipantphysicalID} \\
    \changed{\Species}            & \changed{participant physical type} & \changed{\sboparticipantphysicalID} \\
    \Reaction                     & \changed{event}                     & \changed{\sboeventID} \\
    \Parameter                    & quantitative parameter              & \sboparameterID \\
    \SpeciesReference             & \changed{participant functional type}         & \sboparticipantfunctionalID \\
    \ModifierSpeciesReference     & \changed{participant functional type}         & \sboparticipantfunctionalID \\
    \KineticLaw                   & \changed{rate law}                  & \changed{\sboratelawID} \\
    ...
    \Event                        & \changed{event}                     & \changed{\sboeventID} \\
    \changed{\Trigger}            & \changed{mathematical expression}   & \changed{\sbomathformulaID} \\
    \changed{\Delay}              & \changed{mathematical expression}   & \changed{\sbomathformulaID} \\
...
\changed{The weaknesses are not shared by the annotation scheme described
...
application remain usable in all cases.  \changed{(Moreover,} machine-readable
CV encodings and appropriate software design should \changed{render
possible} \changed{the development of} API libraries that automatically map
...
them to (\eg detecting a reference in a model \changed{to an SBO term} that
...
\changed{If a software tool does not} pay attention to the SBO
