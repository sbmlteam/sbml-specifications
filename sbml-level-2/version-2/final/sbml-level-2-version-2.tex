...
\date{SBML Level 2, Version 2, Issue 1 (Draft)\\[5pt]
  29th May 2005\\
  The latest issue of this specification is available at
  \url{http://sbml.org/specifications/sbml-level-2/version-2/}\\
  This issue of this specification is available at
  \url{http://sbml.org/specifications/sbml-level-2/version-2-1/}}
...
not a valid SBML Level~1 document.

\changed{Minor Releases of SBML are termed \emph{versions}.  This
specification defines the revised standard SBML Level 2 Version 2.
...
\changed{Each SBML version specification document is an evolves as
inevitably the community clarifies and disambiguates the
specification. These changes will be made public as \emph{errata}.
Each errata to the specification will result in an new
\emph{issue} of the specification. Each issue will be numbered.
The first issue of the specification will be numbered 1. The
errata for this specification be recorded in a new section
starting in issue 2. Each errata will be numbered with the issue
in which the errata is introduced. Errata should not change the
fundamental semantics or syntax of SBML but merely clarify and
disambiguate the specification.  (SBML Versions introduce new
syntax and semantics.) A complete list of the errata for SBML
Level 2 Version 2 created since Issue 1 is available at
...
contains a Universal Resource Identifier (URI) string.  This URI
...
resource referenced by the URI.  The URI is \emph{not} a URL and
does not refer to a location on the world wide web.  Instead the
URI string will be recognized by a set of application as referring
to a resource which in the simplest case is purely conceptual.

For example consider instances of software applications that share
a common definition of the URI

    \texttt{http://www.foobar.com/protein}

as representing the concept
of a protein. The following fragment of SBML indicates to those
instances that the species \texttt{MAPK} is a protein.

\begin{example}
<species id="MAPK" definitionURL="http://www.foobar.com/protein"/>
\end{example}

The presence of \attrib{definitionURL} on any SBML structure does
not change the semantics of that structure as described in this
document irrespective of any value contained in the field.  A
parser wishing to interpret the mathematical or structural aspects
of an SBML document can ignore \attrib{definitionURL} values. (The
use of \attrib{definitionURL} fields on MathML elements is covered
by Section~\ref{sec:csymbol-token}.)

Developers are advised and encouraged to take care when designing
syntax schemes for URI to ensure that schemes used by different
application do not overlap.  The best way to ensure this to use
...
operand.  Such a definition is given in this section.

The general form of these operators is as follows:

\begin{equation}
\label{eq:n-ary-operator}
f(x_1 ... x_n) = base_operand operator x_1 operator ... x_n
\end{equation}

where $n$ is any non-negative integer and $base_operand$ is a
constant that is associated with the operator.  The $base_operand$
...
For all the n-ary operators to give optimal computation the
operator can be replaced by (a) the 'base operand' if the operator
has no operands; or (b) replaced by an operand if the operator has
only one operand i.e. the operator is equivalent to identity.
\end{blockChanged}
...
pool contains chemical entities of a type unique to that pool. The
value of \class{speciesType} attributes have no effect on the
numerical interpretation of a model. Simulators and other
numerical analysis software may ignore \class{speciesType}
attributes.  In any \class{Species} structure, that has a
\attrib{speciesType} field value, the pair of \attrib{speciesType}
and \attrib{compartment} field values must be unique in the set of
all \class{Species} structures i.e. a species type cannot be
located more than once in a compartment.
\end{blockChanged}
...
\quantity{concentration} unit.  The units of \quantity{substance}
...
\class{Constraint} structures are used to express mathematical
statements which cannot and should not be used to compute the
dynamical behavior of the model.  A \class{Constraint} structure
is simply a mechanism for expressing the validity of the model's
state during numerical analysis and should be used to encode those
assumptions under which the model is designed to operate.

A \class{Constraint} structure consists of two fields:
\attrib{math} which contains a MathML formula and an optional
\attrib{message} which contains a HTML message to potentially be
displayed to the user.  Figure~\vref{fig:constraint} shows the
definition of the \class{Constraint} structures.  The formula
contained in the \attrib{math} field should be an arbitrary
function of the variables and parameters of the model which
returns a boolean value which is true when the model is in a valid
state.  The \attrib{message} field can be displayed to the user
when a tool detects that the function given by the \attrib{math}
field is false.
...
As an example the constraint that species $S_1$ should only have
values between 1 and 100 could be encoded as:
...
\begin{blockChanged}
\subsubsection{Reaction as Symbol}
...
present.  A \class{KineticLaw} expression cannot contain reaction
symbols. This restriction means that it is not possible to either
create algebraic loops amongst \class{KineticLaw} expressions or
to require a specific order of evaluation of kinetic laws.
\end{blockChanged}
...
containing a \attrib{kind} attribute value of \texttt{mole} or
\texttt{item} and \texttt{exponent} attribute value of 1 (can
default to 1).  See section~\ref{sec:built-in-units}.

\sbmlrule{1203}
\item If the \attrib{id} attribute of a
\class{unitDefinition} element contains \texttt{length} then the element
must simplify to a single \class{unit} element containing a
\attrib{kind} attribute value of \texttt{metre} and \attrib{exponent}
attribute value of 1 (can default to 1).  See
...
must simplify to a single \class{unit} element containing a
\attrib{kind} attribute value of \texttt{metre} and a \attrib{exponent}
attribute value of 2.  See section~\ref{sec:built-in-units}.

\sbmlrule{1205}
\item If the \attrib{id} attribute of a
\class{unitDefinition} element contains \texttt{volume} then the element
must simplify to a single \class{unit} element containing a
\attrib{kind} attribute value of \texttt{litre} or \texttt{metre}. See
section~\ref{sec:built-in-units}.

\sbmlrule{1206}
\item If the \attrib{id} attribute of a
\class{unitDefinition} element contains \texttt{volume} then if the
element must simplifies to \texttt{litre} then the effective
...
\item If the \attrib{id} attribute of a
\class{unitDefinition} element contains \texttt{volume} then if the
element must simplifies to \texttt{metre} is used then the effective
...
must simplify to a single \class{unit} element containing a
\attrib{kind} attribute value of \texttt{second} and \attrib{exponent}
attribute value of 1 (can default to 1).  See
section~\ref{sec:built-in-units}.
...
\sbmlrule{1405}
\item If the spatial dimensions of a
\class{species} element is 1 then the \attrib{spatialSizeUnits}
attribute must a value of `\texttt{length}', `texttt{metre}' or
the \attrib{id} value of a \class{unitDefinition} element that
defines a variant of `texttt{metre}' with an exponent value of 1.
See Section~\ref{sec:species-units}.

\sbmlrule{1406}
\item If the spatial dimensions of a
\class{species} element is 2 then the \attrib{spatialSizeUnits}
attribute must a value of `\texttt{area}' or the \attrib{id} value
of a \class{unitDefinition} element that defines a variant of
`texttt{metre}' with an exponent value of 2. See
Section~\ref{sec:species-units}.

\sbmlrule{1407}
\item If the spatial dimensions of a
\class{species} element is 3 then the \attrib{spatialSizeUnits}
attribute must a value of `\texttt{volume}', `texttt{litre}' or
the \attrib{id} value of a \class{unitDefinition} element that
defines a variant of `texttt{metre}' with an exponent value of 3.
See Section~\ref{sec:species-units}.

\sbmlrule{1408}
\item The \attrib{substanceUnits}
attribute must contain either \texttt{substance}, \texttt{item},
\texttt{moles} or the values of \attrib{id} attributes of
\class{unitDefinition} elements that define variants (i.e. have
only arbitrary \attrib{scale}, \attrib{multiplier} and
\attrib{offset} values) of \texttt{item} or \texttt{moles}.  See
Section~\ref{sec:species-units}.
