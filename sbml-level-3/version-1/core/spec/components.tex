  ...
  \hspace*{-1ex}\begin{tikzpicture}[level distance=1in]
    \node { \emptyClassbox{\textsl{SBase}} }
     ...
     \draw[diamond-,shorten >=-6pt] (b) -- (c) 
       ...
       node[above=-6pt,left=1.2in] {\textsf{0..*}};
  \end{tikzpicture}
...
by the attribute \token{units}.  The value assigned to
the local parameter's \token{units} attribute must be chosen
from one of the following possibilities: one of the base unit
identifiers from Table~\vref{tab:unitkind}; one of the predefined
unit identifiers appearing in first column of
Table~\vref{tab:predefined}; or the identifier of a new unit defined
...
When the local parameter identifier appears in the rate law, the units associated
with the value are those declared by the local parameter's \token{units} attribute.
...
The \LocalParameter object inherits from \SBase the optional \token{sboTerm} attribute of
type \primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this attribute in a
