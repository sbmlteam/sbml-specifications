...
Section~3.1 of the \sbmlthreecore specification defines a number of primitive data types and also uses a number of XML Schema 1.0 data types~\citep{biron:2000}.  More specifically we make use of \primtype{integer}, \primtype{double}, \primtype{string} and \primtype{SIdRef}. In addition we make use of four new primitives \primtype{FbcSId}, \primtype{FbcSIdRef} as well as the enumerations \primtype{FbcType} and \primtype{FbcOperation}, see \ref{fig:fbc_uml} for the interrelation between these entities.
...
This attribute is of type \primtype{FbcSIdRef} and can only refer to an existing \Objective \primtype(FbcSId). This required attribute exists so that when multiple \Objective's are included in a single model, the model will always be well described i.e. there is a single, primary objective function which defines a single optimum and its associated solution space.

\subsubsection{A note on units}
\label{fbcunits}
The only unit definitions that need to be  considered when using the \FBCPackage are the global model definitions of ``extent''  and ``time'' as all \FBC flux related objects i.e. \FluxBound and \FluxObjective have the same implicit unit as the \Reaction that they refer to.
...
Similarly, there is also the potential for a bound to ``seemingly'' conflict with the reaction that it bounds' reversibility, e.g. a reaction is irreversible but has bounds $-\infty <= J <= \infty$. While in this particular case it may seem as if the reversible attribute should be authoritative, the reverse case invalidates this as a general approach i.e. a reaction that is labelled reversible can also be bound such that it carries only a net forward/reverse flux (e.g. $0 <= J <= \infty$). Therefore, in the context of the \FBCPackage, a \FluxBound should be considered authoritative while it is left to the software implementation to deal with obvious inconsistencies.
