...
MathML operators in SBML return results of two types: boolean and numeric.
...
\item the arguments of the logical operators; and
...
optimized by performing replacements on the operators when
certain conditions apply.  Specifically, the operator can be
replaced by the 'base operand' if the operator has no
operands; or by an operand if the operator has only
one operand (i.e.,  the operator is equivalent to identity).
\end{blockChanged}
...
provide a mechanism allowing a modeler to express the validity
of a model's state during numerical analysis.
