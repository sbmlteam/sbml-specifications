...
The attributes have to refer to an existing \Parameter in the model. In the case that equal bounds are required for a reaction, both attributes should point to the same \Parameter.
...
\newtxt{The \FBCPackage defines a \GeneProteinAssociation class that derives
...
one or more genes (or gene products). Where more than one gene (or gene product) is present in an association they can then be written as logical expressions and thereby related to one another using logical `and' and `or' operators.}
...
\newtxt{The \FBCPackage defines an abstract \Association class that is
...
\newtxt{The \FBCPackage defines a \GeneProductRef class that references
a gene (or gene product) declared in \ListOfGeneProducts, a child of
\Model. It is derived from an \Association and thereby inherits the
\SBase attributes \token{metaid} and \token{sboTerm}, as well as the
...
\newtxt{The \FBCPackage defines an \GeneAnd class that is derived from
...
\newtxt{The \FBCPackage defines an \GeneOr class that represents a gene
