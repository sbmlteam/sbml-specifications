...
\token{listOfExternalModelDefinitions} lists.  No loops are allowed: it
...
references to time within that \Submodel must also be modified according
to the \token{timeConversionFactor} attribute.  The effect is multiplicative:
if that \Submodel had its own \token{timeConversionFactor},  references
to time within that \Submodel would then be converted according to the
...
reference to extent in a package. See \sec{conversion-factors} for more details.

If the \Submodel itself references a \Model with its own \Submodel element,
references to extent within that \Submodel must also be modified according
to the \token{extentConversionFactor} attribute.  The effect is multiplicative:
if that \Submodel had its own \token{extentConversionFactor},  references
to extent within that \Submodel would then be converted according to the
two \token{extentConversionFactor} elements multiplied together.
...
the \token{id} attribute whose types inherit from the SID primitive data
type) or the meta identifier (i.e., the \token{metaid} attribute or any
...
\item In mathematical formulas, MathML \token{<cn>} elements that refer
...
\token{<cn>} element in the MathML of a Kinetic Law, it is necessary to
examine the local parameters of the Kinetic Law's parent Reaction
object.  Whether the \token{<cn>} element should be considered to point
...
if that \ReplacedElement had its own \token{conversionFactor}, references
...
\item \emph{Identifiers must be conserved}.  If a replaced element
  ...
  (or their derivatives, such as \token{UnitSIdRef} or the proposed
  \token{SpIdRef}), the replacement element must also define those
