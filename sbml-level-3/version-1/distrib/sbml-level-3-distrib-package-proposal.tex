...
class \class{externalDefinition} or \class{explicitPDF} or
\class{explicitPMF}.
...
As outlined above the \FunctionDefinition is extended to contain the
following classes from \distribshort.
...
This declares a statistical distribution by referring to an external
normative definition. \uncertml is the recommended source of such
definitions. The attributes of the class are defined below.
...
This class defines a probability density function (PDF) explicitly and
so provides a continuous probability distribution that can be
sampled. The PDF is defined as a mathematical function using
\mathml. The distribution can be truncated.

\paragraph{truncated}

See the truncated attribute in section \ref{sec:truncatt}.

\subsection{explicitPMF}

This class defines a discrete probability distribution described by a
user-defined probability mass function (PMF). Here the PMF is defined
as a category and its associated probability.
...
The recommended external definition for distributions is \uncertml. We
have been liaising with the \uncertml team to ensure that they meet
the above criteria.
...
satisfy the \emph{validity upon reduction} rule for packages. This
rules states that the SBML document must be syntactically valid if all
package specific elements are removed from it. To ensure that this is
the case the fallback function used in relation to \distribshort must
