...
Note that \FunctionDefinition does not have a separate field for
defining the units of the value returned by the function.  The
...
The reason is that having a separate field for declaring the units
would not only be redundant, but also lead to the potential for
having conflicting information.  If the declared units were
incorrect relative to the actual value returned by the function,
the only logical conflict-resolution rule would be to assume that
the correct units are those of the expression anyway.)
...
\FunctionDefinition inherits an optional \token{sboTerm} field of type
...
The \Parameter structure inherits from \SBase the optional \token{sboTerm} field of
...
\InitialAssignment inherits from \SBase an optional \token{sboTerm} field of type
\primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
...
and all the fields from \SBase, including \token{sboTerm}.
...
The \Rule structure inherits from \SBase the optional \token{sboTerm} field of type
\primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
...
The \Constraint structure inherits from \SBase the optional \token{sboTerm} field of
type \primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this field in a
...
\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.8]{figs/reaction-uml}
  \caption{\changed{The definitions of \Reaction, \KineticLaw,
      \SpeciesReference and \ModifierSpeciesReference.  Following
      UML notation fields that are inherited from a base class are
      not shown.}}
  \label{fig:reaction}
\end{figure}
...
The \Reaction structure inherits from \SBase the optional \token{sboTerm} field of
type \primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this field in a
...
\ModifierSpeciesReference.  \changed{It is used simply to hold the fields
...
\token{species} and optional fields \token{id} and \token{name}
...
\token{math} and \token{parameter}, in addition to the fields 
it inherits from \SBase.
...
\KineticLaw inherits from \SBase the optional field called
...
\token{name} fields, but in the case of \Event, both are optional.
...
\paragraph{The \token{sboTerm} field on \class{Event}}
\label{sec:event-sboterm}

\changed{As with all SBML components derived from \SBase, an
\Event has an optional field \token{sboTerm} of type
...
As shown in Figure~\ref{fig:event}, the \token{trigger} field of
an \Event must contain exactly one object of class \Trigger.  This
object contains one \token{math} field containing a MathML
...
\paragraph{The optional \token{sboTerm} field on \class{Trigger}}
\label{sec:trigger-sboterm}

As with all SBML components derived from \SBase, \Trigger inherits
the optional field \token{sboTerm} of type \primtype{SBOTerm} (see
...
an optional \token{delay} field of class \Delay.  The \Delay is
...
\paragraph{The optional \token{sboTerm} field on \class{Delay}}

As with all SBML components derived from \SBase, \Delay inherits
an optional \token{sboTerm} field of type \primtype{SBOTerm} (see
Sections~\ref{sec:sboterm-type} and~\ref{sec:sboTerm}).  When a
...
\Event contains a \token{eventAssignment} field, which must
...
contains a required field, \token{variable}, a \token{math} field,
...
\paragraph{The optional \token{sboTerm} field on \class{EventAssignment}}
...
contained in the \token{math} field of \EventAssignment's field
must be consistent with the units of the object identified by the
\token{variable} field.   More precisely:
\begin{itemize}
...
\item \emph{In the case of a parameter}, an \EventAssignment sets
  ...
  \token{variable} field.  (See Section~\ref{sec:parameter-units}
