...
\begin{blockChanged}
\validRule{comp-21011}{A \ReplacedElement may not define a \token{conversionFactor} for a \token{deletion}, and therefore both of those attributes may not be set on the same \ReplacedElement object.
  (References: SBML Level~3 Package Specification for Hierarchical Model
  Composition, Version~1, \sec{replacedelement-conversionfactor}.) }
\end{blockChanged}
