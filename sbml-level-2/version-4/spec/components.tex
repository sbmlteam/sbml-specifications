...
\begin{figure}[htb]
    ...
    \node { \emptyClassbox{\textsl{SBase}} }
      ...
      child {node (a) {
          \begin{classbox}{Sbml}
            ...
            version: positiveInteger \{ use="required" fixed="3" \} \\
          \end{classbox}
        }}
      child {node (b) {
          \emptyClassbox{Model}
  ...
  \end{tikzpicture}
  \caption{The definition of \Sbml for \sbmltwothree.
...
used within the \token{sbml} element.  The URI for SBML Level~2 Version~3
is \uri{http://www.sbml.org/sbml/level2/version3}.  All
...
Versions~2 and~3.  Modelers and software authors are
...
interoperability.  \twotwo and Version~3 acknowledge this reality by reducing
...
<functionDefinition id="Fahrenheit_to_kelvin">
  ...
  \sbmltwotwo and Version~3.  It can be used for other temperature units
...
\sbmltwothree---they have no effect on a model's
...
If the \token{constant} attribute is \changed{\val{true}},
then \changed{the value of the species' quantity}
...
\sbmltwothree provides two ways of assigning initial values to
...
is set to \val{true}.  \sbmltwothree therefore stipulates that if a
...
and Version~3 removed these attributes for several reasons.  First, the introduction
...
identical to a global \changed{identifier} defined in the model takes
precedence over \changed{the value associated} \changed{with
the global identifier.  Note that this introduces the potential
for a local parameter definition to shadow a} \changed{global identifier
\emph{other} than a parameter.  In SBML's simple symbol system, 
there is no separation of symbols} \changed{by class of object;
consequently,  inside the kinetic law mathematical
formula, the value of a local parameter} \changed{having the same
identifier as a species or compartment or other global model
entity will override the global} \changed{value.  Modelers and software
...
\begin{linenomath}
\begin{equation*}
  R_1 = r_1 \cdot V_1 = \changed{k_1} \cdot [S_1] \cdot V_1 \\
\end{equation*}
\end{linenomath}
The second rate expression becomes
\begin{linenomath}
\begin{equation*}
  R_2 = r_2 \cdot V_2 = \changed{k_2} \cdot [S_2] \cdot V_2 \\
\end{equation*}
\end{linenomath}
...
An \Event definition has \changed{two} required parts: \changed{a
...
lack of such an attribute on other \sbmltwothree components involving
...
instance defined in the model.  When the event \changed{is executed}, the value of
