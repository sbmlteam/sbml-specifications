    ...
    \node { \emptyClassbox{\textsl{SBase}} }
      ...
      child {node (a) {
          \begin{classbox}{UnitDefinition}
            id: UnitSId \{ Excludes values from Table~\ref{tab:unitkind} \}         \\
            name: string \{ use="optional" \} \\
          \end{classbox}
        }}
      ...
      child {node[left=-1.1in] (c) { 
          \begin{classbox}{Unit}
            kind: \changed{UnitSId} \{ Restricted to values in Table~\ref{tab:unitkind} \}  \\
...
The \Unit data structure has one required \changed{attribute},
...
For example, the following \Event instance would fail model
validation: the expression inside the \token{math} element does
not have any declared units, and what is required in this context
is units of time:
