...
\newcommand{\Distribution}{\absDefRef{Distribution}{distributionOrSample-class}}
\newcommand{\AbstractUncertainty}{\absDefRef{AbstractUncertainty}{abstractuncertainty-class}}
...
\packageVersion{Version 0.13 (Draft)}
\packageVersionDate{XX July, 2013}
...
0.13 (Draft) & XX July 2013 & Lucian Smith and Stuart Moodie & Modified based PWG discussion, particularly with respect to UncertML.\\
\bottomrule
\end{edtable}
...
Finally, the author(s) would encourage the
...
In order to perform truncation, one or both ends of the distribution are cut off, and the remaining function is re-scaled so the area under the curve is once again 1.0.  Earlier versions of this specification allowed one to specify truncation values for UncertML distributions and explicit PDFs:  upper and lower limits to the distributions, between which values must be sampled.  However, this capability is being introduced in UncertML 3.0, making truncation elements superfluous for \Distribution elements.  Truncation was never considered for inclusion for explicit PMF functions, as truncating a sample is as simple as not including the rejected realizations.  Therefore, since its capabilities are duplicated elsewhere, truncation-specific elements have been dropped from this version of the specification.
...
No inputs are provided.  The four \token{probabilities} children of the \token{CategoricalDistribution} all have equal values and sum to 1.0, as they must.  Each entry in the \token{values} vector are therefore equally likely to be chosen, resulting in this function returning \val{1}, \val{2}, \val{3}, or \val{4}, each with equal probability.
...
Because the \AbstractUncertainty class is abstract, the name of this element will be the same as the name of the derived class in UncertML:  \val{NormalDistribution}, \val{StatisticsCollection}, \val{Variance}, etc.
...
In these examples, a parameter is given an \AbstractUncertainty child to describe its uncertainty in two ways:

\begin{example}
...
  <species id="x" compartment="C" boundaryCondition="false" initialAmount="3.22"
           hasOnlySubstanceUnits="false" constant="false">
    <distrib:uncertainty>
      <StatisticsCollection xmlns="http://uncertml.org/3.0">
        <NormalDistribution>
          <mean> 3.2 </mean>
...
defining the uncertainty about the \emph{initialAmount} of the
species, not the species itself. For example if you
define the uncertainty, but not the initialAmount, what does
the uncertainty describe in this case? If you can only describe the
uncertainty of a parameter then this problem goes away if the
...
...
  <species id="x" compartment="C" boundaryCondition="false" initialAmount="ic"
           hasOnlySubstanceUnits="false" constant="false"/>
...
\end{example}
...
{\color{red}Lucian: In SBML, the only difference between Species and Parameters is that Species can appear in Reactions.  Otherwise, they have exactly the same issues with initial values vs. changing in time.  As such, it doesn't actually gain you anything to restrict the 'uncertainty' element to only be a child of parameters.  However, you're right that the initial value vs. time-variance value needs to be addressed; I've added some text in the \AbstractUncertainty section to this effect.  Is it clear?}

%\subsubsection{Combining \AbstractUncertainty and \UncertMath}

%This example is the same as the previous one, but uses \UncertMath to reference a \FunctionDefinition defined as a normal distribution, and \AbstractUncertainty to note the standard deviation:
...
%           hasOnlySubstanceUnits="false" constant="false">
%    <distrib:uncertainty>
%        <StandardDeviation xmlns="http://uncertml.org/3.0">
%          <values> 0.3 </values>
%        </StandardDeviation>
%      <math xmlns="http://www.w3.org/1998/Math/MathML">
%        <apply>
...
%Again, the \AbstractUncertainty notes that the standard deviation is 0.3, and the \UncertMath notes that the mean is 3.2 and the variance is 0.09.
...
{\color{red}%
  Lucian: I've updated everything to use the UncertML-defined 'AbstractUncertainty' and 'Distribution' instead of 'uncertml'.  Thanks!}
