...
and \token{encoding} attributes.  \thisL uses XML version 1.0 and
requires a document encoding of UTF-8.  Following this
...
attributes (\token{level} and \token{version}), an optional
\token{listOfPackages} element, and a required \token{model}
element.
\todo{MH}{may need changing}

\begin{figure}[htb]
  \centering
  \small
  \includegraphics[scale=0.8]{figs/sbml-uml}
  \caption{The definition of \Sbml for SBML \thisLV.  The class
    \Model is defined in Section~\ref{sec:model}.  Note that
    \Sbml, \Model and \ListOfPackages are all subclasses of
    \SBase, and therefore inherit the attributes of that abstract
    class.}
  \label{fig:sbml}
\end{figure}
...
Readers may wonder why the SBML top-level XML element uses
\emph{both} a namespace URI identifying the SBML level and
version, as well as separate XML attributes giving the level and
version.  Why is the information duplicated?  There are several
reasons.  First, XML is only one possible serialization of SBML
(albeit an extremely popular one at this time).  Though most of
this document is written with XML in mind, it is the intention
behind the design of SBML that its object structure should be
...
from a string.  Second, generic high-level XML parsers may not
give their calling programs access to the value of the
\texttt{xmlns} attribute.  Providing the information via separate
attributes is a good backup measure.  And finally, earlier in the
history of SBML, it was expected that only the level needed to be
encoded as part of the namespace URI (e.g.,
...
\subsubsection{The \token{listOfPackages} element}

As shown in Figure~\ref{fig:sbml}, the SBML document may contain
an optional \token{listOfPackages} element, which (if present)
must contain one or more instances of \Package object.  Each of
the latter constitutes a \emph{package reference}; this is a
declaration that the current SBML document makes use of features
defined in a named SBML Level~3 package.
Section~\ref{sec:package} explains the SBML Level~3 package
concept in more detail.

It is important to note that an additional requirement for XML
conformance is the presence of an XML namespace declaration for
every Level~3 package used in an SBML document.  This declaration
can come on the \token{sbml} element (see Figure~\ref{fig:sbml}),
or it can appear wherever the Level~3 package elements and
attributes are used.  Using an SBML Level~3 package requires
\emph{both} declaring the XML namespace, and referencing the
package in the \token{listOfPackages} element.
...
\label{sec:listofpackages}

The \ListOfPackages class defined in Figure~\ref{fig:sbml} and the
various other \ListOf classes defined in Figure~\ref{fig:model}
...
can declare which packages they support.  The mechanism for
indicating which packages are used in a given SBML model is a list
of \Package object definitions.  The definition of \Package is
shown in Figure~\vref{fig:package}.  A list of one or more
instances of \Package objects can be located in an instance of
\ListOfPackages in \Sbml, as shown in Figure~\vref{fig:sbml}.
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \node (b) {
      \begin{classbox}{\textcolor{black}{Package}}
        namespace: string\\
        required: boolean\\
      \end{classbox}
    };
  ...
  \end{tikzpicture}
  \caption{The definition of \Package.}
  \label{fig:package}
\end{figure}

As mentioned in Section~\ref{sec:sbml}, in the XML encoding of
SBML, an additional requirement exists when including Level~3
packages in a model: the XML namespace for the package must also
be declared.  This declaration is often most conveniently made on
the top-level \token{sbml} element, but XML also permits the
declarations to appear on the individual elements where Level~3
package elements and attributes are used.  Regardless of which
approach is chosen, every package used in an SBML Level~3 package
must be declared \emph{both} using an instance of a \Package
object and an XML namespace declaration.


\subsubsection{The \token{namespace} and \token{required} attributes}
\label{sec:package-attributes}

Every SBML Level~3 package is identified uniquely by an XML
namespace URI.  In an instance of a \Package object appearing in a
model, the \token{namespace} attribute must be set to this
namespace URI string.  The URI must be the same string that is
used in the XML namespace declaration described above.  The reason
for the duplication is that there is no way to use the XML
namespace declaration alone to indicate whether a software tool
must be able to understand and use a given SBML Level~3 package to
fully interpret a model.  This information is carried by the
\token{required} attribute on a \Package object instance.

The \token{required} attribute value indicates whether the package
is in fact required for complete mathematical interpretation of a
model, or whether the package is optional.  A value of
\token{required}=\val{true} indicates that interpreting the
package is required.  If a package is declared optional, it means
that the elements and attributes added by the package can be
ignored without any loss of meaning of the model's full
mathematics.  ``Ignoring'' a package can be accomplished in
multiple ways; a reader could either skip those elements or
attributes altogether during parsing, or read them but not
interpret them, or similar.

No declaration is necessary for the Level~3 Core package, since it
is the base package and support for it is required in any case.  A
model that uses only Level~3 Core and no package would not need a
\token{listOfPackages} element at all.
...
\subsubsection{The \token{extendUnits} attribute}
\label{sec:model-extendUnits}
\todo{SH}{Write me.}
...
referenced throughout a model.
% In addition, every kind of SBML
...
The \token{exponent} attribute on \Unit represents an exponent on
the unit.  A \Unit object also has a \token{scale} attribute; its
...
because $10^0 = 1$.  Lastly, the \token{multiplier} attribute can
be used to multiply the \token{kind} unit by a real-numbered
factor; this enables the definition of units that are not
power-of-ten multiples of SI units.  For instance, a
\token{multiplier} of 0.3048 could be used to define \val{foot} as
a measure of length in terms of a metre.
...
Level~1.)  A missing value for \token{size} for a given
...
other construct except \InitialAssignment.  The default value for the
...
values and defaults to \val{false}.  This attribute's role is to
...
When a species definition has the attribute setting
\token{hasOnlySubstanceUnits}=\val{false} and the size of the
compartment in which it is located changes, the default behavior
in SBML is to \emph{preserve the amount}.  Upon such changes, the
value of the concentration or density must be recalculated from
the simple relationship \emph{concentration} =
...
\EventAssignment, it means that the \emph{concentration} is set by
the rule or event; in that case, it is the amount that needs to be
...
assignments could conceivably reassign both a species quantity and
a compartment size simultaneously.
Section~\ref{sec:eventassignment} describes the handling of
species in event assignments.)

Note that the above only matters if a species has the attribute
\token{hasOnlySubstanceUnits}=\val{false}.  If the attribute's
value is \val{true}, then the identifier of the species always
stands for an amount wherever it appears in a mathematical formula
or is referenced by an SBML construct.
