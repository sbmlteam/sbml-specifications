...
\changed{Unless otherwise specified, all SBML documents must follow the XML specification:  anything allowed in XML is allowed here, and anything disallowed in XML is disallowed here.
...
ellipses \changed{(\val{...})} are used to indicate content elided from this example):
...
\changed{An SBML document may contain at most one model definition}.
...
\fig{fig:model}.  All of the lists are optional, \changed{and, if present, may
    ...
    \node (b) {
      \begin{blockChanged}
        \begin{classbox}{\textcolor{black}{Compartment}}
          \textcolor{black}{id: SId \{ use="required" \}}                   \\
...
The identifier (the \token{id} attribute value) of a \Compartment object may be used in a model's mathematical expressions.  The identifier stands for the value of the compartment's \token{size} attribute, with a unit of measurement as defined in \sec*{sec:compartment-units}.  A compartment's identifier may be the target of an \InitialAssignment, \EventAssignment, or \Rule in the model, to set or redefine the value of the compartment size.
...
The identifier (the \token{id} attribute value) of a \Species object may be used in a model's mathematical expressions.  The identifier stands for the given species' amount or concentration, depending on the value of its \token{hasOnlySubstanceUnits} attribute as described below, with units of measurement as described in \sec{sec:species-substanceunits}.  A species' identifier may be the target of \InitialAssignment, \EventAssignment, or \Rule objects in a model, to set or redefine the value of the species amount or concentration.
\end{blockChanged}
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \draw node[right=1.25in,above=0in] {\changed{\textsf{0..1}}};
  \end{tikzpicture}
...
\InitialAssignment inherits an optional \token{id} attribute of type \primtype{SId} from \SBase.  The identifier of an \InitialAssignment has no mathematical meaning in an SBML \thisLV model.
...
\changed{An \InitialAssignment with no \token{math} child leaves undefined what assignment is to be made to the corresponding \token{symbol}.  A package may define how to include extra information that would define how to apply this \InitialAssignment; otherwise, no assignment is carried out.}
...
The classes derived from \Rule inherit \token{math} and the attributes and elements \changed{that \Rule itself inherits from \SBase, including \token{id}, \token{name}, and \token{sboTerm}}.
...
\Rule inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  The identifier of a \Rule-derived object has no mathematical meaning in \changed{an \thisLV} model.
...
\changed{A \Rule with no \token{math} child leaves undefined how the rule behaves mathematically.  A package may define how to include extra information that would define how to apply the \Rule; otherwise, no change to the model is carried out.}
...
\begin{blockChanged}
Finally, any symbol that appears as the target of a \emph{rateOf} \token{csymbol} may not be determined by an \AlgebraicRule.  This is because the \emph{rateOf} \token{csymbol} is defined as applying only to symbols whose rates of change are easily determinable.
\end{blockChanged}

\begin{blockChanged}
Because these rules indicate what symbols may \emph{not} be determined by an \AlgebraicRule, they can thus also be used to discover what symbol \emph{is} being determined by an \AlgebraicRule.  If three symbols appear in the \token{math} element of an \AlgebraicRule, the first of which is flagged \token{constant}=\val{true}, and the second of which appears as the target of a \emph{rateOf} \token{csymbol}, one may conclude that the \AlgebraicRule must be used to determine the value of the third symbol.  This is, in fact, a principle use (outside of validation) of the \token{constant} attribute: its use in allowing software to properly identify the dependent variable in an \AlgebraicRule.
\end{blockChanged}
...
\token{variable} attribute) of an assignment rule \changed{is of type \primtype{SIdRef}, and must refer to an SBML object in the \primtype{SId} namespace with mathematical meaning and the ability to be assigned.  In SBML Level~3 Core, this consists of}
\Species, \SpeciesReference, \Compartment, 
\changed{and} global \Parameter objects in
the model (but not reactions \changed{nor function definitions}).  The entity identified must
...
(the \changed{required} \token{variable} attribute) of a rate rule  \changed{has type \primtype{SIdRef}, and must refer to an SBML object in the \primtype{SId} namespace with mathematical meaning and the ability to be assigned.  In Level~3 Core, this consists of}
\Species, \SpeciesReference, \Compartment, 
\changed{and} global \Parameter objects in
the model (but not reactions \changed{nor function definitions}).  The
...
\begin{figure}[htb]
  ...
  \caption{The definition of class \Constraint.  The contents of
    ...
    \changed{and will be interpreted as} a Boolean value.  As shown above, an
...
\val{true} \changed{(or any non-zero numerical value)} when the model is in a \emph{valid} state.  The formula
...
\changed{A \Constraint with no \token{math} child does not define a mathematical constraint.  A package may define how to include extra information that would define how to apply the \Constraint; otherwise, no restriction on the model's behavior is implied.}
...
declaration \changed{nor} a DOCTYPE declaration.
...
\changed{inherited by \SpeciesReference and
...
\KineticLaw inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of a \KineticLaw takes on no mathematical meaning; the value of that \token{math} element is instead associated with the enclosing \Reaction object's identifier.
\end{blockChanged}
...
\changed{A \KineticLaw with no \token{math} child leaves undefined the mathematics of the corresponding \Reaction.  A package may define how to include extra information that would define how to apply the \KineticLaw; otherwise, the model behaves mathematically as if no \KineticLaw was defined at all.  This is sometimes useful when the modeler desires to define one or more \LocalParameter objects, which live in the \KineticLaw.}
...
\KineticLaw object \changed{(as well as within the parent \Reaction)}, a local parameter whose identifier is
...
The identifier (the \token{id} attribute value) of a \LocalParameter may be used in the mathematical expression within the enclosing \KineticLaw object.  The identifier stands for the value of the parameter's \token{value} attribute, and within the scope of a \KineticLaw object \changed{and within the scope of the \Reaction itself}, a local parameter's identifier shadows any identical identifiers from the \primtype{SId} namespace of the model.  Because of its limited local scope, the identifier also cannot be the target of an \InitialAssignment, \EventAssignment, or \Rule object in the model.  The units of measurement associated with the identifier are described in \sec{subsec:localparameter-units}.

Note that a \LocalParameter must not take the \token{id} of a \Species referenced by any of its parent \Reaction's \SpeciesReference elements: the principle by which a \LocalParameter shadows any global \token{SId} would mean that the \SpeciesReference would now be referring to a \LocalParameter, which is not, of course, a \Species.

The effective scope of a \LocalParameter \token{id} in \sbmlthree Core is therefore restricted to the \token{math} of the \KineticLaw, since no other construct within a \Reaction references an \token{SId}.  However, \sbmlthree packages may use this fact to introduce new \token{SId} references within a \Reaction that could refer to a \LocalParameter.  In some cases, this could mean that a \KineticLaw is present in the \Reaction with no child \token{math}, but with a \ListOfLocalParameters to provide a new package constuct with locally-scoped parameters to reference.
...
As shown in \fig{fig:event}, an \Event object \changed{may} contain
...
The expression in the \token{math} element \changed{will be interpreted as} a
...
Similarly, a \Trigger with no \token{math} child leaves undefined when the corresponding \Event will trigger.  A package may define how to include extra information that would define how to apply this \Trigger; otherwise, the event is never triggered.
\end{blockChanged}
...
\changed{Similarly, a \Priority with no \token{math} child leaves the priority of the event undefined.  A package may define how to include extra information that would define how to apply this \Priority; otherwise, the event behaves as if it did not have a priority.}
...
\changed{Similarly, a \Delay with no \token{math} child leaves the priority of the event undefined.  A package may define how to include extra information that would define how to apply this \Delay; otherwise, the event is considered to execute as if it had no \Delay.}
...
\changed{An \EventAssignment with no \token{math} child leaves undefined what assignment is to be made to the corresponding \token{symbol}.  A package may define how to include extra information that would define how to apply this \EventAssignment; otherwise, no assignment is carried out.}
