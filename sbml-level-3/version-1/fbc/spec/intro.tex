...
\newtxt{The specification for Flux Balance Constraints Version 1 in SBML Level~3 Version~1 is based on the March 2012 proposal document (Olivier and Bergmann):}
%
\begin{description}
...
\newtxt{\subsection{Relation to previous specifications}}

\newtxt{Version~3 of the \FBCPackage builds upon and maintains backwards compatibility with Version~2. It, primarily, introduces new elements and attributes that include: user-defined constraints, a new variable type and a key-value pair annotation. The only changes to existing attributes is that \Species \token{charge} is no longer restricted to an integer value and \token{chemicalFormula} now allows generic components to be defined.  Previous \FBCPackage specifications:}
%
\newtxt{
\begin{description}
  \item[Specification version 2 (September 2015)]
  \item [] {\small\url{http://identifiers.org/combine.specifications/sbml.level-3.version-1.fbc.version-2.release-1}}
  \item[Specification version 1 (February 2013)]
  \item [] {\small\url{http://identifiers.org/combine.specifications/sbml.level-3.version-1.fbc.version-1.release-1}}
\end{description}
}
