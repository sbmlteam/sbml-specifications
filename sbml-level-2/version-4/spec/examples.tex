...
\changed{$Y_{1c}$}, and $Y_2$) have values in terms of concentrations.
(Readers may wonder why, when their values in the \changed{SBML}
model are given as initial \emph{amounts}.  The reason goes back
to the \Species defaults and the meaning of \changed{the}
\token{hasOnlySubstanceUnits} attribute: if the attribute is not
set and the compartment in which the species \changed{is} located has more
than 0 spatial dimensions, a species' symbol in a model is
interpreted as a concentration \changed{or} density regardless of whether
its initial value is set using \token{initialAmount} or
\token{initialConcentration}.)  We \changed{use} $V_n$ to represent the
size of compartment \val{nucleus} and $V_c$ the
size of compartment \val{cytoplasm}:
\end{blockChanged}
\begin{linenomath}
\begin{equation*}
  V_n = 1, \quad V_c = 5, \quad X = 1, \quad Y_{1n} = 1, \quad Y_{1c} = 0, \quad Y_2 = \changed{1/5}
\end{equation*}
\end{linenomath}
