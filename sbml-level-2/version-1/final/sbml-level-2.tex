...
valid SBML Level~1 document is not a valid SBML Level~2 document and a
...
The equality of \class{SId} values is determined by an exact character sequence match, i.e. in a
...
in a model.  This allows a software package to store its component's
identifiers in an unmodified form in \attrib{name} field even when the
package's internal schema differs significantly from SBML.
For example a species in an SBML model
...
\emph{Assignment}  & left-hand side is a scalar:         & $x = f(W)$\\
...
\item \emph{In the case of a species}, an \class{AssignmentRule} sets the
  ...
  \class{AssignmentRule} structure and a \class{SpeciesReference} structure that refer to
  ...
  more than one \class{AssignmentRule} or \class{RateRule} can refer to a given species in an
...
\item \emph{In the case of a compartment}, an \class{AssignmentRule} sets
  ...
  in \attrib{math}.  The overall units of the formula must be appropriate
  for the compartment; that is, they
  ...
  \emph{Restrictions}: No more than one \class{AssignmentRule} or \class{RateRule} can refer to
...
\item \emph{In the case of a species}, a \class{RateRule} sets the rate of
  ...
  on the \class{Species} structure that defines the species the
  \attrib{boundaryCondition} field is set to ``\attribvalue{true}''.  No
  more than one \class{RateRule} or \class{AssignmentRule} can refer to a given species in an SBML
  model definition.
...
\item \emph{In the case of a compartment}, a \class{RateRule} sets the rate
  ...
  appropriate for the compartment;
  ...
  \emph{Restrictions}: No more than one \class{RateRule} or \class{AssignmentRule} can refer to a
...
\item \emph{In the case of a parameter}, a \class{RateRule} sets the rate
  ...
  \emph{Restrictions}: No more than one \class{RateRule} pr \class{AssignmentRule} can refer to a
  given parameter.
...
This model is a fragment of a larger model of
calcium regulation across the plasma membrane. The
model includes a calcium influx channel, \texttt{Ca\_channel},
and a calcium-extruding PMCA pump, \texttt{Ca\_Pump}. The
model also includes two cytosolic proteins that buffer
calcium via the \texttt{CalciumCalbindin\_gt\_BoundCytosol}
and \texttt{CalciumBuffer\_gt\_BoundCytosol} reactions.
...
\item \emph{Connections}.  This will be a mechanism for describing the
  ...
  possible to create a 2-D array of compartments and then a 4-D array of
...
to be labelled with instances of controlled vocabularies.  The model vocabulary
will describe the SBML features used by the model.
...
\item \emph{Rules for Initial Conditions}.
This will enable the encoding of rules that are only executed at the start of a simulation
i.e. set initial conditions.  These rules will be similar to assignment rules and are evaluated
in the same sequence as assignment rules but only in the very first instance of simulation.

\item \emph{Tie-breaking algorithm}
This will include a controlled vocabulary and associated attribute on a model that indicates
the simultaneous event tie-breaking algorithm that is required to correctly simulate the model.
\end{itemize}
