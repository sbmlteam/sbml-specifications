...
the first phase when the event is \emph{fired} and the second
...
time at which the event \emph{fires}, and the time at which its
assignments are \emph{executed}.  Similarly, it is also possible
to distinguish between the time at which the mathematical
...
the \EventAssignment's \token{variable} attribute.  Specifically,
a model could intend the \EventAssignment expression to be
evaluated either at the time the event is \emph{fired}, or at the
time the event assignments are actually \emph{executed}.  (In the
former case, a model interpreter would have to save the calculated
values until the time of execution.)
...
be the time point when the \Event is \emph{fired}.
...
event will fire again at any subsequent time points when the
trigger makes the transition from \val{false} to \val{true}; in
other words, an event can fire multiple times during a simulation
...
Sometimes, in the time interval between when an \Event object
\emph{fires} (\ie its trigger expression transitions in value from
\val{false} to \val{true}) and the time at which its assignments
are to be \emph{executed}, conditions in the model may change such
that the trigger expression transitions back from \val{true} to
\val{false}.  Should the event's assignments still be made if
...
\val{true}, then once the event is fired, all of its assignments
...
expression does not have to be re-checked after it fires if
...
\val{false} at any time between when the event fired and when it
is to be executed, the event is no longer considered to have fired
...
event is fired, but this then constitutes a whole new event
fire-and-execute sequence.)
...
multiple events therefore can lead to situations where the
execution of one event affects another event's trigger expression
value.  If that other event has \token{persistent}=\val{false},
and its trigger expression evaluates to \val{false} before it is
to be executed, the event must not be executed after all.


\paragraph{The \token{initialValue} attribute on \Trigger}
\label{sec:trigger-initialvalue}

As mentioned above, an event \emph{fires} when the mathematical
...
In order to determine whether an event may fire at $t = 0$, it is
...
means that the trigger expression is assumed to have the value
\val{true}.  If that is the case, the trigger cannot transition in
value at the moment simulation begins, and can only make the
transition from \val{false} to \val{true} sometime after $t = 0$.
Conversely, if \token{initialValue}=\val{false}, then the trigger
expression is assumed to start with the value \val{false}, and
therefore may fire at $t = 0$ if the expression evaluates to
...
events that happen to be executed simultaneously.  If no \Priority
object is present on a given \Event object, no priority is defined
for that event.
...
sum of the time at which a given event's \Trigger is \emph{fired}
...
events must be fired is dictated by the \Priority objects, as
follows.  If the values of the two \Priority objects' mathematical
formulas differ, then the event having the higher priority value
must be executed before the event with the lower value.  If,
instead, the two priority values are mathematically equal, then
the two events must be fired in a \emph{random} order.  It is
important to note that a \emph{random order is not the same as an
  defined order}: given multiple runs of the same simulation
with identical conditions, an undefined ordering would permit a
simulation system to execute the events in (for example) the same
order every time (according to whatever scheme may have been
implemented by the system), whereas the explicit requirement for
random ordering means that the order of execution in different
simulation runs depends on random chance.  In other words, given
two events ``A'' and ``B'', a randomly-determined order must lead
to an equal chance of selecting event ``A'' first or event ``B''
first, every time those two events are executed simultaneously.
...
priorities must fire in the order determined by their \Priority
...
simulator can choose between events as it wishes.  (Since
priorities only matter with respect to other events with
priorities, a single event object with a priority cannot change
the ordering of a set of simultaneous events.)
...
The expression in a \Priority object's \token{math} element must
be evaluated at the time the \Event is to be \emph{executed}, but
before any other \Event is \emph{executed} at the same moment in
simulation time.  The reason for this is so that the priorities of
events may be usefully compared with one another, and the events
ordered appropriately.  If the execution of one event causes
another event with a defined priority and no delay to fire, its
\Priority object formula must be evaluated before the execution of
any other \Event object currently queued for execution at that
time, and based on the priority value thus calculated, the \Event
object must be inserted into the appropriate position in the queue
of events pending execution.  Software developers should take note
that it is possible for the execution of one \Event object to
cause the \Priority value of a different simultaneously-executing
\Event object to change.  Section~\ref{sec:bp:events} provides
suggestions for possible approaches to implementing support for
this scheme.
...
Section~\ref{sec:event-semantics} below for more information
...
\Event object to \emph{fire}.  Such a transition is not possible
...
simultaneously, and it is possible for events to fire other events
(\ie an event assignment can cause an event to fire).  This leads
to several points:
\begin{itemize}
...
\item Any \Event object whose \Trigger \token{persistent}
  ...
  expression reevaluated after the event has been fired and before
  it is executed.  If its trigger expression now evaluates to
  \val{false}, it must be removed from the queue of events pending
  execution and treated as any other event whose trigger
  expression evaluates to \val{false}.  (Note that it is possible
  it may be triggered again as a result of another simultaneous
  event's actions.)
...
\item Events with defined priorities are executed in the order
  ...
  coin-flipping).  Newly-fired events that are to be executed
...
%   causes a different event with a higher priority to fire and
