...
be valid according to the rules defined in \sect{validation-rules}.
...
\item Find all submodels that do not have a unique identifier (i.e., a
  value for their \token{id} attribute) in the containing model's
...
\item For every submodel identifier ``\emph{M}'', verify that no component
  identifier or meta identifier (i.e., the \token{id} or \token{metaid}
  attribute values of the component) begin with the character sequence
  ``\emph{M\_}''.  For example, if a submodel has identifier
  \val{rocket}, change it (for the purposes of this algorithm) to
  \val{rocket\_}.  If a component has identifier ``\emph{M\_}'' already,
...
\item For every component that has not been removed in the submodel,
  ...
  previous step.  That is, change every \primtype{SIdRef} value to the
  form form ``\emph{M\_id}'', where ``\emph{id}'' is the original
  \primtype{SIdRef} value and ``\emph{M}'' is the submodel identifier,
  and change every XML \primtype{ID} value to the form
  ``\emph{M\_metaid}'', where ``\emph{metaid}'' is the original
  \primtype{ID} value.  \textbf{FIXME: original version unclear; need
    finish this}

\item Verify identifier references.  If any identifier reference (values
  of \primtype{SIdRef} and \primtype{ID}) is found pointing to deleted
