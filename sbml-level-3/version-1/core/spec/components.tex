...
used within the \token{sbml} element.  The URI for SBML Level~3 Version~1
is \uri{http://www.sbml.org/sbml/level3/version1/core}.  All
  ...
  \hspace*{-1ex}\begin{tikzpicture}[level distance=1in]
    \node { \emptyClassbox{\textsl{SBase}} }
      ...
      child {node[left=-1in] (c) { 
          \begin{classbox}{Unit}
            kind: UnitSIdRef$\,$\{\textsl{Restricted to values in Table~\ref{tab:unitkind}}.$\,$\}  \\
            exponent: int      \\
            scale: int         \\
            multiplier: double \\
          \end{classbox}
  ...
  \begin{tikzpicture}
    ...
    \node (b) {
      \begin{classbox}{Compartment}
        id: SId \\
        name: string \{ use="optional" \} \\
        spatialDimensions: int \{ maxInclusive="3" minInclusive="0" \} \\
...
The optional attribute \token{outside} of type \primtype{SIdRef} can be
