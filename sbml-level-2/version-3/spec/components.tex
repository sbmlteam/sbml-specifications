...
MathML \token{lambda} element \changed{or a MathML
  \token{semantics} element containing a \token{lambda} element}.
The \token{lambda} element must
...
in the model.  \changed{This restriction applies to the
...
must match the model's units of \quantity{time} (\ie the
definition of the \val{time} units in the model; see
Section~\ref{sec:built-in-units}).  Note that, as in other cases
of MathML use in SBML, units are \emph{not} predefined or assumed
for the contents of the \token{math} element in the \Delay object;
rather, they must be defined explicitly for each instance of a
\Delay object in a model.  This is an important point to bear in
mind when pure numbers are used in delay expressions.

The following \Event example instance would not satisfy the model
consistency rules in SBML described in this specification and
summarized in Appendix~\ref{apdx:validation-rules}): the
expression inside the \token{math} element does not have any
declared units, whereas what is required in this context is that
the units match the model's units of time:
...
\token{delay} above has \emph{no units declared}.  Instead of
simply entering a literal number in the formula, what must be done
is to use a defined parameter or an expression that yields the
proper units.  Here is an example of using a parameter:

\begin{example}
<model>
    ...
    <listOfParameters>
        <parameter id="oneUnitOfTime" value="1" units="time"/>
    </listOfParameters>
    ...
    <listOfEvents>
        <event>
