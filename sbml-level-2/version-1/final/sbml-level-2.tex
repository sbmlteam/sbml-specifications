...
The Systems Biology Markup Language was originally created partly by studying the modeling facilities in
the following systems: \emph{BioSpice}~\citep{arkin:2001},
...
\emph{StochSim}~\citep{bray:2001,morton-firth:1998}, and \emph{Virtual
  Cell}~\citep{schaff:2000,schaff:2001}.  SBML Level~2 was developed with
the help of these packages' authors and with help and collaboration from
the creators of CellML~\citep{hedley:2001b}.
...
\item If all other situations, the referenced identifier must be the
  ...
  \item \emph{Species identifier}: When a species identifier occurs in a
    ...
    broadly).  The units of concentration of a specific species symbol depends
    on how the species is declared on a \class{Species} structure (see
    Section~\ref{sec:species}).  If the \class{Species} structure has an
    \attrib{initialConcentration} attribute value then the concentration units of the
    species symbol is exactly those given on the \class{Species} structure.
    If the \class{Species} structure has an \attrib{initialAmount}
    attribute value then the concentration units of the species symbol is
    ${species substance}/{compartment volume}$ units where $species substance$
    units those given on the \class{Species} structure and $compartment volume$
    units are those given on the \class{Compartment} structure associated with the
    species.
  ...
  \item \emph{Compartment identifier}: When a compartment identifier occurs
    ...
    The units of size of a specific volume symbol are those given on the
    \class{Compartment} structure that declares the symbol (see
...
of units that are not power-of-ten multiples of SI units, such as minutes
and hours.  For instance, a \attrib{multiplier} of 0.3048 could be used to
define ``foot'' as a measure of length in terms of a metre.  The
\attrib{multiplier} field has a default value of ``\attribvalue{1}'' (one).
Finally, the \attrib{offset} field is used to represent the addition of a
constant in the transformation the \attrib{kind} unit.  For example, an
\attrib{offset} value of ``\attribvalue{32.0}'' would be needed to define
Fahrenheit in terms of degrees Celsius.  The \attrib{offset} field has a
default value of ``\attribvalue{0}'' (zero).
...
Table~\vref{tab:builtin}, corresponding to the five types of quantities that
...
Table~\vref{tab:unitkind}, new units defined in unit definitions, or the
five predefined names ``\unit{substance}'', ``\unit{time}'',
``\unit{length}'', ``\unit{area}'', and ``\unit{volume}'' from
...
\begin{table}[htb]
  ...
  \unit{scale} value of ``\attribvalue{0}''.}
  \label{tab:builtin}
\end{table}

A model may change the default scales by reassigning the keywords
``\unit{substance}'', ``\unit{length}'',
``\unit{area}'', ``\unit{time}'', and ``\unit{volume}'' in a
unit definition.  This takes advantage of the \class{UnitDefinition}
structure's facility for defining scales on units.
The column ``Possible Scalable Units'' in Table~\ref{tab:builtin} defines the set of
units that can be used to re-define the built-in quantities.  \attrib{multiplier} and
\attrib{scale} attributes can be used when re-defining the built-in quantities.
...
\subsubsection{\attrib{id} and \attrib{name} Fields}
...
\subsubsection{\attrib{spatialDimensions} Field}
...
\subsubsection{\attrib{size} Field}

A compartment has an optional floating-point field named
...
the absence of geometry information.  A missing value
for \attrib{size} for a given compartment signifies that the value is
either unknown, not required for analysis, or available from an external
data source.  Note in particular that in SBML Level~2, a missing
\attrib{size} value does \emph{not} imply that the compartment size is 1.
(This is unlike the definition of compartment \attrib{volume} in SBML
Level~1.).

\subsubsection{\attrib{units} Field}

The units of a compartment's size may be explicitly set using the optional field
\attrib{units} in \class{Compartment}.  These are the units of the \attrib{size}
field, the units of the compartment's symbol in MathML expressions and the units of
\class{AssignmentRule} structures determining the compartment's size.
\class{RateRule} structures determining the compartment's size have units
${compartment size}/time$ where $compartment size$ is determined by the \attrib{size}
attribute and $time$ is the built-in unit for time.
The named units in the \attrib{unit} attribute on \class{compartment} structures
must be either one of the base units
from Table~\vref{tab:unitkind}, the built-in defaults named \unit{volume},
\unit{area}, \unit{length}, or a new unit defined by a unit definition in
the enclosing model.  The units chosen must agree with the number of
spatial dimensions of the compartment i.e. the chosen units must be multiplied and/or
scaled variants of litre or cubic metres if \attrib{spatialDimensions} is 3; squared
metres if \attrib{spatialDimensions} is 2 and metres if \attrib{spatialDimesions} is
1.

The default units for a compartment
...
\subsubsection{\attrib{outside} Field}
...
\subsubsection{\class{Compartment} Structures in \class{Model} Structures}
...
Section~\ref{sec:model}.)  Species are required to be located in a compartment
in an SBML model.  Thus, a \class{Compartment} structure must be present in a
model if that model contains a \class{Species} structure.
...
\subsubsection{\attrib{id} and \attrib{name} attributes}

\class{Species} has an \attrib{id} field of type \class{SId} and optional
\attrib{name} field of type \class{string}.

\subsubsection{\attrib{compartment} Field}
...
\subsubsection{\attrib{initialAmount} and \attrib{initialConcentration} Fields}
...
compartment.  These attributes are mutually exclusive and \emph{only one}
of these attributes can have a value on any given instance of a
\class{Species} structure.  Also, \attrib{initialConcentration} must not
have a value if the species' compartment has a \attrib{spatialDimensions}
value of ``\texttt{0}''.  (The reason is simply that there cannot be a
concentration defined when the compartment has no spatial extent.)  Missing
...
The units of a species may be
set explicitly using the optional field \attrib{units}.  These units are
the units of the \attrib{initialAmount} or \attrib{initialConcentration} attributes.
The units of the species symbol in MathML expressions and the units
of \class{AssignmentRule} and \class{RateRule} structures that determine the
value of the species concentration are derived from those specified in
the \attrib{units} attribute of the \class{Species} structure.

The units that can be used in the \attrib{units} attribute depend on whether
amounts or concentrations are being used for a given species:
\begin{itemize}
  
\item \emph{Case of \attrib{initialAmount} being used}. The value assigned
  to \attrib{units} must be chosen from one of the following possibilities:
  ...
  in the enclosing model.  The chosen units must be scaled variants of
  \unit{mole} or \unit{item} units.  If absent, the units default to the value
  set by the built-in ``\attrib{substance}'' of Table~\vref{tab:builtin}.
...
\item \emph{Case of \attrib{initialConcentration} being used}.  The value
  ...
  new unit name defined by a unit definition in the enclosing model.  The chosen units
  must be scaled variants of \unit{mole} or \unit{item} units per $size$ where $size$
  is a scaled variant of litre or cubic metres if \attrib{spatialDimensions} is 3;
  squared metres if \attrib{spatialDimensions} is 2 and metres if
  \attrib{spatialDimesions} is 1.  \attrib{spatialDimensions} here is the attribute
  value on the containing compartment.
  If absent, the units default to $substance$/$size$ where $substance$ is the built-in
  unit \unit{substance} (see Table~\ref{tab:builtin}) and $size$ is units assigned
  to the compartment containing the species.  (As noted
  above, \attrib{initialConcentration} cannot be used if the compartment
  where the species is located has a \attrib{spatialDimensions} value of
  ``\attribvalue{0}''.)
\end{itemize}

\subsubsection{\attrib{constant} and \attrib{boundaryCondition} Fields}
...
system.  Table~\vref{tab:specieattrib} shows how to interpret the combined
...
\subsubsection{\attrib{charge} Field}
...
\attrib{id} field identifies the parameter.  

\subsubsection{\attrib{value} Field}
...
The units of the parameter are specified by
the field \attrib{units}.  These units are the units of the \attrib{value} field,
the parameter symbol in MathML expressions and a \class{AssignmentRule} structure
which may determine the value of the parameter.  A \class{RateRule} structure which
may determine the value of the parameter has units ${parameter units}/time$ where
$parameter units$ are the units assigned to the parameter and $time$ is the built-in
\quantity{time} units.
The value assigned to the parameter's \attrib{units} attribute must be
...
structure.  There are no constraints on which units can be chosen from these sets.
...
The effects of an \class{AssignmentRule} structure are in general terms the same,
but differ in the precise details depending on which variable is being set:
...
\item \emph{In the case of a species}, an \class{AssignmentRule} sets the
  ...
  in \attrib{math}.  The units of the expression depends
    on how the species is declared on a \class{Species} structure (see
    Section~\ref{sec:species}).  If the \class{Species} structure has an
    \attrib{initialConcentration} attribute value then the units of the
    the rule is exactly those given on the \class{Species} structure.
    If the \class{Species} structure has an \attrib{initialAmount}
    attribute value then the units of the rule are
    ${amount}/{size}$ units where $amount$
    units those given on the \class{Species} structure and $size$
    units are those given on the \class{Compartment} structure associated with the
    species. 
...
\item \emph{In the case of a compartment}, an \class{AssignmentRule} sets
  ...
  in \attrib{math}. The units of the \class{AssignmentRule} expression
  is exactly that given by the \attrib{units} attribute (or its default) on the
  \class{Compartment} structure associated with the \attrib{variable} field.
...
\item \emph{In the case of a parameter}, an \class{AssignmentRule} sets the
  ...
  \attrib{math}.  The units of the \class{AssignmentRule} expression
  is exactly that given by the \attrib{units} attribute (or its default) on the
  \class{Parameter} structure associated with the \attrib{variable} field. 
...
\item \emph{In the case of a species}, a \class{RateRule} sets the rate of
  ...
  formula in \attrib{math}.  The units of the expression is $concentration/time$
    where the $concentration$ units depends
    on how the species is declared on a \class{Species} structure (see
    Section~\ref{sec:species}).  If the \class{Species} structure has an
    \attrib{initialConcentration} attribute value then the $concentration$ units are
    exactly those given on the \class{Species} structure.
    If the \class{Species} structure has an \attrib{initialAmount}
    attribute value then the concentration units are 
    ${amount}/{size}$ units where $amount$
    units those given on the \class{Species} structure and $size$
    units are those given on the \class{Compartment} structure associated with the
    species.
  ...
  on the \class{Species} structure that defines the species the
  \attrib{boundaryCondition} field is set to ``\attribvalue{true}''.  No
  more than one \class{RateRule} or \class{AssignmentRule} can refer to a
  given species in an SBML model definition.
...
\item \emph{In the case of a compartment}, a \class{RateRule} sets the rate
  ...
  formula in \attrib{math}.  The units of the \class{RateRule} expression
  is $size/time$.  The $size$ units are exactly that given by the
  \attrib{units} attribute (or its default) on the
  \class{Compartment} structure associated with the \attrib{variable} field.

  \emph{Restrictions}: No more than one \class{RateRule} or \class{AssignmentRule}
  can refer to a given compartment in an SBML model definition.
...
\quantity{time} is a built-in unit type (see Table~\vref{tab:builtin}).
...
optional boolean fields.  The \attrib{reversible} field indicates
...
required and thereby optimize such computations.  The default value of
\attrib{fast} is ``\attribvalue{false}''.  (A simulator/analysis package
...
reaction as fast when in fact it is not.)
...
\class{SpeciesReference} structure. This field contains an expression in
dimensionless units.  It is recommended for maximal interoperability that
this field should simply contain an integer value (see the following
example).  The \attrib{math} field is optional and defaults to containing
...
rate of the reaction.  The overall rate expression in \attrib{math} must
...
substance and time for the reaction.
...
Section~\ref{sec:mathmltokens}, the units of each species reference are
...
\subsubsection{\attrib{trigger} Field}
...
\subsubsection{\attrib{delay} Field}
...
\subsubsection{\attrib{timeUnits} Field}
...
taken from the defaults defined by the built-in ``\unit{time}''
of Table~\vref{tab:builtin}.

\subsubsection{\attrib{eventAssignment} Field}
...
$S_1$ and $S_2$ are reactants but their values are not determined by a
kinetic law thus they are both on the boundary of the reaction system and
in SBML have \attrib{boundaryCondition} attribute values of
\attribvalue{true}.  The value of $S_1$ varies over time so in SBML $S_1$
...
<sbml xmlns="http://www.sbml.org/sbml/level2" level="2" version="1">
    <model id="facilitated_ca_diffusion">
        <listOfUnitDefinitions>
            ...
            <unitDefinition name="area">
                <listOfUnits>
                    <unit kind="meter" scale="-6" exponent="-2" />
                </listOfUnits>
            </unitDefinition>
        </listOfUnitDefinitions>
        <listOfCompartments>
