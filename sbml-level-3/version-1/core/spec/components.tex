...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}[level distance=1.05in]
    \node { \emptyClassbox{\textsl{SBase}} }
      ...
      child {node (a) {
          \begin{classbox}{SBML}
            xmlns: string \{ use="required" fixed="http://www.sbml.org/sbml/level3/version1/core" \} \\
...
for \quantity{time} and \quantity{avogadro}.  Functions must be
written so that all model variables they use are passed to them
...
of the form ``the function definition \emph{is an} X'', where X is
...
supplied in the call to the function.  (See also
...
\subsubsection{Semantics and use of \class{Unit} and \class{UnitDefinition}}
...
\end{equation}
\end{linenomath}
where the unknown exponents of the base units are denoted with
...
to a rational number).  If the compartment's \token{constant}
...
\InitialAssignment, but the original \token{size} value in that
...
size.  A discussion of recommended units is given in
Section~\ref{sec:bp:unitdefinitions:recommendedunits}.
\question{MH: Table 2: reworded from ``allowable'' to ``recommended''}
...
\begin{table}[tbh]
  ...
  \begin{tabular}{cll}
    ...
    \token{spatialDimensions}  & \textbf{for inheriting units of size} & \textbf{Recommended units}\\
    \midrule
  ...
  \caption{When a \Compartment object instance does not specify a
    ...
    that case.  The right-hand column lists units
...
indicated in Table~\ref{tab:sboterm-availability}.  The relationship is
of the form ``the compartment  \emph{is an} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
...
<model volumeUnits="litre" ...>
    ...
    <listOfCompartments>
        <compartment id="cytosol" size="1e-15" 
                     constant="true" spatialDimensions="3"/>
        <compartment id="mitochondria" size="0.5e-16" 
                     constant="true" spatialDimensions="3"/>
    </listOfCompartments>
    ...
</model>
...
The relationship is of the form ``the species \emph{is an} X'',
...
When the attribute \token{initialAmount} is set, the units
...
units, where the units of \quantity{amount} are those specified by
the \token{substanceUnits} attribute, and the units of
\quantity{size} are those associated with the definition of the
size of the \Compartment in which the species is located.  Note
that in either case, \quantity{amount} units are involved and
determined by the \token{substanceUnits} attribute.  If the
\token{substanceUnits} attribute is not set on a given \Species
object instance, then the units of \quantity{amount} for that
...
is not set either, then the units of the species' quantity are
...
Instead, that aspect is something controlled by the attribute
...
interpretation applies for a given \Species identifier.  The
...
indicated in Table~\ref{tab:sboterm-availability}.  The relationship is
of the form ``the parameter \emph{is an} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
...
indicated in Table~\ref{tab:sboterm-availability}.  The relationship is
of the form ``the initial assignment \emph{is an} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
...
The collection of \InitialAssignment, \AssignmentRule and
\KineticLaw objects forms a set of assignment statements that must
...
\token{math} attribute.  The directed arcs in this graph start
from statements assigning the symbol and end at statements
containing the symbol in their \token{math} elements.  Such a
...
of the form ``the rule \emph{is an} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
...
indicated in Table~\ref{tab:sboterm-availability}.  The relationship is
of the form ``the constraint \emph{is an} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
...
\token{message} whose content is dicated by object class \Message.
...
indicated in Table~\ref{tab:sboterm-availability}.  The relationship is
of the form ``the reaction \emph{is an} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
...
form ``the species reference \emph{is an} X'', where X is the SBO
...
stoichiometry to a rational number, something that is occasionally
useful in the context of biochemical reaction networks.)  If the
species reference's \token{constant} attribute has the value
\val{false}, the species reference's value may be overridden by an
...
indicated in Table~\ref{tab:sboterm-availability}.  The relationship is
of the form ``the kinetic law \emph{is an} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
...
specified by the optional \LocalParameter attribute \token{units}.
The attribute's value must have the data type
\primtype{UnitSIdRef} (Section~\ref{sec:unitsidref}).  There are
no constraints on the units that can be assigned to a parameter;
there are also no units to inherit from the enclosing \Model
...
The relationship is of the form ``the local parameter \emph{is an}
...
and \token{boundaryCondition}=\val{false} participate in $M$ reactions
...
symbol in an assignment statement \token{math} element. The directed
...
relationship is of the form ``the event \emph{is an} X'', where X
...
intended to be evaluated.  A value of \val{true} indicates that
...
or not).  If \token{useValuesFromTriggerTime}=\val{false}, the
...
\val{false} for this attribute also implies that a \Delay
...
turn must contain a MathML \token{math} element.  The expression
in the \token{math} element must evaluate to a value of type
\primtype{boolean}.  The exact moment at which this expression
evaluates to \val{true} during a simulation is taken to be the
...
relationship is of the form ``the trigger \emph{is an} X'', where X
is the SBO term.  The term chosen should be the most precise
...
The units of the value computed by a \Delay object's \token{math}
expression should match the model's units of \quantity{time} (set
by the \Model \val{timeUnits}; see
...
indicated in Table~\ref{tab:sboterm-availability}.  The relationship is
of the form ``the delay \emph{is an} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
...
indicated in Table~\ref{tab:sboterm-availability}.  The relationship is
of the form ``the event assignment \emph{is an} X'', where X is
the SBO term.  The term chosen should be the most precise (narrow)
