...
Unless otherwise specified, SBML \thisL documents must follow the XML specification:  anything allowed in XML is allowed in SBML documents, and anything disallowed in XML is not allowed in SBML.
...
\verdiff{a}{an optional} \token{model} element.
...
XML form for an SBML \thisLV document. Here,
ellipses (\val{...}) are used to indicate content elided from this example:
...
\subsubsectionVerTwoOnly{The \token{id} and \token{name} attributes}
...
\item Every SBML Level~3 package is identified uniquely by an XML
  ...
  nicknamed \val{comp} and another package nicknamed \val{layout}
  (and here, ellipses are used to indicate content elided from
  this example):
  \begin{example}
<sbml xmlns="http://www.sbml.org/sbml/level3/\thisvlc/core" level="3" version="\sbmlversionnum"
      xmlns:comp="http://www.sbml.org/sbml/level3/\thisvlc/comp/\thisvlc"
  ...
  words, the prefix strings \val{comp} and \val{layout} in the
...
\item SBML Level~3 requires that every package defines the
  ...
  \token{required} attribute indicates whether constructs in that package may change the mathematical interpretation of constructs defined in SBML Level~3 Core.  A \token{required} value of \val{true} indicates that the package may do so.  The value is set by the relevant Level~3 package specification, and does \emph{not} depend on the presence or absence of particular package constructs in a given SBML document: in other words, it is a property of the package.
\verdiff{For example, if a package defines new object classes that can be used to change the mathematical meaning of a core element, such as the Hierarchical Model Composition's \token{replacedElement} construct, that package's \token{required} attribute must always be set to \val{true} for all SBML documents declaring the use of that package's namespace, whether or not the documents actually contain any \token{replacedElement} object.}{For example, if a package defines new object classes with
...
namespace.}  The following is a complete example:
  \begin{example}
<sbml xmlns="http://www.sbml.org/sbml/level3/\thisvlc/core" level="3" version="\sbmlversionnum"
      xmlns:comp="http://www.sbml.org/sbml/level3/\thisvlc/comp/\thisvlc"
      xmlns:layout="http://www.sbml.org/sbml/level3/\thisvlc/layout/\thisvlc"
...
In this example, because the Hierarchical Model Composition (\val{comp}) package can change the mathematical interpretation of a model, the \token{comp:required} attribute must always be set to \val{true}, even if a model is completely empty.  The Graphical Layout (\val{layout}) package, by contrast, cannot ever change the mathematical interpretation of any model; thus, its \token{required} attribute must be set to \val{false}.
  ...
  something similar.  The following are some ways in which packages with \token{required}=\val{true} may change the time-course dynamics of a model:
\begin{itemize}
...
\begin{figure}[htbp]
  \centering
  \verdiff{\includegraphics[scale=0.75]{figs/model-uml-l3v1r2}}
...
\fig{fig:model}.  All of the lists are optional, \verdiff{but if a given list container is present
...
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more}} <functionDefinition> ... </functionDefinition> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfFunctionDefinitions>\\
\>\><listOfUnitDefinitions>\\
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more}} <unitDefinition> ... </unitDefinition> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfUnitDefinitions>\\
\>\><listOfCompartments>\\
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more}} <compartment> ... </compartment> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfCompartments>\\
\>\><listOfSpecies>\\
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more}} <species> ... </species> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfSpecies>\\
\>\><listOfParameters>\\
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more}} <parameter> ... </parameter> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfParameters>\\
\>\><listOfInitialAssignments>\\
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more}} <initialAssignment> ... </initialAssignment> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfInitialAssignments>\\
\>\><listOfRules>\\
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more elements of subclasses of \abstractclass{Rule}}}  \` \sayOptional\\
\>\></listOfRules>\\
\>\><listOfConstraints>\\
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more}} <constraint> ... </constraint> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfConstraints>\\
\>\><listOfReactions>\\
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more}} <reaction> ... </reaction> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfReactions>\\
\>\><listOfEvents>\\
\>\>\>\textrm{\emph{\verdiff{one}{zero} or more}} <event> ... </event> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfEvents>\\
\></model>\\
</sbml>
\end{tabbing}
...
defining a compartment, and defining a species reference in a reaction requires defining
...
\begin{figure}[htb]
  ...
  \small
  \vspace*{-0.75ex}
  \verdiff{\includegraphics[scale=0.78]{figs/functiondefinition-uml-l3v1r2}}
  ...
  \caption{The definition of class \FunctionDefinition.  A
    ...
    \token{semantics} element).  A function definition \verdiff{must}{may}
    contain \verdiff{exactly}{no more than} one \token{math} element defined by the
    ...
    element.  A sequence of \verdiff{one}{zero} or more instances of
...
\subsubsection{The \token{id} \verone{and \token{name}} attribute\verone{s}}

\verdiff{The \token{id} and \token{name} attributes have types \primtype{SId}
...
element can be used.  \verdiff{The}{If present, the} \token{lambda} element must begin with
...
\quantity{time}, \quantity{avogadro}, \verone{and} \quantity{delay}\vertwo{, and \quantity{rateOf}}.
Functions must be written so that all model symbols they use are
...
<model ...>
   ...
   ...
   <listOfReactions>
       <reaction id="reaction_1" reversible="true"\verone{ fast="false"}>
           ...
           <kineticLaw>
               <math xmlns="http://www.w3.org/1998/Math/MathML">
                   <apply> 
...
\begin{figure}[htb]
  ...
  \vspace*{-0.5ex}
  \fi
  \verdiff{\includegraphics[scale=0.78]{figs/unitdefinition-uml-l3v1r2}}
  ...
  \caption{The definition of classes \UnitDefinition and
      \Unit.  A sequence of \verdiff{one}{zero} or more instances of
      ...
      it merely acts as a container for \verdiff{one}{zero} or more instances of
...
\verdiff{\vspace*{-4ex}\paragraph{The \token{id} and \token{name} attributes}}{\vspace*{-4ex}\paragraph{The \token{id} attribute}}

\verdiff{The required attribute \token{id} and optional attribute
...
A \UnitDefinition object may contain a \ListOfUnits container which \verdiff{must contain one or more}{may
...
represented by the symbol $m_i$.  Similarly, let the value of
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \node (b) {
        \begin{classbox}{\textcolor{black}{Compartment}}
          \textcolor{black}{id: SId \verdiff{}{\{ use="required" \}}}  \\
          \verone{\textcolor{black}{name: string \{ use="optional" \}}      \\}
  ...
  \caption{The definition of class \Compartment.  A
      sequence of \verdiff{one}{zero} or more instances of \Compartment objects
...
\subsubsection{\verdiff{The \token{id} and \token{name} attributes}{The \token{id} attribute}}

\verdiff{\Compartment has one required attribute, \token{id}, of type
...
\begin{figure}[htb]
  ...
  \end{tikzpicture}
  \vspace*{-1ex}
  \caption{The definition of class \Species.  \verdiff{One}{Zero} or more
...
\verdiff{As with other major objects in SBML, \Species has a mandatory
...
indicate whether and how the amount of that species can vary
...
\begin{table}[ht]
  ...
  \begin{edtable}{tabular}{lllll}
    ...
    \textbf{value}            & \textbf{value}                     & \textbf{or rate rule?} & \textbf{product?}   & \textbf{the species' amount?} \\
    \midrule
...
reactions, its amount is determined by those reactions.  In
SBML, it is possible to indicate that a given species' amount is
...
\emph{boundary} of the reaction system, and its amount is not
...
amount can be changed at all, regardless of whether by
reactions, rules, or constructs other than \InitialAssignment.  A
value of \val{false} indicates that the species' amount can be
...
amount may still be changed by \AssignmentRule, \RateRule,
...
attributes are \val{true}, then its amount cannot be changed by
...
into ODEs a model with species of mixed
...
\begin{figure}[htb]
  ...
  \caption{The definition of class \Parameter.  A
      sequence of \verdiff{one}{zero} or more instances of \Parameter objects can
...
\verdiff{\Parameter has one required attribute, \token{id}, of type
...
\begin{figure}[htb]
  ...
  \caption{The definition of class \InitialAssignment.  The
    ...
    sequence of \verdiff{one}{zero} or more instances of \InitialAssignment
...
\subsubsectionVerTwoOnly{The \token{id} attribute}
...
\InitialAssignment.  For SBML Level~3 Core, initial assignments can take place for
...
result, $\textbf{V}$ is a vector of symbols that does not
include $x$, and $\textbf{W}$ is a vector of symbols that may
include $x$):
\vertwo{\vspace*{1ex}}
\begin{center}
...
\begin{figure}[htb]
  \centering
  \verdiff{\includegraphics[scale=0.79]{figs/rule-uml-l3v1r2}}
...
The classes derived from \Rule inherit \token{math} and the attributes and elements that \Rule itself inherits from \SBase, including \vertwo{\token{id}, \token{name}, and }\token{sboTerm}.
...
set the values of variables.  The left-hand side (the required
\token{variable} attribute) of an assignment rule is of type \primtype{SIdRef}, and must refer to an SBML object in the \primtype{SId} namespace with mathematical meaning and the ability to be assigned\verdiff{:}{.  In SBML Level~3 Core, this consists of}
\Species, \SpeciesReference, \Compartment, 
and global \Parameter objects in
the model (but not reactions nor function definitions).  The entity identified must
%NB:  changed to make this text match that for RateRule:
have its \token{constant} attribute set to the value \val{false}.  The effects of
...
(the required \token{variable} attribute) of a rate rule  has type \primtype{SIdRef}, and must refer to an SBML object in the \primtype{SId} namespace with mathematical meaning and the ability to be assigned\verdiff{:}{.  In Level~3 Core, this consists of}
\Species, \SpeciesReference, \Compartment, 
and global \Parameter objects in
the model (but not reactions nor function definitions).  The
...
\begin{figure}[htb]
  ...
  \centering
  \small
  \verdiff{\includegraphics[scale=0.8]{figs/constraint-uml-l3v1r2}}
  ...
  \caption{The definition of class \Constraint.  The contents of
    ...
    \verdiff{but it must return}{and will be interpreted as} a Boolean value.  As shown above, an
    ...
    sequence of \verdiff{one}{zero} or more instances of \Constraint objects can
...
declaration nor a DOCTYPE declaration.
...
\begin{figure}[b]
  \vspace*{-2.5ex}
  \centering
  \verdiff{\includegraphics[scale=0.74]{figs/reaction-uml-l3v1r2}}
...
\paragraph{\verdiff{The \token{id} and \token{name} attributes}{The \token{id} attribute}}

\verdiff{As with most other main kinds of objects in SBML, the
...
\verdiff{The \primtype{boolean} attribute \token{fast} is another required \primtype{boolean}
...
be set to \val{false} for all reactions.  Note that in SBML Level~3 Version~2, the \token{fast} flag is removed, and all reactions are equivalent to \thisLV reactions with a \token{fast} value of \val{false}.}
...
\verone{The model's reactions are divided into two sets by the
...
\verone{The correctness of the approximation requires a significant
...
\verone{Note that the \token{fast} attribute has a significant effect on
...
<model ...>
    ...
    <listOfReactions>
        <reaction id="J1" reversible="false"\verone{ fast="false"}>
            <listOfReactants>
...
<model ...>
    ...
    <listOfInitialAssignments>
        ...
        ...
    <listOfReactions>
        <reaction id="J1" reversible="true"\verone{ fast="false"}>
            <listOfReactants>
...
\begin{example}
<reaction id="x" reversible="false"\verone{ fast="false"}>
    <listOfReactants>
...
<reaction id="y" reversible="false"\verone{ fast="false"}>
    <listOfReactants>
...
Because its sibling class \SpeciesReference has mathematical meaning, it is probably worth noting that no meaning is
...
An instance of \KineticLaw can contain a list of \verdiff{one}{zero} or more
...
\emph{local parameter} that are only accessible \verdiff{by the kinetic law formula of}{within the scope of}
...
\verdiff{\LocalParameter has a required attribute \token{id}, of data type
...
\token{variable} or \token{symbol} attributes of \InitialAssignment, \RateRule,
...
\begin{figure}[htb]
  ...
  \small
  \vspace*{-1ex}
  \verdiff{\vspace*{-2ex}\includegraphics[scale=0.76]{figs/event-uml-l3v1r2}}
...
In addition to the attributes and children it inherits from \SBase,
an \Event definition has one required attribute, \token{useValuesFromTriggerTime}, \verone{the optional attributes \token{id} and \token{name}, }and five optional subobjects, \Trigger, \Delay, \Priority, \ListOfEventAssignments, and \EventAssignment.
...
\verdiff{As with most components in SBML, an \Event has \token{id} and
...
As shown in \fig{fig:event}, an \Event object \verdiff{must}{may} contain
...
The expression in the \token{math} element \verdiff{is}{will be interpreted as} a
...
\token{listOfEventAssignments} element \verdiff{must have a non-empty list of one or more}{may have
...
The \EventAssignment required attribute \token{variable} has type \primtype{SIdRef}. The value of the attribute must be the identifier of \verdiff{a}{an object in the \primtype{SId} namespace of the model with mathematical meaning and the ability to be assigned.  In SBML Level~3 Core, the permitted objects classes are} \Compartment, \Species, \SpeciesReference, \verdiff{or}{and} \Parameter.  \vertwo{The set also includes SBML Level~3 package objects with identifiers in the \primtype{SId} namespace of the model and mathematical meaning defined to allow assignment.}
...
element\verdiff{; that is,}{.  For components defined by SBML Level~3 Core, this means that} a species' quantity, species reference's
...
occurring in MathML \token{ci} elements in the
...
\token{variable} attribute value.  More precisely\verdiff{:}{, for objects defined by SBML Level~3 Core:}
\begin{itemize}
...
\item A software package should retest all event triggers after
  executing the assignments from a given event in order to account for the
