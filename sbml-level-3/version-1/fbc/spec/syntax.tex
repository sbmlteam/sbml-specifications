...
This attribute contains a \val{value} of type \primtype{FbcSIdRef} that exclusively refers to an existing \token{objective}. This attribute exists so that in the case where multiple \Objective's are defined in a single model the model will always be complete, i.e. there is a single, primary optimization target.
...
\begin{itemize}
  \item \token{id} an attribute that can take exclusively contain an \primtype{FbcSId}
  \item \token{reaction} an attribute that takes an \primtype{SIdRef} which refers only to an \SBML \Reaction
  \item \token{operation} an \primtype{enum} that can take a limited set of boolean operators (see text for details on the enumerated allowed types)
  \item \token{value} an attribute that takes a \primtype{double} value representing the bound. This may include $\pm\infty$ 
\end{itemize}

\paragraph{The \token{operation} attribute}
The \token{operation} attribute represents the (in)equality in the expression \token{reaction}, \token{operator} \token{value} an enumerated type that can have the following values:
\begin{itemize}
  \item \val{lessEqual} representing $<=$ 
  \item \val{greaterEqual} representing $>=$ 
  \item \val{less} representing $<$
  \item \val{greater} representing $>$ 
  \item \val{equal} representing $=$
  \item \val{unknown} representing an undefined symbol
\end{itemize}
