...
\validRule{layout-20201}{There may be at most one instance of each of the
  following kinds of objects within a \Model object using Layout:
  \ListOfLayouts.  (References:
  SBML Level~3 Package Specification for Layout,
  Version~1, \sec{model-class}.) }

\validRule{layout-20202}{The various
  \textsf{\textbf{ListOf\rule{0.15in}{0.5pt}}} subobjects within a \Model
  ...
  are present on the \Model, it must not be empty: \ListOfLayouts.  (References: SBML Level~3 Package Specification for
...
\validRule{layout-20903}{There may be at most one instance of an \BoundingBox object on a \TextGlyph, no other elements from the Layout namespace are permitted on a \TextGlyph.
  SBML Level~3 Package Specification for Layout,
...
\validRule{layout-20909}{If both attributes \token{layout:\-originOfText} and \token{layout:\-metaidRef} are specified on a \TextGlyph they have to reference the same element of the \Model. (References: SBML Level~3
Package Specification for Layout, Version~1,
\sec{textglyph-class}.) }
...
\validRule{layout-21109}{If both attributes \token{layout:\-reference} and \token{layout:\-metaidRef} are specified on a \ReferenceGlyph they have to reference the same element of the \Model. (References: SBML Level~3
Package Specification for Layout, Version~1,
\sec{referenceglyph-class}.) }
...
\validRule{layout-21305}{If the \token{layout:\-z} attribute of a \Point element on a \BoundingBox is not specified, the attribute \token{layout:\-depth} must also not be specified.  (References: SBML Level~3
Package Specification for Layout, Version~1,
\sec{graphicalobject-class}.) }
