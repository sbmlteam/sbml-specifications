...
semantics of these operators with zero or one operand.  Here we define the
semantics for the purposes of SBML.

The general form of the operators is as follows:
...
consists of two fields, \attrib{variable} and \attrib{math}, as shown
in Figure~\vref{fig:initialAssignment}.  The \attrib{variable} field has type
\class{SId} field and contains the value of an \attrib{id} field
defined for a \class{compartment}, \class{species} or \class{parameter}
elsewhere in the model.
The \class{InitialAssignment} structure sets the initial value of
the constant or variable referred to by the \attrib{variable} field.
...
\attrib{variable} field value in an SBML model definition. Unlike an
...
the units associated with the identifier given in the \attrib{variable} field,
