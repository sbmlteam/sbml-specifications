...
provide meaningful labels suitable for display to humans
...
model between different representations (e.g.
Henri-Michaelis-Menten Vs. elementary steps), or to use it with
different modelling approaches (discrete or continuous). On may
also need to relate the model components with other description
format, using a deeper semantics.  Although an advanced software
...
\token{sboTerm} \changed{attribute} provided on the SBML class
\SBase.  The \token{sboTerm} \changed{attribute} always refers to
...
\emph{interpreted}. Annotating SBML elements with SBO terms adds
an extra layer of semantics that may be used to \emph{convert} the
model into another model, or another format. Athough SBO support
provides an important source of information to understand the
meaning of a model, software needs not to support \token{sboTerm}
to be considered SBML-compliant.
...
``continuous'', ``discrete'', etc.), but also common types of
...
The presence of the label on a compartment, a species or a
reaction, can help to map SBML elements to representations in
other standards, such as (but not limited to) BioPAX
(\url{http://www.biopax.org/}) or the Systems Biology Graphical
Notation (SBGN, \url{http://www.sbgn.org/}) Such a mapping can be
used in conversion procedures, or to build interfaces, SBO
becoming a ``glue'' between standards of representation.
...
The \token{sboTerm} \changed{attribute} data type is always \primtype{SBOTerm},
defined in Section~\ref{sec:sboterm-type}.  When present in a
given model object instance, the \changed{attribute}'s value must be an
...
mathematical formulas encoded using \mathmltwo that express the
parameter using other SBO parameters. The main use of that feature
is to avoid listing all the variants of a mathematical expression,
escaping a combinatorial explosion.
...
precise statement about the rate law in question. Some of the
non-leaf terms also contain rate laws encoded using \mathmltwo.
However, those are generic rate-laws, containing MathML constructs
not yet supported by SBML and need to be expanded before to be
...
\item \emph{Parent(s)}: \token{SBO:0000050} second order
  irreversible mass action kinetics, one reactant (is a);
  \token{SBO:0000163} irreversible mass action kinetics,
  continuous scheme (is a)
...
\token{sboTerm} is an attribute of SBase and can be used in all
derived elements. However, not all SBO terms can be used to
annotate all SBML elements.  Table~\ref{tab:sboterm-availability}
summarizes the relationship between SBML components and the
branches within SBO that apply to that component (There are no
specific SBO term that correspond to sbml, unitDefinition and unit
elements).
...
\item An object can only have one \token{sboTerm} \changed{attribute},
...
\subsection{Relationships to the SBML \token{annotation} \changed{element}}

Another means of providing this kind of information would be to
place SBO terms inside the \token{annotation} \changed{element}
...
\changed{attribute} rather than in the \token{annotation}
\changed{element} of an object.

Although \token{sboTerm} is just another kind of optional
annotation in SBML, the SBO references are \changed{separated out into
...
A software tool must not need to have the ability to access the
...
formats (\sboref). WebServices are also available to provide
