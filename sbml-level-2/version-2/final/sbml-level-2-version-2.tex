...
\authoremail{\{afinney,mhucka\}@sbml.org}
...
\changed{Minor Releases of SBML are termed \emph{versions} and constitute
...
\changed{Each SBML Level-plus-version specification inevitably
...
is adapted from~\citep{jacobs:2004}.)}

\changed{Errata result in new \emph{issues} of the specification.
...
value of a variable can be calculated from other values of other
...
\class{annotation-xml}, \class{csymbol} and \class{semantics} elements; and
...
The MathML ``e-notation'' requires the mantissa and exponent to
...
The N-ary operators (\class{add}, \class{times}, \class{and},
\class{or} and \class{xor}) should follow their definition in the
MathML specification.  Note, however, that the MathML specification
does not define the semantics of these operators with zero or one
operand.  In this section, we define these semantics for the
purposes of SBML.

The general form of these operators is as follows:
...
\begin{table}[bh]
  ...
  \begin{tabular}{ll}
    ...
    \bottomrule
  \end{tabular}
  \caption{The base operands of the N-ary operators of the MathML subset}
...
\begin{figure}[htb]
  ...
  \caption{The definition of \class{Sbml} for SBML Level~2 Version~2.  Following UML notation, additional fields
...
\class{Parameter}, \class{InitialAssignment}, \class{Rule},
\class{Constraint}, \class{Reaction} and \class{Species}
components.} All of these components are optional; that is, the
lists in each of the respective fields are permitted to have zero
length. (However, there are dependencies between components, such
that defining some requires defining others.  For example, as
explained in other sections below, defining a species requires
...
number of spatial dimensions of the compartment; that is, they
must be dimensionless or units of volume if the value of
\attrib{spatialDimensions} is ``\attribvalue{3}''; they must be
units of area if the value of \attrib{spatialDimensions} is
``\attribvalue{2}''; they must be dimensionless or units of length
if the value of \attrib{spatialDimensions} is ``\attribvalue{1}'';
and they must be ``\unit{dimensionless}'' if the value of
\attrib{spatialDimensions} is ``\attribvalue{0}''.} The default
units depends on the value of the compartment's
\attrib{spatialDimensions} field according to the following rule:
for spatial dimensions of 3, 2, 1 or 0, the compartment has the
default units of \unit{volume}, \unit{area}, \unit{length} and
\unit{dimensionless}, respectively.  (See Table~\vref{tab:builtin}
and Table~\vref{tab:unitkind}.)
...
either unknown, set by an assignment rule, not required for
...
\attrib{substanceUnits} must be a variant of \unit{dimensionless},
\unit{kilogramme}, \unit{mole} or \unit{item} units}. The
...
or dimensionless if the value of the compartment's
\attrib{spatialDimensions} is ``\attribvalue{3}''; they must be
units of area or dimensionless if the value of
\attrib{spatialDimensions} is ``\attribvalue{2}''; and they must
be units of length or dimensionless if the value of
\attrib{spatialDimensions} is ``\attribvalue{1}''.} The
\attrib{spatialSizeUnits} must not have a value if
...
\emph{Initial Assignments} provide a way to encapsulate the
calculation of the value of a constant or the initial value of a
variable from the value of another constant or the initial value
of another variable.  An \class{InitialAssignment} structure
consists of two fields \attrib{symbol} and \attrib{math} as shown
in Figure~\vref{fig:initialAssignment}.  \attrib{symbol} is an
\class{SId} field and contains the value of an \attrib{id} field
of a \class{compartment}, \class{species} or \class{parameter}.
The \class{InitialAssignment} structure sets the initial value of
...
to calculate the value of the constant or variable.
...
other initial value assigned to the given symbol except for that
value calculated by an \class{AssignmentRule} structure. Two or
more \class{InitialAssignment} structures cannot have the same
\attrib{symbol} field value in an SBML model definition. Unlike a
\class{AssignmentRule} structure a \class{InitialAssignment}
...
units of the formula, the \attrib{math} field, are identical to
the units associated with the symbol, the \attrib{variable} field,
when that variable appears in other formulae.
...
The ordering of \class{InitialAssignment} structures is
significant: they are always evaluated in the order given in SBML.
...
In general \class{InitialAssignment} structures are much less
constrained than \class{AssignmentRule} structures.
...
\begin{blockChanged}
A rule cannot determine the value if a reaction rate by
referencing the \attrib{id} value of a \class{Reaction} structure.
However a rule can determine other variable values by reference to
the rate of a reaction through by referencing the \attrib{id}
value of a \class{Reaction} structure in MathML \class{cin}
elements.
...
but may be over determined e.g. equations can't refer to
...
\class{Model}.  It is permissible for a
species to be referenced simultaneously from the \attrib{modifier},
\attrib{product}, and/or
\attrib{reactant} fields of the same reaction, if appropriate
for the kind of reaction kinetics involved.}
...
As was previously stated, the overall rate expression in the
...
multiplied variant of \unit{dimensionless}, \unit{moles} or
...
encode rate expression has the final units of
...
can occur as the content of a MathML \class{ci} element. Such a
...
A \class{KineticLaw} expression itself cannot contain reaction
identifiers.  This restriction is in place to prevent the
creation of algebraic loops among \class{KineticLaw} expressions
and prevent dependencies on the order of evaluation of kinetic laws.
\end{blockChanged}
...
defaults, ensuring size is in liters and time is in seconds.  The
...
concentration values in millimoles per liter.  All the
...
and milliMoles per liter, respectively.
...
\item \emph{\attrib{definitionURL} on SBase}

In Version 2 the \class{SBase} structure has an addition field
\attrib{definitionURL} which contains the URI of a resource with a
`is a' relationship with the containing SBML structure.  This can
be used for example to label a species as being a protein.
...
\attrib{definition} which refers to another\\
...
directly from kilogramme rather from mole.  This facilitates the
...
\class{RateRule} structures.  A reaction symbol cannot occur
within the math expression of within a \class{KineticLaw} (this is the
simplest restriction to avoid algebraic loops amongst
\class{KineticLaw} structures).
...
/begin{blockChanged}
/begin{verbatim}
        <xsd:attribute name="definitionURL" type="xsd:anyURI" use="optional"/>
/end{verbatim}
/end{blockChanged}
/begin{verbatim}
    </xsd:complexType>
...
“\texttt{true}” if the value of the \attrib{spatialDimensions} field is 0. See
...
contain either \texttt{dimensionless}, \texttt{kilogramme},
...
\texttt{moles}, \texttt{kilogramme} or \texttt{dimensionless}. See
Section~\ref{sec:species-units}.
...
\attrib{id} attributes of \class{unitDefinition} elements that
define variants of \\
\texttt{dimensionless}, \texttt{kilogramme},
