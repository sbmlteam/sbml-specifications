...
The Distributions package allows modelers to include new MathML \token{csymbol} contructs that define draws from distributions, and the ability to define the uncertainty in any element with mathematical meaning.  Both could be employed as-is by users of the Spatial package:  the \token{csymbol} constructs can be used in spatial MathML, and in core MathML to affect spatial elements through the \SpatialSymbolReference link, and any element with mathematical meaning could be given an \class{Uncertainty} child defining its uncertainty.
