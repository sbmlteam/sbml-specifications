...
of the format contained in the light gray box \changed{(\token{vCard:N}, \token{vCard:EMAIL}, and \token{vCard:ORG})} can occur in any
order.  The elements of the format contained between \texttt{[}
and \texttt{]} \changed{(\token{vCard:EMAIL} and \token{vCard:ORG})} are optional, \changed{but everything else is required}.  The precise form of the whitespace,
and the specific XML namespace prefixes used \changed{(\val{dc}, \val{rdf}, and \val{vCard})} are not constrained.
