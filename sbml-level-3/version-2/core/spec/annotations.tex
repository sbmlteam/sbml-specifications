...
of the format contained in the light gray box \relchange{(\token{vCard:N}, \token{vCard:EMAIL}, and \token{vCard:ORG} for \token{vCard} version 3, and \token{vCard4:fn} or \token{vCard4:hasName}, plus \token{vCard4:hasEmail}, and \token{vCard4:organization-name} for \token{vCard} version 4)} can occur in any
order.  The elements of the format contained between \texttt{[}
and \texttt{]} \relchange{(\token{vCard:EMAIL}, \token{vCard:ORG}, \token{vCard4:organization-name}, and \token{vCard4:hasEmail})} are optional, \relchange{but everything else is required}.  The precise form of the whitespace,
and the specific XML namespace prefixes used \relchange{(\val{dcterms}, \val{rdf}, \val{vCard}, and \val{vCard4})} are not constrained.
...
The \token{dcterms:created}, \token{dcterms:modified}, \token{vCard:N}, \relchange{\token{vCard:ORG}, and \token{vCard4:hasName},}  
elements
