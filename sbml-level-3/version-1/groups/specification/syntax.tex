...
The Groups package uses a number of the primitive data types described in Section~3.1 of the \sbmlthreecore specification, \changed{including \primtype{SId}, clarified below,} and adds \changed{the \primtype{GroupKind} type}.
...
The \ListOfMembers class is defined in \fig{member-uml}, and must have one or more \Member children.  Since \ListOfMembers is derived from \SBase, it inherits the \token{sboTerm} and \token{metaid} attributes, as well as the optional children \Notes and \Annotation.  Unlike most lists of objects in SBML, however, the \token{sboTerm} attribute and the \Notes and \Annotation children are taken here to apply directly to every SBML element referenced by each child \Member of this \ListOfMembers, if that referenced element has no such definition.  \changed{Thus,} if a referenced element has no defined \token{sboTerm}, \changed{child \Notes, or child \Annotation, that element should be considered to now have the \token{sboTerm}, child \Notes, or child \Annotation of the \ListOfMembers.}

If multiple \ListOfMembers \changed{have child \Member elements that} reference the same SBML element, and more than one \changed{\ListOfMembers or \Member has a value for an \token{sboTerm} attribute}, \Notes, or \Annotation element, those \changed{\Member} elements should be consistent with each other:  the \token{sboTerm} attributes should either be identical, or one should inherit from the other; \Notes should say the same or similar things; and \Annotation elements should not conflict.  \changed{Interpreters may choose to resolve any such conflicts arbitrarily.}
...
Note that even if used in conjunction with the SBML Level~3 Hierarchical Model Composition package, this attribute is not allowed to reference elements \changed{that reside within} other \Model objects in the same SBML Document.  Referenced elements must be normal members of the parent \Model containing the \Member object, and submodel elements may be \changed{normally accessed} by creating replacements.


\subsubsection{Members referencing other Groups and Members}
\label{nested-groups}

If a \Member references a \Group, it is \changed{the \Group itself} that is considered to be a member of the \changed{parent} \Group, not the corresponding referenced element (or elements).  This is also true for elements from other packages that point to other elements, such as \SBaseRef elements from the SBML Level~3 Hierarchical Model Composition package.  \changed{However, if a \Member references a \ListOfMembers object, it is the elements referenced in that list that are considered to be part of the parent \Group.}
...
\item If the annotation or SBO term is on a \Member object, it is \changed{an annotation about the \Member itself, and not about its referenced element}.

\item If the annotation or SBO term is on \ListOfMembers, it is a short-hand that means the annotation applies to each individual referenced member, as if the annotation were put on the \changed{referenced SBML elements} directly.
