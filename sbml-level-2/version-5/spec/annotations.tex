...
\changed{The placeholder \texttt{NESTED\_CONTENT} refers to
  additional, nested \texttt{RELATION\_ELEMENT} elements in a
...
The placeholder \texttt{NESTED\_CONTENT} in the syntax summary
above refers to additional nested annotations.  The format of
each element comprising \texttt{NESTED\_CONTENT} is identical to
the syntax of \texttt{RELATION\_ELEMENT}; in other words,
\texttt{NESTED\_CONTENT} consists of one or more of the
following written sequentially:
...
This can be used to clarify or elaborate the
\texttt{RELATION\_ELEMENT} in which the annotation appears; for
example, it could be used to describe protein modifications on
species, or to add evidence codes for an annotation.  The
\texttt{NESTED\_CONTENT} content relates to its containing
\texttt{RELATION\_ELEMENT}, not the other way around, and it
qualifies but does not change the meaning of the containing
relation.  Ignoring \texttt{NESTED\_CONTENT} does not affect the
information in the \texttt{RELATION\_ELEMENT}.  The
\texttt{NESTED\_CONTENT} may be nested to arbitrary depth, with
each successive layer describing or clarifying the annotation
...
more \token{rdf:li} elements, \changed{and may contain nested content providing additional annotations about the contents of the \token{rdf:Bag}}.  The \token{rdf:li} elements must
...
% Not really necessary to provide another example.  I'm removing
% it so that i can achieve better page breaks.
...
% \hspace*{0.25in}\uri{http://www.genome.jp/dbget-bin/www\_bget?ec:3.5.4.4}
...
between SBML elements and resources, \changed{qualifier}
