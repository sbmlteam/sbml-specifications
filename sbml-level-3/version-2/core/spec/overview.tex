...
\item \emph{Initial Assignment}: A mathematical
  ...
  the value of a \relchange{symbol} can be calculated from other values
  and \relchange{symbols} at the start of simulated time.
...
\item \emph{Rule}: A mathematical expression added to the set of
  ...
  Rules can be used to define how a \relchange{symbol}'s value can be
  calculated from other \relchange{symbols}, or used to define the rate of
  change of a \relchange{symbol}.  The set of rules in a model can be used
...
\item \emph{Constraint}: A means of detecting out-of-bounds
  ...
  \relchange{symbols}.  An SBML constraint applies
...
\item \emph{Event}: A statement describing an instantaneous,
  discontinuous change in one or more \relchange{symbols} of any type
