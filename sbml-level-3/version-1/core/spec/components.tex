...
\token{substanceUnits} attribute on either \Model or \Species.
...
\token{timeUnits} attribute on the \Model object instance.  Since
SBML \thisL does not define default values, then if the \Model
\token{timeUnits} attribute has no value, it means that the units
...
% This seems to be too confusing.  Let's just make the right
% statements where the Delay and csymbol delay are described.
%
% An additional point worth noting concerns the lack of built-in
% assumptions about the units of measurement associated with the
% mathematical expression in the \Delay element within \Event
% (Section~\ref{sec:event-delay}) and the \emph{delay}
% \token{csymbol} (Section~\ref{sec:csymbol}).  In keeping with SBML
% \thisL's general principle of avoiding defaults, these components
% do not have any particular default units.  Models must associate
% time units with the values explicitly.  An approach for doing this
% for \Delay is discussed in Section~\ref{sec:event-delay}.
...
\token{volumeUnits}, \token{areaUnits} or \token{lengthUnits}
attributes (as appropriate), or the \token{units} attribute on
...
involve events of some sort.  A single reaction event is one
in which some set of entities (known as ``reactants'' and
``modifiers'' in SBML) interact, once.  As time moves forward in a
system, the reaction keeps occurring as long as there are entities
left to participate in the reaction.  The \emph{extent} of the
reaction is how many times the reaction has happened after the
system has started.  The time derivative of this extent gives the
rate at which the reaction is occurring.  Thus, what is
colloquially referred to as the ``rate of the reaction'' is in
fact equal to the rate of change of the extent.

The units to be associated with reaction extent in a given model
can be set by the attribute \token{extentUnits} on the \Model
instance.  These units establish how reaction events are meant to
be counted.  The units of extent are combined with the
\token{timeUnits} attribute on \Model to specify the units of the
rate of change of reaction extent; this defines how the numerical
value of each \KineticLaw's mathematical formula
(Section~\ref{subsec:kinetic-law}) is meant to be interpreted.  A
list of recommended units is given in
Section~\ref{sec:bp:unitdefinitions:recommendedunits}.
...
only one global \token{extentUnits} and one global
\token{timeUnits}.
...
\begin{table}[bht]
  ...
  \caption{Base units defined in SBML.  These symbols are
    ...
    handling certain common situations.  The unit  ``\token{dimensionless}''
...
SI unit \token{mole} is technical \emph{a unit of measure for
...
\item The \token{scale} attribute can be used to set the exponent
  ...
  must be set to \val{0} (zero), because $10^0 = 1$.
...
value of \token{spatialDimensions} does affect the interpretation
of the units associated with a compartment's \token{size} value,
as described in the next section.  In addition, some SBML Level~3
packages may use the value of \token{spatialDimensions} in ways
...
Table~\vref{tab:comp-size-units}.  If neither the \token{units}
...
\begin{table}[tbh]
  ...
  \begin{edtable}{tabular}{cl}
    \toprule
    \textbf{Value of attribute} & \class{Model} \textbf{attribute used} \\[-2pt]
    \token{spatialDimensions}  & \textbf{for inheriting units of size} \\
    \midrule
    \val{3}                     & \token{volumeUnits} \\
    \val{2}                     & \token{areaUnits} \\
    \val{1}                     & \token{lengthUnits} \\
  ...
  \caption{When a \Compartment object instance does not specify a
    ...
    \token{spatialDimensions} attribute, and the right-hand column
...
associated with its value are \quantity{substance}/\quantity{size}
units, where the units of \quantity{substance} are those specified
by the \token{substanceUnits} attribute, and the \quantity{size}
...
case, \quantity{substance} units are involved and determined by
the \token{substanceUnits} attribute.  If the
...
the \token{substanceUnits} attribute on the enclosing \Model
...
% OLDER TEXT -- REMOVE ONCE THE FINAL SENTENCE IS MERGED IN
%  in the \token{initialAmount}
% or the \token{initialConcentration} attribute are determined by a
% combination of factors described in
% Section~\ref{sec:species-units} below.  The interpretation of
% species quantities under different conditions is explained in
% Section~\ref{sec:species-meaning}.
...
\primtype{boolean} attribute \token{hasOnlySubstanceUnits}
...
transport, binding or other kind of process that can change the
quantity of one or more species in a model.  The classical meaning
of a \emph{reaction} is a process in which some entities (\ie
species) interact in some way, at some rate.  In SBML, the notion
of a reaction is slightly generalized to allow entities that may
not be chemical substances; thus, a reaction in SBML does not
necessarily have to be a biochemical reaction---a biochemical
reaction is just one possible kind of process.

At minimum, to describe a reaction in SBML, it is necessary to
define its \emph{structural} properties, \ie the participating
reactants and/or products (and their corresponding
...
\Reaction component is optional.  For some modeling purposes,
models containing reactions without defined rates is an acceptable
...
inspecting its rate expression.  However, a reaction is not
required to have a rate formula (because \KineticLaw is an
optional component of \Reaction), and besides, when a rate
expression is present, it may not always be possible to deduce the
reversibility by inspecting it.  Having a separate attribute for
\token{reversible} allows certain kinds of structural analysis,
such as elementary mode analysis, even in these cases.
...
would be interpreted as an assertion that the rate expression will
not have negative values during a simulation.  Software developers
may wish to provide their software systems with a means of testing
...
possibility that a model could contain contradictory information,
but this would be considered as a problem of the encoded model,
rather than as an invalid SBML encoding.
...
choosing correct rate laws, and visualization purposes.
...
of the model.  This optional identifier allows \SpeciesReference
and \ModifierSpeciesReference instances to be referenced from
other objects.  In the case of \SpeciesReference objects, the
identifier also has a special meaning: \emph{it stands for the
  value of the \token{stoichiometry} attribute} of a
\SpeciesReference object instance.  This allows the stoichiometry
of a species to be accessed from other parts of the model.  This
feature is discussed in more detail in
Section~\ref{subsec:speciesreference} below.
...
and which species act as products in a reaction.  The
\SpeciesReference structure inherits the mandatory attribute
\token{species} and optional attributes \token{id}, \token{name},
and \token{sboTerm} from the parent type \SimpleSpeciesReference;
see Section~\ref{subsec:simplespeciesreference} for their
definitions.  It also defines the optional attribute
\token{stoichiometry} and the mandatory attribute
\token{constant}, described below.

The \token{species} attribute value must be the identifier of an
existing species defined in the enclosing \Model; the species is
thereby designated as a reactant or product in the reaction.
Which one it is (\ie reactant or product) is indicated by whether
the \SpeciesReference appears in the \Reaction's \token{reactant}
or \token{product} lists.
...
The \SpeciesReference object has a mandatory boolean attribute
named \token{constant}, used to indicate whether the stoichiometry
can vary during a simulation.  A value of \val{true} for this
attribute means that the corresponding species' stoichiometry in
the reaction cannot be changed by other constructs elsewhere in
the model except by an \InitialAssignment.  A value of \val{false}
means the stoichiometry can be changed by other SBML constructs
such as rules (see Section~\ref{sec:rules}), as described above in
the section on the \token{stoichiometry} attribute.
...
elsewhere in the model.  When the identifier appears in \token{ci}
...
% 2009-08-12 <mhucka@caltech.edu>
% Redundant with what's written in previous 2 subsections:
%
% As explained above, the identifier of a \SpeciesReference object
% can be the subject of an \InitialAssignment object.  (An example
% of this is given below.)  In addition, if the \SpeciesReference
% object's \token{constant} attribute has the value \val{false}, the
% \token{id} can also appear in \RateRule, \AssignmentRule, or
% \EventAssignment constructs in the model; this thereby provides a
% means of defining variable stoichiometries, for those models where
% it is appropriate.
...
\item The units of the \token{math} element of a \RateRule that
  ...
  identical to 1/\quantity{time}, where \quantity{time} is the
  model-wide \token{time} units (Section~\ref{sec:timeunits}).
...
\LocalParameter objects that each associate an identifier with a
value; this identifier can then be used in the kinetic law.  The
definition of \LocalParameter is shown in
Figure~\vref{fig:reaction}.
...
in Section~\ref{sec:identifiers}, and about the use of names in
...
The units of the parameter are used in the following way: when the
local parameter's identifier appears in the content of the
\token{math} element of the \KineticLaw object, the units
associated with the value are those declared by the local
parameter's \token{units} attribute.
...
species identifiers, as discussed in Section~\ref{sec:ci-token}.
The only \Species identifiers that can be used in \token{math} are
those declared in the lists of reactants, products and modifiers
in the \Reaction object (see
Sections~\ref{subsec:simplespeciesreference},
\ref{subsec:speciesreference} and~\ref{subsec:modifierreference}).
\LocalParameter identifiers can only be taken from the
\KineticLaw's list of local parameters (see below); \Parameter
identifiers refer to the parameters defined globally on the \Model
instance.
...
definition to shadow a global identifier \emph{other} than a
parameter.  In SBML's simple symbol system, there is no separation
of symbols by class of object; consequently, inside the kinetic
law mathematical formula, the value of a local parameter having
the same identifier as a species or compartment or other global
model entity will override the global value.  Modelers and
...
\paragraph{SBML ``kinetic laws'' compared to traditional rate
  laws}
...
normally used in traditional rate laws are problematic when used
for defining reactions between multiple compartments.
...
expression will therefore be $k \cdot [S_1]$, with the units of
$k$ being $1/\emph{time}$.  Then:
...
In multicompartment models, if one wants to be able to specify a
rate law only once and then use it unmodified in equations for
different substances, the rate law needs to be expressed in terms
of \quantity{substance}/\quantity{time}, rather than the more
typical \quantity{substance}/\quantity{size}/\quantity{time}.  As
a result, modelers and software tools in general cannot insert
textbook rate laws unmodified as the rate expression in the
\token{math} element of a \KineticLaw.  The unusual term ``kinetic
law'' was chosen to alert users to this difference.
...
event is counted.  The value of the mathematical expression in the
...
this reaction) describes the contribution of the reaction to the
rate of change of the quantity of the species in the overall
system.
...
When constructing a set of equations dictating the rates of change
of the different species in an SBML model, we only need to
consider those species with \Species object attributes
\token{constant}=\val{false} and
\token{boundaryValue}=\val{false}, because as discussed in
Section~\ref{sec:species-constant}, species not meeting these
criteria are not affected by the reactions in the model.  (The
latter kind of species may be affected by other SBML constructs,
but here, we are focusing specifically on the implications of
reactions.)  Assume now a model in which $N$ species $S_{1}$,
$S_{2}$, \ldots{}, $S_{N}$ having \token{constant}=\val{false} and
\token{boundaryValue}=\val{false} participate in $M$ reactions
...
Let \vrj represent the rate or velocity of reaction \rj as given
by the formula in the \token{math} element of \KineticLaw object
for \rj.  The units associated with this rate expression are
\emph{extent}/\emph{time} units, where the extent and time units
are specified by the corresponding attributes on the \Model
object.  Let \stoichij represent the overall stoichiometry of
species \si in reaction \rj.  (By ``overall stoichiometry'', we
mean the number that results from taking the sum of the
stoichiometry values of all references to \si in \rj's
...
In Section~\ref{sec:bp:reactions}, we discuss some recommendations
and examples of how to encode models and reactions in SBML.

\paragraph{Dimensions of the kinetic law}

One important point about ODEs and kinetic laws in SBML is that the
left hand side of the ODE is always the rate of change of the \emph{amount}
of the species, never the \emph{concentration }(or, more generally,
the \emph{density}, i.e. the amount per size of the compartment)\emph{.
}In other words, the left hand side is an \emph{extensive }quantity.
Correspondingly the right hand side and therefore the result of the
kinetic law has to be an extensive quantity as well. This means the
kinetic law does \emph{not} describe how often the reaction would
take place in a compartment of unit size but how often it takes place
(per time unit) given the actual compartment sizes of the model. The
dimension of the kinetic law is therefore \emph{number of reaction
events per time.}

This definition of the kinetic law differs from the traditional kinetic
laws that can be found in many publications and textbooks where it
is often given in units of \emph{concentration per time }(\emph{intensive
}quantity)\emph{.}

The reason that SBML uses a different definition is that the formalism
using traditional kinetic laws only works well for the description
of models for which certain assumptions hold, whereas SBML supports
a more general class of models. Some of these assumptions are:

\begin{itemize}
\item All species that participate in a given reaction are located in one
compartment. SBML allows transport reactions and other reactions that
involve several compartments.
\item Compartment volumes are constant. SBML allows variable compartment
sizes.
\item Kinetic laws in units of concentration per time assume that the reactions
take place homogeneously in the compartment volume, not e.g. at a
membrane. Mathematically this means the reaction rate scales with
the compartment volume. SBML does not make this assumption.
\end{itemize}

\paragraph{Translating traditional kinetic laws to SBML kinetic laws}
...
\begin{center}
$R$: \ce{S_1 ->[v_R] S_2}, with $v_{R}=\frac{v_{max}\cdot[S_{1}]}{K_{M}+[S_{1}]}$
\par\end{center}
...
In the discussion of best practices in
...
\item \emph{In the case of a species}, an \EventAssignment sets
  ...
  \quantity{amount of substance}) to the value determined by the
  formula in \token{math}.  The overall units of the \token{math}
...
\item \emph{In the case of a species reference}, an
  ...
  should be \token{dimensionless}, because reactant and product
  stoichiometries in reactions are dimensionless quantities.
