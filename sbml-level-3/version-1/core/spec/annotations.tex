...
\newcommand{\otherContentNS}   {\fbox{\texttt{\scalefont{1.2}{+++}}}}
...
\newcommand{\historyNS}        {\colorbox[gray]{.8}{\texttt{[HISTORY]}}}
...
\begin{example}
<\sbmlElement \otherContent metaid="\sbmlMetaNS" \otherContent >
  \otherContent
  <annotation>
    \otherContent
...
\sbmlElement refers to any SBML element name that can contain an
\token{annotation} element; \sbmlMeta is an XML \primtype{ID}
string; \relationElement refers to element names in either the
namespace \uri{http://biomodels.net/biology-qualifiers/} or
\uri{http://biomodels.net/model-qualifiers/}; and \resourceURI is
a URI (See Section~\ref{sec:uri-in-annotation}).  \history refers
to an optional section described in
