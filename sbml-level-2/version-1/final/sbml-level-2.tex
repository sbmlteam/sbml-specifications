...
\item If all other situations, the referenced identifier must be the
  ...
  \item \emph{Species identifier}: When a species identifier occurs in a
    ...
    on the \class{Compartment} structure where the species is declared to be
    located.
  ...
  \item \emph{Compartment identifier}: When a compartment identifier occurs
    ...
    The units associated with the the size of the compartment are those
...
substance, volume, area, length and time.  SBML defines defaults for these
...
\texttt{multiplier} values of one.  The various components of a model,
