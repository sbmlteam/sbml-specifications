...
\markboth{\textit{SBML Level 2 Proposal}}{\textit{SBML Level 2 Proposal}}
...
\date{SBML Level 2, Version 1\\[5pt]\fbox{Working Draft Revision 2}\\[5pt]
  \today{}}
...
\item Formulas in Level~2 are expressed using MathML~\citep{w3c:2000b}.
  ...
  \class{KineticLaw} and \class{Rule} structures has been replaced by an
  element named \class{math} containing MathML content; see
...
\class{SId} is a data type derived from the basic XML type \class{string},
...
structures have \class{math} subelements, and a function definition has a
single MathML \class{lambda} subelement.  The XML namespace for all of
...
A \class{Model} data object may contain optional lists of \class{Species},
\class{Compartment}, \class{FunctionDefinition}, \class{UnitDefinition},
\class{Parameter}, \class{Reaction}, and \class{Rule} components (they are
optional because the lists in fields \attrib{species},
\attrib{compartment}, \attrib{functionDefinition}, \attrib{unitDefinition},
\attrib{parameter}, \attrib{reaction}, and \attrib{rule} are permitted to
have zero length).

The \class{Model} structure has an optional \attrib{id} field that can be
...
default value for the \attrib{constant} field is \texttt{true} because in
...
the basic \class{AssignmentRule} type.  The \attrib{parameter} field
has type \class{SId} and identifies the parameter affected by the
...
expression in the \attrib{math} field.  No more than one
...
corresponding structure has the \attrib{constant} set to \texttt{true}.
...
are always evaluated in the order given in SBML.  The \attrib{math} field
of a \texttt{scalar} rule structure can contain any identifier in a MathML
\class{ci} element except for the following: (a) those identifiers for
...
the \attrib{listOfSpecies} container of the \class{Model} data structure
(see Section~\ref{sec:model}).  Lists of modifiers, products and reactants
in \class{Reaction} type structures refer to those species.  The connection
between the products and reactants in a reaction definition and the species
names listed in the enclosing \class{Model} definition is achieved using
the \class{SpeciesReference} type data structure defined in
...
\class{Model}-type structure.  Refer to Table~\ref{tab:specieattrib} to
determine the attribute values of species that can be referenced by
\class{SpeciesReference} structures.
...
reaction and the species names listed in the enclosing \class{Model}
definition is achieved using the \class{ModifierSpeciesReference} data
structure defined in Figure~\vref{fig:reaction}.  The field
\attrib{species}, of type \class{SId}, must refer to the name of a species
defined in the enclosing \class{Model}-type structure.
...
The field \attrib{math} can contain MathML expressions to express the rate
...
\attrib{math} are those listed in the \class{Reactant}, \class{Product} and
\class{Modifier} fields of the \class{Reaction} structure.
...
%-----------------------------------------------------------------------------
We propose that the \attrib{compartment} field of the \class{Species} type is optional.  By default, when this field doesn't have a value, the species is placed in a compartment of unit volume.  This means that a compartment structure must be always occur in a model with a species structure.
