...
\subsection{The \FBC \class{listOfKeyValues}}
\label{listofkeyvalues-class}

The \class{listOfKeyValues} forms the basis of a controlled annotation defined by the \FBCPackage. While there is a clear need in the modelling community for a `structured note' or `descriptive list' of keys and associated values:
%
\begin{quote}
\begin{description}
  \item[key X] some arbitrary value
  \item[keyZZ] alternative measure of beard
  \item[covfefe] intangible metaphysical property
\end{description}
\end{quote}
...
As shown in \ref{fig:fbc_v3_uml_keyvalue} the \ListOfKeyValues defines the \token{listOfKeyValues}. It \newtxt{does not} inherit the
...
\newtxt{Sarah notes that the introduction of url can create precedence and validation issues. I need to look into this and in general expand the KVP text.}
