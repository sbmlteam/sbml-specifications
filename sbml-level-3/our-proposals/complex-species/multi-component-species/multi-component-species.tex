...
\end{example}
\caption{The \texttt{malate\_aspartate\_shuttle3} model which has duplicate reactions for each compartment.} 
\label{fig:malate_aspartate_shuttle3-xml}
\end{figure}

Under this proposal we can replace the 2 reactions in Figure~\ref{fig:malate_aspartate_shuttle3-xml} with a single reaction structure as shown in Figure~\ref{fig:malate_aspartate_shuttle4-xml}
...
for a \class{SimpleSpeciesReference} to refer to set of closely related species that have a 
similar but not identical chemical structure.  
This is represented using \class{GenericBond} which is an
alternative \class{Bond} type.  The reactions containing containing a \class{GenericBond} can potentially
...
structures referenced by the \class{SpeciesTypeInstances} in a single \class{SpeciesType}
...
\attrib{otherBindingSiteReference}.
\attrib{otherBindingSiteReference} is optional.
If \attrib{otherBindingSiteReference} is not present the \class{SpeciesBond} structure represents
the state where \attrib{bindingSite} is unbound.
...
\subsection{Use generic bonds in \class{SpeciesType} structures}
