...
The \token{substanceUnits} attribute specifies the default units
of measurement associated with the quantity of each \Species
object in the model.  The attribute's value must be of type
\primtype{UnitSIdRef} (Section~\ref{sec:unitsidref}).  A list of
...
quantity is assumed to be in the units specified by the \Model
\token{substanceUnits} attribute.  If the \Model does not define a
value for this attribute, then there is no default for the model,
and all species that do not specify individual
\token{substanceUnits} values then have \emph{no} declared units
for their quantities.  As discussed elsewhere in this
specification document, leaving the units of species quantities
...
all models specify the units of measurement for every species
quantity, using the \token{substanceUnits} attribute on either
\Model or \Species.
...
units of time for a given model.  It is a global attribute, which
means that time is measured in the same units everywhere in the
model.  This is particularly relevant to the definition of
reactions in a model: all \Reaction object instances in an SBML
generate per-time values, and the time units are those given by
the \token{timeUnits} attribute on the \Model object instance.
Since there are no default values in \thisL, if the
\token{timeUnits} attribute has no value, it means that the units
of time are not defined for the model's reactions.  Leaving the
units unspecified in an SBML model does not result in an invalid
model; however, as a matter of best practice, we strongly
recommend that all models specify the units of measurement for
time.

An additional point worth noting in this context concerns the lack
of built-in assumptions about the units of measurement associated
with the mathematical expression in the \Delay element within
\Event (Section~\ref{sec:event-delay}) and the \emph{delay}
...
units with the values explicitly.  An approach for doing this for
\Delay is discussed in Section~\ref{sec:event-delay}.
...
\token{lengthUnits} together are used to set the default units of
measurement for the sizes of compartments.  The different
...
If a given \Compartment object definition does not specify a value
for its \token{units} attribute, then that compartment's size is
taken to be measured in the units specified by the \Model
\token{volumeUnits}, \token{areaUnits} or \token{lengthUnits}, as
appropriate based on the \Compartment object's
\token{spatialDimensions} value.  If the model does not define the
relevant attribute, then there is no default for the units of
compartment size for compartments having the corresponding
\token{spatialDimensions} value.  As discussed elsewhere in this
specification document, leaving the units of species quantities
undefined in an SBML model does not render the model invalid;
however, as a matter of best practice, we strongly recommend that
all models specify the units of measurement for every compartment
size, using either the \token{volumeUnits}, \token{areaUnits} or
\token{lengthUnits} attributes (as appropriate), or the
\token{units} attribute on \Compartment.

The use of three separate attributes is a carry-over from a scheme
used in SBML Level~2.  Note that it is quite possible for a model
to define a value for two or more of the attributes
\token{volumeUnits}, \token{areaUnits} and \token{lengthUnits}
simultaneously, because SBML models may contain mixtures of
