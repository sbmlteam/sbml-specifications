...
\newtxt{However, the use of \textsf{R}, \textsf{X} and $(\ldots)_n$ is not encouraged as, it may hinder the balancing of any Reaction in which such a \Species occurs. Therefore, any \token{chemicalFormula} that contains any of the aforementioned, non-Hill compatible symbols will raise a `best practices' warning on model validation.}
...
Analogous to the attributes described in \ref{reaction-class-ga} the \token{lowerBound} and \token{upperBound} values bound the \UserConstraint.
%
\begin{eqnarray}
...
The \UserConstraint contains a \ListOfUserConstraintComponents or a linear combination of \UserConstraintComponent{s}. Similar to a \FluxObjective each \UserConstraintComponent contains a coefficient--variable pair where the \token{coefficient} refers to a \Parameter. In addition, the \UserConstraintComponent allows the \token{variable} to refer to non-constant \Parameter thus allowing the definition of non-reaction, artificial, variables.
