...
%\usepackage{pstricks} % this library may not be required. 
\usepackage{dashbox}
\usepackage{datetime}
\usepackage{verbatim}
...
\newcommand{\listOfSpeciesFeatureValues}{\objFont{list\-Of\-Species\-Feature\-Values}}
...
\ifnum\pdfstrcmp{#1}{\colorVersion}=0
\begin{blockChanged}
\fi}
...
% conditional textChange
\newcommand{\mTextChanged}[2]{%
...
\newcommand{\mExampleFile}[2]{%
\ifnum\pdfstrcmp{#1}{\thisVersion}=0
\lstset{style=XML}\examplespacing\lstinputlisting{#2}\regularspacing%
\else
\exampleFile{#2}%
\fi
}
