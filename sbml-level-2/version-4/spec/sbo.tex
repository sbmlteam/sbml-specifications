...
identifying most elements of SBML}. The relationship implied by an \token{sboTerm} value on an SBML model component is ``is a'': the
thing defined by that SBML component ``is an'' instance of the
thing described in SBO by the term having the identifier given by the \token{sboTerm} attribute value. By adding SBO term
...
mathematical expression, and (6) relationship or process}.
...
\begin{figure}[htb]
  ...
  \centering
  \includegraphics
  [scale = 0.8]{figs/sbo-participant-role}
  \caption{Partial expansion of some of the terms in the
...
\begin{figure}[tbh]
  ...
  \centering
  \includegraphics
  [scale = 0.8]{figs/sbo-quantitative-parameter}
...
\begin{figure}[tbh]
  \centering
  \includegraphics
  [scale = 0.8]{figs/sbo-relationship-process}
  \caption{Partial expansion of some of the terms in the \changed{\emph{relationship or process} branch}.}
  \label{fig:sbo-event}
\end{figure}
\begin{blockChanged}
...
element of an object. If the \token{annotation} path is choose, the qualifiers (Section~\ref{sec:qualified-dc-annotation}) linking SBML element and SBO term should then be choosen extremely carefully, since it will no longer be possible to assume an ``instance to class'' relationship. 
\end{blockChanged}
