...
\changed{Note that the \distrib package is being released as an \threetwo package, but that it may be used with \threeone as well.  When there is a difference, the difference will be noted.}
...
The \UncertValue class inherits from the \threetwo \SBase class, meaning it has the optional attributes \token{id}, \token{name}, \token{metaid}, and \token{sboTerm}, in addition to having the optional children \token{notes} and \token{annotation}.  The class has these features even in the context of an \threeone document, where the \SBase class did not have an \token{id} nor a \token{name}.  The \UncertValue \emph{does} have mathematical meaning which may be used in other contexts, but that meaning may not be set directly by any other SBML element of any level, version, or package.  If setting the value is desired, the \token{var} attribute should be used, and that referenced element set as per normal SBML procedures.  The meaning is provided mostly to allow access to the \token{val} attribute, which otherwise would be undiscoverable to any other SBML element.  Do note, however, that the scope of the \UncertValue \token{id} is limited to its \FunctionDefinition parent in that context.  In other contexts, its \token{id} has the scope of its parent \Model.  
...
The \ExternalParameter inherits from \UncertValue, and adds the required attribute \token{definitionURL}, of type \primtype{ExternalRef}, and an optional child \ListOfExternalParameters.  The \token{definitionURL} must be a URI that defines a valid distribution-related parameter.  It is again strongly recommended that modelers use distributions from ProbOnto (\url{http://probonto.org/}), as consistently referencing a single ontology will improve exchangeability.
...
No inputs are provided.  Three of the four \Category children of the \CategoricalDistribution all have equal values for their \token{probability} children, with the fourth not having its probability set, so that and sum to 1.0, as they must.  Each \token{name} is therefore equally likely to be chosen, resulting in this function returning \val{1}, \val{2}, \val{3}, or \val{4}, each with equal probability.
...
%\begin{example}
%<sbml xmlns:distrib="http://www.sbml.org/sbml/level3/version2/distrib/version1" xmlns="http://www.sbml.org/sbml/level3/version2/core" level="3" version="2" distrib:required="true">
%  <model>
%    <listOfFunctionDefinitions>
...
\begin{figure}[bh]
  ...
  \caption{The definition of the extended \SBase class to include a new optional \Uncertainty child, which in turn has an \Uncertainty child in the \uncertml namespace.  Intended for use with any element with mathematical meaning, or with a \Math child.}
