...
\changed{Overall, the \token{rotate} attributes define \emph{where} the rotation is to occur, and the \token{rotateAngleInRadians} define \emph{how much} rotation is to take place.  A \token{rotateAngleInRadians} value of \val{0} would therefore define a \CSGRotation that remained fixed in space.}
