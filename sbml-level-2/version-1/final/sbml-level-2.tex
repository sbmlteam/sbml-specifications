...
\item Formulas in Level~2 are expressed using MathML~\citep{w3c:2000b}.
  ...
  discontinuous piece wise logical operators, something that was not
  included in SBML Level~1.
  
\item All attributes which contain initial conditions or parameter values,
  including compartment sizes and species concentrations,
...
\item The \class{Compartment} structure has a new positive integer attribute \attrib{spatialDimension} which specifies
  the number of dimensions of space the compartment is composed of.  The units of species concentration in SBML Level 2 depends
  on the spatial dimensions of the compartment where the species is located.  To support this new feature two new built-in units
  \emph{area} and \emph{length} have been introduced to Level 2.  The size of a compartment is given by its \attrib{size} attribute.
  See Section~\ref{sec:compartments}.
  
\item The \class{Species} structure has an additional attribute \attrib{initialConcentration}
  which allows the initial value of a species variable to be set in $concentration$ units.  See Section~\ref{sec:species}.
...
\item \class{Rule} structures have been changed and simplified.  Instances of
  \class{AssignmentRule} and \class{RateRule} replace instances of \class{ParameterRule},
  \class{SpeciesConcentrationRule} and \class{CompartmentVolumeRule}.
  The \attrib{type} attribute is not present on rule structures in level 2.
...
\item All elements, including \class{Sbml} and
  \class{listOf}\rule{0.5in}{0.5pt} elements, are derived from the base
  type \class{SBase}.  The elements contained inside \class{SBase} are not
  derived from \class{SBase}.

\item SBML Level 2 is encoded in UTF-8.  SBML documents must refer to the
  UTF-8 encoding in their XML prefix. See Section~\ref{sec:sbml}.

\item the unit kinds \texttt{meter} and \texttt{liter} are removed in SBML
  Level 2.  See Section~\ref{sec:unitdefinitions}.

\item \class{Unit} structures have additional attributes
  \attrib{multiplier} and \attrib{offset} to enable
  the definition of non-SI units.  See Section~\ref{sec:unitdefinitions}. 
...
in Figure~\vref{fig:top-level}.  In addition to this, all elements, including \class{sbml}, elements
used to form fields (e.g. \attrib{trigger} on \class{Event}) and
\class{listOf}\rule{0.5in}{0.5pt} elements, are derived from \class{SBase}.  The elements
contained inside \class{SBase} are not derived from \class{SBase}
...
match the value of an \attrib{id} field of a component.  When a species
identifier occurs in a \class{ci} element, it represents the concentration
of the species. The units of a species symbol is $substance/size$ where
$size$ is either $volume$, $area$ or $length$ when the containing
compartment's spatial dimension is 3, 2, or 1 respectively. If the
compartment's spatial dimension is 0 then the species symbol's units is
$substance$. When a compartment identifier occurs in a \class{ci} element,
it represents the size of the compartment where units are either $volume$,
$area$, $length$, $dimensionless$ when the compartment's spatial dimension
is 3, 2, 1, or 0 respectively. The units of substance, volume, area and
length are determined from the built-ins \texttt{substance},
\texttt{volume}, \texttt{area} and \texttt{length} of
Table~\vref{tab:builtin} respectively.
...
\texttt{http://www.sbml.org/sbml/level2}.  All SBML Level 2 elements should be encoded
using this URI by assigning this URI to either the default namespace or a
tag prefix.
The character encoding for SBML is UTF-8.  SBML documents should include the
\attrib{encoding} attribute with the value \attribvalue{UTF-8} in the XML prologue.

In the transformation of UML to
...
respective fields are permitted to have zero length).  If a \class{Species}
structure is present then a \class{Compartment} structure must be present.
If a \class{Reaction} structure is present then a \class{Species} must be present.
...
translated into lists of XML elements enclosed within elements of the form
...
\begin{table}[bh]
  ...
  \begin{tabular}{llllll}
    ...
    \underline{dimensionless} & \underline{item}
                        & lumen     & pascal    & tesla\\
    \bottomrule
  \end{tabular}
...
\texttt{kind="second"} and \texttt{exponent="-2"}. The
...
The optional \attrib{multiplier} double attribute can be used to
pre-multiply the \attrib{kind} unit by any real scale factor.
For instance, a multiplier of 0.45359237 is used to define a pound
in terms of a kilogram. The \attrib{multiplier} attribute has a default value
of 1.  The \attrib{offset} attribute is used to represent the addition of a
constant in the transformation the \attrib{kind} unit.
For example, an \attrib{offset} attribute value of "32.0" is needed to define Fahrenheit in
terms of Celsius. The \attrib{offset} attribute has a default value of 0. 
The \attrib{offset} is applied to the unit of type \attrib{kind} \emph{after}
the \attrib{multiplier} and \attrib{scale} attributes i.e. the conversion from
a value $x_old$ in \attrib{kind} units to its equivalent defined units is given by

\begin{equation}
(multiplier \, 10^{scale} \, x_{old}) + offset
\end{equation}
...
Table~\ref{tab:builtin}, corresponding to the five types of quantities
that play roles in biochemical reactions: amount of substance, volume, area, length and
...
three predefined names ``\texttt{substance}'', ``\texttt{time}'', ``\texttt{length}'',
``\texttt{area}'', and ``\texttt{volume}'' from Table~\ref{tab:builtin}.  The latter usage
...
\begin{table}[htb]
  ...
  \begin{tabular}{lllc}
    ...
    \midrule
    \texttt{substance} & moles \emph{or} no. of molecules     & mole \\
    \texttt{volume}                & litre          & litre \\
    \texttt{area}                & square metre          & square metre \\
    \texttt{length}                & metre           &  metre \\
    \texttt{time}                  & second          & second \\
    \bottomrule
  \end{tabular}
  \end{blockChanged}
...
\class{string}.  A \class{Compartment} structure has the optional
positive integer field \attrib{spatialDimension} which contains the
number of spatial dimensions over which the species, contained in or
on the compartment, are distributed.  The maximum value of \attrib{spatialDimension} is 3.
\attrib{spatialDimension} has a default value of 3 meaning that by default a compartment has volume.

A compartment also has an optional floating-point field
called \attrib{size}, representing the total size of the
compartment.  The \attrib{size} field enables concentrations of species
to be calculated in the absence of geometry information.
\attrib{size} has the default units of \texttt{volume},
\texttt{area}, \texttt{length} or \texttt{dimensionless} when the
\attrib{spatialDimension} attribute has values 3, 2, 1 or 0
respectively (See Tables~\vref{tab:builtin} and~\vref{tab:unitkind}).
The \attrib{size} field is optional.  A missing \attrib{size} value
implies that the value is either unknown, not required for analysis,
or available from an external data source.  A missing \attrib{size}
value does not imply that the compartment size is 1.
...
indicates that the compartment's size can be determined by rules, and the
value of the \attrib{size} field should be taken to be the initial size
...
The units of compartment size, that is the unit of the \attrib{size} field only,
may be explicitly set using the optional field
\attrib{units} in \class{Compartment}; the named units must be either one
of the base units from Table~\vref{tab:unitkind}, the built-in defaults
named \texttt{volume}, \texttt{area}, \texttt{length}, or a new unit defined
by a unit definition in the enclosing model.  
...
provide for spatial characteristics aside from compartment size and
...
A \class{Compartment} structure must be present in a model if that model
contains a \class{Species} structure.
...
compartment in which the species is located.  There is no default value for the
\attrib{compartment} field.  The field must instead refer to a \class{Compartment}
structure by the value of that structure's \attrib{id} field.  A \class{Compartment}
structure must be present in a model if that model contains a \class{Species} structure.

The optional fields \attrib{initialAmount} and \attrib{initialConcentration}, of
type \class{double}, are used to set the initial amount of the species in the
named compartment.  These attributes are mutually exclusive and only one these
attributes can have a value on any given instance of a \class{Species}
structure.  Missing \attrib{initialAmount} or \attrib{initialConcentration}
values implies that their values are either unknown, not required for analysis
or available from an external data source.  \attrib{initialConcentration} must
not have a value if the species' compartment has a spatial dimension of 0.

The units of the \attrib{initialAmount} attribute value
...
The units of the \attrib{initialConcentration} attribute value
may be explicitly set using the optional field \attrib{units}.  The
value assigned to \attrib{units} must be either one of the unit names from
Table~\vref{tab:unitkind} or a new unit name defined by a unit definition in
the enclosing model.  If absent, the units default to $substance/volume$,
$substance/area$ or $substance/length$ units when the spatial dimension of
the species' compartment is 3, 2 or 1 respectively. $substance$, $volume$,
$area$ and $length$ are taken from Table~\vref{tab:builtin}.
...
Table~\vref{tab:unitkind}; one of the built-in unit names in first column of
Table~\ref{tab:builtin}); or the name of
...
Section~\ref{sec:namespaces} for further details.)  Parameters local to a reaction cannot
be changed by rules and therefore are implicitly always constant.
Parameters local to a reaction should not have a \attrib{constant} attribute value.
...
\begin{tabular}{ll}
  ...
  \emph{Assignment} rules, left-hand side is a scalar:         & $x = f(X)$\\
...
SBML uses an abstract \class{Rule} structure
...
distinguish the case of algebraic, assignment and rate rules.
Figure~\vref{fig:rules} gives the definitions of \class{Rule} and the
subtypes derived from it.  The figure shows there are three subtypes,
\class{AlgebraicRule}, \class{AssignmentRule} and \class{RateRule} defined directly from
\class{Rule}.  These correspond to the cases \emph{Algebraic}, \emph{Assignment} and
...
the assignment and rate rule cases.  An example of the use of \class{AlgebraicRule}
structures is given in Section~\ref{sec:algeraiceg}.

\subsubsection{\class{AssignmentRule}}

The rule type \class{AssignmentRule} is used to express equations that determine the value of variables.
...
\item \emph{In the case of a species}: the rule sets the referenced species's concentration
  to the value determined by the formula in \attrib{math}.  The units are in
  terms of $substance/size$, where the $size$
  ...
  \emph{Restrictions}: An assignment rule and a \class{SpeciesReference} structure (see
  ...
  set to ``\attribvalue{true}''.  No more than one \class{AssignmentRule} can refer to
  a given species in an SBML model definition.
  
\item \emph{In the case of a compartment}: the rule sets the referenced compartment's size to
  the size determined by the formula in \attrib{math}.  The units are units are either $volume$,
  $area$ or $length$ when the compartment's spatial dimension is 3, 2 or 1 respectively.
  
  \emph{Restrictions}: No more than one \class{AssignmentRule} can refer to
  a given compartment in an SBML model definition.
  
\item \emph{In the case of a parameter}: the rule sets the referenced parameter's value to
  that determined by the formula in \attrib{math}.  The units are the same as those used
  for the parameters initial value.
...
$substance$, $volume$, $area$ and $length$ are built-in unit types (see Table~\ref{tab:builtin}).

\subsubsection{\class{RateRule}}

The rule type \class{RateRule} is used to express equations that determine the rates
of change of variables.  The left-hand sides of rate rules can be the identifiers of species,
compartments, or parameters.  The effects are in general terms the same,
but have different effects depending on what variable is being set:
