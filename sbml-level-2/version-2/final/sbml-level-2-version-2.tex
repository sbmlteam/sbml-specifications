...
% macros for SBO
\newcommand{\sboref}{\emph{placeholder: reference to sbo}}
\newcommand{\sboproduct}{\emph{SBO:0000011} product}
\newcommand{\sboreactant}{\emph{SBO:0000010} reactant}
\newcommand{\sbomodifier}{\emph{SBO:0000019} modifier}
\newcommand{\sboparameter}{\emph{SBO:0000002} (quantitative parameter)}
\newcommand{\sborule}{\emph{placeholder: sbo rule term}}
\newcommand{\sboconstraint}{\emph{placeholder: sbo constraint term}}
\newcommand{\sbokineticlaw}{\emph{SBO:0000001} (rate law)}
\newcommand{\sboinitialassignment}{\emph{placeholder: sbo initial assignment term}}

\newcommand{\sbomodellingframework}{\emph{SBO:0000004} modelling framework}
\begin{document}
...
\author{Andrew Finney, Michael Hucka, Nicolas Le Nov\`{e}re}

\authoremail{\{afinney,mhucka\}@sbml.org, lenov@ebi.ac.uk}

\address{The SBML Team\\
California Institute of Technology\\
Pasadena, CA 91125, USA\\[3pt]
\url{http://sbml.org}}

\date{SBML Level 2, Version 2, Issue 1\\[5pt]
...
\begin{center}
The latest issue of the SBML Level 2 Version 2 specification is available at
\url{http://sbml.org/specifications/sbml-level-2/version-2/}\\[10pt]

This issue of the SBML Level 2 Version 2 specification is available at
\url{http://sbml.org/specifications/sbml-level-2/version-2/issue-1/}\\[10pt]

The list of errata for all issues of the SBML Level 2 Version 2 specification is available at
...
\begin{minipage}{5in}
  The Systems Biology Markup Language (SBML) is a machine-readable format
  for representing computational models of biochemical reaction
  networks and processes
  in a way that can be used by software systems to communicate and exchange
  those models.  This document completely describes SBML Level 2 Version 2
  and its translation into XML, the Extensible Markup Language.
\end{minipage}
\end{center}
...
language.  The version of SBML defined in this document, SBML Level~2,
...
to Level~2.  In addition, a large subset of the structures in Level~2 can
...
revisions within an SBML Level to adjust and refine language features.
The present specification document defines the SBML Level~2 Version~2.
Appendix~\ref{apdx:level1-level2} lists the differences between this version
and SBML Level~1 Version~2.  Differences in the specification
...
Each SBML Level+Version specification inevitably
requires minor editorial changes as the community of users
discovers errors and ambiguities.  As a practical reality, these
discoveries occur over time. Such problems are publicly announced
as \emph{errata} to the specification which clarify and
disambiguate the problems.  Errata identify the following types of
required changes to the specification: (a) formatting changes that
do not result in changes to text content; (b) corrections that do
not affect conformance; and (c) corrections that \emph{may} affect
conformance, but add no new features. A change that affects
conformance is one that: turns conforming data, processors, or
...
ambiguity or under-specified part of the specification in such a
way that an software whose conformance was once unclear becomes
clearly conforming or non-conforming.  [This definition of errata
is adapted from~\cite{jacobs:2004}.]

Errata result in new \emph{issues} of the SBML specification.
Each issue is numbered. The first issue of the specification is
numbered 1.  The errata for this specification be recorded in a
new section starting in issue 2. Each errata will be numbered with
the issue in which the errata is introduced. Errata do not change
the fundamental semantics or syntax of SBML, but rather clarify
and disambiguate the specification and correct errors.  (New
syntax and semantics are only introduced in SBML Versions and
Levels.) A complete list of the errata for SBML Level~2 Version~2
created since Issue~1 is made publicly available at
...
The Systems Biology Markup Language has been, and continues to be,
developed in collaboration with a large international community of
researchers and software developers.  As is the case with many contemporary
projects, the primary mode of interaction between members is electronic
mail, with discussions taking place on the community mailing list,
\texttt{sbml-discuss@caltech.edu}.  The mailing list archives are available
at \url{http://sbml.org/forums/}.  In Section~\ref{sec:acknowledgements},
we attempt to acknowledge as many contributors to SBML's development as we
can, but as SBML evolves, it becomes increasingly difficult to detail the
individual contributions on a project that has truly become a community
effort.
...
\item \emph{Compartment}: \changed{A well-stirred container of
finite size for substances.}
...
expression used to determine the initial conditions of the model.
This type of structure can only be used to define define how the
value of a variable can be calculated from other values and
variables at the start of simulated time.}

\item \emph{Rule}: A mathematical expression used in
  combination with the differential equations constructed based on the set
  of reactions in a model; \changed{it can be used to define how a variable
  can be calculated from other variables,
  or used to define the rate of change of a variable.  The set of rules can be
...
\class{SBase} contains four fields, all of which are optional:
\attrib{metaid}, \attrib{notes}, \attrib{annotation} and
\attrib{definitionURL}. These fields are discussed separately in
...
field, every data object derived from \class{SBase} can have its own value for \attrib{annotation}.
...
%-----------------------------------------------------------------------------
\subsection{Guidelines for the \changed{General} Use of the \attrib{annotation} Field in
  \class{SBase}}
\label{sec:annotation-use}
%-----------------------------------------------------------------------------

The \attrib{annotation} field in the definition of \class{SBase}
is formally unconstrained in order that software developers may
attach any information they need to the structures in an SBML
model.  However, it is important that this facility not be
misused.  In particular, it is critical that information essential
to a model definition \changed{or that can be easily encoded in
existing SBML structures} is \emph{not} stored in
\attrib{annotation}. Parameter values, functional dependencies
between model structures, etc., should not be recorded as
annotations.
...
model with items in a database.  \changed{For case (c) SBML Level
2 Version 2 contains a standard form for use within an
\attrib{annotation} field.  This form should be used in preference
to proprietary syntaxes, see Section~\ref{sec:finney-novere}.}
...
and simple way of creating a unique name string.
...
%-----------------------------------------------------------------------------
\subsection{Standard Form for the \attrib{annotation} Field in
  \class{SBase} when Referencing External Resources}
\label{sec:finney-novere}
%-----------------------------------------------------------------------------

This section describes the standard non-proprietary format for
referring to controlled vocabulary terms and database identifiers
from \class{annotation} elements.

%=============================================================================
\subsubsection{General Syntax for dublin-core annotation}
%=============================================================================

In the following description text of the form \class{rdf:*} and
\class{dc:*} refer to elements of the
\url{http://www.w3.org/1999/02/22-rdf-syntax-ns\#} and
\url{http://purl.org/dc/elements/1.1/} namespaces respectively.
These elements must conform to the RDF/XML Syntax Specification
recommendation from the W3C \citep{RDFsyntax} and the Guidelines
for implementing Dublin Core in XML \citep{DCguidelines}
respectively. Text of the form \class{dcterm:*} refers to the
\url{http://purl.org/dc/terms/} namespace which is described in
\citep{qualifiedDC, DCelements}.

In this format the annotation of an element is located in a single
\class{rdf:RDF} element contained within an SBML
\class{annotation} element. The format described in this section
only defines the form of the \class{rdf:RDF} element.  The
containing SBML \class{SBase} element must have a \attrib{metaId}
field value. (As this attribute is of the type \class{ID} its
value must unique to the entire SBML document.)
...
\texttt{\#<string>} where the string component is the same as the
value of the \attrib{metaId} field of the containing SBML element.

The \class{rdf:Description} element must contain a sequence of one
or more Dublin Core relation elements.  The specific form of these
elements will vary depending on the relationship between the SBML
component and referenced information or resources.  The specific
form of these elements is described in the following 2 sections.

The Dublin Core relation elements must contain a single
\class{rdf:Bag} element which in turn must contain one or more
\class{rdf:li} elements.  The \class{rdf:li} elements must have a
\attrib{rdf:resource} attribute containing a URI referring to an
...
To enable interoperability, the community has agreed on a set of
standard valid URI syntaxes. These URIs will always be composed as
"resource\#id". URI syntax rules are not a fixed part of the SBML
standard but are extendable independently from specific SBML
Levels and Versions. The web page \url{FIX ME} lists URI syntaxes
and possible physical links to controlled vocabulary and
databases. This list enumerates the set of strings that precede
the '\#', and for each member of this list there is a brief
summary of the syntax for the identifier following the '\#'.  The
list will evolve with the evolution of databases and resources.
This annotation format doesn't require software to access this
list to operate.

The annotation can be located at different depths within a model
component as is appropriate.

Two flavors of annotations can be used: with or without
qualifications.  These are described in the following sections.

%=============================================================================
\subsubsection{Non-qualified dublin-core annotation}
\label{sec:non-qualified}
%=============================================================================

The first type of annotation uses unqualified Dublin Core
annotation \citep{unqualifiedDC}. The SBML component is linked to
a precise external piece of information and the relationship is
assumed to be an 'is a' relationship from the SBML component to
the external resource.  In this case the Dublin core relation
elements used must be \class{dc:relation}.


The use of elements \texttt{rdf:Bag} allows multiple links to
external resources for a given SBML object as shown in the
following example, describing the two components of the human
complex calcium/calmodulin kinase II by their UniProt and KEGG
compound accessions and a Gene Ontology term.
...
The following reaction contains annotation, but its kineticlaw is
also annotated, referring to a ratelaw control vocabulary.

\begin{example} <reaction id="calciumBinding"
metaid="jb007">
  <annotation>
    <rdf:RDF
      xmlns:dc="http://purl.org/dc/elements/1.1/"
    ...
    <annotation>
      ...
      >
        <rdf:Description rdf:about="#hal9000">
          <dc:relation>
            <rdf:Bag>
              <rdf:li rdf:resource="http://www.biomodels.net/CVs/rateLaw/\#XYZ:0000001"/>
            </rdf:Bag>
          </dc:relation>
        </rdf:Description>
      </rdf:RDF>
...
%=============================================================================
\subsubsection{Qualified Dublin-Core annotation}
\label{sec:qualified}
%=============================================================================

Although useful in most of the cases, the unqualified annotation
is sometimes not sufficient. For instance, how is the following
annotation to be interpreted?
...
How can a parser tell the difference between the previous and the
following example.
...
In the former case, the species represents a complex between the
human proteins cdc2, cdc13 and rum1. The annotations point to
components, parts of the species. In contrast, in the second case,
the species represent a chemotaxis receptor, and the annotations
point to homologous receptors. They can all be represented by the
species, but not at the same time.

To enable the format to encode relationships other than 'is a'
between SBML elements and resource  a subset of \class{dcterms:*}
elements can be contained immediately within the
\class{rdf:Description} elements as well as \class{dc:relation}
elements. The valid set of \class{dcterms:*} elements consists of
\class{dcterms:hasPart}, \class{dcterms:isPartOf},
\class{dcterms:isVersionOf}, \class{dcterms:hasVersion} and
\class{dcterms:isReferencedBy}.  These elements take on the
following semantics:

\begin{tabular}{p{0.2\textwidth}p{0.8\textwidth}}
\hline
hasPart        & The SBML component includes the subject of the referenced resource, either physically or logically \\
isPartOf       & The SBML component is a physical or logical part of the subject of the referenced resource \\
isVersionOf    & The SBML component is a version, an instance of the subject of the referenced resource \\
hasVersion     & The subject of the referenced resource is a version, an instance of the SBML component \\
isReferencedBy & The SBML component is referenced, cited, or pointed to by the referenced resource \\
\hline
\end{tabular}

(These qualifiers were defined by the Dublin Core for a slightly
different purpose. A relation \texttt{hasPart} meant that the
described document physically contained the referenced document,
and a relation \texttt{isVersionOf} meant that the described
element was derived from the referenced document. In this format,
the component of the model described by the element is linked to a
piece of knowledge described in the reference resource. Although
the two concepts are on different logic level, the relationship
between the object described and the object referenced are the
same.)
...
beta chain''. This is the case for instance in the comatic
...
Because in this case we need an annotation representing the other
genes such as gamma or delta. One could enumerate all the known
proteins, but such an approach would almost surely lead to
...
``Calcium/calmodulin-dependent protein kinase type II alpha
chain'' is a part of the abovementioned ``calcium- and
...
Finally, one can mix different qualified sets in the same
annotation element. The following phosphorylation is annotated by
its exact KEGG counterpart, and by the generis GO term
``phosphorylation''.
...
%-----------------------------------------------------------------------------
\subsection{The \attrib{id} and \attrib{name} Fields on SBML
Components} \label{sec:idnameattribs}
%-----------------------------------------------------------------------------
...
(Section~\ref{sec:functions}), rules (Section~\ref{sec:rules}),
constraints (Section~\ref{sec:constraints}), reaction kinetics
...
\changed{\class{InitialAssignment}, \class{Constraint}},
...
is \emph{not valid} in MathML~2.0.  Elsewhere in SBML, when a
field value is declared to have type ``\attribtype{double}'' (a
...
\item If \class{ci} appears in the body of a function definition
...
\item \changed{In all other situations, the referenced identifier
must be the
...
be dereferenced as Internet addresses.  Also, the content of the
\class{csymbol} element is for rendering purposes only and can be ignored
by the parser.
...
MathML operators in SBML return results of two types: boolean and
numeric. The relational operators (\class{eq}, \class{neq},
...
boolean values. The type of an operator referring to a function
definition is the type of the top level operator of the function
definition.  All other operators, values and symbols return
numeric results.
...
\class{or}, \class{xor}, \class{not}); and

\item the \attrib{math} field of a \class{Constraint} structure.
...
\item the first arguments of \class{piece} and \class{otherwise}
operators; and
...
assigning this URI to either the default namespace or a tag
...
xmlns="http://www.sbml.org/sbml/level2/version2" level="2"
version="2">
  ...
</sbml>
\end{example}
...
\class{Event}, \class{Parameter}, \class{InitialAssignment},
...
\attrib{metaid}, \attrib{notes}, \changed{\attrib{annotation} and \attrib{definitionURL}} fields.
...
The \class{Model} structure has an optional field,
\attrib{sboTerm} which contains an Systems Biology Ontology
(SBO)\sboref term identifier referring to a modelling framework
...
defines the mathematical framework used in the model.
...
Section~\ref{sec:mathmlsubset} \emph{except} \texttt{lamdba}.
...
The term \emph{compartment type} refers to a set of compartments
of the same type where a compartment is a specific well stired
constainer of substances. The \class{CompartmentType} data
structure is intended to represent these compartment types.  Its
definition is shown in Figure~\vref{fig:comaprtmentType}.
...
\class{CompartmentType} structures are included in SBML to enable
\class{Compartments} (Section~\ref{sec:compartments}) of the same
type to be related together.  It is a conceptual construct; the
existence of \class{CompartmentType} structures in a model has no
effect on the model's numerical interpretation.  Simulators and
other numerical analysis software may ignore
\class{CompartmentType} structures and references to them in a
model.
...
determined by rules (see Section~\ref{sec:rules}) \changed{or initial assignments
(see Section~\ref{sec:initialAssignment})}, and the value of the
...
compartment definition a virtual compartment type is assumed for
that compartment, and no other compartment can belong to that
compartment type.

The value of \class{compartmentType} attributes on compartment
...
The units of the value in the \attrib{initialAmount} field is that
given by the \attrib{substanceUnits} field of the species
...
field is deprecated: parsers are free to ignore this field and
generators do not need to create this field.}
...
The \class{Parameter} structure has an optional field,
\attrib{sboTerm} which contains an Systems Biology Ontology
...
of other constants or the initial values of variables.  An
...
has type \class{SId} field and contains the value of an
...
The value calculated by an \class{InitialAssignment} overrides any
other initial value assigned to the given symbol. Unlike an
\class{AssignmentRule} structure, an \class{InitialAssignment}
structure only applies at the start of simulated time and can set
the value of a constant. In all cases, as would be expected, the
units of the formula in the \attrib{math} field are identical to
the units associated with the identifier given in the
\attrib{symbol} field, when that variable appears in other
formulae.
...
set of assignment rules that should be considered as a whole. This
set should not contain more than one assignment for the same
...
set of assignment rules should not contain algebraic loops: a
chain of dependency between these rules should terminate. Formally
consider the directed graph of assignment rules where nodes are
rules and directed arcs exist for each occurrence of a symbol in a
assignment rule \attrib{math} field.  The directed arcs start from
the rule defining the symbol to the rules that contain the symbol
in their math fields. Such a graph must be acyclic. Examples of
valid and invalid set of assignment rules are given in
Section~\ref{sec:ruleconstraints}.

\subsubsection{The \attrib{sboTerm} Field}

The \class{InitialAssignment} structure has an optional field,
\attrib{sboTerm} which contains an Systems Biology Ontology
(SBO)\sboref term identifier referring to an initial assignment
SBO term, that is a term derived from \sboinitialassignment.  The
...
The following example shows how the symbol $x$ can assigned the
initial value $2y$:
...
The \class{Rule} structure has an optional field, \attrib{sboTerm}
which contains an Systems Biology Ontology (SBO)\sboref term
identifier.  The \attrib{sboTerm} field should contain a SBO term
identifier referring to a SBO Term derived from \sborule.  The
\class{Rule} structure should have a 'is a' relationship with the
SBO term. The SBO term chosen should be the most precise (narrow)
term that matches the form of the \class{Rule}.
...
that variable appears in other formulae.  \quantity{time} is a
...
The \attrib{math} field of an assignment rule structure can
contain any identifier in a MathML \class{ci} element except for
the following: (a) identifiers for which there exists a subsequent
assignment rule, and (b) the identifier for which the rule is
defined.  \changed{These constraints are designed to eliminate
algebraic loops among the assignment rules}; eliminating algebraic
loops ensures that assignment rules can be evaluated any number of
times without the result of those evaluations changing.  As an
example, consider the following equations, in the order shown:
\begin{equation*}
  \begin{array}{lll}
...
A rule cannot be used to set the value of a reaction rate, that is, the
value of a \attrib{variable} field in a rule cannot be \attrib{id} value of a
\class{Reaction} structure.  However, the \attrib{math} field of a rule can
use of the value of a reaction rate by referencing the \attrib{id}
value of a \class{Reaction} structure within the MathML expression (using
the MathML \class{cin} element).
...
but may be overdetermined e.g. equations can't refer to
...
shows the definition of the \class{Constraint} structures.
...
field \attrib{message}, which contains an XHTML message that may
...
return a boolean value which is true when the model is a
...
predication of the behavior of the biochemical reaction network
...
The \class{Constraint} structure has an optional field,
\attrib{sboTerm} which contains an Systems Biology Ontology
(SBO)\sboref term identifier referring to a constraint SBO term,
that is a term derived from \sboconstraint.  The
...
The \class{Reaction} structure defines a symbol, the value of the
\attrib{id} field, that can be used in mathematical formulae see
Section~\ref{subsec:reaction-as-symbol} for further details.
...
below.  The abstract type \class{SimpleSpeciesReference} is \changed{used
simply to hold} the common field, \attrib{species},
...
The \class{Reaction} structure has an optional field,
\attrib{sboTerm} which contains an Systems Biology Ontology
(SBO)\sboref term identifier referring to a modelling framework
SBO term, that is a term derived from \sbomodellingframework. The
SBO term chosen should be the most precise (narrow) term that
...
\changed{The \class{SpeciesReference} structure has an optional
field, \attrib{sboTerm} which contains an Systems Biology Ontology
(SBO)\sboref term identifier referring to a product or reactant
SBO term, that is a term derived from \sboproduct or \sboreactant
respectively.  A product term should only be used when the
\class{SpeciesReference} structure is contained in the
\attrib{product} field of the containing \class{Reaction}
structure.  Similarly a reactant term should only be used when the
\class{SpeciesReference} structure is contained in the
\attrib{reactant} field of the containing \class{Reaction}
...
because it acts as a catalyst or inhibitor).  In SBML, such species are
called \emph{modifiers}.
...
list of reactants and products lists of the same reaction where it is
...
field, \attrib{sboTerm} which contains an Systems Biology Ontology
(SBO)\sboref term identifier referring to a modifier SBO term,
that is a term derived from \sbomodifier. The
\class{ModifierSpeciesReference} structure should have a 'is a'
relationship with the SBO term. The SBO term chosen should be the
most precise (narrow) term that captures the role of the given
...
\begin{blockChanged}
The \class{KineticLaw} structure has an optional field,
\attrib{sboTerm} which contains an Systems Biology Ontology
...
A \class{KineticLaw} structure forms an assignment rule assigning
the evaluated value of the \attrib{math} field to the symbol value
contained in the \class{Reaction} \attrib{id} field. No other
structure can assign a value to such a reaction symbol i.e. the
\attrib{variable} fields of \class{InitialAssignment},
...
The combined set of \class{InitialAssignment},
\class{AssignmentRule} and \class{KineticLaw} structures form a
set of assignment rules that should be considered as a whole. The
combined set of assignment rules should not contain algebraic
loops: a chain of dependency between these rules should terminate.
Formally consider the directed graph of assignment rules where
nodes are rules and directed arcs exist for each occurrence of a
symbol in a assignment rule \attrib{math} field.  The directed
arcs start from the rule defining the symbol to the rules that
contain the symbol in their math fields. Such a graph must be
acyclic. Examples of valid and invalid set of assignment rules are
...
\changed{A variable can't be assigned a value in an
...
appears in other formulae. However, the precise details, which are
...
defaults, ensuring size is in litres and time is in seconds.  The
...
and Technology Corporation (JST) under the ERATO Kitano Symbiotic Systems
...
Genome Research Institute (USA); the National Institute of General Medical
...
August 2002. The final set of features in SBML Level~2 Version~1 described
...
constituting the world-wide \emph{SBML Forum} that it has become infeasible
...
BALSA, BASIS, BIOCHAM, BioCharon, biocyc2SBML, BioGrid, BioModels,
BioNetGen, BioPathway Explorer, Bio Sketch Pad, BioSens, BioSPICE
Dashboard, BioSpreadsheet, BioTapestry, BioUML, BSTLab, CADLIVE,
CellDesigner, Cellerator, CellML2SBML, Cellware, CL-SBML, COPASI,
Cytoscape, DBsolve, Dizzy, E-CELL, ecellJ, ESS, FluxAnalyzer, Fluxor,
Gepasi, INSILICO discovery, Jarnac, JDesigner, JigCell, JSIM, JWS Online,
Karyote, KEGG2SBML, Kinsolver, libSBML, MATLAB SBToolbox, MathSBML, MesoRD,
MetaboLogica, MetaFluxNet, MMT2, Modesto, Moleculizer, Monod, NetBuilder,
PANTHER Pathway, PathArt, PathScout, Pathway Tools, PathwayBuilder, PaVESy,
PNK, Reactome, ProcessDB, PROTON, pysbml, PySCeS, runSBML, SBML ODE Solver,
SBMLeditor, SBMLR, SBMLSim, SBMLToolbox, SBW, SCIpath, Sigmoid, SigPath,
SigTran, SIMBA, Simpathica, SimWiz, SmartCell, SRS Pathway Editor,
StochSim, STOCKS, TERANODE Suite, Trelis, Virtual Cell, WinSCAMP, and
XPPAUT.
...
In Section~\ref{sec:annotation-use}, previous Levels and Versions of SBML
reserved set of XML Namespace names corresponding to software tools known
to exist at the time.  Due to the explosion of SBML-compatible software
tools in recent years, it has become infeasible to maintain such a list;
moreover, informal discussions with SBML users revealed that no one paid
much attention to the list anyway.  Beginning with SBML Level~2
Version~2, no such namespaces are defined in the SBML specification.

\item \emph{sboTerm}

In Version 2 \class{SimpleSpeciesReference}, \class{KineticLaw},
\class{Parameter}, \class{Reaction}, \class{Model}, \class{Rule},
\class{InitialAssignment} and \class{Constraint} structures have
an addition field \attrib{sboTerm}.  These fields can only contain
valid Systems Biology Ontology (SBO) \sboref term values (that is
they are of type SBOTerm).
...
\class{CompartmentType} structures.  A \class{CompartmentType}
structure represents a type of compartment and consists of just
...
possible to explicitly assign a value of the symbol using
\class{EventAssignment}, \class{AssignmentRule} or
\class{RateRule} structures.
...
\item page 9 Section 3.6.1: The MathML standard doesn't specify
the result of n-ary operators when the number of operands is
critically small, for example for \class{times} and \class{add}
elements, when the number of operands are zero or one.  The
specification should clarify the correct behavior.
...
\item page 18 section 4.5.6: the value of '''outside''' for a
given compartment must be the '''id''' (not name) of another
compartment
...
\item page 24 Section 4.8.4: This section not refer to assignment
rules using the term `scalar rule'.
...
\sbmlrule{9999} \item The value of an \class{id} attribute of a
reaction cannot be used in \class{ci} elements within the MathML
contained in the \class{kineticLaw} element that is contained in
any \class{reaction} element. (see
section~\ref{subsec:reaction-as-symbol}).
...
the element must simplify to a single \class{unit} element
containing a \attrib{kind} attribute value of
\unit{dimensionless}, \unit{mole} or \unit{item} and
...
element must simplify to a single \class{unit} element containing
either:
...
\sbmlrule{1204}
\item If the \attrib{id} attribute of a
\class{unitDefinition} element contains \texttt{area} then the element
must simplify to a single \class{unit} element containing either:
...
\sbmlrule{1205}
\item If the \attrib{id} attribute of a
\class{unitDefinition} element contains \texttt{volume} then the element
must simplify to a single \class{unit} element containing a
\attrib{kind} attribute value of \texttt{dimensionless}, \texttt{litre} or \texttt{metre}. See
section~\ref{sec:built-in-units}.

\sbmlrule{1206}
\item If the \attrib{id} attribute of a
\class{unitDefinition} element contains \texttt{volume} then if the
element simplifies to \texttt{litre} then the effective
\attrib{exponent} attribute must be 1 (can default to 1). See
section~\ref{sec:built-in-units}.

\sbmlrule{1207}
\item If the \attrib{id} attribute of a
\class{unitDefinition} element contains \texttt{volume} then if the
element simplifies to \texttt{metre} is used then the effective
\attrib{exponent} attribute must be 3. See
section~\ref{sec:built-in-units}.

\sbmlrule{1208}
\item If the \attrib{id} attribute of a
\class{unitDefinition} element contains \texttt{time} then the element
must simplify to a single \class{unit} element containing either:
...
�texttt{true}�if the value of the \attrib{spatialDimensions} field is 0. See
...
attribute of a \class{assignmentRule} must not contain the same
value.  See sections~\ref{sec:initialAssignment}.
...
\sbmlrule{1700}
\item The \attrib{variable} attribute of a
\class{assignmentRule} element must contain the value of a
\attrib{id} attribute of a \class{compartment}, \class{species},
or \class{parameter} element.  See
...
a \class{rateRule} element must contain the value of a \attrib{id}
attribute of a \class{species}, \class{compartment}, or
\class{parameter} element.  See Sections~\ref{sec:raterule}.
...
\sbmlrule{9999}
\item \class{ci} elements contained within
an \class{assignmentRule} element cannot contain the value of any
