...
\class{Aspartate} species have an initial concentration.
...
of the given \class{BindingSite} and is thus generalized to cover
all states of that \class{BindingSite}.
...
\item The \attrib{stoichiometry} field of all
\class{SpeciesReference} structures must have a value of one (the
default value).  \emph{This restriction applies because it is not
possible (at least within this scheme) to identify the $n$
seperate entities that would match with a \class{SpeciesReference}
...
\item the reaction must have a \attrib{reversible} attribute of
...
\class{SpecificBonds} within the new normalized
\class{SpeciesGraph}.
...
\attrib{compartment} attrib on the \class{SimpleSpeciesReference}
...
\item If \class{Reaction} does not contain \class{GenericBond}
