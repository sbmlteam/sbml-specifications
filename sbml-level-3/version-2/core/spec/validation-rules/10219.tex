The number of arguments used in a call to a function defined by a
\FunctionDefinition object must equal the number of arguments accepted by
that function\vertwo{, if defined}.  In other words, it must equal the number of MathML
\token{bvar} elements inside the \token{lambda} element of the function
definition\vertwo{, if present}.  (References: \tworefs{4.3.4}{\ref{sec:functiondefinition-calling}}.)
