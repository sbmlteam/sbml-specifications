...
Beginning \changed{with \sbmltwotwo}, a new low-volume,
...
Major \changed{editions} of SBML are termed \emph{levels} and represent
substantial changes to the composition and structure of the
language.  The \changed{edition} of SBML defined in this document, \sbmltwo,
represents \changed{an evolution} of the language resulting from
...
changes within \changed{a Level} to correct, adjust, and refine
...
Announcements of errata, \changed{releases} of the SBML specification
...
\begin{table}[htb]
  ...
  \begin{edtable}{tabular}{l>{\hspace*{15pt}}cc}
    \toprule
    & \multicolumn{2}{c}{\textbf{Change relative to Level~2 Version~1}}\\
    \cmidrule(rl){2-3}
    \textbf{Feature} 		& \textbf{in Version 2} & \textbf{in Version 3}\\
    \midrule
    Default value for attribute \token{fast} on \Reaction
    & changed
    & changed
    \\
    Attribute \token{charge} on \Species
    & deprecated
    & deprecated
    \\    
    Attribute \token{offset} on \UnitDefinition
    & removed
    & removed
    \\
    Predefined unit \val{Celsius} in \token{UnitKind}
    & removed
    & removed
    \\
    Attribute \token{substanceUnits} on \KineticLaw
    & removed
    & removed
    \\
    Attribute \token{timeUnits} on \KineticLaw
    & removed
    & removed
    \\
    Attribute \token{timeUnits} on \Event
    & 
    & removed
    \\
    Attribute \token{spatialSizeUnits} on \Species
    & 
    & removed
    \\
    \bottomrule
...
distinguish \changed{objects and data types} from other kinds of entities:
...
\item \token{SomeThing}, \token{otherThing}: \changed{Attributes}
...
\token{[0..1]} to signify exactly zero or one.  This notation
appears throughout this specification document.
