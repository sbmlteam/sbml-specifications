...
\date{SBML Level 2, Version 1\\[5pt]
  \today{}}
...
\item \emph{Rule}: A mathematical expression that is used in
...
\begin{figure}[ht]
  ...
  \caption{The definition of \class{SBase}.  Text enclosed in braces next
    ...
    constraints on the possible field values.  We use XML Schema
...
By default these units are either composed or used directly to define the
...
the value of \attrib{hasOnlySubstanceUnits} is ``\attribvalue{true}''.
...
(i.e. \quantity{concentration} units given a broad definition of concentration)
...
\attrib{spatialDimensions} is ``\attribvalue{0}'' or if the value of
\attrib{hasOnlySubstanceUnits} is ``\attribvalue{true}''.
...
\item The
   \attrib{math} field of the
  \class{AssignmentRule} structure determining the species' concentration
...
indicating that the species \emph{is} part of the reaction
...
\item \emph{In the case of a species}, an \class{AssignmentRule} sets the
  ...
  \attrib{species} field that have the same
...
\item \emph{In the case of a species}, a \class{RateRule} sets the rate of
  ...
  \emph{Restrictions}: In a given SBML model, there cannot be both a
  \class{RateRule} \attrib{variable} field and a \class{SpeciesReference}
  \attrib{species} field that have the same
  value. (See Section~\ref{sec:reactions} for the definition of
  \class{SpeciesReference}.)  This means that an assignment rule cannot be
  defined for a species that is created or destroyed in a reaction.  The
...
interoperability, that the \attrib{stoichiometry} field is used in preference
to the \attrib{stoichiometryMath} field and that the \attrib{stoichiometry} 
field contains integer values. Parsing software should expect all possible
values of the \attrib{stoichiometry} and \attrib{stoichiometryMath} fields
including, for example non-integer values for \attrib{stoichiometry}, in
SBML Level 2 streams and handle them appropriately.
