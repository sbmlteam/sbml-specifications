...
\token{http://www.sbml.org/sbml/level3/version1/distrib/version1}
\end{center}
...
can be seen in the UML representation in \fig{fig:funcdef}. The redefined \FunctionDefinition can optionally
contain a single \token{drawFromDistribution} child.
...
It must also have one other child:  a \PredefinedPDF if it is intended to represent a pre-defined distribution, a \ExplicitPMF if it is intended to represent a PMF, or an \ExplicitPDF if it is intended to represent an explicit PDF.  Each of these situations is described below.  In all cases, the strings defined in the \Input objects are used as the variables which define the distribution, and which can be linked as arguments to the function to explicit values or to SBML-defined variables.

\subsection{The \class{Input} class}
\label{input-class}

The \Input class mimics the \token{bvar} elements of MathML lambda functions.  It has a single text node child, which must contain a string of type \primtype{UncertId}.  These IDs represent the arguments to the function, and serve as local identifiers, referenced only by the other children of the \DrawFromDistribution class. See the examples in \ref{sec:fd-examples} for more details.
...
Note \notice that this use of UncertML \token{ExplicitPMF} elements replaces the old \ExplicitPMF class in previous versions of this specification.
  ...
  <listOfFunctionDefinitions>
    <functionDefinition id="normal">
      ...
      <drawFromDistribution xmlns="http://www.sbml.org/sbml/level3/version1/groups/version1">
         <input> avg </input>
         <input> var </input>
         <distribution xmlns:un="http://uncertml.org/3.0">
...
\begin{figure}[htb]
\includegraphics[width=0.75\linewidth]{gaussian_distribution_function.png}
...
As seen in \fig{extended-sbase-uml}, 
...
The \distrib package is designed to be agnostic to the types of the values used as function arguments, and the types of the values it returns.  As seen, certain uses of this package require the use of arrays or matrices, and to accomplish this, the \arrays package (or the equivalent) must be used.  Certain uses of the \ExplicitPMF element will return \primtype{string} values instead of \primtype{doubles}, and there is no existing SBML package that supports this data type, though the Qualitative Models package may introduce this type in the future.  Until such a time that those packages are accepted, then, the use of this \distrib package will be effectively constrained to use \primtype{double} and \primtype{boolean} values, though 'non-simulatable' models that use the other types are possible before then.
