...
\primtype{integer}, \primtype{double}, \primtype{string} and \primtype{SIdRef}. 
In addition we make use of four new primitives
\primtype{FbcSId}, \primtype{FbcSIdRef} as well as the enumerations \primtype{FbcType} 
and \primtype{FbcOperation}, see \ref{fig:fbc_uml} for the
...
attribute exists so that when multiple \Objective's are included in a single model, 
the model will always be well described i.e. there is a single,
