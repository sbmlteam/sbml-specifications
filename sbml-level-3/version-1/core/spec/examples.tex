...
of \emph{millimoles}, but \val{P} is given in \emph{grams}.  The 
\Model declares the \token{extentUnits} to be \emph{mole} and the 
...
\KineticLaw, \emph{extent}/\emph{time}, is \emph{mole}/\emph{second}.  Let
...
and the values of $k\sub{on}$ and $k\sub{off}$ have been adjusted
to reflect the units of the species.
