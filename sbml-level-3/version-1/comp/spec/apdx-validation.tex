...
\validRule{comp-10301}{(Extends validation rule \#10301 in the SBML
  Level~3 Version~1 Core specification.) \begin{blockChanged}Within a \Model 
  object,\end{blockChanged} the values of the attributes
  ...
  objects defined by the Hierarchical Model Composition package\begin{blockChanged}, plus
  any objects defined by any other package with \token{package:id}
  attributes defined as falling in the 'SId' namespace.\end{blockChanged}
  (References: SBML Level~3 Package Specification for Hierarchical Model
...
\validRule{comp-20203}{The value of attribute \token{comp:required} on
  ...
  the \SBML object contains any \Submodel with \Compartment, \Constraint,
  \Event, \InitialAssignment, \Parameter, \Reaction, \Rule, or \Species
  objects that have not been either
...
\validRule{comp-20204}{The value of attribute \token{comp:required} on
  ...
  \Submodel objects that are present contain no \Compartment, \Constraint,
  \Event, \InitialAssignment, \Parameter, \Reaction, \Rule, or \Species
  objects that have not been either
  directly or indirectly replaced.  (References: SBML Level~3 Package
  Specification for Hierarchical Model Composition, Version~1,
...
\validRule{\begin{blockChanged}comp-20205\end{blockChanged}}{There may be at most one instance of the
...
\validRule{comp-20210}{A \ListOfExternalModelDefinitions object may have
  ...
  namespace or the \begin{blockChanged}Hierarchical
  Model Composition namespace\end{blockChanged} are permitted on a
...
\validRule{comp-20302}{An \ExternalModelDefinition object may have the
  ...
  other subobjects from the SBML Level~3 Core namespace \begin{blockChanged}or the Hierarchical
  Model Composition namespace \end{blockChanged}are permitted in
...
\validRule{comp-20303}{An \ExternalModelDefinition object must have the
  ...
  they are required, and may have the \begin{blockChanged}optional attributes \token{comp:name},
  \token{comp:modelRef}, and \token{comp:md5}\end{blockChanged}.  No other attributes from the Hierarchical
...
\validRule{comp-20304}{The value of the \token{comp:source}
  ...
  Level~3 \begin{blockChanged}Version~1\end{blockChanged} document.  (References: SBML Level~3 Package Specification for
  Hierarchical Model Composition, Version~1,
  \sec{externalmodeldefinition-class}.) }
...
\validRule{comp-20305}{The value of the \token{comp:modelRef}
  ...
  an \token{id} attribute on a \Model \begin{blockChanged}or \ExternalModelDefinition 
  object \end{blockChanged}in the SBML document
...
\validRule{comp-20607}{A \Submodel object must have the attributes
  ...
  and may also have the optional attributes \begin{blockChanged}\token{comp:name},\end{blockChanged}
...
\validRule{comp-20803}{A \Port object must have a value for the required
  ...
  \token{comp:metaIdRef}.  \begin{blockChanged}It may also have the optional attribute \token{comp:name}.\end{blockChanged}
...
\validRule{comp-20903}{A \Deletion object must have a value for \begin{blockChanged}one, 
  and only one, of the
  attributes \token{comp:portRef}, \token{comp:idRef},
  \token{comp:unitRef}, or \token{comp:metaIdRef}.  It may also have the
  optional attributes \token{comp:id} and \token{comp:name}.\end{blockChanged}  No other
