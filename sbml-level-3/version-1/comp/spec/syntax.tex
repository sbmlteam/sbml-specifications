...
The type \primtype{PortSId} is derived from \primtype{SId} (\sbmlthreecore specification Section~3.1.7) and has identical syntax.
...
from a location accessible on the Internet, say via the HTTP protocol, but
which has no defined \token{id} attribute for its \token{<model>} element.
...
\begin{enumerate}
  ...
  \item A port cannot refer to any another port of the same model (including
    itself).
...
Because the model namespace is defined per document, this
...
\RateRule, and \KineticLaw elements.  See \sect{conversion-factors} for
more details.
...
that should be considered ``replaced''.  The object \emph{holding}
...
\item New attributes of a type defined in the package specification that 
 ...
 \primtype{SpIdRef}, defined in the proposed Spatial Processes specification 
 (April 2011). 
\end{itemize}
...
text of the element matched the SId of another element in the model.  Do
note that local parameters may only effectively be replaced by global
parameters, since references to its \primtype{SId} are only valid from
within the \Reaction element to which it belongs.
...
The \ReplacedElement class's \token{conversionFactor} attribute, if present
...
factor should equal a single unit of the replacement element, and the units
...
value for its \token{hasOnlySubstanceUnits} attribute, thus changing amount
...
It is likewise only legal to use a \token{conversionFactor} attribute on a 
\ReplacedElement that points to an element with mathematical meaning.  For
\sbmlthreecore, this means \Compartment, \Parameter, 
\Reaction, \Species, and \SpeciesReference elements.
...
\item \emph{Identifiers must be conserved}.  If a replaced element
  defines one or more attributes of type \token{SIdRef} or \token{IDREF}
  (or their derivatives, such as \token{UnitSIdRef} or the proposed \token{SpIdRef}),
...
attribute.  These \token{metaId}s may be referenced by the \SBaseRef \token{metaIdRef}
...
\begin{table}[bht]
  ...
  \begin{edtable}{tabular}{p{1.6in}p{2.3in}l}
    ...
    \sprd{4}{MathML \token{<cn>} element}	& \sprd{4}{$\frac{1}{\textsf{\persymb}}$}\\
    \sprd{4}{MathML csymbol \token{time}} & \sprd{4}{$\frac{1}{\token{timeConversionFactor}}$}\\
    The time argument to a MathML \token{csymbol} \emph{delay} function & \sprd{4}{\token{timeConversionFactor}}\\
    \Parameter				& 1\\
    \Priority				& 1\\
...
\val{1} as well.  For example, if the conversion factor \token{extentConversionFactor} is
defined but \token{timeConversionFactor} is not, kinetic laws are
converted according to the \token{extentConversionFactor}, \emph{divided
  by~1}.
