...
\newcommand{\species}[1]{\textit{#1}\xspace}
\newcommand{\concent}[1]{\ensuremath{[\textit{#1}\,]}\xspace}
...
In Equations~\ref{eq:v1} and~\ref{eq:v2}, the values of
\species{E}, \species{S} and \species{ES} are all in
...
\begin{linenomath}
  \begin{align*}
    k_{\text{on (new)}}  & = k_{\text{on (original)}} \cdot ???\\
    k_{\text{off (new)}} & = k_{\text{off (original)}} \cdot ???\\
    k_{\text{cat (new)}} & = k_{\text{cat (original)}} \cdot ???
  \end{align*}
\end{linenomath}
...
is \emph{mole}.  Our approach here is to introduce constants into
the reaction rate expressions to convert the substance units to
\emph{mole} and multiply each occurence of a concentration by that
constant.  A separate constant is necessary for each \Species
object appearing in a \KineticLaw object, although it turns out
that in the particular situation under consideration here, the
constants are all identical:
\begin{linenomath}
  \begin{equation*}
      \convE = \convS = \convES = 10^{-3}\;\emph{mole} \;\emph{millimole}^{\,-1}
  \end{equation*}
\end{linenomath}
Next, we define the rate equations to be put in the \Reaction
object definitions.  Let \csg stand for the value of the \Model
object's \token{conversionFactor} attribute.  Using the guidelines
described in Section~\ref{sec:about-kinetic-laws}, the
rate-of-change equation for species \species{S} is:
\begin{linenomath}
\begin{equation*}
...
their values have units of \quantity{substance amount}, not
\{\quantity{substance amount}\}/\quantity{size}.  A second point is that, as a
