...
formats, using a deeper semantics\changed{, such as SBGN (\url{http://www.sbgn.org/})}.  Although an advanced software
...
\changed{keeping} SBML components in mind, and are classified into subsets that
...
The Systems Biology Ontology (SBO) provides \changed{terms for
...
reaction, can help map SBML elements to \changed{equivalent} \changed{concepts in
other standards, such as (but not limited to) BioPAX
(\url{http://www.biopax.org/}), PSI-}\changed{MI (\url{http://www.psidev.info/index.php?q=node/60})}, or the Systems Biology Graphical
...
vocabularies in addition to understanding \changed{SBML}.
...
of controlled vocabularies with \changed{six} main divisions: \changed{(1) entity, (2) participant role, (3) quantitative parameter, (4) modeling framework, (5)
mathematical expression, and (6) interaction}.
Figure~\vref{fig:sbo-top-level} illustrates the highest level of
SBO.

Each of the \changed{six} branches of Figure~\ref{fig:sbo-top-level} have a
...
\begin{figure}[tbh]
  ...
  \vspace*{1ex}
  \caption{The \changed{six} controlled vocabularies (CVs) that
...
\begin{figure}[htb]
  ...
  \caption{Partial expansion of some of the terms in the
    \changed{\emph{entity}} branch of SBO.}
  \label{fig:expanded-species}
\end{figure}

Figure~\vref{fig:expanded-speciesRef} shows the structure
for the \changed{\emph{participant role} branch}, also grouping the concepts in a hierarchical manner. For example, in reaction
...
of this is easy to capture in the \changed{ontology}.  As more agreement is
reached in the modeling community about how to define and name
modifiers for different cases, the \changed{ontology} can grow to accommodate it.
...
\begin{figure}[htb]
  ...
  \caption{Partial expansion of some of the terms in the
    \changed{\emph{participant role} branch} of SBO.}
  \label{fig:expanded-speciesRef}
\end{figure}

\changed{The controlled vocabulary for quantitative
...
\begin{figure}[tbh]
  ...
  \caption{Partial expansion of some of the terms in the \emph{quantitative
      parameter} \changed{branch}.}
  \label{fig:expanded-parameter}
\end{figure}

The terms of the SBO quantitative parameter \changed{branch} contain
...
illustrates the structure of this \changed{branch}, which is at this point
...
\begin{figure}[tbh]
  ...
  \caption{Partial expansion of some of the terms in the
    \emph{modeling framework} \changed{branch}.}
...
hierarchy.  Rate law \changed{or conservation law} formulas are part of the mathematical
...
common reaction \changed{or rule} types.  Other types of mathematical expressions
may be included in the future in order to be
able to further characterize mathematical components of a model, \changed{such as} initial assignments, assignment rules, rate rules,
...
in the CV act \changed{as} templates for what to put into an SBML construct such as
...
\token{k}) and SBO participant role (for \token{R}) \changed{branches}.
This makes it possible for software applications to interpret the
intended meanings of the parameters in the expression.  \changed{This also permits to convert an expression into another, by using the \mathmltwo formula} \changed{contained in the SBO terms associated with the parameters.}

The \changed{\emph{interaction}} branch of SBO defines types of biological processes\changed{, events or relationship involving entities}.  It lists the types of biochemical reactions, such as binding,
...
\begin{figure}[tbh]
  ...
  \caption{Partial expansion of some of the terms in the \changed{\emph{interaction} branch}.}
...
\begin{table}[bht]
  ...
  \begin{edtable}{tabular}{lll}
    \toprule
    \textbf{SBML Component} & \textbf{SBO Branch} & \textbf{\changed{Branch Identifier}} \\
    \midrule
    ...
    \SpeciesReference   & \changed{participant role}          & \sboparticipantroleID \\
    \ModifierSpeciesReference & \changed{participant role}    & \sboparticipantroleID \\
    ...
    \Event              & \changed{interaction}     & \sbointeractionID \\
  ...
  may be assigned to them.  The \changed{identifiers of the highest-level SBO terms in each branch} are provided
  for guidance, but actual \changed{values used for \token{sboTerm} attributes} should \changed{be} more specific
  child terms \changed{within these branches}. \changed{Note that the important aspect here is the set of specific SBO identifiers, not the SBO term names, because the names may change as SBO continues to evolve.} See text for further explanations.}
...
annotation in SBML, \changed{SBO references} are separated into
their own attribute on SBML components, both to simplify their use
for software tools and because \changed{doing so asserts} a stronger and
more focused connection in a more regimented fashion.  \changed{SBO
