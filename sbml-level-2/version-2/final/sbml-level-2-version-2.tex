...
must be a decimal number.  An omitted \token{E} or \token{e} and
exponent means that a value of 0 is assumed.  The \token{E} or
\token{e}, if present, must be followed by an integer or an error
results.  The following are examples of legal literal
