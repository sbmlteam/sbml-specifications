...
identifier (\ie the identifier given in the \changed{attribute} \token{symbol} in
\InitialAssignment, the \changed{attribute} \token{variable} in \AssignmentRule, and the
\changed{attribute} \token{id} on the \KineticLaw's enclosing \Reaction).  Each of these
...
(References: L2V2 Section~4.11.5; \changed{L2V3 Section~\ref{sec:ruleconstraints}.})
