...
We could extend \texttt{malate\_aspartate\_shuttle2} using \class{SpeciesType} structures
...
if the reaction was not reversible.  All the \class{SimpleSpeciesReference} structures (that is
...
In SBML Level 2 the model's \attrib{species} list is a complete enumeration of the
...
Just as it is not possible to explicitly locate a \class{SpeciesType} in \class{Compartment} more than
once implied species cannot duplicate each other or explicit \class{Species} structures.
...
Implied species, which don't match any \class{Species} structures, always have an initial
...
Two
\class{SimpleSpeciesReference} structures that refer to the same implied species should have
the same units.
...
In this proposal \class{SpeciesType} structures can be composed from instances of other
\class{SpeciesType} structures.
These instances are encoded as \class{SpeciesTypeInstance} structures.
...
This proposal makes distinction between \texttt{simple} \class{SpeciesType} structures and
\texttt{complex} \class{SpeciesType} structures.  Simple \class{SpeciesType} structures
do not contain any \class{SpeciesTypeInstance} structures whereas complex \class{SpeciesType} structures
do.
...
The bond information on a \class{SimpleSpeciesReference} or a \class{SpeciesType} is a
graph linking the \class{SpeciesTypeInstance} structures together.
The description of a structure using chemical
bonds requires the identification of binding sites on \class{SpeciesType} structures, using
\class{BindingSite} structures, and
then the enumeration of the bonds on those binding sites using \class{SpecificBond} structures.
\class{SpecificBond} structures consist of pairs of \class{BindingSiteReference} structures.
...
\class{SpecificBond} structures can be
used to indicate unbound binding sites, simply by containing a single \class{BindingSiteReference}
...
\class{SpeciesGraph} structures, that is \class{SpeciesType} and \class{SimpleSpeciesReference}
structures, can contain a number of disconnected components (as described previously in
Section~\ref{sec:multicomponentspecies}.  This means that a list of \class{Bond}
structures in a \class{SpeciesGraph} does not have to comprise a connected graph.
In this case the \class{SpeciesGraph} still represents a single entity where the complete set of
...
\class{GenericBond} is a alternative \class{Bond} type.  Reactions containing containing a
...
an unspecified chemical entity.  The whatever is assigned to the \class{GenericBond}
...
The values of \class{GenericBond} \attrib{id} fields are specific
...
This proposal adds to a \class{Model} structure a
\attrib{speciesType} field which consists of a list of
\class{SpeciesType} structures. This list is divided into
\emph{simple} and \emph{complex} types where those containing one
or more \class{SpeciesTypeInstance} structures are complex. The
set of complex species types in the modelled system is not
restricted to those listed in the \attrib{speciesType} field. The
complete set of complex species types can be inferred by
traversing the reaction network, defined by the set of
\class{Reaction} structures, from the contents of the
\attrib{speciesType} field.

This proposal changes the definition of the \attrib{species} list
on \class{model}. This list does not necessarily comprise the
complete set of pools of chemical entities in the model. The set
of \class{Species} structures in this list can have an undefined
...
network, defined by the set of \class{Reaction} structures, from
...
\class{SimpleSpeciesReference} structures match with
\class{Species} in the same \class{Compartment} across all
...
one in which the \class{BindingSite} \emph{is} referenced by a
\class{GenericBond} structure with a unique identity occurring
...
of the given \class{BindingSite} is generalised to cover all
states of that \class{BindingSite}.
...
In this proposal a \class{SimpleSpeciesReference} can refer to a
set of \class{Species} both those that are explicitly defined and
...
\item \attrib{id}, this optional \class{SId} field is not
...
\texttt{ci} elements, within the \class{KineticLaw} structure of
the enclosing \class{Reaction} structure.
...
structures before following the \class{Species} semnatics.
\class{SimpleSpeciesReference} structures that match with a
\class{Species} structure should default to the \class{Species}
attributes and/or be exactly equivalent to them.  Two
\class{SimpleSpecisReference} structures that refer to the same
...
\class{Compartment} where the matching species are located. If
this field is not present a given \class{SimpleSpeciesReference}
structure then it must not be present on any other
\class{SimpleSpeciesReference} structure in the same enclosing
\class{reaction}.
...
\class{SpeciesType} associated with it.
...
nodes are \class{SpeciesTypeInstances} and the arcs are
...
entity. \class{SpeciesType} is derived from \class{SpeciesGraph},
...
The bonds list inherited from \class{SpeciesGraph} must only
contain \class{SpecificBond} structures.

A simple example of the use \class{SpeciesType} structures is given in section~\ref{sec:commonspecies}.

Consider all the \class{BindingSite} structures of the
...
\class{SpecificBond} structure enclosed in the \class{SpeciesType}
structure.  This means that the status of a \class{BindingSite}
can't be left undefined or ambiguous by a \class{SpeciesType}.
...
\class{SpeciesType} or \class{SimpleSpecies} structure) is
normalized if the set of \class{SpeciesType} structures, refereed
to by the set of \class{SpeciesTypeInstance} structures, are
...
into the same `outer' \class{Bond} structures as the initial
complex \class{SpeciesType} structure.
...
\class{Species} structures defines the complete set of pools of
chemical entities that are created by, destroyed by and affect the
set of reactions. In this proposal the set of \class{Species}
...
\item \emph{\attrib{speciesType} and \attrib{compartment}
attributes are present}, in this case the match is with the
species located in the given \class{Compartment} which has a
\class{SpeciesType} equivalent to the given \class{SpeciesType}.
...
If the \attrib{compartment} attribute is not present on the
\class{SimpleSpeciesReference} then the match is with any species
 matching the \class{SimpleSpeciesReference} located in the same
\class{Compartment} as all the other species involved in the
reaction. If the \attrib{compartment} attribute is present then
the match is with any species matching the
\class{SimpleSpeciesReference} located in the given
\class{Compartment}.

Before matching can occur between a \class{SimpleSpeciesReference}
...
\class{SimpleSpeciesReference} and a species we first require a
definition of a \emph{generic graph} and definition of
\class{SimpleSpeciesReference} in terms of a \emph{generic graph}.
(The use \emph{generic graph} here is to capture the semantics of
\class{GenericBond} structures which may be contained within a
...
A match of generic graph (from a \class{SimpleSpeciesReference})
into a simple graph (from a \class{SpeciesType} as described in
...
the corresponding reactant \class{SpeciesReference} structures.
...
\class{SpecificBond} structures via the mapping $m_{X}$ created by
...
\subsubsection{Matching more than one \class{SimpleSpeciesReference} to a single species}
