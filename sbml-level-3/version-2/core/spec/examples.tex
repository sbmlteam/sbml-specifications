...
\newcommand{\kon} {\ensuremath{k\sub{on}}\xspace}
\newcommand{\koff}{\ensuremath{k\sub{off}}\xspace}
\newcommand{\kcat}{\ensuremath{k\sub{cat}}\xspace}
...
%-----------------------------------------------------------------------------
\subsection{A simple example using the \tokenNC{conversionFactor} attribute}
\label{sec:eg:conversionfactor}
%-----------------------------------------------------------------------------

This example involves the same enzymatic reaction as in the
example of \sec{sec:modeleg}:
\begin{center}
  \ce{\species{E} + \species{S} <=>[\kon][\koff] \species{ES} ->[\kcat] \species{E} + \species{P}}
\end{center}
...
the example of \sec{sec:modeleg}; that is, the overall unit of extent in
...
\sec{sec:eg:conversionfactor2} we illustrate a second
...
\sec*{sec:eg:conversionfactor2} does not change the kinetic
...
the approach of \sec{sec:eg:conversionfactor2} may be more
appropriate.
...
the guidelines described in \sec{sec:about-kinetic-laws}.
...
%-----------------------------------------------------------------------------
\subsection{An alternative formulation of the \tokenNC{conversionFactor} example}
...
reaction as in the example of \sec{sec:modeleg}:
\begin{center}
  \ce{\species{E} + \species{S} <=>[\kon][\koff] \species{ES} ->[\kcat] \species{E} + \species{P}}
\end{center}
As in \sec{sec:eg:conversionfactor}, we define the overall
...
\sec{sec:about-kinetic-laws}, and in this case, the
...
(\sec{sec:sboTerm}) is to enable such interpretation
...
of the fast reaction explicitly rather than using the \vertwo{old} \token{fast}
attribute on a reaction element.  The system modeled is
\begin{larray*}
  X_0 & \overset{\underrightarrow{k_1 [X_0]}}{}           & S_1 \\[6pt]
  S_1 & \overset{\underleftrightarrow{k\sub{f} [S_1] - k\sub{r} [S_2]}}{} & S_2 \\[6pt]
  S_2 & \overset{\underrightarrow{k_2 [S_2]}}{}           & X_1
\end{larray*}\vspace*{-1em}
\begin{larray*}
  k_1 = 0.1, \quad k_2 = 0.15, \quad k\sub{f} = K\sub{eq} 10000, \quad k\sub{r} = 10000, \quad K\sub{eq} = 2.5.
\end{larray*}
...
reaction given in the example of \sec{sec:modeleg}.
\begin{center}
  \ce{\species{E} + \species{S} <=>[k1\sub{on}][k1\sub{off}] \species{ES} ->[k2] \species{E} + \species{P}}
\end{center}
...
\subsection{Example with combinations of
  \tokenNC{boundaryCondition} and \tokenNC{constant} values on \class{Species}
...
\fig{fig:multicomp} illustrates the arrangement of
...
\fig{fig:multicomp}.  The species variables in this SBML
...
\token{initialConcentration}.)  We use $V\sub{n}$ to represent the size
of compartment \val{nucleus} and $V\sub{c}$ the size of compartment
\val{cytoplasm}:
\begin{larray*}
  V\sub{n}    &=& 1\; \unit{litre}\\
  V\sub{c}    &=& 5\; \unit{litre}\\
  X      &=& 1\; \unit{mole}\\
  Y\sub{1n} &=& 1\; \unit{mole}\\
  Y\sub{1c} &=& 0\; \unit{mole}\\
  Y_2    &=& 1/5\; \unit{mole}
\end{larray*}
...
\begin{linenomath}
  \begin{align*}
    \dfrac{d [X]}{d t}    &= 0 \\[6pt]
    V\sub{n} \dfrac{d [Y\sub{1n}]}{d t} &= k_1 [X] [Y\sub{1n}] V\sub{n} - K_T \big([Y\sub{1n}] - [Y\sub{1c}]\big) V\sub{c}
...
\vspace*{-1em}
\begin{larray*}
  [P\sub{delayed}] && \text{is } \text{delay}([P], \Delta_t) \text{ or }[P]\text{ at } t - \Delta_t\\[-2pt]
