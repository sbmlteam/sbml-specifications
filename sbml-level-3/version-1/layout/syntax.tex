...
For a complete example that uses SBML Level 2 see \sec{l2example}.
...
\primtype{String}. It allows one to specify a human readable name for the 
\LayoutClass. 
...
If the \token{compartment} attribute is used together with the \token{metaidRef} 
...
which case the \token{order} attribute will help to disambiguate the drawing
order. 
...
If the \token{species} attribute is used together with the \token{metaidRef} 
they need to refer to the same object in the \Model.
...
If the \token{speciesReference} attribute is used together with the \token{metaidRef} 
they need to refer to the same object in the \Model.
...
recommended to use the \token{sboTerm} on the \SpeciesReference. If both
\token{role} and \token{sboTerm} are specified and conflicting, it is the 
\token{role} that takes precedence. 
...
The element names of the children of \ListOfSubGlyphs are the de-capitalized forms of their class: \token{graphicalObject}, \token{compartmentGlyph}, \token{generalGlyph}, \token{reactionGlyph}, \token{referenceGlyph}, \token{speciesGlyph}, \token{speciesReferenceGlyph}, or \token{textGlyph}.
