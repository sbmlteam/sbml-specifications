...
This is document is non-normative i.e. doesn't define any aspect of the SBML standard on the
SBML specification documents define the SBML standard.
...
(see Section~\ref{sec:workinggroups}).
...
libSBML supports Level 1 will support Level 2 shortly.  libSBML is a native library and provides C and C++ APIs.
libSBML is available from the \hef{http://sourceforge.net/project/showfiles.php?group_id=71971&release_id=164086}{SBML Sourceforge project}.
...
\attrib{initialConcentration} is the form most commonly used.
...
\subsubsection{Species Referenencs}
...
Avoid the use discontinuous operators like \texttt{piecewise}.

\subsubsection{Delay}

Avoid the use of the builtin delay operator.
...
Do not expect tools to interpret units.
...
You should write using the most interoperable form as possible.  To maximize interoperability beyond this requires
...
into future SBML Levels is collected by the editors.  The editors then establish what the consensus is
...
by the community the specification becomes the new standard.
...
team has been an active participant in the SBML development process.)  We view CellML and
