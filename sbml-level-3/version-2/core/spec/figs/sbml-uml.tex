\begin{tikzpicture}[level distance=0.5in]
  \node { \emptyClassbox{\textsl{SBase}} }
    ...
    child {node[below=-10pt] (a) {
        \begin{classbox}{SBML}
          xmlns: string \{ use=\texttt{"}required\texttt{"} fixed=\texttt{"}http://www.sbml.org/sbml/level3/\relverdiff{version1}{version2}/core\texttt{"} \} \\
          level: positiveInteger \{ use=\texttt{"}required\texttt{"} fixed=\texttt{"}3\texttt{"} \}   \\
          version: positiveInteger \{ use=\texttt{"}required\texttt{"} fixed=\texttt{"}\relverdiff{1}{2}\texttt{"} \} \\
   ...
   \draw[diamond-, above=0pt] (a) -- (b) 
     ...
     node[above=6pt,left=0.25in] {\relverdiff{}{\textsf{0..1}}};
