...
\item If all other situations, the referenced identifier must be the
  ...
  \item \emph{Species identifier}: When a species identifier occurs in a
    ...
    concentration associated with the species are
    \quantity{species~substance}/\quantity{compartment~volume} units, where
    the units of
...
easier.  Further details of how \class{listOf}\rule{0.5in}{0.5pt} elements
...
that follow the place of its definition in an SBML model.  (These
...
These are the motivations for the provision of the \class{UnitDefinition} and
\class{Unit} structures in SBML, whose definitions are shown in
...
\class{UnitDefinition} \class{Unit} list, a \class{Unit} representing $meter$
with a \class{Unit} representing $second^{\,-2}$.
...
\emph{built-in units} that
play roles in biochemical reactions: amount of substance, volume, area,
length and time.  SBML defines defaults for these built-in units, see the 3rd
column of Table~\ref{tab:builtin}, all with
default \texttt{scale}, \texttt{multiplier}, and \texttt{offset} values of zero.
The various components of a model,
...
five predefined names of built-in units ``\unit{substance}'', ``\unit{volume}'',
...
used in redefining a given built-in unit.  Note that, in order for unit
...
The units of the compartment size, as defined by the \attrib{unit} attribute,
are used in all contexts where a
...
The \attrib{units} field value also determines the units associated of the
...
\item \emph{Case of \attrib{initialAmount} being used}. The value assigned
  ...
  one of the unit names from Table~\vref{tab:unitkind}, the built-in unit
  ...
  value set by the built-in unit ``\attrib{substance}'' shown in
  Table~\vref{tab:builtin}.
  
\item \emph{Case of \attrib{initialConcentration} being used}.  The value
  assigned to \attrib{units} must be either one of the unit names from
  Table~\vref{tab:unitkind} or a new unit name defined by a unit definition
  in the enclosing model.  The
  ...
  value of ``\attribvalue{0}''.  If an attribute value for \attrib{units} is
  absent from a \class{Species} structure, the units default to units of
...
\item \emph{In the case of a parameter}, a \class{RateRule} sets the rate
  ...
  \attrib{math}.  The overall units of the formula must be of the form
