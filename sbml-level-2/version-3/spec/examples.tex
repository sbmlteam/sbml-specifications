...
sets the default substance units in the model.  The \changed{\token{volume}}
and \token{time} built-ins are left to their defaults, meaning
\changed{volume} is in litres and time is in seconds.  The result is that, in
...
\changed{attribute} on a reaction element.  The system modeled is
\begin{linenomath}
\begin{equation*}
  \begin{array}{@{}ccc@{}}
...
\AlgebraicRule \changed{object}.  The model contains a different
...
\subsection{Example with combinations of
  ...
  with \class{RateRule} \changed{objects}}
...
\token{boundaryCondition} and \token{constant} \changed{attributes}.  The model
...
\token{constant} \changed{attribute} with a default value of \val{false}.  The
values of $S_2$ and $S_3$ are fixed so in SBML they have a
\token{constant} \changed{attribute} values of \val{true}.  $S_3$ only occurs as
a modifier so the value of its \token{boundaryCondition} \changed{attribute} can
...
\token{boundaryCondition} and \token{constant} \changed{attributes}.
...
\changed{$G_1$} and \changed{$G_2$}.  \changed{$G_1$} is initially
on and \changed{$G_2$} is initially off.  When turned on, the two
genes lead to the production of two products, $P_1$ and $P_2$,
respectively, at a fixed rate.  When $P_1$ reaches a given
concentration, \changed{$G_2$} switches on.  This system can be
represented mathematically as follows:
\begin{linenomath}
\begin{eqnarray*}
  \dfrac{d P_1}{d t} & = & \changed{k_1 (G_1 - P_1)}\\[3pt]
  \dfrac{d P_2}{d t} & = & \changed{k_2 (G_2 - P_2)}\\
  G_2 & = &
    \begin{cases}
...
\begin{linenomath}
\begin{equation*}
  \changed{G_1 = 1, \quad G_2 = 0}, \quad \tau = 0.25, \quad P_1 = 0, \quad P_2 = 0, \quad \changed{k_1 = k_2 = 1}.
\end{equation*}
\end{linenomath}
