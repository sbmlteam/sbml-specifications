...
The \colorString primitive type is a string encoding the hexidecimal color code. Color values are specified as a 6 to 8 digit hex string which 
...
The following defines a dark red color, with a red component of 0x20, green 
...
The \ListOfLocalStyles object derives from the \class{ListOf} class and inherits
...
The \token{id} of a \ColorDefinition specifies \val{none} cannot be used as a \token{stop-color}. It is also considered an error to specify the id of another gradient as the value of a \token{stop-color} attribute.
