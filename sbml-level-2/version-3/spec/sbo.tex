...
rate expressions, parameters, and a few others. The use of \token{sboTerm} attribute is optional, and the presence of \token{sboTerm} on an element does not change the way the model is \emph{interpreted}. Annotating SBML elements with SBO terms add an extra layer of semantics that may be used to \emph{convert} the model into another model, or another format. 
