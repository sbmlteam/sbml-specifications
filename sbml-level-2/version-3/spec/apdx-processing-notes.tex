...
\section{Processing and validating \changed{\class{SBase}
 \token{notes} and \class{Constraint} \token{message} content}}
\label{apdx:processing-notes}

\changed{In Section~\ref{sec:notes} and
...
the \token{notes} \changed{and \token{message}} may not be a
...
Based on the three forms of \changed{\SBase \token{notes}} content described in
Section~\ref{sec:notes} \changed{and the identical forms for
...
\token{notes} \changed{and \token{message}} to be viewed or edited:
\begin{description}
...
\item \emph{Step 2}. The application should look at the first
  element inside the \token{notes} \changed{or \token{message}}
...
<html xhtml="http://www.w3.org/1999/xhtml">
    ...
    \changed{or \token{message}}, and finally
...
<html xhtml="http://www.w3.org/1999/xhtml">
    ...
    <body>\end{example}
    then insert the content of the \token{notes} \changed{or \token{message}}, and finally
