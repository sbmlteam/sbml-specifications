...
% $Id: components.tex 12125 2010-10-03 18:22:04Z mhucka $
% $HeadURL: https://svn.code.sourceforge.net/p/sbml/code/trunk/specifications/sbml-level-3/version-1/core/spec/components.tex $
% ----------------------------------------------------------------
...
<sbml xmlns="http://www.sbml.org/sbml/level3/version1/core" level="3" version="1"
  ...
  \changed{time-course dynamics of the model can be correctly
...
indicated in Table~\ref{tab:sboterm-availability}.  \changed{The
...
definitions \changed{could, if desired, be implemented as textual substitutions.}
...
contain references to \changed{identifiers} other than the
variables declared \changed{in} the \token{lambda} itself.  That
is, the contents of MathML \token{ci} elements inside the body of
the \token{lambda} can only be \changed{one of two kinds of
...
\quantity{time}, \quantity{avogadro} \changed{and \quantity{delay}}.
...
must \emph{not} \changed{be equal to one of the reserved base unit
...
expressed in \emph{this} \changed{v}ersion of SBML Level~3 should
...
once.  If the compartment's attribute \token{constant} \changed{has}
...
alternative methods \changed{exist} for setting the size of a
compartment, the \token{size} attribute must be defined as
optional; however, \emph{it is good practice to specify a value
\changed{for} the size of every compartment in a model}, no matter what method
...
\Species object.  \changed{(Setting both is an error.)}
...
The following example shows \changed{a species definition} within an
...
additional information about \changed{how $t \leq 0$ should be
  handled}.
...
\changed{There are no} restrictions on duplicate \Constraint
...
a system is not required to \changed{attempt detecting} whether
...
The model's \changed{reactions} are divided into two sets by the
...
<model>
    ...
    ...
    <listOfEvents>
        <event \changed{useValuesFromTriggerTime="true"}>
            <trigger \changed{initialValue="false" persistent="true"}>
                <math xmlns="http://www.w3.org/1998/Math/MathML">
