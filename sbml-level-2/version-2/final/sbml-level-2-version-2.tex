...
\newcommand{\FunctionDefinition}{\class{\hyperref[fig:functionDefinition]{Function}}\xspace}
...
\item Section~\ref{sec:annotation-standard} describes a recommend format
  for storing certain kinds of information in annotations on SBML
  components;
...
\primtype{ID}.  This type consists of strings of characters restricted as
shown in Figure~\ref{fig:id}.
...
\begin{figure}[htb]
  ...
  \end{tabular}
  \color{black}
  \caption{The definition of the XML Schema type \primtype{ID} expressed in
    the variant of BNF used by the XML 1.0
    specification~\protect\citep{bray:2000}.  The characters \texttt{(} and
    \texttt{)} are used for grouping, and the character \texttt{*}
    indicates ``zero or more times''.  The productions \token{CombiningChar}
    and \token{Extender} are relatively long lists of Unicode characters and
    are omitted here; please consult the XML 1.0
    specification~\protect\citep{bray:2000} for the complete definition.}
  \label{fig:id}
\end{figure}
...
The type \primtype{SId} is the type of the \token{id} field found on the
majority of SBML components.  \primtype{SId} is a data type derived from the
basic XML type \primtype{string}, but with restrictions about the types of
characters permitted and the sequence in which they may appear.  Its
definition is shown in Figure~\vref{fig:sid}.
...
The equality of \primtype{SId} values is determined by an exact character sequence
match; i.e., comparisons of these identifiers must be performed in a
case-sensitive manner.  This applies to all uses of \token{SId} including
the identifiers of unit definitions.

The \primtype{SId} is purposefully not derived from the XML \primtype{ID} type.
Using XML's \primtype{ID} would force all SBML identifiers to exist in a
...
model/submodel composition.  Further, the use of the \primtype{ID} type for
SBML identifiers would have limited utility because MathML \token{ci}
elements are not of the type \primtype{IDREF} (see
Section~\ref{sec:formulas}).  Since the \primtype{IDREF}-\primtype{ID} linkage
cannot be exploited in MathML constructs, the utility of the XML \primtype{ID}
...
\changed{As explained above, the base data types in SBML are taken directly
from XML Schema~\citep{biron:2000,fallside:2000,thompson:2000}.  SBML
defines additional object classes beyond this.}  The overall SBML
...
specifically in the context of the \token{annotation} field on
\SBase.  The namespace issue here is unrelated to the namespaces
discussed in Section~\ref{sec:namespaces} in the context of \primtype{SId} and
symbols in SBML.
...
fields: \token{id} and \token{name}.  The \token{id} field is usually
...
this identifier.  Section~\ref{sec:sid} provides a definition of the data type \primtype{SId}
used for the \token{id} field, and Section~\ref{sec:namespaces} describes
...
fields are not defined on \SBase.
...
In contrast to the \token{id} field, the \token{name} field is optional
...
component.  The data type of the \token{name} field is the type
\primtype{string} defined in XML Schema~\citep{biron:2000,thompson:2000} and
discussed further in Section~\ref{sec:primitive-types}.  SBML imposes no
restrictions as to the content of \token{name} fields beyond those
restrictions defined by the \primtype{string} type in XML Schema.

The recommended practice for handling \token{name} is as follows.  If a
software tool has the capability for displaying the content of
\token{name} fields, it should display this content to the user as a
component's label instead of the component's \token{id} field.  If the
user interface does not have this capability (e.g., because it cannot
display or use special characters in symbol names), or if the \token{name}
field is missing on a given component, then the user interface should
display the value of the \token{id} field instead.  (Script language
interpreters are especially likely to display \token{id} fields instead of
\token{name} fields.)

As a consequence of the above, authors of systems that automatically
generate the values of \token{id} fields should be aware some systems may
display the \token{id}'s to the user.  Authors therefore may wish to take
some care to have their software create \token{id} values that are
reasonably easy for humans to type and read.

\begin{blockChanged}
An additional point worth mentioning is although there are restrictions on
the uniqueness of \token{id} values (see Section~\ref{sec:namespaces}
below), there are no restrictions on the uniqueness of \token{name} values
...
\token{id})
  of functions, compartment types, compartments, species types, species, reactions,
...
mandatory on all components.  Second, the \SBase abstract type is
used as the base type for certain structures such as \Unit,
\AssignmentRule, \AlgebraicRule, etc., which do not have
identifiers at all because they are subordinate to other structures that
\emph{do} have identifiers.  If instead \SBase had an \token{id}
...
more importantly, the presence of an SBML identifier field (\token{id})
necessarily requires specifying scoping rules for the corresponding
identifiers.  However, the \SBase abstract type is used as the basis
...
\SBase where to have an \token{id} field, then the specification of
\SBase would need a default scoping rule and this would then have to
...
\token{id} on \SBase would require all of these lists in a model to
...
the rest of an SBML model.  Finally, because the \Sbml top-level
object is derived from \SBase, it too would have to be assigned a
...
\begin{blockChanged}
%-----------------------------------------------------------------------------
\subsection{The \token{sboTerm} Field on SBML Components and the \primtype{SBOTerm} type}
\label{sec:sboTerm}
%-----------------------------------------------------------------------------

The structures \InitialAssignment,
\SimpleSpeciesReference, \Parameter,
\KineticLaw, \Rule, \Model,
\Constraint and \Reaction all have \token{sboTerm}
optional fields of the type \primtype{SBOTerm}. These fields when
present must contain valid Systems Biology Ontology\sboref
identifiers that refer to actual SBO terms. The \primtype{SBOTerm}
...
the SBOTerm referenced by the \token{sboTerm} field.
...
\primtype{sboTerm} fields in preference to using application specific
annotation schemes.
\end{blockChanged}
...
\StoichiometryMath, ventAssignment,
...
\begin{figure}[htb]
  ...
  \caption{The definition of \Sbml for SBML Level~2 \changed{Version~2}.  Following UML notation, additional fields
...
The \Model structure has an optional \primtype{SBOTerm} field,
\token{sboTerm} (see Section~\ref{sec:sboTerm}).  This field when
...
The \Parameter structure has an optional \primtype{SBOTerm}
...
\SpeciesReference structure should have a `is a'
...
The \InitialAssignment structure has an optional
\primtype{SBOTerm} field, \token{sboTerm} (see
Section~\ref{sec:sboTerm}).  This field when present must contain
an Systems Biology Ontology (SBO)\sboref term identifier.  The
\InitialAssignment structure should have a `is a'
relationship with the SBO term. The SBO term chosen should be the
...
\changed{The \Rule structure has an optional
\primtype{SBOTerm} field, \token{sboTerm} (see
Section~\ref{sec:sboTerm}).  This field when present must contain
an Systems Biology Ontology (SBO)~\sboref term identifier.  The
\token{sboTerm} field should contain a SBO term identifier
referring to an actual SBO Term. The \Rule structure should
...
The \Constraint structure has an optional \primtype{SBOTerm}
field, \token{sboTerm} (see Section~\ref{sec:sboTerm}).  This
field when present must contain an Systems Biology Ontology
(SBO)\sboref term identifier referring to an actual SBO term.  The
\Constraint structure should have a `is a' relationship
...
structure includes a required \token{id}\changed{, of type \primtype{SId},} and an optional
\token{name}\changed{, of type \primtype{string}}.  These must be used according to the guidelines described in
Section~\ref{sec:idnameattribs}.
...
The \Reaction structure has an optional \primtype{SBOTerm}
field, \token{sboTerm} (see Section~\ref{sec:sboTerm}).  This
field when present must contain an Systems Biology Ontology
...
simply to hold the fields \token{species}, \token{id}, and \token{name}
that are common to the latter structures.  The optional identifier stored
in the \token{id} field gives the reference an identifier for use in the
...
\primtype{SId} data type described in Section~\ref{sec:sid}.  The \token{id}
value (whether it is in a \SpeciesReference or
\ModifierSpeciesReference object) exists in the global namespace of
the model, as described in Section~\ref{sec:idnameattribs}.
\SimpleSpeciesReference also has an optional \token{name} field, of
type \primtype{string}.  The \token{name} and \token{id} fields must be used
...
\changed{The \SpeciesReference structure also has an optional
\primtype{SBOTerm} field, \token{sboTerm} (see
...
only be used when the \SpeciesReference structure is
contained in the \token{product} field of the containing
\Reaction structure.  Similarly a reactant term should only
be used when the \SpeciesReference structure is contained
in the \token{reactant} field of the containing \Reaction
structure. The \SpeciesReference structure should have a
...
The \ModifierSpeciesReference structure has an optional
\primtype{SBOTerm} field, \token{sboTerm} (see
Section~\ref{sec:sboTerm}).  This field when present must contain
a Systems Biology Ontology (SBO)\sboref term identifier referring
to a modifier SBO term, that is a term derived from \sbomodifier.
The structure containing the \token{sboTerm} field should have a
``is a'' relationship with the SBO term. The SBO term chosen should
be the most precise (narrow) term that captures the role of the
given species in the reaction.
...
ventAssignment structures cannot contain the value of a
...
\Event and ventAssignment are derived from \SBase
...
\begin{figure}[htb]
  ...
  \caption{The definitions of \Event and ventAssignment.
...
ventAssignment structures.  This field is implemented as
a \token{listOfEventAssignments} element containing one or more
\token{eventAssignment} elements.  The ventAssignment
...
ventAssignment structures within an \Event
...
ventAssignment structures is not significant; the effect
...
ventAssignment structures represent the value of the
identifier at the point when the \Event is \emph{fired}.

\changed{A variable cannot be assigned a value in an
ventAssignment structure and be assigned a value by an
\AssignmentRule structure i.e. the value of a
\token{variable} attribute on a ventAssignment structure
...
In all cases, as would be expected, the units of the formula
\changed{ in an ventAssignment} are identical to the
...
\item \emph{In the case of a species}, an ventAssignment sets the
  referenced species' quantity (\quantity{concentration} or
  \quantity{amount of substance})
  to the value determined by the formula
  in \token{math}.  The units of the formula are the \emph{units of the species} as
...
\item \emph{In the case of a compartment}, an ventAssignment sets
  the referenced compartment's size to the size determined by the formula
  in \token{math}.  The overall units of the formula are the units
  specified for the size of the compartment identified by the value of the
  ventAssignment's \token{variable} field.  (See
  Section~\ref{sec:compartment-units} for an explanation of how the units of the
  compartment's size are determined.)

\item \emph{In the case of a parameter}, an ventAssignment sets the
  referenced parameter's value to that determined by the formula in
  \token{math}.  The overall units of the formula are the units
  defined for the parameter identified by the value of the
  ventAssignment's \token{variable} field.  (See
...
at times $t_1$ and $t_2$.  The ventAssignment structure
...
\InitialAssignment, ventAssignment,
...
ventAssignment element should be unique among the set of
assignments within an \Event element if not the effect of
event assignment is ambiguous.

\item page 31 Section 4.10.5: A \token{variable} attribute on a
ventAssignment element should not have the same value as
...
ventAssignment structure will have effect at $t_1+d$ and
...
an \token{eventAssignment} must contain the value of a \token{id}
...
a ventAssignment structure cannot be the same as the
