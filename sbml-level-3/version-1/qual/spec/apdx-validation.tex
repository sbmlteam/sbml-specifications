...
\validRule{qual-20310}{A \QualitativeSpecies with attribute \token{qual:\-constant} set to \token{true} cannot only be refferred to by an \Input. It cannot be the subject of an \Output in a \Transition.    (References: SBML Level~3 Package
  Specification for Qualitative Models, Version~1,
  \sec{qualSpecies-class}.) }
...
\validRule{qual-20406}{The various
  \ListOfFunctionTerms subobject with a \Transition
