...
\begin{figure}[thb]
  ...
  \newcounter{rownum}
  \setcounter{rownum}{0}
  \begin{edtable}{tabular}{>{\stepcounter{rownum}\therownum.}rp{5.9in}}
    \toprule
...
\validRule{comp-10101}{To conform to the Hierarchical Model
...
\validRule{comp-20303}{An ExternalModelDefinition object must have the
  ...
  may have the optional attribute \token{comp:\-model\-Ref}.  
  No other attributes from the Hierarchical Model
...
\validRule{comp-20622}{The \token{comp:\-timeConversionFactor} attribute
  ...
  (References: SBML Level~3 Package Specification for Hierarchical Model
  Composition, Version~1, \sec{submodel-timeconversionfactor}.) }
