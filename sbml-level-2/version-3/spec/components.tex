...
the characters \token{<?xml}.  \sbmltwo uses XML version 1.0 and
requires a document encoding of UTF-8.

Following this XML declaration, the outermost portion of a model
expressed in SBML \changed{\thisLV} consists of an object of class
\Sbml, defined in Figure~\ref{fig:sbml}.  This class contains two
required attributes, \token{level} and \token{version}, and one
required subelement called \token{model} whose class is \Model.
...
\token{sbml} element.  The XML namespace URI for \sbmltwothree is
the string \uri{http://www.sbml.org/sbml/level2/\changed{version3}}.  All
Level~2 Version~\changed{3} elements must be placed in this namespace either
...
The \Model structure has an optional \changed{attribute}, \token{id}, used to
...
Instances of these objects are placed inside instances of classes
...
\ListOfEvents.  The ``list'' classes are also defined in
Figure~\ref{fig:model}.  All the contents of these lists are
optional; that is, the lists are permitted to have zero length.
However, as shown in the definition of Figure~\ref{fig:model}, the
lists container elements themselves are mandatory.  The resulting
XML data object has the form illustrated by the following skeletal
model:
...
<sbml xmlns="http://www.sbml.org/sbml/level2/\changed{version3}" level="2" version="\changed{3}">
    <model id="My_Model">
        <listOfFunctionDefinitions>
            \textrm{\emph{zero or more}} <functionDefinition> ... </functionDefinition> \textrm{\emph{elements}}
        </listOfFunctionDefinitions>
        <listOfUnitDefinitions>
            \textrm{\emph{zero or more}} <unitDefinition> ... </unitDefinition> \textrm{\emph{elements}}
        </listOfUnitDefinitions>
        <listOfCompartmentTypes>
            \textrm{\emph{zero or more}} <compartmentType> ... </compartmentType> \textrm{\emph{elements}}
        </listOfCompartmentTypes>
        <listOfSpeciesTypes>
            \textrm{\emph{zero or more}} <speciesType> ... </speciesType> \textrm{\emph{elements}}
        </listOfSpeciesTypes>
        <listOfCompartments>
            \textrm{\emph{zero or more}} <compartment> ... </compartment> \textrm{\emph{elements}}
        </listOfCompartments>
        <listOfSpecies>
            \textrm{\emph{zero or more}} <species> ... </species> \textrm{\emph{elements}}
        </listOfSpecies>
        <listOfParameters>
            \textrm{\emph{zero or more}} <parameter> ... </parameter> \textrm{\emph{elements}}
        </listOfParameters>
        <listOfInitialAssignments>
            \textrm{\emph{zero or more}} <initialAssignment> ... </initialAssignment> \textrm{\emph{elements}}
        </listOfInitialAssignments>
        <listOfRules>
            \textrm{\emph{zero or more subclasses of \abstractclass{Rule}}}
        </listOfRules>
        <listOfConstraints>
            \textrm{\emph{zero or more}} <constraint> ... </constraint> \textrm{\emph{elements}}
        </listOfConstraints>
        <listOfReactions>
            \textrm{\emph{zero or more}} <reaction> ... </reaction> \textrm{\emph{elements}}
        </listOfReactions>
        <listOfEvents>
            \textrm{\emph{zero or more}} <event> ... </event> \textrm{\emph{elements}}
        </listOfEvents>
    </model>
</sbml>
\end{example}

Note that there are dependencies between SBML components, such
...
\SBase's various attributes and subelements such as \token{notes}
and \token{annotation}.  The list classes do not add any
attributes of their own.

Readers may wonder about the motivations for the use of the
\class{ListOf\rule{0.5in}{0.5pt}} containers.  A simpler approach
to creating the lists of components in XML would be to place them
all directly at the top level.  We chose instead to group them
within XML elements named after
\class{listOf}\rule{0.5in}{0.5pt}\class{ClassNames}, in part
...
The \Model structure has an optional \token{sboTerm} \changed{attribute} of type
\primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  \changed{Values} \changed{of this attribute must
be chosen from identifiers} referring to
...
the \changed{values of \token{sboTerm} attributes} on each \Reaction in a model may still
...
\begin{figure}[htb]
  ...
  \caption{\changed{The definition of class \FunctionDefinition.
      ...
      sequence of zero or more instances of \FunctionDefinition
...
SBML model.  \changed{The units of the} \changed{following
  ...
  the formula.}  Rather than requiring a complete unit definition
on every structure, SBML provides a facility for defining units
that can be referenced throughout a model.  In addition, every
kind of SBML mathematical entity has units assigned to it from a
set of built-in defaults (see Section~\ref{sec:built-in-units}
below, and also Sections~\ref{sec:compartment-units},
\ref{sec:species-units} and~\ref{subsec:kinetic-law}).  By
redefining these built-in default units, it is possible to change
the units used throughout a model in a simple and consistent
manner.
...
\begin{figure}[htb]
  \centering
  \small
  \begin{tikzpicture}[level distance=1in]
    \node { \emptyClassbox{\textsl{SBase}} }
      ...
      child {node[left=-0.25in] (b) { \emptyClassbox{ListOfUnits}
        }}
      child {node (c) { 
          \begin{classbox}{Unit}
            kind: UnitKind                                     \\
     ...
     ;
     \draw[diamond-] (a) -- (b) 
       node[above=6pt,left=0.5in] {\textsf{listOfUnits}};
     \draw[diamond-,shorten >=-6pt] (b) -- (c) 
       node[above=6pt,left=1.6in] {\textsf{unit}}
       node[above=6pt,left=1.25in] {\textsf{1..*}};
  \end{tikzpicture}
  \caption{\changed{The definition of classes \UnitDefinition and
      \Unit.  A sequence of zero or more instances of
      \UnitDefinition objects can be located in an instance of
      \ListOfUnitDefinitions in \Model, as shown in
      Figure~\protect\ref{fig:model}.  Note how
      \class{ListOfUnits} has no attributes (beyond those it
      inherits from class \SBase); it merely acts as a container
      for one or more instances of \Unit objects.}}
...
(Table~\ref{tab:unitkind}).  The \changed{attribute} \token{kind} indicates the
chosen base unit, whereas the three \changed{attributes} \token{exponent},
\token{scale}, and \token{multiplier} define how the base unit is
being transformed.  These various \changed{attributes} are described in detail
below.

Compatibility note: in \sbmltwoone, \Unit had an additional \changed{attribute}
called \token{offset}.  This \changed{attribute} has been removed entirely in
\sbmltwotwo \changed{and Version~3}.  Modelers and software authors are instead directed
...
The \Unit data structure has one required \changed{attribute}, \token{kind},
...
\Unit structure also has an optional \token{scale} \changed{attribute}; its
...
\item The value of the \token{unitKind} \changed{attribute} in the \Unit object
...
\begin{figure}[htb]
  ...
  \caption{\changed{The definition of class \CompartmentType.  A
      sequence of zero or more instances of \CompartmentType
      objects can be located in an instance of
...
In \changed{\sbmltwothree}, a compartment type only has an identity, and this
...
\changed{\sbmltwothree}---they have no effect on a model's mathematical
...
\begin{figure}[htb]
  ...
  \caption{\changed{The definition of class \SpeciesType.  A
      sequence of zero or more instances of \SpeciesType objects
...
\begin{figure}[htb]
  ...
  \caption{\changed{The definition of class \Compartment.  A
      sequence of zero or more instances of \Compartment objects
...
A \Compartment structure has an optional \changed{attribute}
...
A compartment's size is set by its \token{size} \changed{attribute} exactly
once.  If the compartment's \token{constant} \changed{attribute} has the value
...
the compartment size differ in that the \token{size} \changed{attribute} can
...
\token{constant} \changed{attribute} has the value \val{false}, the
...
\begin{table}[h]
  ...
  \begin{edtable}{tabular}{cllll}
    \toprule
    \textbf{Value of \changed{attribute}}    & \texttt{size} & \texttt{units} &  & \textbf{Default value}\\
  ...
  \caption{The types of units permitted for the units of
    ...
    \val{0}, the \token{units} \changed{attribute} must not be present
...
\changed{attribute}, are used in the following ways when the compartment has a
\token{spatialDimensions} value greater than \val{0}:
\begin{itemize}

\item The value of the \token{units} \changed{attribute} is used as the units of
  the \token{size} \changed{attribute} of the compartment structure (see
...
\item The value of the \token{units} \changed{attribute} is used as the units of
  the \token{math} \changed{element} provided on \AssignmentRule and
...
\item In a \RateRule structure that sets the rate of change of the
  ...
  the rule's \token{math} \changed{element} are those in the compartment's
  \token{units} \changed{attribute} divided by the default \quantity{time}
...
\begin{figure}[htb]
  ...
  \caption{\changed{The definition of class \Species.  A sequence
      of zero or more instances of \Species objects can be located
...
Previous definitions of \Species in SBML Level~2 included an
additional attribute called \token{spatialSizeUnits}, which
allowed explicitly setting the units of \emph{size} for initial
...
species was located.  Since the \token{spatialSizeUnits} attribute
determined the size units of a species when that species appeared
in reaction rate formulas, and the compartment may have its own
different spatial units, when both the species and the compartment
appeared in reaction rate formulas, one of the quantities would
have had to be converted to have the same spatial units as the
other.  In other words, modelers and software tools would have had
to insert explicit conversion factors into kinetic rate formulas,
which can be difficult and prone to errors.  Second, it is 
(SEE STEFAN'S REASON)
For these reasons, and because few software packages seemed to
take account of the implicit unit conversion, it was deemed better
to remove the \token{spatialSizeUnits} attribute from \Species.
...
represented by the \Species structure. The \changed{attribute}'s value must be
...
structures having a value for the \token{speciesType} \changed{attribute}, the
pair
\begin{center}
...
\token{initialConcentration} \changed{attributes} exactly once.  If the species'
\token{constant} \changed{attribute} has the value \val{true} (the default),
...
\token{initialConcentration} \changed{attributes} can only be used to set the
species quantity to a literal scalar value, whereas
\InitialAssignment allows the value to be set using an arbitrary
mathematical expression.  If the species' \token{constant} \changed{attribute}
...
\changed{attribute} of the species structure.  The units of the
...
\subsubsection{The \token{substanceUnits} and
    \token{hasOnlySubstanceUnits} \changed{attributes}}
...
\token{hasOnlySubstanceUnits}, \changed{and by the units of the
  size defined for} \changed{the compartment object in which the
  species are located.}
...
\begin{table}[htb]
  ...
  \changed{attribute} of the \Species structure.}
...
\item The \token{math} \changed{element} of \AssignmentRule and
...
\item In \RateRule structures that set the rate of change of the
  ...
  rule's \token{math} \changed{element} are the \emph{units of the species}
...
The \Species structure has two optional boolean \changed{attributes} named
...
\begin{table}[ht]
  ...
  \caption{\changed{How to interpret the values of the \token{constant} and
      \token{boundaryCondition} \changed{attributes} of the \Species structure.
...
The optional \changed{attribute} \token{charge} takes an integer indicating the
charge on the species (in terms of electrons, not the SI unit
coulombs). This may be useful when the species is a charged ion
such as calcium ($\text{Ca}^{2+}$).  The \token{charge} \changed{attribute} is
...
\begin{figure}[htb]
  ...
  \caption{\changed{The definition of class \Parameter.  A
      sequence of zero or more instances of \Parameter objects can
...
by the \changed{attribute} \token{units}.  These units are relevant when the
...
units assigned to the parameter using the \token{units} \changed{attribute} and
\quantity{time} is the built-in \token{time} units.  The value
assigned to the parameter's \token{units} \changed{attribute} must be chosen
...
The \Parameter structure has an optional boolean \changed{attribute} named
...
The \Parameter structure inherits from \SBase the optional \token{sboTerm} \changed{attribute} of
...
\begin{figure}[htb]
  ...
  \caption{\changed{The definition of class \InitialAssignment.  The
    ...
    sequence of zero or more instances of \InitialAssignment
    objects can be located in an instance of
...
\InitialAssignment contains the \changed{attribute} \token{symbol}, of type
\primtype{SId}.  The value of this \changed{attribute} in an \InitialAssignment
object can be the identifier (\ie the value of the \token{id}
\changed{attribute}) of a \Compartment, \Species or \Parameter elsewhere in the
model.  The purpose of the \InitialAssignment is to define the
initial value of the constant or variable referred to by the
\token{symbol} \changed{attribute}.  (The \changed{attribute}'s name is \token{symbol} rather
...
\token{math} \changed{element} are taken to be the units associated with the
...
abstract \Rule structure containing an \changed{element}, \token{math}, to hold
the right-hand side expression, then to derive subtypes of \Rule
that add \changed{attributes} to distinguish the cases of algebraic, assignment
...
A \Rule structure has a required \changed{element} called \token{math},
...
The \Rule structure inherits from \SBase the optional \token{sboTerm} \changed{attribute} of type
\primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
...
\begin{figure}[htb]
  ...
  \caption{\changed{The definition of class \Constraint.  The
      ...
      zero or more instances of \Constraint objects can be located
...
A \Constraint structure has a required \changed{element}, \token{math},
...
A \Constraint structure has an optional \changed{element} called
...
with the element \token{<html>} and ending with \token{</xhtml>};
...
The \Constraint structure inherits from \SBase the optional \token{sboTerm} \changed{attribute} of
type \primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this \changed{attribute} in a
...
times $t >= 0$.  Each \Constraint's \token{math} \changed{element} is first
...
\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.779]{figs/reaction-uml}
  \caption{\changed{The definitions of classes \Reaction, \KineticLaw,
      ...
      \ListOfModifiers, and \ListOfParameters.}}
  \label{fig:reaction}
\end{figure}
...
structure includes a mandatory \changed{attribute} called \token{id}, of type
...
boolean \changed{attribute} in the \Reaction data structure.  The \changed{attribute}'s
...
The \Reaction structure inherits from \SBase the optional \token{sboTerm} \changed{attribute} of
type \primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this \changed{attribute} in a
...
of the \token{sboTerm} \changed{attribute} on the \Model element.  If the
\Model's \token{sboTerm} \changed{attribute} has a value, it should be
...
\changed{attribute} on \KineticLaw within \Reaction must still be given a value,
...
\token{listOfSpecies} \changed{element} of the top-level \Model data structure
...
and elements that are common to the latter two structures.}
...
The \SimpleSpeciesReference structure has a mandatory \changed{attribute},
...
\changed{The class} \SimpleSpeciesReference inherits from \SBase the optional
\token{sboTerm} \changed{attribute} of type \primtype{SBOTerm} (see
...
\ModifierSpeciesReference, all have \token{sboTerm} \changed{attributes}.  When
a value is given to this \changed{attribute}, it must be a valid SBO referring
...
structure is contained in the \token{product} \changed{element} of the
...
the \token{reactant} \changed{element} of the containing \Reaction structure.
...
The value of the \token{species} \changed{attribute} must be the identifier of
...
\ModifierSpeciesReference structure inherits the mandatory \changed{attribute}
...
\token{math} and \token{parameter}, in addition to the
...
\KineticLaw inherits from \SBase the optional \changed{attribute} called
\token{sboTerm} of type \primtype{SBOTerm} (see
Section~\ref{sec:sboTerm}).  When a value is given to this \changed{attribute},
...
The value of the \token{id} \changed{attribute} of a \Reaction structure can be
...
is defined in Figure~\vref{fig:event}.  \changed{The structures
...
\paragraph{The \token{sboTerm} \changed{attribute} on \class{Event}}
\label{sec:event-sboterm}

\changed{As with all SBML components derived from \SBase, an
\Event has an optional \changed{attribute} \token{sboTerm} of type
\primtype{SBOTerm} }\changed{ (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).}  When a value is given to this
\changed{attribute}, it must be a valid term derived from \sboevent in SBO.  The
...
the optional \changed{attribute} \token{sboTerm} of type \primtype{SBOTerm} (see
Sections~\ref{sec:sboterm-type} and~\ref{sec:sboTerm}).  When a
value is given to this \changed{attribute}, it must be a valid term representing
...
numbers in SBML have default units of \val{dimensionless} and what
...
\token{delay} above has units of \val{dimensionless}.  To make the
...
The identifiers occurring in the MathML \token{ci} \changed{attribute}s of the
...
\item \emph{In the case of a parameter}, an \EventAssignment sets
  ...
  \token{variable} \changed{attribute}.  (See Section~\ref{sec:parameter-units}
  for an explanation of how the units of the parameter are
  determined.)  If no units are defined for the parameter in
  question, then the formula contained in \token{math} must have
  units \val{dimensionless}.

\end{itemize}

It is worth noting that the formula placed in the \token{math}
\changed{element} has no assumed units.  The consistency of the
units of the formula, and the units of the entity which the
assignment affects, must be explicitly established just as in the
case of the value of \token{delay}.  A similar approach as the one
discussed in Section~\ref{sec:event-delay} may be used.
