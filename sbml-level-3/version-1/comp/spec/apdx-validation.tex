...
\begin{figure}[thb]
  ...
  \begin{edtable}{tabular}{>{\stepcounter{rownum}\therownum.}rp{6.05in}}
    ...
    & Transform the remaining submodel objects as follows:
    ...
    & For every object that has not been removed in submodel
    ``\emph{M}'', adjust every reference to the identifiers that were
    transformed in the previous step as follows.  Change every
    \primtype{SIdRef} value by prepending ``\emph{P}'' to the original
    value, and change every XML \primtype{IDREF} value by prepending
    ``\emph{P}'' to the original value.  \emph{However}, if the submodel
    has been replaced by the application of either a \ReplacedElement or
    \ReplacedBy construct, change each \primtype{SIdRef} and
    \primtype{IDREF} value that pointed into the submodel to be the
...
\validRule{comp-10303}{Within a \Model or \ExternalModelDefinition
  ...
  for Hierarchical Model Composition, Version~1, \sec{namespaces}.) }
...
\validRule{comp-10308}{The value of a \token{comp:submodelRef}
  attribute on \ReplacedElement and \ReplacedBy objects must always conform to the
...
\validRule{comp-20303}{An \ExternalModelDefinition object must have the
  ...
  they are required, and may the optional attribute
  \token{comp:modelRef}.  No other attributes from the Hierarchical
...
\validRule{comp-20607}{A \Submodel object must have the attributes
  ...
  and may also have the optional attributes
...
\validRule{comp-20803}{A \Port object must have a value for the required
  ...
  \token{comp:metaIdRef}.  No other attributes from the Hierarchical
...
\validRule{comp-20903}{A \Deletion object must have a value for the
  ...
  attributes \token{comp:portRef}, \token{comp:idRef},
  \token{comp:unitRef}, or \token{comp:metaIdRef}.  No other
...
\validRule{comp-21006}{The value of a \token{conversionFactor}
...
\validRule{comp-21103}{A \ReplacedBy object must have a value for
  the required attribute \token{comp:submodelRef}, and a value for one,
  and only one, of the following attributes: \token{comp:portRef},
  \token{comp:idRef}, \token{comp:unitRef}, or \token{comp:metaIdRef}.
  No other attributes from the
  Hierarchical Model Composition namespace are permitted on a
  \ReplacedBy object.  (References: SBML Level~3 Package
  Specification for Hierarchical Model Composition, Version~1,
  \sec{replacedby-class}.) }
