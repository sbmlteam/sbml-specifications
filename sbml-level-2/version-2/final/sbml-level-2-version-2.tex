...
(\eg \token{xhtml:center} as in the prevous case).  Other
approaches are also possible.

SBML does not require the content of \token{notes} to be any
particular XHTML element.  The only restriction is that
\token{notes} must not contain an XML declaration (\ie
\token{<?xml version="1.0" encoding="UTF-8"?>}) nor a DOCTYPE
declaration.  The notes can otherwise be can be any single or
multiple XHTML elements, subject to the restriction that the
character encoding cannot be changed away from UTF-8 (the encoding
for SBML).


\paragraph{Processing \token{notes} Content}

If \token{notes} can contain any of a number of possible XHTML
elements, how should an application process the contents?  Let us
take the case of an SBML-reading application that needs to take
...
the \token{notes} may not be a complete XHTML document, so the
application will have to perform some processing before handing it
to the XHTML editor.  How should this be done?
...
Version~2, these attributes must be set to \val{2} and
\val{2}, respectively.}
...
example of these essential XML elements for a proper \sbmltwotwo
document:
...
exactly once.  Unless subsequently changed, this value is in
effect at all times, including simulation time $t <= 0$.  An
\InitialAssignment or an \AssignmentRule for the same compartment
identifier overrides this setting (see
...
\token{initialConcentration} fields exactly once.  Unless
subsequently changed, this value is in effect at all times,
including simulation time $t <= 0$.  An \InitialAssignment or an
...
\token{initialAmount} or \token{initialConcentration} on a
species and also redefine the value using an initial assignment or
an assignment rule, but the \token{initialAmount} or
...
exactly once.  Unless subsequently changed, this value is in
effect at all times, including simulation time $t <= 0$.  An
\InitialAssignment or an \AssignmentRule for the same parameter
identifier overrides this setting (see
Sections~\ref{sec:initialAssignment}
and~\ref{sec:assignmentrule}).  It is not an error to define
...
values that are in effect at ``initial time'' ($t = 0$).  The
system of equations defined by a model's \Reaction{}s,
\RateRule{}s, \AlgebraicRule{}s, \Constraint{}s, and \Event{}s can
first be evaluated only after all initial assignments have been
executed.  For purposes of evaluating expressions that involve the
\emph{delay} \token{csymbol} (Section~\ref{sec:csymbol-token}),
initial assignments are considered to apply at $t <= 0$.  In
combination with values defined by species, compartments and
parameters definitions, and values assigned by \AssignmentRule
constructs, this enables delay expressions to be evaluated even
though they may refer to values of variables taken at $t < 0$.
(See Section~\ref{sec:meaning-of-time}.)
...
initial assignments \emph{and} assignment rules for
...
\begin{blockChanged}
An important question is whether an event can fire at initial
simulation time, \ie $t = 0$.  The answer is no: an event can only
be triggered immediately after $t = 0$.  The reason is that when a
