...
\attrib{notes} and \attrib{annotations}.  The field \attrib{notes} is a
...
second field, \attrib{annotations}, is provided for software-generated
annotations.  It is a container for arbitrary data (XML type \class{any})
and is intended to store information not intended for human viewing.  As
with the user-visible \attrib{notes} field, every data object can have its
own \attrib{annotations} value.
...
\attrib{annotations}: (a) Information about graphical layout of model
...
\end{verbatim}
  \end{minipage}
  \caption{The definition of the type \class{SName}, expressed in conventional
    ...
    limits the allowable characters in \class{SName} to plain ASCII text
...
operators in formulas return \class{double} values.
...
that define a given model.  Only one component of type \class{Model} is
...
definition of the \class{Model} structure is shown in
...
A model must contain at least one \class{Specie}, one \class{Reaction} and
one \class{Compartment} data element, but SBML does not impose restrictions
on the total number of these.  A \class{Model} data object may optionally
have lists of \class{UnitDefinition}, \class{Parameter} and \class{Rule}
components (they are optional because the lists in fields
\attrib{unitDefinition}, \attrib{compartment}, and \attrib{rule}
are permitted to have zero length).  It may also have an optional
\attrib{name} field that can be used to give the model a name.  The name
must be a text string conforming to the syntax permitted by the
\class{SName} data type described in Section~\ref{sec:name}.
...
$second^{\,-2}$.  The \class{Unit} data structure has a \attrib{kind} field
whose value must be taken from \class{UnitKind}, an enumeration of SI
units.  The possible values of \class{UnitKind} are listed in
Table~\vref{tab:unitkind}.  The \attrib{exponent} field on \class{Unit}
...
located.  The definition of \class{Compartment} is shown in
...
A \class{Compartment} data object has a \attrib{name} field of type
\class{SName}.  A compartment also has a floating-point field called
...
information.  The \attrib{volume} field is optional and defaults to a value
of ``\texttt{1}'' (one).
...
value of \attrib{outside} for a given compartment should be the name of the
...
proteins).  The \class{Specie} data structure is intended to represent
these entities.  Its definition is shown in Figure~\vref{fig:specie}.

\begin{figure}[htb]
  \centering
  \includegraphics[scale = 0.68]{specie}
  \caption{The definition of \class{Specie}.  As usual, fields inherited from
    \class{SBase} are omitted here but are assumed.}
  \label{fig:specie}
\end{figure}

\class{Specie} has a \attrib{name} field of type \class{SName}.  The field
\attrib{compartment}, also of type \class{SName}, is used to identify the
...
quantity may be explicitly set using the optional field \attrib{units}.
The value assigned to \attrib{units} must be chosen from one of the
following possibilities: one of base unit names from
Table~\vref{tab:unitkind}, the name ``\attrib{volume}'', or a new unit name
defined by a unit definition in the enclosing model.  If absent, the units
default to the value set by the built-in ``\attrib{substance}'' of
...
simulation.  \attrib{boundaryCondition} defaults to a value of
...
heading \attrib{listOfSpecies} in the model:
...
A \class{Parameter} structure is used to associate a symbol with a
...
\class{Parameter} has a \attrib{name} field of type \class{SName}.  This
name represents the symbol.  The field \attrib{value} determines the value
(of type \class{double}) assigned to the symbol.  The units of the
parameter \attrib{value} are specified by the field \attrib{units}.  The
...
the three names ``\attribvalue{substance}'', ``\attribvalue{time}'', or
``\attrib{volume}'' (see Table~\ref{tab:builtin}); or the name of a new
...
\class{SpecieConcentrationRule}, and \class{ParameterRule} are all derived
...
enumeration of type \class{RuleType} that determines whether a rule
falls into category 2 or 3 in the list of cases above.  In SBML Level~1,
the enumeration has two possible values: ``\class{scalar}'' and
``\class{rate}''.  The former means that the expression has a scalar value
...
\attrib{specie}, to the basic \class{AssignmentRule} type.  The field
\attrib{specie} has type \class{SName} and is used to identify the specie
...
if the value is ``\class{rate}'', the rule sets the rate of change of the
...
that are declared on the referenced \class{Specie} element, and the
$volume$ units are those declared on the \class{compartment} element that
contains the \class{Specie}.
...
parameter.  The \attrib{units} field acts in the same way as in the case of
the \class{Parameter} structure (Section~\ref{sec:parameters}).  The value
...
by the expression in the \attrib{formula} attribute.


\subsubsection{Guidelines for Evaluating Rules}

Rules of type ``\class{scalar}'' should be evaluated first in the order
given in the SBML model definition, followed by the rules of type
``\class{rate}'' in the order given.  Following this, the ``\class{rate}''
type rules can be evaluated any number of times before the
``\class{scalar}'' type rules are re-evaluated.
...
implemented using lists of \class{SpecieReference} structures (defined in
Section~\ref{subsec:speciesreference} below).  The \class{SpecieReference}
structure contains fields for recording the names of species and their
stoichiometries.  \attrib{kineticLaw} is an optional field of type
\class{KineticLaw} (defined in Section~\ref{subsec:kinetic-law} below),
...
\subsubsection{\class{SpecieReference}}
\label{subsec:speciesreference}

Each unique specie involved in a reaction is listed once in a model, in a
list contained in the \attrib{specie} field of the \class{Model} data
structure discussed in Section~\ref{sec:model}.  Lists of products and
reactants in \class{Reaction} type structures refer to those species.  The
connection between the products and reactants in a reaction definition and
the specie names listed in the enclosing \class{Model} definition is
achieved using the \class{SpecieReference} type data structure defined in
Figure~\vref{fig:reaction}.

The field \attrib{specie} of type \class{SName} in \class{SpecieReference}
must refer to the name of a specie defined in the enclosing
\class{Model}-type structure.  The two fields \attrib{stoichiometry} and
\attrib{denominator} together set the stoichiometry value for a specie in a
reaction.  Both are integers, and both have default values of
``\attribvalue{1}'' (one).  The use of these separate terms allows a
...
the use of specie references and the \class{KineticLaw} structure:
\begin{example}
<model>
    ...
    <listOfReactions>
...
can override this scale locally.  The \attrib{volume} and \attrib{time}
...
\class{specie} elements set the initial amount of every given specie to $1$
