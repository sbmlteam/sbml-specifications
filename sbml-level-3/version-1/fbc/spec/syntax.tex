...
represents the type of exponent contained by the \FluxObjective. For example, where $J$ represents a steady-state flux in the \token{reaction} attribute, the \primtype{FbcVariableType} allows the definition of either a \val{linear}, $J^1$ or \val{quadratic}, $J^2$ term. In addition, a \val{quadratic} component may be defined using the optional attribute \token{reaction2}. For example, with \token{reaction2} represented by $J_{R2}$ it is possible to define a two variable \val{quadratic} \FluxObjective of the form $J^{1} \times J_{R2}^{1}$.}
