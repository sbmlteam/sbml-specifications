...
The \FBCPackage defines a \GeneAssociation class that derives from \SBase and inherits the attributes \token{metaid} and \token{sboTerm}, as well as the subcomponents for \Annotation and \Notes. As shown in Fig.~ \ref{fig:fbc_uml_ga_all} the \GeneAssociation class extends \Reaction with one or more genes (or gene products). Where more than one gene are present in a single gene associations they are are expressed as a logical expression.
...
Each \GeneAssociation contains a single \Association, however, as described in \ref{association-class} an \Association is an abstract class such that an \token{association} should always contain an instance of one of its sub-classes \GeneAnd, \GeneOr or \Gene.
...
The \FBCPackage defines a \Gene class that represents a gene (or gene product). It is derived from an \Association and thereby inherits the \SBase attributes \token{metaid} and \token{sboTerm}, as well as the subcomponents for \Annotation and \Notes as described in \ref{fig:fbc_uml_ass}.
