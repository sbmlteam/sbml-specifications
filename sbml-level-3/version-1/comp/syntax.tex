...
\primtype{IDREF}, \primtype{SId}, \primtype{SIdRef}, and
...
\primtype{PortSId} attribute in the model.  In other words, the value of
a \primtype{PortSIdRef} attribute must be an existing port identifier in
the model.  As with \primtype{PortSId}, the equality of
...
If a model from a an external SBML document is needed, it can be
...
The optional attribute \token{unitIdRef} takes a value of type
\primtype{UnitIdRef}.  This attribute is used to refer to the identifier
of a \UnitDefinition object.  The namespace of the \primtype{UnitIdRef}
...
\token{sbaseRef} will refer to an SBML component inside that \Model or
...
the identifier \val{p1} inside a submodel \val{m1}.  The following XML
...
Although the use of nested \SBaseRef objects allows a model to refer to
...
class.  The extensions are diagrammed in \fig{extended-model-uml}, and
...
\token{portRef}, \token{idRef}, \token{unitRef} and \token{metaIdRef},
and the recursive subcomponent under \token{sbaseRef}.  In addition,
\Port one required attribute, \token{id}, described below.
...
It is legal for the model by \token{modelRef} to have its own submodels.
...
have its \token{portRef} attribute value se to the port identifier of
the object being replaced.  If it does not have a port identifier, but
has a regular identifier (typically an attribute named \token{id}), then
...
\primtype{SIdRef}.  The value be the identifier of a \Deletion object in
the model, that is, the value of some \Deletion object's \token{id}
attribute.  
