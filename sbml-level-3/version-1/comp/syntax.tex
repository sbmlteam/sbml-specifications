...
package specification, in particular \primtype{boolean}, \primtype{ID},
\primtype{IDREF}, \primtype{SId}, \primtype{SIdRef}, \primtype{UnitSId}, \primtype{UnitSIdRef}, and
\primtype{string}.  The Hierarchical Model Composition package also
makes use of or defines other primitive types; they are described below.


\subsubsection{Type \hspace*{1pt}\primtype{anyURI}}
...
\subsubsection{Type \hspace*{1pt}\primtype{PortSId}}
...
\subsubsection{Type \hspace*{1pt}\primtype{PortSIdRef}}
...
a \primtype{PortSIdRef} attribute must be an existing port identifier in
...
\item \Reaction objects introduce a local namespace for \LocalParameter
  ...
  the implication is the the \SBaseRef class (\sect{sbaseref-class})
...
\begin{figure}[hbt]
  ...
  \caption{The definitions of the extended \SBML class as well as the
    ...
    color conventions are explained in \protect\sect{sec:conventions}.}
  \label{hierarchical-sbml-uml}
  \label{sbml-uml}
\end{figure}


\subsubsection{Lists of internal and external model definitions}
...
instead, puts them in a separate list, and the list is a child of the
SBML object itself.  Like other
\textsf{\textbf{ListOf\rule{0.25in}{0.5pt}}} classes in SBML, the
\ListOfModelDefinitions is derived from \SBase but adds no especial
attributes of its own.  It inherits the attributes \token{metaid} and
\token{sboTerm}, as well as the subcomponents for \Annotation and
\Notes, that \SBase provides.

If a model from an external SBML document is needed, it can be
referenced with an object of the \ExternalModelDefinition class
(\sect{externalmodeldefinition-class}).  The
\ListOfExternalModelDefinitions is the container used to hold such
references inside the \SBML object in an SBML document.  It is derived
from \SBase but adds no especial attributes of its own.  Like the other
\textsf{\textbf{ListOf\rule{0.25in}{0.5pt}}} classes, it inherits the
attributes \token{metaid} and \token{sboTerm}, as well as the
subcomponents for \Annotation and \Notes, that most SBML components
have.
...
\fig{hierarchical-sbml-uml}.  It contains several scalar attributes:
\token{source}, \token{id}, \token{modelRef} and \token{md5}.  These
...
not supported by this package.  The \token{source} attribute must have a
...
\primtype{SIdRef}, in is used to identify a \Model object within in the
...
value to give to the \token{modelRef} attribute in \ExternalModelDefinition.
If \token{modelRef} does not have a value, then the main model (i.e., the
\token{<model>} element within the \token{<sbml>} element) in the
...
referenced by \token{source}.  This can be used as a data integrity
...
\subsubsection{The attributes on \class{SBaseRef}}
...
particular object), and all child \SBaseRef objects in the chain will refer to an SBML component inside the \Model
...
the identifier \val{p1} inside the \Submodel \val{m1}.  The following XML
...
If the desired element is within a submodel of a submodel (or deeper) this nested construct can be extended to an arbitrary depth:  as long as a \SBaseRef object points to a \Submodel, particular elements of that submodel (including other submodels) may be referenced by a child \SBaseRef.
...
As mentioned in \sect{namespaces}, \primtype{SId}, \primtype{UnitSId}, and \primtype{PortSId} attributes need only be unique on a per-\Model basis.  Therefore, the \token{idRef}, \token{portRef}, and \token{unitRef} attributes must know to which \Model they are referring.  In addition, even though \primtype{IDREF} attributes are unique on per-document level, the same SBML element may be instantiated in multiple submodels in any number of \Models, and therefore the \token{metaIdRef} attribute must also know to which \Model instantiation it is referring.  This will vary based on \SBaseRef sub-class, and will be explained in those sections.  For 'bare' \SBaseRef objects (which only exist as children of other \SBaseRef objects, as explained above) the \Model instance to which they are referring is the one referenced by the \Submodel to which its parent is pointing.
...
fact, a \Model object in a new context.  The SBML Level~3 Core \Model class is extended here, and
consists of two new lists: one for holding submodels
(\token{listOfSubmodels}, of class \ListOfSubmodels), and one for
holding a list of ports (\token{listOfPorts}, of class \ListOfPorts).
The rest of this section defines the extended \Model class and the \Port
class, while the \Submodel class is described in \sect{submodel-class}.
...
construct.  Software developers who wish to include restrictions are encouraged to experiment here, and add new attributes in a namespace of their own devising.  Future versions of this package may incorporate these attributes to provide additional
...
\token{portRef}, \token{idRef}, \token{unitRef} and \token{metaIdRef},
and the recursive subcomponent \token{sbaseRef}.  In addition,
\Port adds one required attribute, \token{id}, described below.
...
the options described in \sect{sbaseref-class} for referencing other
objects are available to \Port objects.  For example, a port could be
created by using the \token{metaIdRef} attribute to identify the object
...
As evident from \fig{submodel-uml}, \Submodel has numerous attributes as
well as a subcomponent, the \token{listOfDeletions}.  We describe each
of the attributes below, then turn to \token{listOfDeletions} in
...
a single attribute, \token{id}.


\paragraph{The \hspace*{1pt}\token{id} attribute}

The \Deletion class adds one optional attribute, \token{id}, to the
attributes defined by the parent class, \SBaseRef.  The \token{id} can
...
list of replacements shown in \fig{extended-sbase-uml} defines all of the
...
become more clear in the \sect{replacedelement-class} below.
...
when one species in one submodel is the conceptual replacement for a second species in a second submodel. 
In practice, the lack of a direct
mechanism for horizontal replacements is not a true limitation: to
achieve the same effect as in the example, a local species would be created in the containing model replacing both species from the two submodels, setting \token{identical=''true''} for the first, and \token{identical=''false''} for the second.
...
\token{\rule{0.25in}{0.5pt}ConversionFactor} dictate how any submodel
...
\begin{table}[bht]
  ...
  \begin{edtable}{tabular}{p{1.1in}p{2.57in}l}
    ...
    \sprd{6}{\Species}	& \hosu=\val{false} replaced by a \Species having \hosu=\val{true}
	    & \sprd{6}{$\frac{\token{substanceConversionFactor} * \textsf{\emph{(Compart. size)}}}{\percomp}$}\\
