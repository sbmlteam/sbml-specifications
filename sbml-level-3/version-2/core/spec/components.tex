...
\sec*{sec:notation-uml} for defining classes of objects.
We also illustrate the use of SBML components by giving partial
model definitions in XML.  \sec*{sec:xml-rep} provides many
complete example models encoded in SBML.

\relchange{Unless otherwise specified, SBML \thisL documents must follow the XML specification:  anything allowed in XML is allowed in SBML documents, and anything disallowed in XML is not allowed in SBML.
All well-formed SBML documents must begin with an \emph{XML
  declaration}, specifying both the version of XML used
and the document character encoding.  The declaration begins with
the characters \token{<?xml} followed by the attributes \token{version}
and \token{encoding}.  SBML requires XML version 1.0, and UTF-8 as the document encoding.}
...
\begin{figure}[htb]
  ...
  \caption{The definition of class \SBML for SBML \thisLV.  The
    class \Model is defined in \sec{sec:model}.  Note that
...
XML form for an SBML \thisLV \relchange{document. Here,
ellipses (\val{...})} are used to indicate content elided from \relchange{this example}:
...
\uri{http://www.sbml.org/sbml/level3/\thisvlc/core}.  All SBML
...
\sec{sec:sbml-packages}.  For purposes of checking
...
\vertwo{\vspace*{-4ex}\subsubsection{The \token{id} and \token{name} attributes}}

\vertwo{\begin{minipage}{\linewidth}Because \SBML inherits from \SBase, it has optional \token{id}, \token{name}, \token{sboTerm} and \token{metaid} attributes.  SBML \thisLVNC does not define a purpose for these attributes; moreover, being outside of the \Model namespace, the \token{id} attribute is not subject to the uniqueness constraints of \primtype{SId} values inside \Model objects.\end{minipage}
}
...
object and its use are described in \sec{sec:model}.
\relverdiff{Every SBML document must contain one model definition.  (As a result of extension packages defined in SBML Level 3, it is possible that a model is composed of multiple submodels; however, there must still be one top-level model defining the structure of the overall composition.)}{An SBML document may contain at most one model definition.}
...
\item Every SBML Level~3 package is identified uniquely by an XML
  ...
  nicknamed \relchange{\val{comp}} and another package nicknamed \val{layout}
  (and here, ellipses are used to indicate content elided from
  this example):
  \begin{example}
<sbml xmlns="http://www.sbml.org/sbml/level3/\thisvlc/core" level="3" version="\sbmlversionnum"
      xmlns:\relchange{comp}="http://www.sbml.org/sbml/level3/\thisvlc/\relchange{comp}/\thisvlc"
  ...
  words, the prefix strings \relchange{\val{comp}} and \val{layout} in the
...
\item SBML Level~3 requires that every package defines the
  ...
  \token{required} attribute indicates whether \relchange{constructs in that package may change the mathematical interpretation of constructs defined in SBML Level~3 Core.  A \token{required} value of \val{true} indicates that the package may do so.  The value is set by the relevant Level~3 package specification, and does \emph{not} depend on the presence or absence of particular package constructs in a given SBML document: in other words, it is a property of the package.}
\relverdiff{\relchange{For example, if a package defines new object classes that can be used to change the mathematical meaning of a core element, such as the Hierarchical Model Composition's \token{replacedElement} construct, that package's \token{required} attribute must always be set to \val{true} for all SBML documents declaring the use of that package's namespace, whether or not the documents actually contain any \token{replacedElement} object.}}{For example, if a package defines new object classes with
mathematical meanings and with identifiers that may be used in core \Math
elements, that package's \token{required} attribute must always be set to
\val{true} for all SBML documents declaring the use of that package's
namespace.}  The following is a \relchange{complete} example:
  \begin{example}
<sbml xmlns="http://www.sbml.org/sbml/level3/\thisvlc/core" level="3" version="\sbmlversionnum"
      xmlns:\relchange{comp}="http://www.sbml.org/sbml/level3/\thisvlc/\relchange{comp}/\thisvlc"
      xmlns:layout="http://www.sbml.org/sbml/level3/\thisvlc/layout/\thisvlc"
      \relchange{comp}:required="true" layout:required="false">
  \relchange{<model />}
</sbml>   \end{example}

\relchange{In this example, because the Hierarchical Model Composition (\val{comp}) package can change the mathematical interpretation of a model, the \token{comp:required} attribute must always be set to \val{true}, even if a model is completely empty.  The Graphical Layout (\val{layout}) package, by contrast, cannot ever change the mathematical interpretation of any model; thus, its \token{required} attribute must be set to \val{false}.}

  If a package is declared optional \relchange{(i.e., its \token{required} flag is set to \val{false})}, it means the
  ...
  ``Ignoring'' a package can be accomplished in multiple ways\relchange{:} a
  reader could either skip those elements or attributes altogether
  during parsing, or read them but not interpret them, or do
  something similar.  \relchange{The following are some ways in which packages with \token{required}=\val{true} may change the time-course dynamics of a model:}
\begin{blockRelChange}
\begin{itemize}
  \item An element with a (potential) mathematical meaning may be left undefined in the core specification of SBML Level~3, but may have a value defined by a package.
  \item An element with mathematical meaning may have a value defined by a package such that the value overrides the value given in the core SBML specification.
  \item The definition of how an element changes over time may be altered or augmented by a package.
  \itemvertwo{An element may be defined within a package to have mathematical meaning, and its \primtype{SId} may be used to determine one or more mathematical functions in core SBML.  (However, no package may ever give mathematical meaning to the \primtype{SId} of an element that was defined as not having mathematical meaning: no \primtype{SId} of this type may ever appear in any mathematical context.)}
\end{itemize}
\end{blockRelChange}
\end{enumerate}
...
The definition of \Model is shown in \fig{fig:model}.
...
\begin{figure}[htbp]
  \centering
  \relverdiff{\includegraphics[scale=0.75]{figs/model-uml-l3v1r2}}
...
\fig{fig:model}.  All of the lists are optional, \relverdiff{but if a given list container is present
within the model, the list must not be empty; that is, it must have length one or more}{and, if present, may
be empty; this is semantically equivalent to omitting the list}.  The
...
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more}} <functionDefinition> ... </functionDefinition> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfFunctionDefinitions>\\
\>\><listOfUnitDefinitions>\\
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more}} <unitDefinition> ... </unitDefinition> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfUnitDefinitions>\\
\>\><listOfCompartments>\\
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more}} <compartment> ... </compartment> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfCompartments>\\
\>\><listOfSpecies>\\
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more}} <species> ... </species> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfSpecies>\\
\>\><listOfParameters>\\
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more}} <parameter> ... </parameter> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfParameters>\\
\>\><listOfInitialAssignments>\\
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more}} <initialAssignment> ... </initialAssignment> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfInitialAssignments>\\
\>\><listOfRules>\\
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more elements of subclasses of \abstractclass{Rule}}}  \` \sayOptional\\
\>\></listOfRules>\\
\>\><listOfConstraints>\\
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more}} <constraint> ... </constraint> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfConstraints>\\
\>\><listOfReactions>\\
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more}} <reaction> ... </reaction> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfReactions>\\
\>\><listOfEvents>\\
\>\>\>\textrm{\emph{\relverdiff{one}{zero} or more}} <event> ... </event> \textrm{\emph{elements}}  \` \sayOptional\\
\>\></listOfEvents>\\
\></model>\\
</sbml>
\end{tabbing}
...
defining a compartment, and defining a \relchange{species reference in a reaction} requires defining
...
\sec*{sec:sboterm-type} and \sec*{sec:sboTerm}).  When a
...
indicated in \tab{tab:sboterm-availability}.  The
...
As discussed in \sec{sec:sboTerm}, SBO labels are optional
...
(\sec{sec:unitsidref}).  A list of recommended units is
given in \sec{sec:bp:unitdefinitions:recommendedunits}.
...
\Species (described in \sec{sec:species}), then the
...
\sec*{sec:species-substanceunits} provides more information
...
\sec*{sec:species-units} and \sec*{sec:species-meaning}
...
\sec*{sec:compartment-units} provides more information
...
(\sec{subsec:kinetic-law}) is meant to be interpreted in a
...
\sec{sec:bp:unitdefinitions:recommendedunits}.
...
(\sec{sec:sidref}) and refer to a \Parameter object
...
inherit.  \sec{sec:about-kinetic-laws} describes how to
...
\sec{sec:species} and \sec{sec:reactions}.
...
The various \ListOf classes defined in \fig{fig:model} are
...
derived from the abstract class \SBase (\sec{sec:sbase}),
...
\vertwo{Lists are allowed to be empty for two reasons.  First, this allows model writers to add \Annotation and \Notes objects to a given list even when the list is empty in a model; this can be useful, for instance, to let a modeler explain \emph{why} the components are absent from the model.  Second, it enables SBML Level~3 package specifications to define new elements to be children of these lists, and for these child elements to be used without the requirement that at least one SBML Level~3 Core element always be present.}
...
\FunctionDefinition is shown in \fig{fig:mathdefinition}.

\begin{figure}[htb]
  \centering
  \small
  \vspace*{-0.75ex}
  \relverdiff{\includegraphics[scale=0.78]{figs/functiondefinition-uml-l3v1r2}}
  ...
  \caption{The definition of class \FunctionDefinition.  A
    ...
    \token{semantics} element).  A function definition \relverdiff{must}{may}
    contain \relverdiff{exactly}{no more than} one \token{math} element defined by the
...
\subsubsection{The \token{id} \reltwo{and \token{name}} attribute\reltwo{s}}

\relverdiff{The \token{id} and \token{name} attributes have types \primtype{SId}
and \primtype{string}, respectively, and operate in the manner
described in Section~\ref{sec:idnameattribs}.}
{\FunctionDefinition inherits the \token{id} attribute from \SBase, but defines \token{id} as being \emph{required} rather than optional.  The \token{id} attribute otherwise obeys the behavior described in \sec{sec:idnameattribs}.\vspace*{1ex}
\\
A function's identifier can be used in \token{math} elements as the first target of an \token{apply} element, but the identifier has no value associated with it, and may not be the target of an \InitialAssignment, \EventAssignment, or \Rule object in the model.}
MathML \token{ci} elements in an SBML model can refer to the function defined by a \FunctionDefinition using the value of its \token{id} attribute.

\subsubsection{The \token{math} element}
\label{sec:function-definition-math}

The \vertwo{optional }\token{math} element is a container for MathML content that
...
element can be used.  \relverdiff{The}{If present, the} \token{lambda} element must begin with
zero or more \token{bvar} elements, followed by any other of the
elements in the MathML subset listed in
\sec{sec:mathmlsubset} \emph{except} \token{lambda} (\ie a
...
\relchange{symbols} declared in the \token{lambda} itself.  That
is, the contents of MathML \token{ci} elements inside the body of
the \token{lambda} can only be one of three kinds of
  identifiers: (i) the \relchange{symbols} declared by its \token{bvar}
...
\quantity{time}, \quantity{avogadro}, \reltwo{and} \quantity{delay}\vertwo{, and \quantity{rateOf}}.
Functions must be written so that all model \relchange{symbols} they use are
passed to them via their parameters.

\vertwo{If the \token{math} element is not present in the \FunctionDefinition, the function has no mathematical meaning defined in SBML Level~3 Core.  This situation may arise when models are incomplete, or when additional meaning or subobjects are provided by an SBML Level~3 package.  (However, from the perspective of a model reader that only understands SBML Level~3 Core, the additional meaning will not be recognized.)}
...
class \SBase (see \sec*{sec:sboterm-type}
and \sec{sec:sboTerm}).  When a value is given to this
...
indicated in \tab{tab:sboterm-availability}.  The relationship is
...
definition.  \vertwo{If the \FunctionDefinition has no \token{math} child, SBML Level~3 Core places no restriction on the number of arguments the function may take when used in MathML elsewhere in a model.  Additional restrictions may be placed by SBML Level~3 packages.  However, without additional content or meaning defined by SBML Level~3 packages, a call to a function that has no \token{math} content is mathematically meaningless in a model.}
...
<model ...>
   ...
   ...
   <listOfReactions>
       <reaction id="reaction_1" reversible="true"\reltwo{ fast="false"}>
           ...
           <kineticLaw>
               <math xmlns="http://www.w3.org/1998/Math/MathML">
                   <apply> 
...
\fig{fig:unitdefinition} and explained in more detail
below.
...
model.  We revisit this topic in \sec{sec:using-units},
and illustrate it again with the example given in
\sec{sec:eg:conversionfactor}.
...
\sec{sec:bp:unitdefinitions:offset} discusses approaches
...
defined in \fig{fig:unitdefinition}.  Once a unit is
...
\begin{figure}[htb]
  ...
  \vspace*{-0.5ex}
  \fi
  \relverdiff{\includegraphics[scale=0.78]{figs/unitdefinition-uml-l3v1r2}}
...
\relverdiff{\vspace*{-4ex}\paragraph{The \token{id} and \token{name} attributes}}{\vspace*{-4ex}\paragraph{The \token{id} attribute}}

\relverdiff{The required attribute \token{id} and optional attribute
...
of names.}
{\UnitDefinition inherits the \token{id} attribute from \SBase, but defines \token{id} as being required rather than optional, and in addition, refines the data type of \token{id} to be \primtype{UnitSId} instead of \primtype{SId}.  This is done to provide each unit definition with a unique identifier by which other components of a model may refer to it.  (See \sec{sec:idnameattribs} for more details.)}
...
must \emph{not} be equal to one of the reserved base unit
  names listed in \tab{tab:unitkind}, the list of reserved
...
A \UnitDefinition object may contain a \ListOfUnits container which \relverdiff{must contain one or more}{may
contain zero or more} \Unit objects. \sec{sec:unit-structure} explains
the meaning and use of \Unit.   

\vertwo{In SBML \thisLV, the list of units in a \UnitDefinition object may be empty.  This is permitted for the same reasons that other lists are permitted to be empty.  (See the discussion in \sec{sec:listof}.)  An empty unit definition is synonymous with an undefined unit.  For example, suppose a given component in a model is defined to have a unit of measurement \val{u}.  If the \UnitDefinition object with identifier \val{u} has an empty \ListOfUnits subobject, then this is identical to leaving the unit undefined on the component.}
...
base unit chosen from the list in \tab{tab:unitkind}.  The
...
\tab{tab:unitkind}.  These reserved symbols are defined in
...
\begin{table}[bht]
  ...
  \caption{Base units defined in SBML.  These symbols are
    ...
    (\sec{sec:unitsid}).  All are names of base or derived
...
The presence of \token{avogadro} in \tab{tab:unitkind}
...
\sec{sec:csymbol}.  The numerical value is taken to be the
...
\tab{tab:unitkind}.  The formula underlying this definition
is the following:
\begin{linenomath}
\begin{equation}
...
\tab{tab:unitkind}; in a given \Unit object, it is
...
\tab{tab:unitkind} and specifies how it should be
...
represented by the \relchange{symbol} $m_i$.  Similarly, let the value of
...
of an abbreviation \val{mmls} for the unit $\mathrm{millimoles}\ \mathrm{l}^{-1}\
\mathrm{s}^{-1}$:
...
\sec{sec:bp:unitdefinitions:offset} provides suggestions
...
of \Compartment is shown in \fig{fig:compartment}.
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \node (b) {
      \begin{blockVerChange}
        \begin{classbox}{\textcolor{black}{Compartment}}
          \textcolor{black}{id: SId \relverdiffNC{}{\{ use="required" \}}}  \\
          \reltwo{\textcolor{black}{name: string \{ use="optional" \}}      \\}
          \textcolor{black}{spatialDimensions: double \{ use="optional" \}} \\
          \textcolor{black}{size: double \{ use="optional" \}}              \\
          \textcolor{black}{units: UnitSIdRef \{ use="optional" \}}         \\
          \textcolor{black}{constant: boolean}                              \\
        \end{classbox}
      \end{blockVerChange}
    };
  ...
  \caption{The definition of class \Compartment.  A
      sequence of \relverdiff{one}{zero} or more instances of \Compartment objects
      can be located in an instance of \ListOfCompartments in
      \Model, as shown in \protect\fig{fig:model}.}
...
\subsubsection{\relverdiff{The \token{id} and \token{name} attributes}{The \token{id} attribute}}

\relverdiff{\Compartment has one required attribute, \token{id}, of type
...
{\Compartment inherits the \token{id} attribute from \SBase; however, \Compartment defines \token{id} as being required rather than optional.  The attribute otherwise behaves as described in \sec{sec:idnameattribs}.}

\vertwo{The identifier (the \token{id} attribute value) of a \Compartment object may be used in a model's mathematical expressions.  The identifier stands for the value of the compartment's \token{size} attribute, with a unit of measurement as defined in \sec*{sec:compartment-units}.  A compartment's identifier may be the target of an \InitialAssignment, \EventAssignment, or \Rule in the model, to set or redefine the value of the compartment size.}
...
detail below in \sec{sec:compartment-units}.
...
\ref{sec:before-t0} provides additional information about
...
assignment (\sec{sec:initialAssignment}) or other SBML
...
for this are explained in \sec{sec:bp:size}.
...
\tab{tab:comp-size-units}.  If the \Model object does not
...
\item The \token{math} elements of \AssignmentRule,
  ...
  and \sec{sec:initialAssignment}).

\item In a \RateRule object that defines a rate of change for a
  compartment's size (\sec{sec:raterule}), the unit of the
...
A \Compartment also has a mandatory \primtype{boolean} attribute called
...
construct except \InitialAssignment.  \sec{sec:size}
...
\begin{figure}[htb]
    ...
    \node (b) {
      \begin{blockVerChange}
        \begin{classbox}{\textcolor{black}{Species}}
          \textcolor{black}{id: SId \relverdiffNC{}{\{ use="required" \}}}     \\
          \reltwo{\textcolor{black}{name: string \{ use="optional" \}}         \\}
          \textcolor{black}{compartment: SIdRef}                               \\
          \textcolor{black}{initialAmount: double \{ use="optional" \}}        \\
          \textcolor{black}{initialConcentration: double \{ use="optional" \}} \\
          \textcolor{black}{substanceUnits: UnitSIdRef \{ use="optional" \}}   \\
          \textcolor{black}{hasOnlySubstanceUnits: boolean}                    \\
          \textcolor{black}{boundaryCondition: boolean}                        \\
          \textcolor{black}{constant: boolean}                                 \\
          \textcolor{black}{conversionFactor: SIdRef \{ use="optional" \}}     \\
        \end{classbox}
      \end{blockVerChange}
  };
    \draw[open triangle 60-] (a) -- (b);
  \end{tikzpicture}
  \vspace*{-1ex}
  \caption{The definition of class \Species.  \relverdiff{One}{Zero} or more
...
\relverdiff{As with other major objects in SBML, \Species has a mandatory
...
{\Species inherits the \token{id} attribute from \SBase; however, \Species defines \token{id} as being required rather than optional.  The attribute otherwise behaves as described in \sec{sec:idnameattribs}.}

\vertwo{The identifier (the \token{id} attribute value) of a \Species object may be used in a model's mathematical expressions.  The identifier stands for the given species' amount or concentration, depending on the value of its \token{hasOnlySubstanceUnits} attribute as described below, with units of measurement as described in \sec{sec:species-substanceunits}.  A species' identifier may be the target of \InitialAssignment, \EventAssignment, or \Rule objects in a model, to set or redefine the value of the species amount or concentration.}
...
\primtype{SBOTerm} from its parent class \SBase (see
\sec*{sec:sboterm-type} and \sec*{sec:sboTerm}).  Values
for this attribute should be SBO identifiers taken the branch for
type \Species indicated in \tab{tab:sboterm-availability}.
...
(\sec{sec:initialAssignment}) or other SBML construct
...
constructs are mutually exclusive; see \sec{sec:rules}
and \sec{sec:events}.)  It is not an error to define
...
that case is ignored.  \ref{sec:before-t0} provides
...
type \primtype{UnitSIdRef} (\sec{sec:unitsidref}).  When
...
set (\sec{sec:species-substanceunits}), SBML allows
...
by either (i) determining whether the
...
\item In a \RateRule object that defines the rate of change of the
  ...
  quantity (\sec{sec:species-units}) divided by the
  model-wide unit of \quantity{time}
  (\sec*{sec:timeunits}); in other words, \{unit of
...
The \Species object has two other mandatory \primtype{boolean} attributes
named \token{constant} and \token{boundaryCondition}, used to
indicate whether and how the \relchange{amount} of that species can vary
during a simulation.  \tab{tab:specieattrib} shows how to
...
\begin{table}[ht]
  ...
  \begin{edtable}{tabular}{lllll}
    ...
    \textbf{value}            & \textbf{value}                     & \textbf{or rate rule?} & \textbf{product?}   & \textbf{the species' \relchange{amount}?} \\
    \midrule
...
reactions, its \relchange{amount} is determined by those reactions.  In
SBML, it is possible to indicate that a given species' \relchange{amount} is
...
\emph{boundary} of the reaction system, and its \relchange{amount} is not
determined by the reactions.  The \primtype{boolean} attribute
...
\relchange{amount} can be changed at all, regardless of whether by
reactions, rules, or constructs other than \InitialAssignment.  A
value of \val{false} indicates that the species' \relchange{amount} can be
...
\relchange{amount} may still be changed by \AssignmentRule, \RateRule,
...
attributes are \val{true}, then its \relchange{amount} cannot be changed by
...
The example model in \sec{sec:constantspecieseg} contains
all four possible combinations of the \token{boundaryCondition}
and \token{constant} attributes on \token{species} elements.
\sec{sec:odeeg} gives an example of how one can translate
into ODEs a model \relchange{with species of mixed
\token{boundaryCondition} attribute values}.

\relchange{Finally, it is worth clarifying that while the \token{constant} and \token{boundaryCondition} attributes restrict whether and how the species amount changes, the same is not true of a species' concentration.  In SBML, the concentration of a species is a quantity that depends on the size of the compartment in which it is located.  A compartment's size may change, and therefore, so can the concentration of a species even if the \emph{amount} of the species remains unchanged.  A species' concentration may vary even if the \Species object's \token{constant}=\val{true} in a model.}
...
\sec{sec:constructing-equations}.  Because the value of
...
size simultaneously.  \sec{sec:eventassignment} describes
...
\Parameter is shown in \fig{fig:parameter}.
...
\begin{figure}[htb]
    ...
    \node (b) {
      \begin{blockVerChange}
        \begin{classbox}{\textcolor{black}{Parameter}}
          \textcolor{black}{id: SId \relverdiffNC{}{\{ use="required" \}}}  \\
          \reltwo{\textcolor{black}{name: string \{ use="optional" \}}      \\}
          \textcolor{black}{value: double \{ use="optional" \}}             \\
          \textcolor{black}{units: UnitSIdRef \{ use="optional" \}}         \\
          \textcolor{black}{constant: boolean}                              \\
        \end{classbox}
      \end{blockVerChange}
  ...
  \caption{The definition of class \Parameter.  A
      sequence of \relverdiff{one}{zero} or more instances of \Parameter objects can
      be located in an instance of \ListOfParameters in \Model, as
      shown in \protect\fig{fig:model}.}
  \label{fig:parameter}
\end{figure}
...
the \primtype{boolean} flag \token{constant} to indicate which flavor the
...
\relverdiff{\Parameter has one required attribute, \token{id}, of type
...
\primtype{string}.  Identifiers and names must be used according
to the guidelines described in Section~\ref{sec:idnameattribs}.}
{\Parameter inherits the \token{id} attribute from \SBase, but on \Parameter it is defined as being required instead of optional.  The attribute otherwise behaves as described in \sec{sec:idnameattribs}.}

\vertwo{A parameter's identifier (its \token{id} attribute value) may be used in a model's mathematical expressions.  The identifier stands for the value of the parameter, with a unit of measurement as described in \sec{sec:parameter-units}.  It may be the target of \InitialAssignment, \EventAssignment, or \Rule objects elsewhere in a model, to set or redefine the value of the parameter.}

\vertwo{A \Parameter \token{id} will most often represent a \primtype{double} value, but the identifier may be used in other contexts. For example, it is possible to have a \Parameter that is only assigned Boolean values and only used in Boolean contexts.  The units of such a \Parameter should be \token{dimensionless}.  In such cases, it would also be appropriate to set the \Parameter's \token{sboTerm} attribute to the value for ``logical parameter'' (\val{SBO:0000602}).}
...
initial assignment (\sec{sec:initialAssignment}) or 
...
(\sec*{sec:parameter-constant}) has the value \val{true},
...
exclusive; see \sec*{sec:rules} and \sec*{sec:events}.)  It
...
(\sec{sec:unitsidref}).  There are no constraints on the
...
\item In a \RateRule object that defines the rate of change of the
  parameter's value (\sec{sec:raterule}), the unit
...
The \Parameter object has a mandatory \primtype{boolean} attribute named
...
\token{value} is presented in \sec{sec:parameter-value}.
...
(\sec*{sec:parameters}).  However, this approach is not
...
\InitialAssignment is shown in \fig{fig:initialAssignment}.
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \draw[diamond-,shorten <=-4pt,shorten >=-6pt] (b) -- (c)
        node[above=6pt,left=1.85in] {\textsf{math}};
    \draw node[right=1.25in,above=0in] {\vertwo{\textsf{0..1}}};
  \end{tikzpicture}
  \vspace*{0.5ex}
  \caption{The definition of class \InitialAssignment.  The
    contents of the \Math class can be any \mathml
    permitted in SBML; see
    \protect\sec{sec:mathmlsubset}.  A
    sequence of \relverdiff{one}{zero} or more instances of \InitialAssignment
...
(\sec{sec:assignmentrule}).  The main differences are (a)
...
\subsubsectionVerOnly{The \token{id} attribute}
\label{sec:initialassignment-id}

\vertwo{\InitialAssignment inherits an optional \token{id} attribute of type \primtype{SId} from \SBase.  The identifier of an \InitialAssignment has no mathematical meaning in an SBML \thisLV model.}


\subsubsectionVerChange{The \token{symbol} attribute}

\InitialAssignment contains the mandatory attribute \token{symbol}, of type \primtype{SIdRef}.  The purpose of \InitialAssignment is to define the initial value of the constant or variable referred to by the \token{symbol} attribute.  (The attribute's name is \token{symbol} rather than \token{variable} because it may assign values to constants as well as variables in a model; see \sec{sec:initial-assignment-semantics}.)

\relchange{The value of the \token{symbol} attribute must be the identifier of an object in the \primtype{SId} namespace of the model; moreover, the object must be of a class that is defined to have mathematical meaning in SBML.  \relverdiff{The}{In SBML Level~3 Core, the} types of objects whose identifiers are permitted as the values of \InitialAssignment \token{symbol} attributes are \Compartment, \Species, \SpeciesReference and (global) \Parameter objects in the model.}  \vertwo{In addition, classes of objects defined by SBML Level~3 packages to have mathematical meaning may also be defined by those packages to be permissible targets of \InitialAssignment objects; in other words, \InitialAssignment \token{symbol} attributes may also reference identifiers in the \primtype{SId} namespace defined by SBML Level~3 packages.}

An initial assignment cannot be made to reaction identifiers; that is, the \token{symbol} attribute value of an \InitialAssignment cannot be an identifier that is the \token{id} attribute value of a \Reaction object in the model.  This is identical to a restriction placed on rules (see \sec{sec:ruleconstraints}).  \relchange{It may also not reference the \token{id} of a \FunctionDefinition.}

\vertwo{If the \token{symbol} attribute of an \InitialAssignment object references an object in an SBML namespace that is not recognized by the interpreter reading a given SBML document (that is, if the object is defined by an SBML Level~3 package that the software does not support), the assignment must be ignored---the symbol must not be assigned by the construct if the interpreter cannot understand the package.  If an interpreter cannot establish whether a referenced object is missing from the model or instead is defined in an SBML namespace not recognized by the interpreter, it may produce a warning to the user.  (The latter situation can only arise if an SBML package is present in the SBML document with a \token{package:required} attribute of \val{true}.)}


\subsubsectionVerChange{The \token{math} element}
...
\vertwo{An \InitialAssignment with no \token{math} child leaves undefined what assignment is to be made to the corresponding \token{symbol}. The absence of a \token{math} element is permitted because it is possible for SBML Level~3 packages to add constructs that extend \InitialAssignment and define how a value is to be computed.  In the absence of such constructs in a given model, no assignments or other changes to the model are carried out when there is no \token{math} element.  This leaves the model unchanged: any SBML component that had a value will continue to have that value; any component whose value was undefined will continue to have an undefined value.  A simulator encountering this situation may choose to produce a warning.  No other validation rules are affected by the absence of a \token{math} child: it is still invalid to have an \InitialAssignment and an \AssignmentRule that assign to the same model element, for example.}


\subsubsectionVerChange{The \token{sboTerm} attribute}
...
\subsubsectionVerChange{Semantics of initial assignments}
...
\InitialAssignment.  \relchange{For SBML Level~3 Core,} initial assignments can take place for
...
\item \emph{In the case of a species}, an \InitialAssignment sets
  ...
  \sec{sec:species-units} for an explanation of how a
...
\item \emph{In the case of a compartment}, an \InitialAssignment
  ...
  \sec{sec:compartment-units} for more information about
  compartment units.)
...
\item \emph{In the case of a parameter}, an \InitialAssignment
  ...
  \sec{sec:parameter-units} for more information about
  parameter units.)

\itemvertwo{\emph{In the case of an object from an SBML Level~3 package}, an \InitialAssignment sets
...
\sec{sec:before-t0}: if an \InitialAssignment object
...
(\sec{sec:reactions}) nor by the assignment of an initial
...
result, $\textbf{V}$ is a vector of \relchange{symbols} that does not
include $x$, and $\textbf{W}$ is a vector of \relchange{symbols} that may
include $x$):
\vertwo{\vspace*{1ex}}
\begin{center}
...
\fig{fig:rules} gives the definitions of \Rule and the
...
\begin{figure}[htb]
  \centering
  \relverdiff{\includegraphics[scale=0.79]{figs/rule-uml-l3v1r2}}
             {\includegraphics[scale=0.79]{figs/rule-uml}}%
  \vspace*{-1ex}
...
The classes derived from \Rule inherit \token{math} and the attributes and elements \relchange{that \Rule itself inherits} from \SBase, including \vertwo{\token{id}, \token{name}, and }\token{sboTerm}.


\paragraphVerOnly{The \token{id} attribute}
\label{sec:rule-id}

\vertwo{\Rule inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  The identifier of a \Rule-derived object has no mathematical meaning in an \thisLV model.}


\paragraph{The \token{math} element}

A \Rule object has an \vertwo{optional }element called \token{math},
...
described in more detail in \sec*{sec:algebraicrule},
\sec*{sec:assignmentrule} and \sec*{sec:raterule}.

\vertwo{A \Rule with no \token{math} child element leaves undefined how the rule behaves mathematically. The \Rule \token{math} element is defined as optional in SBML Level~3 Core because it is possible for SBML Level~3 packages to add constructs that extend \Rule and define the missing behavior. In the absence of such constructs in a given model, no assignments or other changes to the model are carried out when there is no \token{math} element.  This leaves the model unchanged: any SBML component that had a value will continue to have that value; any component whose value was undefined will continue to have an undefined value.  A simulator encountering this situation may choose to produce a warning.  No other validation rules are affected by the absence of a \Rule \token{math} element: it is still invalid to have an \InitialAssignment and an \AssignmentRule that assign to the same model element, for example.}
...
\RateRule indicated in \tab{tab:sboterm-availability}.  The relationship is
...
\token{boundaryCondition}=\val{false} or else not be involved in
any reaction at all.  These restrictions are explained in more
detail in \sec*{sec:ruleconstraints} below.
...
\vertwo{Finally, any symbol that appears as the target of a \emph{rateOf} \token{csymbol} may not be determined by an \AlgebraicRule.  This is because the \emph{rateOf} \token{csymbol} is defined as applying only to symbols whose rates of change are easily determinable.}

\relchange{Although the rules above directly stipulate the symbols that may \emph{not} be determined by the \AlgebraicRule construct, note that they can also be used to discover the symbol that \emph{is} determined by a given \AlgebraicRule object.  For instance, if three symbols appear in the \token{math} portion of \AlgebraicRule, and the first has \token{constant}=\val{true} and the second symbol is a \Reaction identifier, one may deduce that the \AlgebraicRule is being used to determine the value of the third symbol that appears in the mathematical expression.  This is, in fact, one of the principal uses (in addition to model validation) of the \token{constant} attribute: it allows software to identify the dependent variables in a model's \AlgebraicRule objects.}
...
set the values of variables.  The left-hand side (the \relchange{required}
\token{variable} attribute) of an assignment rule \relchange{is of type \primtype{SIdRef}, and must refer to an SBML object in the \primtype{SId} namespace with mathematical meaning and the ability to be assigned}\relverdiff{\changed{:}}{.  In SBML Level~3 Core, this consists of}
\Species, \SpeciesReference, \Compartment, 
\relchange{and} global \Parameter objects in
the model (but not reactions \relchange{nor function definitions}).  The entity identified must
%NB:  changed to make this text match that for RateRule:
\relchange{have its \token{constant} attribute set to the value \val{false}}.  The effects of
...
\item \emph{In the case of a parameter}, an \AssignmentRule sets
  ...
  attribute value.  (See \sec*{sec:parameter-units} for
  more information about parameter units.)

\itemvertwo{\emph{In the case of an object from an SBML Level~3 package}, an \AssignmentRule sets
...
\sec{sec:initial-assignment-semantics}.)
...
(the \relchange{required} \token{variable} attribute) of a rate rule  \relchange{has type \primtype{SIdRef}, and must refer to an SBML object in the \primtype{SId} namespace with mathematical meaning and the ability to be assigned}\relverdiff{:}{.  In Level~3 Core, this consists of}
\Species, \SpeciesReference, \Compartment, 
\relchange{and} global \Parameter objects in
the model (but not reactions \relchange{nor function definitions}).  The
...
\item \emph{In the case of a species}, a \RateRule sets the rate
  ...
  (\sec{sec:species-units}) divided by the model-wide unit
  of \quantity{time} (\sec{sec:timeunits}), or in other
...
\itemvertwo{\emph{In the case of an object from an SBML Level~3 package}, a \RateRule sets the rate of change of the referenced object's value (as defined by that package) to the value of the formula in \token{math}.    The unit of measurement associated with the value produced by
  the formula should be the same as that object's \token{units}
  attribute value (if it has such an attribute) divided by the model-wide unit
  of \quantity{time}, or be equal to
  the units of model components of that type (if objects of that class
...
As mentioned in \sec*{sec:assignmentrule} for
...
is discussed in more detail in \sec{sec:bp:rules}.
...
different quantities in a model.  \fig{fig:constraint}
shows the definition of the \Constraint object class.

\begin{figure}[htb]
  \relchange{\vspace*{2ex}}%
  \centering
  \small
  \relverdiff{\includegraphics[scale=0.8]{figs/constraint-uml-l3v1r2}}
             {\includegraphics[scale=0.85]{figs/constraint-uml}}
  \relchange{\vspace*{-1.5ex}}
  \caption{The definition of class \Constraint.  The contents of
    the \Math class can be any \mathml permitted in SBML,
    \relverdiff{but it must return}{and will be interpreted as} a Boolean value.  As shown above, an
    ...
    explained in \sec{sec:notes} for notes on \SBaseUpright also
    apply to the XHTML within messages in a \Constraint. A
    sequence of \relverdiff{one}{zero} or more instances of \Constraint objects can
    be located in an instance of \ListOfConstraints in \Model, as
    shown in \protect\fig{fig:model}.}
  \relchange{\vspace*{1.25ex}}
...
\vertwo{\Constraint inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  The identifier of a \Constraint object has no mathematical meaning in an SBML \thisLV model.}
...
\relchange{\vspace*{-0.5ex}}
\subsubsection{The \token{math} element}

\Constraint has one \vertwo{optional }subelement, \token{math},
containing a MathML formula defining the condition of the
constraint.  This formula must return a Boolean value of
\val{true} \vertwo{(or any non-zero numerical value) }when the model is in a \emph{valid} state.  The formula
...
\sec{sec:notes}.  In particular, the element must declare
the use of the XHTML XML namespace, and must not contain an XML
declaration \relchange{nor} a DOCTYPE declaration.
...
subobject (\sec{sec:constraint-message}) may optionally be
...
\begin{figure}[b]
  \vspace*{-2.5ex}
  \centering
  \relverdiff{\includegraphics[scale=0.74]{figs/reaction-uml-l3v1r2}}
...
\Reaction object.  As shown in \fig{fig:reaction}, it
contains several scalar attributes and several lists of other
objects.


\paragraph{\relverdiff{The \token{id} and \token{name} attributes}{The \token{id} attribute}}

\relverdiff{As with most other main kinds of objects in SBML, the
...
attributes must be used as described in
Section~\ref{sec:idnameattribs}.}
{\Reaction inherits the \token{id} attribute from \SBase; however, \Reaction defines \token{id} as being required rather than optional.  The attribute otherwise behaves as described in \sec{sec:idnameattribs}.}

\vertwo{The identifier (the \token{id} attribute value) of a \Reaction object may be used in mathematical expressions in a model.  The identifier stands for the reactions rate; this role and the units of measurement associated with the reaction identifier are explained in more detail in \sec{subsec:reaction-as-symbol}.  A reaction's identifier cannot be the target of an \InitialAssignment, \EventAssignment, or \Rule object, nor may its value be determined by an \AlgebraicRule object in a model.}
...
\sec*{subsec:speciesreference}
and \sec*{subsec:modifierreference} below.  Throughout this text,
...
\item The ability of a species to appear as a reactant or product
  ...
  \sec{sec:species-constant} for more information.
...
techniques, including simulation.  \relchange{In the absence of any additional definition, some simulators choose to give an error and refuse to simulate models that have a \Reaction with no \KineticLaw.  Others assume that the effective rate of a \Reaction with no \KineticLaw is zero.  Still others define this value to be ``not a number'' (\token{NaN}).  This behavior is not standardized, and should not be relied on when exchanging models for simulation.}


\paragraph{The \token{reversible} attribute}
\label{sec:reversible}

The mandatory \primtype{boolean} attribute \token{reversible} on \Reaction
...
\paragraph{The \token{fast} attribute \vertwo{(removed)}}
\label{sec:fast}

\relverdiff{The \primtype{boolean} attribute \token{fast} is another required \primtype{boolean}
...
be set to \val{false} for all reactions.  \changed{Note that in SBML Level~3 Version~2, the \token{fast} flag is removed, and all reactions are equivalent to \thisLV reactions with a \token{fast} value of \val{false}.}}
{In SBML Level~3 Version~2, the \token{fast} attribute has been removed: every \Reaction in a \thisLV model is equivalent to an SBML Level~3 Version~1 \Reaction with a \token{fast} value of \val{false}.  This means that for \thisLV, the speed of every \Reaction will always be determined by its \KineticLaw.  To achieve the same or similar effects as setting the \token{fast} attribute to \val{true} in a previous version of SBML, the \KineticLaw should be constructed to produce a value in the desired time scale, or the reaction can be replaced with an \AssignmentRule or \AlgebraicRule object as in the example of \sec{apdx:rules-eg}.}

\reltwo{The model's reactions are divided into two sets by the
...
\reltwo{The correctness of the approximation requires a significant
...
\reltwo{Note that the \token{fast} attribute has a significant effect on
...
\sec{sec:model}).  The lists of products, reactants and
...
\SimpleSpeciesReference, defined in \fig{fig:reaction}, is
...
\relchange{inherited by \SpeciesReference and
...
(see \sec*{sec:sboterm-type} and \sec*{sec:sboTerm}).  When
...
\tab{tab:sboterm-availability}.  The relationship is of the
...
\sec{subsec:simplespeciesreference}, \SpeciesReference
...
\paragraph{\relchange{The \token{id} attribute}}

\relchange{The \token{id} attribute \vertwo{that \SpeciesReference inherits from \SBase\xspace}is \vertwo{still }optional, and behaves as described in \sec{sec:idnameattribs}.  It also acquires the mathematical meaning of the value of its \token{stoichiometry} attribute, and may be the target of an \InitialAssignment, \EventAssignment, or \Rule object defined in the enclosing model.}
...
optional \token{stoichiometry} on \SpeciesReference object.  \relchange{The
...
by an initial assignment (\sec{sec:initialAssignment}) or
other SBML construct elsewhere in the model.
...
\sec{sec:before-t0} provides additional information about
the semantics of assignments, rules and values for simulation time
$t \leq 0$.  \relchange{Real-world examples of variable stoichiometries in models include genome-scale models \citep{obrien:2013}, and can generally be found in searches such as \url{https://scholar.google.com/scholar?q=model+variable+stoichiometry}.}

An explanation of how exactly the stoichiometry is used in the
mathematical interpretation of the model is given in
\sec{sec:about-kinetic-laws}.


\paragraph{The \token{constant} attribute}

The \SpeciesReference attribute \token{constant} is a
mandatory \primtype{boolean} attribute used to indicate whether the
...
<model ...>
    ...
    <listOfReactions>
        <reaction id="J1" reversible="false"\reltwo{ fast="false"}>
            <listOfReactants>
...
<model ...>
    ...
    <listOfInitialAssignments>
        ...
        ...
    <listOfReactions>
        <reaction id="J1" reversible="true"\reltwo{ fast="false"}>
            <listOfReactants>
...
\begin{example}
<reaction id="x" reversible="false"\reltwo{ fast="false"}>
    <listOfReactants>
    ...
    <listOfProducts>
        ...
        <speciesReference species="b" stoichiometry="1" constant="true"/>
    </\relchange{listOfProducts}>
</reaction>
<reaction id="y" reversible="false"\reltwo{ fast="false"}>
    <listOfReactants>
...
\sec{sec:sbo}).  The \Reaction object class provides a way
...
\relchange{Because its sibling class \SpeciesReference has mathematical meaning, it is probably worth noting that} no meaning is
...
\fig{fig:reaction}, \KineticLaw has elements called
...
\vertwo{\KineticLaw inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of a \KineticLaw takes on no mathematical meaning; the value of that \token{math} element is instead associated with the enclosing \Reaction object's identifier.}


\paragraph{The \token{math} element}

As shown in \fig{fig:reaction}, \KineticLaw has an element
...
\sec*{subsec:simplespeciesreference},
\sec*{subsec:speciesreference} and \sec*{subsec:modifierreference}).

\sec*{sec:about-kinetic-laws} provides important
discussions about the meaning and interpretation of SBML ``kinetic
laws''.

\vertwo{A \KineticLaw with no \token{math} child leaves undefined the mathematics of the corresponding \Reaction.  The absence of a \token{math} element is permitted because it is possible for SBML Level~3 packages to add constructs that extend \KineticLaw and define other ways for a value to be computed. In the absence of such package constructs, the model behaves mathematically as if the \KineticLaw is not defined.  This can be sometime useful, for example, when a model needs to define one or more \LocalParameter objects (which are placed within \KineticLaw objects) but has no mathematical formula for the kinetics of the reaction involved.}


\paragraph{The list of local parameters}

An instance of \KineticLaw can contain a list of \relverdiff{one}{zero} or more
\LocalParameter objects (\sec{subsec:localparameter})
...
As discussed in \sec{sec:identifiers}, reactions introduce
...
\emph{local parameter} that are only accessible \relverdiff{by the kinetic law formula of}{within the scope of}
...
\relverdiff{\LocalParameter has a required attribute \token{id}, of data type
...
{\LocalParameter inherits the \token{id} attribute from \SBase; however, \LocalParameter defines \token{id} as being required rather than optional, and in addition, its type is defined to be the derived type of \primtype{LocalSId} instead of \primtype{SId}.  It otherwise behaves as described in \sec{sec:idnameattribs}.}

\vertwo{The identifier (the \token{id} attribute value) of a \LocalParameter may be used in the mathematical expression within the enclosing \KineticLaw object.  The identifier stands for the value of the parameter's \token{value} attribute, and within the scope of a \KineticLaw object and within the scope of the \Reaction itself, a local parameter's identifier shadows any identical identifiers from the \primtype{SId} namespace of the model.  Because of its limited local scope, the identifier also cannot be the target of an \InitialAssignment, \EventAssignment, or \Rule object in the model.  The units of measurement associated with the identifier are described in \sec{subsec:localparameter-units}.}

\vertwo{A \LocalParameter must not have the same \token{id} value as a \Species referenced by any of the parent \Reaction's \SpeciesReference objects.  (If this were permitted, the local scoping would mean that the \SpeciesReference would refer to a \LocalParameter, which is not a \Species---which would create an inconsistency.)}

\vertwo{The effective scope of where a \LocalParameter \token{id} can be used in \sbmlthree Core is restricted to the \token{math} of the \KineticLaw, since no other construct within a \Reaction can reference an \primtype{SId}.  However, \sbmlthree packages may introduce new ways of referencing \primtype{SId} values within a \Reaction that could also refer to \LocalParameter objects.  Packages could therefore introduce cases where a \KineticLaw can exist without a \token{math} element but with a new package construct that (for example) uses a \ListOfLocalParameters to provide the package construct with locally-scoped parameters to reference.}
...
\LocalParameter indicated in \tab{tab:sboterm-availability}.
...
\sec{sec:species}), and so the rate expression $k \cdot
...
\tab{tab:rate-law-problems} along with the problems they
entail.
...
\sec{sec:species-constant}, these are the only species
...
\item \emph{Global conversion factor defined}.  If the \Model
  ...
  \sec{sec:model-conversionFactor}).  The formula for the
...
\item \emph{Conversion factor defined for the species}.  If the
  ...
  attribute value (see \sec{sec:species-conversion}).  The
...
\token{variable} \relchange{or \token{symbol}} attributes of \InitialAssignment, \RateRule,
...
\sec{sec:sbase}) and are defined in
\fig{fig:event}.  An example of a model which uses events
is given in \sec{sec:examples}.

\begin{figure}[htb]
  \centering
  \small
  \vspace*{-1ex}
  \relverdiff{\vspace*{-2ex}\includegraphics[scale=0.76]{figs/event-uml-l3v1r2}}
...
\relchange{In addition to the attributes and children it inherits from \SBase,}
an \Event definition has one required attribute, \token{useValuesFromTriggerTime}, \reltwo{the optional attributes \token{id} and \token{name}, }\relchange{and five optional subobjects, \Trigger, \Delay, \Priority, \ListOfEventAssignments, and \EventAssignment.}
These various features of \Event are described below.


\relchange{\vspace*{-0.25ex}}
\paragraph{\relverdiff{The \token{id} and \token{name} attributes}{The \token{id} attribute}}
\label{sec:event-id-name}

\relverdiff{As with most components in SBML, an \Event has \token{id} and
\token{name} attributes, but in the case of \Event, both are optional.
These attributes operate in the manner described in
Section~\ref{sec:idnameattribs}.}
{\Event inherits an optional \token{id} attribute of type \primtype{SId} from \SBase.  The identifier of an \Event has no mathematical meaning in an SBML \thisLV model.}
...
\sec{sec:sboterm-type} and \sec{sec:sboTerm}).  When this
attribute is present on a given \Event object instance, its value
should be an SBO identifier belonging to the branch for type
\Event indicated in \tab{tab:sboterm-availability}.  The
...
\relchange{\vspace*{-1.75ex}}
\subsubsection{\class{Trigger}}
\label{sec:trigger}
\label{sec:event-trigger}

As shown in \fig{fig:event}, an \Event object \relverdiff{must}{may} contain
exactly one object of class \Trigger.  This object in turn must
contain two attributes, \token{persistent} and
\token{initialValue}, and \vertwo{may contain }a MathML \token{math} element.
The expression in the \token{math} element \relverdiff{is}{will be interpreted as} a
...
event will trigger again at any subsequent moments when the
...
determined in part by the \primtype{boolean} flag
\token{initialValue}, discussed below.

\vertwo{If an instance of an \Event contains no \Trigger child, that \Event has no way of being triggered or executed in a model according to the SBML \thisLV specification.  The \Event may nevertheless be present in a model as part of (e.g.)\ a partially unknown system, for annotation purposes, or as part of constructs defined by an SBML Level~3 package.}

\vertwo{A \Trigger with no \token{math} child leaves undefined when the corresponding \Event will trigger.  The absence of a \token{math} element is permitted because it is possible for SBML Level~3 packages to add constructs that extend \Trigger and define how a value is to be computed.  In the absence of such constructs, the event is never triggered.  A simulator encountering this situation may choose to produce a warning.}


\paragraphVerOnly{The \token{id} attribute on \class{Trigger}}
\label{sec:trigger-id}

\vertwo{\Trigger inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  The identifier has no mathematical meaning in an SBML \thisLV model.}
...
If the \primtype{boolean} attribute \token{persistent} has a value of
...
\sec{sec:event-priority}), SBML does not mandate a
particular ordering of event execution in the case of simultaneous
events (see \sec{sec:event-semantics}).  Models with
...
\primtype{boolean} attribute \token{initialValue}.  A value of \val{true}
...
\primtype{SBOTerm} from its parent class \SBase (see
\sec*{sec:sboterm-type} and \sec*{sec:sboTerm}).  The value
...
As shown in \fig{fig:event}, an \Event object can contain
...
\vertwo{A \Priority with no \token{math} child leaves the priority of the event undefined.  The absence of a \token{math} element is permitted because it is possible for SBML Level~3 packages to add constructs that extend \Priority and define how a value is to be computed. In the absence of such constructs, the event behaves as if it does not have a priority.  A simulator encountering this situation may choose to produce a warning.}
...
satisfied.  See \sec{sec:trigger-persistent} for more
...
\sec{sec:event-semantics} includes additional discussion
of these topics.
...
execution next.  \sec{sec:bp:events} provides further
...
\paragraphVerOnly{The \token{id} attribute on \class{Priority}}
\label{sec:priority-id}

\vertwo{\Priority inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  The identifier has no mathematical meaning in an SBML \thisLV model.}
...
should be an SBO identifier belonging to the branch for type
\Priority indicated in \tab{tab:sboterm-availability}.  The
...
\vertwo{Similarly, a \Delay with no \token{math} child leaves the delay of the event undefined.  The absence of a \token{math} element is permitted because it is possible for SBML Level~3 packages to add constructs that extend \Delay and define how a value is to be computed. In the absence of such constructs, the event is considered to execute as if it had no \Delay.  A simulator encountering this situation may choose to produce a warning.}
...
\quantity{time} (see \sec{sec:model-timeUnits}).  Note
...
\sec{sec:operator-arg-types}, expressions containing only
...
\paragraphVerOnly{The \token{id} attribute on \class{Delay}}
\label{sec:delay-id}

\vertwo{\Delay inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  The identifier has no mathematical meaning in an SBML \thisLV model.}
...
\token{listOfEventAssignments} element \relverdiff{must have a non-empty list of one or more}{may have
zero or more} \token{eventAssignment} elements of class
\EventAssignment.  The object class \EventAssignment has one
required attribute, \token{variable}, and the \vertwo{optional }element,
...
\emph{executed}.)  See \sec{sec:event-semantics} below for
more information about events and event assignments.


\paragraph{The \token{variable} attribute}

The \relchange{\EventAssignment required} attribute \token{variable} has type \primtype{SIdRef}. \relchange{The value of the attribute must be the identifier of \relverdiff{a}{an object in the \primtype{SId} namespace of the model with mathematical meaning and the ability to be assigned.  In SBML Level~3 Core, the permitted objects classes are} \Compartment, \Species, \SpeciesReference, \relverdiff{or}{and} \Parameter.  \vertwo{The set also includes SBML Level~3 package objects with identifiers in the \primtype{SId} namespace of the model and mathematical meaning defined to allow assignment.}}
...
element\relverdiff{; that is,}{.  For components defined by SBML Level~3 Core, this means that} a species' quantity, species reference's
...
\item The \token{variable} attribute must not contain the
  identifier of a reaction; \vertwo{in core,} only species, species references,
...
\vertwo{If the \token{variable} attribute of an \EventAssignment object references an object in an SBML namespace that is not recognized by the interpreter reading a given SBML document (that is, if the object is defined by an SBML Level~3 package that the software does not support), the event assignment must be ignored---the object's value must not be assigned  if the interpreter cannot understand the package.  If an interpreter cannot establish whether a referenced object is missing from the model or instead is defined in an SBML namespace not recognized by the interpreter, it may produce a warning to the user.  (The latter situation can only arise if an SBML package is present in the SBML document with a \token{package:required} attribute of \val{true}.)}
...
\paragraphVerOnly{The optional \token{id} attribute on \class{EventAssignment}}
\label{sec:eventassignment-id}

\vertwo{\EventAssignment inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  The identifier has no mathematical meaning in an SBML \thisLV model.}
...
occurring in MathML \token{ci} \relchange{elements} in the
...
\token{variable} attribute value.  More precisely\relverdiff{:}{, for objects defined by SBML Level~3 Core:}
\begin{itemize}
...
\item \emph{In the case of a compartment}, an \EventAssignment
  ...
  produced by the \token{math} formula should be the same as that
  specified for the compartment's size.  (See
  \sec{sec:compartment-units} for more.)
...
\itemvertwo{\emph{In the case of an object from an SBML Level~3 package}, an \EventAssignment sets
  the referenced object's value (as defined by that package) to the value of the formula in
  \token{math}.  The unit of measurement associated with the value produced by
  the formula should be the same as that object's \token{units}
  attribute value (if it has such an attribute), or be equal to
...
\item A software package should retest all event triggers after
  executing \relchange{the assignments from a given event} in order to account for the
  possibility that the assignments cause another event trigger to
