...
\newcommand{\sbomodelingframework}{\emph{SBO:0000004}, ``modeling framework''\xspace}
...
%the same field available on \mathmltwo objects used in SBML.  Rather than
...
%such agreed-upon standard format; moreover, as is the case in \mathmltwo,
...
\begin{blockChanged}
\mathmltwo defines ``whitespace'' in the same way as XML does, i.e., the
...
\sbomodelingframework).  The SBO term chosen should be the most
...
\primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this field in an
\AlgebraicRule, \AssignmentRule, or \RateRule, the value must be a
valid SBO term identifier.  The rule should have a ``is a''
relationship with the SBO term.  When a value is given to this
field the value must be an SBO term identifier referring to a
mathematical formula term defined in SBO (i.e., terms derived from
\sbomathformula).  The SBO term chosen should be the most precise
(narrow) term that matches the form of the \AlgebraicRule,
\AssignmentRule, or \RateRule.
...
\sbomodelingframework).  The \Reaction structure should have a
...
given more meaningful labels suitable for display to humans
...
to interpret models without additional human intervention.  For
...
to be in some given model).  An advanced software tool
\emph{might} be able to deduce the purposes of some model
components by detailed analysis of the kinetic rate expressions
and other parts of the model, but this quickly becomes infeasible
for any but the simplest of models.
...
provided in many SBML object classes.  The SBML object classes
\Model, \Parameter, \InitialAssignment, \AlgebraicRule,
\AssignmentRule, \RateRule, \Constraint, \Reaction,
\SpeciesReference, \ModifierSpeciesReference, and \KineticLaw, all
have optional \token{sboTerm} fields.  In this section, we discuss
the \token{sboTerm} field, the motivations and theory behind its
introduction, guidelines for its use, and provide examples for
illustration.
...
tools can interpret mechanically.
...
an object \emph{must not change the fundamental mathematical
  meaning} of the model.  An SBML model must stand on its own and
not require the additional information added by SBO terms for a
correct mathematical interpretation.  There are two nuances to
this.  First, it would be too limiting to require that all
software tools be able to understand the SBO vocabularies in
addition to understanding \sbmltwotwo.  Supporting SBO is not only
additional work for the software developer; for some kinds of
applications, it may not make sense.  If the SBO terms on a model
are optional, it follows that the SBML model \emph{must} remain
unambiguous and fully interpretable without them, because an
application reading the model may ignore the terms.  Second, we
believe allowing the use of \token{sboTerm} to alter the
mathematical meaning of a model would give software authors too
much leeway to shoehorn incompatible concepts into SBML,
ultimately reducing the interoperability of the models.
...
mentioned, the relationship between the SBML object and the URI
is:
\begin{quote}
...
extent possible the the nature of each reaction rate equation in a
...
quantitative parameter, and (4) mathematical formula.
...
is not meant to be complete or comprehensive.  The web site for
SBO~\cite{sbo} should be consulted for the current version of the
ontology.
...
Figure~\ref{fig:expanded-species} shows anticipated examples for
the \emph{participant role} CV.  The structure of this CV reflects
the hierarchical conceptual groupings of the concepts.  For
example, in reaction rate expressions, there are a variety of
possible modifiers.  Some classes of modifiers can be further
subdivided and grouped.  All of this is easy to capture in the CV.
As more agreement is reached in the modeling community about how
to define and name modifiers for different cases, the CV can grow
to accommodate it.
...
will need to be distinguished along this dimension, but some will.
...
Finally, there is the \emph{mathematical formula} framework.  
...
in a database, but it may be helpful to get a sense for the kinds
of data fields that are likely to be available for each entry in
SBO.  The following are the minimum anticipated data fields:
\begin{itemize}\setlength{\parskip}{-0.2ex}
...
\item \emph{Parent(s)}: The term(s) from which a given term is
  derived.  The given term has an ``is a'' relationship to its
  parents.

\item \emph{Children}: The term(s), if any, which are children of
  a given term.
...
\begin{table}[bht]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    \Model			& modeling framework	  & \token{SBO:0000004} \\
    \Parameter			& quantitative parameter  & \token{SBO:0000002} \\
    \InitialAssignment		& 			  & \\
    \AlgebraicRule		&			  & \\
    \AssignmentRule		&			  & \\
    \RateRule			&			  & \\
    \Constraint			&			  & \\
    \Reaction			& modeling framework	  & \token{SBO:0000004} \\
    \SpeciesReference		& participant role	  & \token{SBO:0000003} \\
    \ModifierSpeciesReference	& participant role	  & \token{SBO:0000003} \\
    \KineticLaw			& mathematical formula	  & \\
    \bottomrule
  \end{tabular}
...
\subsubsection{Is Another Ontology Needed?}
...
is that the SBML model must be self-contained and stand on its
own.  Therefore, in cases where they differ, the definitions in
the model take precedence over the definitions referenced by the
CV.  In other words, the model (and its MathML) is authoritative.
...
once a core set of terms is defined, applications can cache the
controlled vocabulary and not make network accesses to the master
SBO copy unless something forces them to (\eg detecting a
