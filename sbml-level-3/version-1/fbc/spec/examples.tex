...
pathway that transforms metabolite \textit{IN} to \textit{OUT}. The model was created and analysed using the \textsf{SBW Flux Balance} \FBC implementation \cite{sbw} In \SBML each reaction is represented as a chemical process transforming reactants to products, e.g. reaction \textit{R1} is encoded in XML as (see also the complete example provided at the end of this section):
%
...
%
\exampleFile{examples/spec-example1-bnd.txt}
