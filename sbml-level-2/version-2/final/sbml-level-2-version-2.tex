...
rather than plain HTML for \token{notes} content is to balance
several conflicting goals: support an international formatting
standard so that users have more control over the appearance of
notes and can predict to some degree how their notes will be
displayed in different tools and environments; chose a format for
notes that is compatible with the XML form of SBML (plain HTML
would not be); and achieve these goals using an approach that is
hopefully easy enough for software developers to support using
modern programming libraries.  Libraries for displaying and
...
The content of \token{notes} elements must declare the use of the
XHTML namespace.  There are multiple ways in which this can be
done.  One way is to place a namespace declaration for the XML
namespace URI \uri{http://www.w3.org/1999/xhtml} on the top-level
\Sbml object (see Section~\ref{sec:sbml}) and then reference the
namespace in the \token{notes} content.  The following example
...
% Mike the scheme as originally written wouldn't work for example
% if the content of the notes field was a <title> element the wrapping
% scheme wouldn't have worked.  We need to state that the notes field MUST
% contain wrappable content.

SBML does not require the content of \token{notes} to be a
specific XHTML element however there are restrictions the
following restrictions on the content of the \token{notes} field:
\begin{itemize}

\item \token{notes} must not contain an XML declaration (\ie
\token{<?xml version="1.0" encoding="UTF-8"?>}) nor a DOCTYPE
declaration;

\item at no pointin the \token{notes} field can the character
encoding be changed away from UTF-8 (the encoding for SBML);

\item the \token{notes} content should be in a form that can be
processed as as described below to form a valid XHTML 1.0
document.
...
\item If the first element inside \token{notes} is \token{<head>},
  the application could assume that the contents is an XHTML
  document without the \token{<html> ... </html>} wrapper around
  it.  It could therefore take the contents of the \token{notes}
  element, wrap it with \token{<html
    xhtml="http://www.w3.org/1999/xhtml">} and \token{</html>}
  elements, append the result to the XML and DOCTYPE declarations,
  and finally hand it to the XHTML editing function.

\item If the first element is \token{<body>}, the application
  could take the \token{notes} contents and wrapt it with
  \begin{example}
<html xhtml="http://www.w3.org/1999/xhtml">
    <head>
        <title></title>
    </head>\end{example}
  and \token{</html>} elements, append the result to the XML and
...
The above restriction means that arbitrary XHTML elements cannot
be placed in \token{notes} field because only a subset of elements
can form valid documents with the above wrapping scheme.  For
example a single XHTML \token{title} element cannot be wrapped as
described to form a valid XHTML document.
...
\item \token{bqbiol:is} The object represented by the SBML
...
\item \token{bqbiol:hasPart} The object represented by the SBML
component includes the subject of the referenced resource, either
physically or logically.

\item \token{bqbiol:isPartOf} The object represented by the SBML
component is a physical or logical part of the subject of the
referenced resource.

\item \token{bqbiol:isVersionOf} The object represented by the
...
\item \token{bqbiol:isHomologTo} The object represented by the
