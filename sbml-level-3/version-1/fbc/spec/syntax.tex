...
and one required element \token{listOfFluxObjectives} which contains a \ListOfFluxObjectives.
...
The \ListOfFluxObjectives is derived from and functions like a typical \SBML \textsf{\textbf{ListOf\rule{0.15in}{0.5pt}}} class with the proviso that it cannot be empty and must contain one or more \token{fluxObjective} elements of type \FluxObjective (see \ref{fluxobjective-class}).
...
\begin{itemize}
  \item \token{reaction} an attribute that contains an \primtype{SId} that is restricted to refer only to an \SBML \Reaction,
