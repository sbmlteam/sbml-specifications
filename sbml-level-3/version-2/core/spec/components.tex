...
\relverdiff{\paragraph{The \token{id} and \token{name} attributes}}{\paragraphChanged{The \token{id} attribute}}
...
% Tentative editor decision 2014-08-03 is to remove this entirely.

% \changed{\Constraint definitions \emph{cannot and should not} be used to
...
\paragraph{The \token{fast} attribute \changed{(deprecated)}}
\label{sec:fast}
...
\emph{local parameter} that are only accessible \changed{within the scope of}
