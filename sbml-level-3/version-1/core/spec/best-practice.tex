...
% --------------------
% [2010-09-15] This just doesn't seem like it belongs here.  A FAQ
% might be better.

% Perhaps the largest issue facing simulator developers is in defining in
% what situations, exactly, two events would \emph{execute} simultaneously.
% The model defines this situation mathematically (that is, exactly), while
% the limits of computational numerical precision make it such that a
% simulator may be only able to determine things numerically (that is, to
% some degree of precision).  This can cause events that were not intended to
% be simultaneous, simultaneous anyway, and events that were intended to be
% simultaneous, no longer so.  A good model writer can address this by
% writing robust models that do not require exact solutions to produce
% expected behavior, but a good simulator writer can help by doing things
% such as caching the calculated times at which embedded boolean elements in
% Trigger equations switched their truth state: given two events with
% triggers $((s1>5) \&\& (s2>7))$ and $((s1>5) \&\& (s3>10))$, for example,
% the time at which $s1$ changes from being less than 5 to greater than 5 can
% be calculated only once, guaranteeing that if s3 is greater than 10 and s2
% is greater than 7, both events fire simultaneously once s1 becomes greater
% than 5, as the modeler clearly intended.
%
% The modeler is likewise encouraged to write mathematically identical
% boolean elements in numerically-identical ways: don't use `$9$' in one
% place and `$pow(3,2)$' in another.
...
There are actually a variety of simple approaches that can satisfy
the ``undefined ordering'' requirement.  For example, a software
