...
You cannot refer to \class{SpeciesType} structures from MathML
...
\class{SpeciesTypeInstance} structure and is enclosed within
another rectangle representing a \class{SpeciesType}.
...
perpendicular to the outer edge of \class{SpeciesType} structures
represent \class{BindingSite} structures. \class{SpecifcBond}
...
simple a reference to \class{SpeciesType} \texttt{D}.  The form
used, although redundant, demonstrates that the
\class{SimpleSpeciesReference} structures of a reaction can
...
Whatever is assigned to the \class{GenericBond} in the set of
...
Note that this proposal restricts the use of \class{GenericBond}
structures to ensure that is feasible to interpret systems of
reactions containing \class{GenericBond} structures. In particular
a reaction containing \class{GenericBonds} can't be reversible.
...
not contain \class{SpeciesTypeInstance} structures \emph{simple}
...
\item A \class{SpeciesTypeInstance} \attrib{id} value must be
unique across the set of reactant \class{SpeciesReference}
structures on a given reaction. A \class{SpeciesTypeInstance}
\attrib{id} value must be unique across the set of product
\class{SpeciesReference} structures on a given reaction.
...
\class{SimpleSpecies-} \class{Reference} can refer to a set of
...
\attrib{spatialSizeUnits}.  \emph{This means that a reaction may
...
\class{SpeciesTypeInstance} structures then \attrib{stoichiometry}
...
match with a \class{SpeciesReference} structure with a
...
\attrib{Compartment} more than once.
...
\class{SpeciesType} structures referenced by the \class{Species-}
\class{TypeInstance} structures in a given \class{SpeciesType}
structure. Each of these \class{BindingSite} structures should be
referenced exactly once by a \class{BindingSiteReference}
structure enclosed in the \class{SpeciesType} structure.  This
means that the status of a \class{BindingSite} can't be left
undefined or ambiguous by a \class{SpeciesType}.
...
\attrib{bindingSiteReference} and
\attrib{otherBindingSiteReference}. \attrib{bindingSiteReference}
is inherited from \class{Bond}. \attrib{otherBindingSiteReference}
is optional.

The \class{SpecificBond} structure represents the state in which
\attrib{bindingSite} is unbound if \attrib{otherBindingSite-}
\class{Reference} is not present. If
\attrib{otherBindingSiteReference} is present the
\attrib{bindingSite} and \attrib{otherBinding-}
\class{SiteReference} structures represent the 2 binding sites
...
\item a \class{SpeciesGraph} is \emph{simple} if it contains no
\class{SpeciesTypeInstance} structures;

\item a \class{SpeciesGraph} is \emph{complex} if it contains one
or more \class{SpeciesTypeInstance} structures.

\item a \emph{standard} \class{SpeciesGraph} is a complex
\class{SpeciesGraph} that does not contain any \class{BindingSite}
structures.

\item a \class{SpeciesGraph} structure is \emph{normalized} if the
\class{SpeciesGraph} is standard and the set of
\class{SpeciesType} structures, referred to by the set of
\class{SpeciesTypeInstance} structures, are simple i.e. it
consists of one hierarchical level.

\end{itemize}

The above definitions apply to both types of \class{SpeciesGraph}:
\class{SpeciesType} or \class{SimpleSpeciesReference}.
...
types are transformed into a standard \class{SpeciesGraph} as
described in Section~\ref{sec:trans-type}. Second these
\class{SpeciesGraph} are normalized (flattened), as described in
...
\subsubsection{Transforming a species type into a standard \class{SpeciesGraph}}
\label{sec:trans-type}

The transformation of a \class{SpeciesType} into a standard
\class{SpeciesGraph} depends on whether it is complex or not:

\begin{itemize}

\item A simple \class{SpeciesType} structure is transformed by
initially creating a \class{SpeciesGraph} that contains one
\class{SpeciesTypeInstance} structure which refers to the original
simple \class{SpeciesType}. All binding sites of the simple
\class{SpeciesType} are referenced as unbound in a set of
\class{SpeciesBond} structures within the new normalized
\class{SpeciesGraph}.  The result is a new standard complex
\class{SpeciesGraph} which refers to the original simple species
type.  The complex \class{SpeciesGraph} is used for equivalence
evaluation i.e its passed to the following stages.

\item A complex \class{SpeciesType} with one or more
\class{BindingSite} structures is transformed to remove the those
\class{BindingSite} structures. Each \class{BindingSite} structure
is replaced by a \class{SpecificBond} structure containing just
the \class{BindingSiteReference} structure that was contained in
the \class{BindingSite} structure.  This means that the original
...
simply reduces the given standard \class{SpeciesGraph} to a single
hierarchical level.

The normalization process consists of iteratively replacing any
\class{SpeciesTypeInstance} structures that refer to complex
\class{SpeciesType} structures with a set of new
...
matched using \class{GenericBond} structures.}
