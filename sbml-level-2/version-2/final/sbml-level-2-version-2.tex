...
\newcommand{\xnew}{\ensuremath{x}\xspace}
\newcommand{\xbase}{\ensuremath{x_b}\xspace}
...
a quantity having numerical value \xnew when expressed in units
\unew.  The relationship between \xnew and a quantity \xbase
expressed in base units \ubase is
\begin{equation}
  \xbase \, \ubase = \xnew \, \unew \left( \frac{w \, \ubase}{\unew} \right)
\label{eq:new-to-old}
\end{equation}
...
\begin{equation}
  \unew = (\token{multiplier} \cdot 10^\token{scale} \cdot \ubase)^\token{exponent}
...
\begin{equation*}
  \unew = \token{multiplier} \cdot 10^\token{scale} \cdot \ubase 
\end{equation*}
...
\intertext{leading to a conversion between quantities of}
  \xbase \, \text{metres} = 0.3048 \cdot \xnew \, \text{feet} 
\end{align*}
Given a quantity of, say, $x = 2$, the conversion results in
$\xbase = 0.6096$.
...
offset as part of their definition.  \emph{When temperature values
  in units other than kelvin are being considered, then a
  different interpretation must be made}, as discussed below.  In
all other cases, the straightforward unit transformation scheme
