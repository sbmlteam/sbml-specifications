...
of a constant or the initial value of a variable from the value of
a constant or the initial value of a variable.  An
...
other initial value assigned to the given symbol except for the
value calculated by an \class{AssignmentRule} structure. Unlike an
...
significant: they should be evaluated in the order given in SBML.
...
constrained than \class{AssignmentRule} structures. In the case of
simulation \class{InitialAssignment} structures are only executed
once to establish the state of the simulated system at the
beginning of simulated time.  A model can contain more than one
\class{InitialAssignment} structure with the same \attrib{symbol}
value (the last one assigning the value to the given symbol at the
start of the simulation).  The values of symbols contained in the
\attrib{math} fields of \class{InitialAssignment} are either those
given on the structure declaring the symbol (i.e. either a
\class{Compartment}, \class{Species} or \class{Parameter}
structure) or the value assigned to the symbol by a previous
\class{InitialAssignment} structure.  As a result the
\attrib{math} field of a \class{InitialAssignment} structures can
refer to symbol before another (or the same)
\class{InitialAssignment} structure makes an assignment to the
given symbol.
