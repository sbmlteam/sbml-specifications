...
\item The \class{ParameterRule} structure no longer has a \attrib{units} field.
...
\class{Model} serves as a container for \class{UnitDefinition},
\class{Compartment}, \class{Species}, \class{Parameter}, \class{Rule}, and
\class{Reaction} components.  All of these components are optional (the
lists in each of the respective fields are permitted to have zero length).
An instance of a \class{Model} may also have an optional \attrib{name}
field that can be used to give the model a name.  The name must be a text
string conforming to the syntax permitted by the \class{SName} data type
...
\attrib{volume} attribute defaults to a value of ``\texttt{1}'' (one).  The
...
\begin{blockChanged}
The following is an example of using \attrib{outside} to model cell
...
\begin{blockChanged}
  ...
  the term occurs (i.e., \class{species} as well as \class{specie},
  \attrib{speciesReference} as well as \attrib{specieReference}, and
  \class{speciesConcentrationRule} as well as
...
Level~1, the enumeration has two possible values: ``\class{scalar}'' and
...
Future releases of SBML may add to the possible values of \class{RuleType}.
\end{blockChanged}
...
\begin{blockChanged}
  The \class{ParameterRule} structure adds one field, \attrib{name} to the
  basic \class{AssignmentRule} type.  The \attrib{name} field has type
  \class{SName} and identifies the parameter affected by the assignment.
  The effect of this rule depends on the value of the \attrib{type} field
  in \class{AssignmentRule}: if the type is ``\texttt{scalar}'', the rule
  sets the referenced parameter's value to that determined by the formula
  in \attrib{math}; if the type is ``\texttt{rate}'', the rule sets the
