...
Format;][]{lassila:1999}.  It has a data type of \class{ID} (the
XML identifier type), and serves as anchors for metadata
references.  Metadata expressed using RDF can be placed anywhere
within an \class{sbml} element and its subelements, \emph{except}
within MathML elements.  The metadata elements can include RDF
\class{description} elements in which the RDF \attrib{describes}
attributes contain the values of the \attrib{metaid} fields of
SBML elements in the model.
\end{blockChanged}
...
\begin{blockChanged}
%-----------------------------------------------------------------------------
\subsection{A standard Format for the \attrib{annotation} Field}
...
\begin{table}[bh]
  ...
  \begin{tabular}{lll}
    ...
    \class{dcterm} & \url{http://purl.org/dc/terms/} & \citep{kokkelink:2002, DCMIUB:2005} \\
...
The general outline of the format syntax is shown in
Figure~\ref{fig:finney-novere-syntax}. In this format the
annotation of an element is located in a single \class{rdf:RDF}
element contained within an SBML \class{annotation} element. The
annotation element can contain other elements in any order as
described in Section \emph{FIX ME}.  The format described in this
section only defines the form of the \class{rdf:RDF} element. The
...
<SBML_ELEMENT +++ metaid="SBML_META_ID" +++ >
  ...
  \end{example}
  \caption{
  An illustrative overview of the syntax of the standard annotation scheme for
  referring
  to external resources.  The order of elements is as shown here. The capitalized identifiers refer to generic strings of a
  particular type: \texttt{SBML\_ELEMENT} refers to any SBML element name that can contain
  an \class{annotation} element; \texttt{SBML\_META\_ID} is a XML \class{ID} string;
  \texttt{DUBLIN\_CORE\_RELATION\_ELEMENT} refers to a restricted subset of element names in the
  \url{http://purl.org/dc/elements/1.1/} and \url{http://purl.org/dc/terms/} namespaces; and
  \texttt{URI} is a URI.  \texttt{[MODEL\_HISTORY]} refers to an optional section described in
  Section~\ref{sec:model-history-annotation} which can
  only be present within SBML model elements. `\texttt{+++}' is a placeholder for either no content
  or valid XML
  syntax that is not defined by the standard annotation scheme but is
  consistent with the relevant
  standards for the enclosing elements.
  `\texttt{...}' is a placeholder for zero or more elements of the same form as the immediately
  preceding element.  The precise form of whitespace and the XML namespace prefix definitions
  is not constrained.
  See main text for a definitive description of the format.
  }
  \label{fig:finney-novere-syntax}
\end{figure}
...
set of standard valid URI syntax rules. This set of rules is not
...
composed as "resource\#id". The web page \url{FIX ME} lists URI
syntaxes and possible physical links to controlled vocabulary and
databases. The list will evolve with the evolution of databases
and resources. The annotation format described in this section
does not require software to access this list to operate.

To enable consistent and thus useful links to external resources
the URI syntax rule set must have a consistent view of the
concepts represented by the different SBML elements for the
purposes of this format.  The following lists the concepts
represented by SBML elements for the purposes of this format:

\begin{itemize}

\item \class{compartmentType}

\item \class{compartment}

\item \class{speciesType}

\item \class{species}

\item \class{reaction}

\item \class{kineticLaw}

\item \class{speciesReference}

\item \class{event}

\item \class{parameter}

\item \class{rule}

\item \class{constaint}

\item \class{initialAssignment}

\end{itemize}

%=============================================================================
\subsubsection{Relation Elements}
...
SBML.  This format assumes that the biological entity represented
by the element is the concept linked to the reference resource.
This pattern doesn't apply in the case of
\class{dcterms:isReferencedBy} because the referenced object is
literature however the SBML element is still assumed to represent
...
The following is a list of the semantics of the relation elements
derived from the Dublin Core standard taking into account the
concepts represented by SBML elements when linked in this format
...
In the described format, when enclosed in an SBML \class{model}
element, can include additional elements immediately before the
first Dublin Core relation elements to encode information on the
model creator and a sequence of dates recording changes to the
model. The syntax for this section is outlined in
Figure~\ref{fig:model-history-annotation}.

These additional elements consist in sequence of a
\class{dc:creator} element, a \class{dcterms:created} element and
zero or more \class{dcterms:modified} elements.
...
{<dcterms:modified rdf:parseType="Resource">
  ...
  \end{example}
  \caption{
  \emph{An illustrative overview of the syntax of the standard annotation scheme for
    recording the history of a model.}  The order of elements is as shown here except that
    elements of the format contained between \texttt{[[} and \texttt{]]}
  can occur in any order.  The capitalized identifiers refer to generic strings of a
  particular type:
  \texttt{FAMILY\_NAME} is the family name of a person who created the model;
  \texttt{GIVEN\_NAME} is the family name of the same person who created the model;
  \texttt{EMAIL\_ADDRESS} is the email address of the same person who created the
  model; and
  \texttt{ORGANIZATION\_NAME} is the name of the organization with which the same person
  who created the model is affiliated.
  The elements of the format contained between \texttt{[} and \texttt{]} are optional.
  `\texttt{+++}' is a placeholder for either no content or valid XML
  syntax that is not defined by the standard annotation scheme but is
  consistent with the relevant standards for the enclosing elements.
  `\texttt{...}' is a placeholder for zero or more elements of the same form as the immediately
  preceding element.
  The precise form of whitespace and the XML namespace prefix definitions
  is not constrained.
  See main text for a definitive description of the format.
  }
  \label{fig:model-history-annotation}
\end{figure}

\emph{FIXME: more on this part of the format here}
...
The SBML structures \class{Model}, \class{Reaction},
\class{SimpleSpeciesReference}, \class{Parameter},
\class{KineticLaw}, \class{Rule}, \class{Constraint} and
\class{InitialAssignment} all have \attrib{sboTerm} optional
fields of the type \class{SBOTerm}.  These fields when present
must contain valid Systems Biology Ontology\sboref identifiers
that refer to actual SBO terms. The \class{SBOTerm} type restricts
the field values to strings of the form \texttt{sbo:XXXXXXX} where
\texttt{X} is a decimal digit. The containing SBML structure
should have an 'is a' relationship with the SBOTerm referenced by
the \attrib{sboTerm} field.
...
identifier referring to a modelling framework SBO term, that is a
...
definition is shown in Figure~\vref{fig:compartmentType}.
...
by the field \attrib{units}.  These units are used when the
parameter identifier appears in MathML expressions and in
\changed{\class{AssignmentRule} and \class{InitialAssignment}
structures} setting the value of the parameter.  A
...
directed arcs start from the statement assigning the symbol and
...
The \class{Rule} structure has an optional \class{SBOTerm} field,
\attrib{sboTerm} (see Section~\ref{sec:sboTerm}).  This field when
present must contain an Systems Biology Ontology (SBO)\sboref term
identifier.  The \attrib{sboTerm} field should contain a SBO term
identifier referring to an actual SBO Term.  The \class{Rule}
structure should have a 'is a' relationship with the SBO term. The
SBO term chosen should be the most precise (narrow) term that
matches the form of the \class{Rule}.
...
The \class{ModifierSpeciesReference} structure should have a 'is
a' relationship with the SBO term. The SBO term chosen should be
the most precise (narrow) term that captures the role of the given
...
resources is introduced. Whilst CellML metadata can be included in
SBML the specification does not make any specific mention of
CellML metadata.
...
element, a \class{compartmentType} element, a
