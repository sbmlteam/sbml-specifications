...
given list container is present within the model, the list may
be empty; that is, it may have no children.  Semantically, this is identical to the list not being present in the model.}  The
...
defining a compartment, and defining a \changed{species reference} requires defining
...
and mutually-recursive functions are permitted \changed{only if the value of the \token{isRecursive} attribute is set to \val{true}}.  The purpose
...
The \token{id} attribute that \FunctionDefinition inherits from \SBase is changed here to be required instead of optional, and otherwise behaves as described in Section~\ref{sec:idnameattribs}.  It has mathematical meaning in that it may be used in \token{math} elements as the first target of an \token{apply} element, but has no value associated with it, and may not be the target of an \InitialAssignment, \EventAssignment, or \Rule.


\subsubsection{The \token{isRecursive} attribute}
...
Recursive functions must terminate, however, as they might through the use of a \token{piecewise} function.
\end{blockChanged}
...
The \token{id} attribute that \UnitDefinition inherits from \SBase is changed here to be required instead of optional, and additionally given the derived type \primtype{UnitSId} instead of \primtype{SId}.  This is done to give the defined unit a unique identifier by which other parts of the model may refer to the unit; see Section~\ref{sec:idnameattribs} for more details.
\end{blockChanged}
...
The \token{id} attribute that \Compartment inherits from \SBase is changed here to be required instead of optional, and otherwise behaves as described in Section~\ref{sec:idnameattribs}.  It also acquires the mathematical meaning of its \token{size}, and may be the target of an \InitialAssignment, \EventAssignment, or \Rule.
\end{blockChanged}
...
The \token{id} attribute that \Species inherits from \SBase is changed here to be required instead of optional, and otherwise behaves as described in Section~\ref{sec:idnameattribs}.  Its mathematical meaning is the value of its amount or concentration, depending on the state of its \token{hasOnlySubstanceUnits} attribute.  It may therefore be the target of an \InitialAssignment, \EventAssignment, or \Rule.
\end{blockChanged}
...
The \token{id} attribute that \Parameter inherits from \SBase is changed here to be required instead of optional, and otherwise behaves as described in Section~\ref{sec:idnameattribs}.  It also acquires the mathematical meaning of its \token{value}, and may be the target of an \InitialAssignment, \EventAssignment, or \Rule.
\end{blockChanged}
...
attribute \changed{must be the identifier of an element in the \primtype{SId} namespace of the model that has mathematical meaning.  In core, this includes any \Compartment, \Species, \SpeciesReference or global \Parameter elsewhere in the model.  In addition, any package element in the same \Model with an identifier in the \primtype{SId} namespace with mathematical meaning may also be the target of the \token{symbol} attribute.}
...
\begin{blockChanged}
If the \token{symbol} references an element in an SBML namespace that is not
  ...
  it may produce a warning.  This situation may only arise if a package is present in the SBML document with a \token{package:required} attribute of \val{true}.
\end{blockChanged}
...
\InitialAssignment inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of an \InitialAssignment takes on no mathematical meaning, and the value of its \token{math} child may not be changed directly.
\end{blockChanged}
...
\InitialAssignment.  \changed{For core elements,} initial assignments can take place for
...
\item \emph{In the case of an element from a package}, an \InitialAssignment sets
  ...
  the formula should be the same as that element's \token{units}
...
\token{sboTerm} \changed{and \token{id}}.
...
\Rule inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of a \Rule takes on no mathematical meaning, and the value of its \token{math} child may not be changed directly.
\end{blockChanged}
...
\token{variable} attribute) of an assignment rule \changed{is of type \primtype{SIdRef}, and can refer to any changeable SBML element in the \primtype{SId} namespace with mathematical meaning in the \Model.  In core, this includes any}
...
\changed{have its \token{constant} attribute set to \val{false}}.  The effects of
...
\begin{blockChanged}
\item \emph{In the case of an element from a package}, an \AssignmentRule sets
...
\begin{blockChanged}
If the \token{variable} references an element in an SBML namespace that is not
...
(the \changed{required} \token{variable} attribute) of a rate rule \changed{is of type \primtype{SIdRef}, and can refer to any changeable SBML element in the \primtype{SId} namespace with mathematical meaning in the \Model.  In core, this includes any}
\Species, \SpeciesReference, \Compartment, 
or global \Parameter object in
...
\begin{blockChanged}
\item \emph{In the case of an element from a package}, a \RateRule sets the rate of change of the referenced element's value (as defined by that package) to the value of the formula in \token{math}.  The unit associated with the value produced by the formula should be equal to that element's \token{units} attribute value divided by the model-wide unit of \quantity{time}, should it have such an attribute, or be equal to the units of elements of that type divided by the model-wide unit of \quantity{time}, should elements of that type all be defined as having the same units.
\end{blockChanged}
...
\begin{blockChanged}
Similarly, the combined set of \RateRule and \Reaction objects constitute a set of definitions of the rates of change of various symbols (the \token{variable} attributes of the \RateRule objects, and the \token{species} attributes of the \SpeciesReference objects in each \Reaction).  These rates of change may be referenced directly using the \emph{rateOf} \token{csymbol}, but may not thereby contain algebraic loops---dependency chains between these statements must terminate.  To put his more formally, consider a directed graph in which nodes are the rates of change of variables, and directed arcs exist for each appearance of a \emph{rateOf} \token{csymbol} in the model that appears in any \RateRule or \KineticLaw.  Let the directed arcs point from the variable referenced by the \emph{rateOf} \token{csymbol} to the variable or variables that are determined by the \Math in which it appears.  This graph must be acyclic.
\end{blockChanged}
...
\Constraint inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of a \Constraint takes on no mathematical meaning, and the value of its \token{math} child may not be changed directly.\end{blockChanged}
...
\begin{blockChanged}
Due to a number of factors, including the slow uptake of support for \Reaction constructs with a \token{fast} flag of \val{true}, confusion about how exactly to implement said support, and the advent of new modeling techniques which allow more nuanced approaches to reactions that occur at different timescales (particularly for hierarchical and multi-scale modeling), the \token{fast} flag is now considered deprecated for \sbmlthree.  The \token{fast} flag will be removed in future versions of SBML, and the speed of any \Reaction will be solely determined by its \KineticLaw.  It is now considered best practice to only produce models with a \token{fast} flag of \val{false}.  To achieve the same or similar effects as setting \token{fast} to \val{true}, one should instead either set the \KineticLaw to a value in the desired timescale, or achieve an instantaneous effect with an \AssignmentRule or \AlgebraicRule.
\end{blockChanged}
...
The \token{id} attribute that \SpeciesReference inherits from \SBase is still optional, and behaves as described in Section~\ref{sec:idnameattribs}.  It also acquires the mathematical meaning of the value of its \token{stoichiometry}, and may be the target of an \InitialAssignment, \EventAssignment, or \Rule.
\end{blockChanged}
...
\KineticLaw inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of an \KineticLaw takes on no mathematical meaning:  the value of that \token{math} is instead used as the value of the \Reaction \token{id} identifier.  
\end{blockChanged}
...
The \token{id} attribute that \LocalParameter inherits from \SBase is changed here to be required instead of optional, and is additionally given the derived type of \primtype{LocalSId} instead of \primtype{SId}.  It otherwise behaves as described in Section~\ref{sec:idnameattribs}.  It also acquires the mathematical meaning of its \token{value}, but may not be the target of an \InitialAssignment, \EventAssignment, or \Rule, as the scope of the targets of those elements do not include elements in the \primtype{LocalSId} namespace.  The scope of the \token{id} of a \LocalParameter is limited to its parent \KineticLaw, and may, in core, only be used as the text of a \token{ci} element in the \KineticLaw's \token{math}.  However, in that context, it overrides any meaning that any other SBML element with the same \token{id} may or may not have.
\end{blockChanged}
...
\Trigger inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of a \Trigger takes on no mathematical meaning, and the value of its \token{math} child may not be changed directly.
\end{blockChanged}
...
\Priority inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of a \Priority takes on no mathematical meaning, and the value of its \token{math} child may not be changed directly.
\end{blockChanged}
...
\Delay inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of a \Delay takes on no mathematical meaning, and the value of its \token{math} child may not be changed directly.
\end{blockChanged}
...
The \EventAssignment attribute \changed{\token{variable}} has type \primtype{SIdRef}.
\changed{The value of this attribute must be the identifier of an element in the \primtype{SId} namespace of the model that has mathematical meaning.  In core, this includes any \Compartment, \Species, \SpeciesReference or global \Parameter elsewhere in the model.  In addition, any package element in the same \Model with an identifier in the \primtype{SId} namespace with mathematical meaning may also be the target of the \token{variable} attribute.}
...
element.  \changed{For core elements, this will mean that} a species' quantity, species reference's
...
\begin{blockChanged}
\item If the \token{variable} references an element in an SBML namespace that is not
  understood by the interpreter, the event assignment must be ignored---the 
  referenced object will not need to be changed, as the interpreter could not
  understand that package.  If an interpreter cannot tell whether 
  a referenced object does not exist or if it exists in an unparsed namespace,
  it may produce a warning.  This situation may only arise if a package is present in the SBML document with a \token{package:required} attribute of \val{true}.
...
\EventAssignment inherits an optional \token{id} attribute from \SBase, of type \primtype{SId}.  Despite having a \token{math} child, the \token{id} of an \EventAssignment takes on no mathematical meaning, and the value of its \token{math} child may not be changed directly.
\end{blockChanged}
...
\token{variable} attribute value.  More precisely\changed{, for core elements:}:
\begin{itemize}
