...
it possible for other SBML elements to interact with these parameters depending 
on the value of the \token{fbc:\-strict} attribute of the \Model (see also 
\sec{model-class}). 

For example, if the value of the \token{strict} attribute is 
\val{false}, the value of a flux bound could be calculated based on \InitialAssignment
...
one or more genes (or gene products). Where more than one gene is present 
in an association they are then expressed as a logical expression where 
genes are related to one another using logical `and' and `or' operators.}
