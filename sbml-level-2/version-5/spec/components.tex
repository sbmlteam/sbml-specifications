...
attributes, for the XML namespace (\token{xmlns}) and the SBML \token{level} and \token{version}, and
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}[level distance=0.85in]
    \node { \emptyClassbox{\textsl{SBase}} }
      ...
      child {node (a) {
          \begin{classbox}{Sbml}
            xmlns: string \{ "http://www.sbml.org/sbml/level2/version5" \} \\
            level: positiveInteger \{ use="required" fixed="2" \}   \\
            version: positiveInteger \{ use="required" fixed="5" \} \\
          \end{classbox}
        }}
      child {node (b) {
          \emptyClassbox{Model}
...
an SBML Level~2 Version~5 document:
...
used within the \token{sbml} element.  The URI for SBML Level~2 Version~5
is \uri{http://www.sbml.org/sbml/level2/version5}.  All
...
the same model.  This restriction also applies to the
  \token{csymbol} for \quantity{time} and to the \token{csymbol} for \quantity{delay}.  Functions must be written
...
Versions~2--\sbmlversion.  Modelers and software authors are
...
in more detail below.  Another change in Version~3--\sbmlversion is the
...
\intertext{then,}
  \ybase \, \uone^{x_1} \utwo^{x_2} \ldots \un^{x_n}
    &= \ynew \, \unew \left(
...
interoperability.  SBML Level~2 Versions 2--\sbmlversion acknowledge this reality by reducing
...
<functionDefinition id="Fahrenheit_to_kelvin">
  ...
  SBML Level~2 Versions 2--\sbmlversion.  It can be used for other temperature units
...
answer is that such a compartment's identifier must not appear
...
whether and how the amount of that species can vary during a
...
\emph{not} affected by the set of reactions even when that
...
The \token{constant} attribute indicates whether the species' amount
...
\val{false}, indicating that the species' amount can be changed,
...
species' \token{constant} attribute is \val{true}, then its amount
...
definitions within \Reaction objects must \emph{not} have
...
identifier as any other global model entity (\Compartment, \CompartmentType, \Event, \FunctionDefinition, \Model, \ModifierSpeciesReference, \Parameter, \Reaction, \Species, \SpeciesReference, or \SpeciesType)
...
occur during simulation.  All triggered simultaneous events must fire, and the order in which they are executed is not defined.  They may be executed randomly, alphabetically, arbitrarily, or in any other order determined by the simulation software.
