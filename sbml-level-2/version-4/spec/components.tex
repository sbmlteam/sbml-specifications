...
\changed{\AlgebraicRule is unlike \AssignmentRule and \RateRule in that
there are no specific definitions of the meaning of the
mathematical expression in the \AlgebraicRule.}
...
type \primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this attribute in a
constraint definition, the value \changed{should} be a valid SBO identifier
...
When a model contains \changed{a} true value for \token{fast} on any of its
reactions, \changed{it indicates} that the creator of the model is
...
definition, the value \changed{should} be a valid SBO identifier referring to
an \changed{interaction} (\ie terms derived from \changed{\sbointeraction}).  The \Reaction
object should have an ``is a'' relationship with the SBO term.
The SBO term chosen should be the most precise (narrow) term that
defines the \changed{interaction} represented by the reaction as a whole.  For
example, a given reaction in a model might represent
\changed{\emph{non-covalent binding}}, which has a term in SBO (identifier
...
attribute, it \changed{should} be a valid SBO identifier
...
\sboreactant \changed{sub-branch}; if a product, then it should be a term in
the \sboproduct \changed{sub-branch}; and if a modifier, then it should be a
term in the \sbomodifier \changed{sub-branch}.  The \SpeciesReference and
...
stoichiometry \changed{should} incorporate a conversion factor for converting
...
units \changed{should} match the \quantity{substance} units of the species.
...
attribute, the value \changed{should} be an SBO identifier referring
...
What happens if the definition of the SBML \changed{predefined} unit
...
What happens if instead the definition of the SBML \changed{predefined} unit
...
And finally, what happens if the definition of the SBML \changed{predefined}
...
As with all SBML components derived from \SBase, an
  ...
  Sections~\ref{sec:sboterm-type} and~\ref{sec:sboTerm}).  When a
value is given to this attribute, it \changed{should} be a valid
term derived from \changed{\sbointeraction} in SBO.  The \Event should have an
...
evaluated at the same time the \changed{assignments} are
...
\primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this
attribute, it \changed{should} be a valid term representing a
...
The expression in \changed{the \Delay object's \token{math} element} must be evaluated at the time the
...
\token{math} expression \changed{should} match the model's units of
...
Sections~\ref{sec:sboterm-type} and~\ref{sec:sboTerm}).  When a
value is given to this attribute, it \changed{should} be a valid term derived from
...
Sections~\ref{sec:sboterm-type} and~\ref{sec:sboTerm}).  When a
value is given to this attribute, it \changed{should} be a valid SBO term
identifier referring to a mathematical expression (\ie terms
...
\EventAssignment \changed{should} be consistent with the units of the object
...
\item \emph{In the case of a species}, an \EventAssignment sets
  ...
  \changed{should} be identical to the \emph{units of the species} as defined
  in Section~\ref{sec:species-units}.
...
\item \emph{In the case of a compartment}, an \EventAssignment
  ...
  \changed{should} be identical to the units specified for the size of the
