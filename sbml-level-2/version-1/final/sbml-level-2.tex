...
\item The rule structures are simpler compared to SBML Level~1.  The new
  structures \class{AssignmentRule} and \class{RateRule} replace Level~1's
  \class{ParameterRule}, \class{SpeciesConcentrationRule} and
  \class{CompartmentVolumeRule}, and there is no \attrib{type} attribute on
  \class{Rule}. (Section~\ref{sec:rules}.)
...
\item The \class{Reaction} structure has a new list of \emph{modifiers} in
  ...
  are neither created nor destroyed by the reaction.  (See
...
enumeration of units.  The possible values of \class{UnitKind} are listed
in Table~\vref{tab:unitkind}.
...
\item \emph{In the case of a species}: the rule sets the referenced
  ...
  \attrib{math}.  The units are in terms of $substance/size$, where the
...
\item \emph{In the case of a species}: the rule sets the rate of change of the species's
  ...
  terms of $substance/size/time$, where the $size$
...
of \texttt{s}, \texttt{k}, \texttt{k2}, \texttt{k3} and \texttt{A} are
