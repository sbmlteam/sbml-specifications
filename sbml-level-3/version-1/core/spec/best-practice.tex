...
\token{name}.  If a software tool has the capability for
displaying the content of \token{name} attributes, it should
...
automatically generate the values of \token{id} attributes should
be aware some systems may display the \token{id}'s to the user.
...
the \token{id} attribute is an abbreviated form of the name
attribute value.
...
\token{stoichiometry} attribute.  A missing value in any of these
...
\token{constant} on the \Compartment, \Species, \SpeciesReference and
\Parameter components.  A \token{constant} attribute value of
\val{true} means that the SBML object in question will not be
changed by other constructs in SBML (with the exception of an
\InitialAssignment).  A value of \val{false} indicates an
...
In the description of \SBase's \token{annotation} element, we
already made the point that it is critical not to put data
essential to understanding a model into an \token{annotation}.
...
stored in \token{annotation} elements: 
\begin{itemize}
...
model should be encoded in the \token{<annotation>} element
enclosed within the SBML \token{<species>} element representing
...
\begin{table}[thb]
  ...
  \begin{tabular}{p{1.7in}p{4.5in}}
    ...
    &
    \token{litre}, \token{cubic metre}, \token{dimensionless}, or
    ...
    \begin{minipage}{2.9in}
      ...
      \begin{tabular}{cp{3in}}
        ...
        \val{3}
        & 
        \token{litre}, \token{cubic metre},
        \token{dimensionless}, or units derived from these
        \\[7pt]
        \val{2}
        &
        \token{square metre}, \token{dimensionless}, or units
        derived from these
        \\[7pt]
        \val{1}
...
reason is simply that SBML \Reaction{}s are defined in units of
...
of \quantity{substance}/\quantity{time}, and when species
quantities are given in terms of concentrations or densities, the
compartment sizes become factors in the reaction rate expressions.
