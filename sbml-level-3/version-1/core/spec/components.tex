...
measurement associated with the quantity of \Species objects in
the model when the objects do not otherwise specify the units.
The attribute's value must be of type \primtype{UnitSIdRef}
...
measurement via the \token{substanceUnits} attribute on \Species
...
The \Unit object class has four required attributes, the first
being the attribute \token{kind}, whose value must be taken from
the list of reserved words given in Table~\vref{tab:unitkind}.
These reserved symbols are defined in the value space of the data
type \primtype{UnitSId} (Section~\ref{sec:unitsid}).
...
using \val{mole} loosely where substances are not involved, and
...
To understand the meaning of the three other \Unit attributes
\token{exponent}, \token{scale} and \token{multiplier}, and how
they work together, we first need to define the meaning of
\UnitDefinition more precisely.  The fundamental purpose of the
unit scheme in SBML is to permit expressing new units in terms of
the base units listed in Table~\vref{tab:unitkind}.  The formula
underlying this definition is the following:
\begin{linenomath}
\begin{equation}
...
Table~\ref{tab:unitkind}; in a given \Unit definition, it is
...
\item The \token{scale} attribute can be used to set the exponent
  ...
  \token{scale} value of \val{-3} signifies $10^{-3} \times gram$,
...
information on ways of handling units requiring offsets.
...
(Section~\ref{sec:rules}) elsewhere in the model.  Further, due to
the fact that alternative methods available for setting the size
of a compartment, the \token{size} attribute must be defined as
...
\begin{table}[tbh]
  ...
  \begin{edtable}{tabular}{cl}
    \toprule
    \textbf{Value of attribute} & \Model \textbf{attribute used} \\[-2pt]
...
attribute.  (This also implies that every model must define at
...
(Section~\ref{sec:initialAssignment}) or rule
...
intended \emph{units of the species} are of the form
...
The fact that the \token{units} attribute value is means that
models can define parameters with undeclared units.  Leaving the
units of parameter values undefined in an SBML model does not
...
are always considered to be \token{dimensionless}.
...
\item When the species reference's identifier appears in mathematical
  formulas expressed in MathML in a model, the units associated
  with it are \token{dimensionless}.

\item The units of the \token{math} element of an \AssignmentRule,
  \InitialAssignment or \EventAssignment setting the stoichiometry of the
  species reference should be \token{dimensionless}.

\item The units of the \token{math} element of a \RateRule that
  acts on the species reference's stoichiometry should be 
  identical to 1/\quantity{time}, where \quantity{time} is the model-wide 
  \token{time} units.
...
\val{10~000}?)  In the discussion of best practices in
