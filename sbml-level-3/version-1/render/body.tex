...
\token{programVersion} of type \primtype{string} which can be used to store the version number of the program used to create the render information.
...
applies to the graphical object as shown in the snippet below. Note this relationship is only valid if there is no render information object that is more specific. For example another \LocalStyle could be defined with \token{idList} that references the \token{layout:id="go1"} explicitly, in which case that style would be  chosen. For more information see also \sec{style-res}. 
...
\primtype{string} which is a space separated list of ids of layout objects to which this \Style should be applied.
