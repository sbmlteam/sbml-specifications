...
$dt$ is $[c_1P(P-1)/2]dt$. This interpretation leads to a Markov
...
However, this is a simulation algorithm and not a precise
mathematical description. Such a description can be provided by
writing the model as a time change of a pair of independent unit
Poisson processes. Let $N_1(t)$ and $N_2(t)$ be the counting
functions of these processes, so that for each $i=1,2$, $t>0$,
$N_i(t)\sim \operatorname{Poisson}(t)$. Then, writing $P(t)$ and
$P_2(t)$ for the numbers of molecules of $P$ and $P_2$ at time
$t$, we have:
\begin{align*}
...
\texttt{DJW TO CHECK AND FINISH THIS!}
