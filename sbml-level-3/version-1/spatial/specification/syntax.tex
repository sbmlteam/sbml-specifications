...
\subsubsection{The \token{arrayDataLength} attribute}
...
\subsubsection{The \token{ArrayData} text child}
...
\subsubsection{The \token{pointIndexLength} attribute}
...
\subsubsection{The \token{PointIndex} text child}
...
This defines a pyramid with four triangular faces.  The first triangle is defined by points [0,0,0], [0,1,0], and [0,1,1]; the second triangle by points [0,0,0], [1,1,0], and [0,1,0]; the third by [0,0,0], [0,1,1], and [1,1,0]; and the fourth by [0,1,0], [1,1,0], and [0,1,1].  \changed{Note also that the order of the faces is consistent, defining each face in a counter-clockwise order, as observed from 'outside' the pyramid.  This can be seen, for example, in the last two faces defined, where the shared edge is defined in the order '5 6' in the first face, but '6 5' in the secod.}
...
\subsubsection{The \token{Samples} text child}
