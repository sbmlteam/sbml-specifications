...
attributes, for the \begin{blockChanged}\token{xmlns} and the\end{blockChanged} SBML \token{level} and \token{version}, and
...
the same model.  This restriction also applies to the
  \token{csymbol} for \quantity{time} \begin{blockChanged}and to the \token{csymbol} for \quantity{delay}\end{blockChanged}.  Functions must be written
...
\item The \token{math} element of an \AssignmentRule or
  \InitialAssignment referring to this compartment \begin{blockChanged}should\end{blockChanged} have
...
\item In \RateRule objects that set the rate of change of the
  ...
  the rule's \token{math} element \begin{blockChanged}should\end{blockChanged} be identical to
...
answer is that such a compartment's identifier \begin{blockChanged}must\end{blockChanged} not appear
...
whether and how the \begin{blockChanged}amount\end{blockChanged} of that species can vary during a
...
\begin{blockChanged}Note that while these restrict whether
...
\begin{table}[ht]
  ...
  \begin{edtable}{tabular}{lllll}
    ...
    \textbf{\token{constant}} & \textbf{\token{boundaryCondition}} & \textbf{assignment}   & \textbf{reactant or} & \textbf{species' \begin{blockChanged}amount\end{blockChanged}} \\
...
reactions, its \begin{blockChanged}amount\end{blockChanged} is determined by those reactions.  In
SBML, it is possible to indicate that a given species' quantity is
\emph{not} \begin{blockChanged}affected\end{blockChanged} by the set of reactions even when that
...
The \token{constant} attribute indicates whether the species' \begin{blockChanged}amount\end{blockChanged}
...
\val{false}, indicating that the species' \begin{blockChanged}amount\end{blockChanged} can be changed,
...
definitions within \Reaction objects \begin{blockChanged}must\end{blockChanged} \emph{not} have
