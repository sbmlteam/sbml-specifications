...
\token{substanceConversionFactor}, \token{timeConversionFactor}, 
and \token{extentConversionFactor}.
...
\item \emph{Replacements must be unique}.  Unless the \ReplacedElement 
  ...
  \token{deletion} attribute, a given object
...
to have been converted appropriately by the \token{conversionFactor}
attribute of the \ReplacedElement object.  It is only 
the elements that remain unreplaced in the Submodel to which the
\token{\rule{0.2in}{0.5pt}ConversionFactor} attributes apply.  Note that
targets of \ReplacedBy objects are still converted by the
\token{\rule{0.2in}{0.5pt}ConversionFactor} elements.

%LS NOTE:  I am going to assume for now that ReplacedBy elements do not
% need conversionFactors, and that the element should be considered to
% be 'left behind' in the submodel, where it will be converted
% as normal by the Submodel conversion factors.
...
\begin{table}[bht]
  ...
  \begin{edtable}{tabular}{p{1.1in}p{2.57in}l}
    ...
    Implied rate of change of a species	& \sprd{4}{$\frac{\token{substanceConversionFactor}}{\token{timeConversionFactor}}$}\\
    \Parameter				& 1\\
    \Priority				& 1\\
    \sprd{4}{\RateRule} 		& \sprd{4}{$\frac{\textsf{\persymb}}{\token{timeConversionFactor}}$}\\
    \Species				& \token{substanceConversionFactor}\\
    \SpeciesReference			& 1\\
...
\token{substanceConversionFactor}.  Non-replaced elements with defined unit
...
converted according to the \token{extentConversionFactor}, 'divided by 1'.
