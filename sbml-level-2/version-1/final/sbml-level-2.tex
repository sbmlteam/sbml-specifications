...
\markboth{\textsf{SBML Level 2 Proposal}}{\textsf{SBML Level 2 Proposal}}
...
\item Instead of using text strings, formulas in Level~2 are expressed
  using MathML~\citep{w3c:2000b}.  The field named \attrib{formula}
  previously available on the \class{KineticLaw} and \class{Rule}
  structures has been replaced by an element named \class{math} containing
...
\item A new field called \attrib{id} replaces the \attrib{name} field
  ...
  SBML model.  The new \attrib{id} field has a type of \class{SId}, whose
  definition is similar to \class{SName} in Level~1; see
  Section~\ref{sec:idnameattribs}.  The \attrib{name} field still remains
  available but is optional and its data type has been widened to be the
  \class{string} type defined by XML Schema.
...
\item The top-level \class{Model} structure can contain an optional list of
  global function definitions of type \class{FunctionDefinition}.  See
...
\item The namespace for identifiers in a model does not contain any
  ...
  \class{FunctionDefinition} mechanism.  SBML Level~2 uses a feature
...
\class{SBase} has an optional field named \attrib{metaid} of type
\class{ID}.  RDF \class{description} elements can be created in which the
RDF \attrib{describes} attributes contain the values of the \attrib{metaid}
...
model.  \class{SBase} contains two fields to support the attachment of
arbitrary information, both of which are optional: \attrib{notes} and
...
Nearly all SBML structures include two particular fields: \attrib{id} and
\attrib{name}.  The \attrib{id} field is required and is used to identify
...
The \attrib{name} field is optional.  Its data type is the type
...
delimiter characters~\citep{biron:2000}.  The purpose of the \attrib{name}
...
the \class{string} type of XML Schema.

The recommended practice for treating \attrib{name} is as follows.  If a
...
rules).  \class{SId} is a data type derived from the basic XML type
\class{string}, but with restrictions about the types of characters
...
\class{piecewise}, \class{piece} and \class{otherwise} elements facilitates
...
A \class{Model} data object may optionally have lists of \class{Species},
...
The \class{FunctionDefinition} data structure associates an identifier
with a function.  The function can be used in any subsequent
MathML \class{apply} element.  The definition of
\class{FunctionDefinition} is shown in
...
The \class{FunctionDefinition} has three fields, \attrib{id},
...
\class{FunctionDefinition} using the value of the \attrib{id} field.
...
The following is an example of a \class{FunctionDefinition} structure,
