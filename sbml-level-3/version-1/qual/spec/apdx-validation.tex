...
\consistencyRule{qual-10202}{The MathML \token{math} element in a \FunctionTerm object should not use the \token{csymbol} elements \val{time} and \val{delay} as these expliciltly introduce time into the model. As yet time is not considered within the Qualitative Models package specification.
  (References: SBML Level~3 Package Specification for Qualitative Models, Version~1, \sec{sec:function-term}.) }
...
\consistencyRule{qual-20311}{A \QualitativeSpecies that is referenced by an \Output with the \token{qual:\-transitionEffect} attribute set to \val{assignmentLevel} cannot be referenced by any other \Output with this \token{transitionEffect} throughout the set of transitions for the containing model.   (References: SBML Level~3 Package Specification for QualitativeModels, Version~1 \sec{output-class})}
...
\validRule{qual-20413}{No element of the \ListOfFunctionTerms object can cause the \emph{level} of a \QualitativeSpecies to exceed the value \token{qual:-maxLevel} attribute.  (References: SBML Level~3 Package
  Specification for Qualitative Models, Version~1,
  \sec{math-interpret}.) }

\validRule{qual-20414}{No element of the \ListOfFunctionTerms object can cause the \emph{level} of a \QualitativeSpecies to become negative.  (References: SBML Level~3 Package
  Specification for Qualitative Models, Version~1,
  \sec{math-interpret}.) }
...
\validRule{qual-20605}{When the value of the attribute \token{qual:\-transitionEffect} of a \Output object is set to \val{production} the attribute \token{qual:\-outputLevel} for that particular \Output object must have a value set.    (References: SBML Level~3 Package
  Specification for Qualitative Models, Version~1,
  \sec{output-class}.) }
