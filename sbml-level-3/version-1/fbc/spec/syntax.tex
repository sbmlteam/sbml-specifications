...
The \SBML \Model class is extended by adding a mandatory boolean attribute \token{strict}
and an optional %\token{listOfFluxBounds},
...
\newtxt{
The mandatory attribute \token{strict} of type \primtype{boolean} is used to apply 
additional restrictions to the model in order to make it easier for software to 
ensure that the models are complete.  If set to \val{true} the following 
restrictions apply to the model:}

\begin{newsection}
\begin{itemize}
	\item { All Reactions must have the attributes \token{lowerFluxBound} and
	        \token{upperFluxBound} defined and they need to point to \Parameter
					objects in the model.}
	\item { These \Parameter objects must have their \token{constant} attribute
	        set to \val{true} and have its \token{value} attribute set to a value
					other than \val{NaN}.
	}
	\item { \SpeciesReference elements of {\Reaction}s must have their \token{stoichiometry} 
	        attribute set to a value that is neither \val{NaN}
	        nor \val{-INF} nor \val{INF}. Additionally their \token{constant} attribute
					needs to be set to \val{true}.
	}
	\item { \InitialAssignment elements may not target the {\Parameter} elements
	         referenced by the attributes \token{lowerFluxBound} and
	        \token{upperFluxBound} of reactions, as well as any \SpeciesReference.
	}
	\item { All defined \FluxObjective elements must have their \token{coefficient}
	        attribute set to a value that is neither \val{NaN}
	        nor \val{-INF} nor \val{INF}. 
	}
	\item { A \token{lowerFluxBound} attribute may not have a value of \val{INF}.}
	\item { A \token{upperFluxBound} attribute may not have a value of \val{-INF}.}

\end{itemize}

If the value of the \token{strict} attribute is \val{false} these restrictions do 
not apply. That means that for example the \InitialAssignment construct may be used
to determine the value of the \Parameter elements used as flux bounds. It could 
also mean that the bounds are updated dynamically and will require re-evaluation. 
