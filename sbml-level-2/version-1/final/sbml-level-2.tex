...
\item The namespace for identifiers in a model does not contain any
  ...
  defined by a model.  (Section~\ref{sec:mathmltokens}.)
...
\item The \class{Compartment} structure has a new field,
  ...
  built-in units for area and length.  (Sections~\ref{sec:compartments} and
  \ref{sec:unitdefinitions}.)
...
summarizes the UML-based notation used in this document, and
...
structure in an SBML model.  The definition of \class{SBase} is presented
...
information they need to different structure in an SBML model.  However, it
is important that this facility not be misused accidentally.  In
...
two delimiter characters, 0xFFFE and 0xFFFF~\citep{biron:2000}.  No
...
function definition has a single \class{lambda} subelement.
...
examples below illustrate the use of this namespace and MathML in SBML.
...
%%%%%%%%% FIXME: is this saying it's the global units of substance? what about
%%%%%%%%% the units on the species definition?
  ...
  \item \emph{Species identifier}: When a species identifier occurs in a
    ...
    broadly).  The units of the species' concentration is
    \quantity{substance}/\quantity{size} where \quantity{size} is either
    \quantity{volume}, \quantity{area} or \quantity{length} when the value
    of the containing compartment's \attrib{spatialDimensions} is
    ``\attribvalue{3}'', ``\attribvalue{2}'', or ``\attribvalue{1}'',
    respectively.  If the compartment's \attrib{spatialDimensions} has a
    value of ``\attribvalue{0}'', then the species' units is
    \quantity{substance}.
  ...
  \item \emph{Compartment identifier}: When a compartment identifier occurs
    ...
    The units associated with the size can only be one of
    \quantity{volume}, \quantity{area}, \quantity{length}, or
    \quantity{dimensionless} when the value of the compartment's
    \attrib{spatialDimensions} is ``\attribvalue{3}'', ``\attribvalue{2}'',
    ``\attribvalue{1}'', or ``\attribvalue{0}'' respectively.
...
These are the motivations for the provision of the \class{UnitDefinition}
data structure in SBML, whose definition is shown in
...
enumeration of units.  The possible values of \class{UnitKind} are given in
...
include other units defined by \class{UnitDefinition} structures.  This
...
one-dimensional line), and ``\texttt{0}'' (for a point).  The default value
is ``\texttt{3}''.
...
unit definition in the enclosing model.  The units chosen must agree with
the number of spatial dimensions of the compartment.
...
a value if the species' compartment has a \attrib{spatialDimensions} value
of ``\texttt{0}''.
...
above: $x$ can be the identifier of a compartment (to set its volume), a
...
of an assignment rule) can refer to the identifier of a species,
compartment, or parameter.  The effects are in general terms the same, but
...
\item \emph{In the case of a species}, an \class{AssignmentRule} sets the
  ...
  in \attrib{math}.  The overall units of the formula must work out to be
  species concentration, as \quantity{substance/size}, where the
  \quantity{size} units are either \quantity{volume}, \quantity{area} or
  \quantity{length} when the species' compartment
  \attrib{spatialDimensions} field has a value of ``\attribvalue{3}'',
  ``\attribvalue{2}'' or ``\attribvalue{1}'', respectively.  If the
  compartment's \attrib{spatialDimensions} field has a value of
  ``\attribvalue{0}'', then the rule's units are \quantity{substance}.
  (The term ``concentration'' is admittedly being used somewhat loosely,
  because of the different possible spatial characteristics of
  compartments.)
  
  \emph{Restrictions}: In a given SBML model, there cannot be both an
  assignment rule and a \class{SpeciesReference} structure that refer to
  the same species.  (See Section~\ref{sec:reactions} for the definition of
...
\item \emph{In the case of a compartment}, an \class{AssignmentRule} sets
  ...
  in \attrib{math}.  The overall units of the formula must work out to be
  the same units as those associated with the compartment; that is, they
  must be either \quantity{volume}, \quantity{area}, \quantity{length}, or
  \quantity{dimensionless}, depending on whether the compartment's
  \attrib{spatialDimensions} field has value ``\attribvalue{3}'',
  ``\attribvalue{2}'', ``\attribvalue{1}'' or ``\attribvalue{0}'',
  respectively.
...
\item \emph{In the case of a parameter}, an \class{AssignmentRule} sets the
  ...
  \attrib{math}.  The units of the formula must work out to be the same as
  those defined for the parameter's initial value (see
  Section~\ref{sec:parameters}).
...
\quantity{substance}, \quantity{volume}, \quantity{area} and
\quantity{length} are built-in unit types (see Table~\ref{tab:builtin}).
...
\item \emph{In the case of a species}, a \class{RateRule} sets the rate of
  ...
  formula in \attrib{math}.  The overall units of the formula must work out
  to be species concentration per time, as \quantity{substance/size/time},
  where the \quantity{size} units are either \quantity{volume},
  \quantity{area} or \quantity{length} when the species' compartment
  \attrib{spatialDimensions} field has a value of ``\attribvalue{3}'',
  ``\attribvalue{2}'' or ``\attribvalue{1}'', respectively.  If the
  compartment's \attrib{spatialDimensions} field has a value of
  ``\attribvalue{0}'', then the rule's units are \quantity{substance}.
  (Once again, the term ``concentration'' is admittedly being used somewhat
  loosely, because of the different possible spatial characteristics of
  compartments.)
  
  \emph{Restrictions}: In a given SBML model, there cannot be both a rate
  rule and a \class{SpeciesReference} structure that refer to the same
  species.  (See Section~\ref{sec:reactions} for the definition of
  \class{SpeciesReference}.)  This means that an assignment rule cannot be
  defined for a species that is created or destroyed in a reaction.  The
  only exception is when the given species is a boundary condition; i.e.,
  on the \class{Species} structure that defines the species the
  \attrib{boundaryCondition} field is set to ``\attribvalue{true}''.  No
  more than one \class{RateRule} can refer to a given species in an SBML
  model definition.
  
\item \emph{In the case of a compartment}, a \class{RateRule} sets the rate
  of change of the compartment's volume to the volume determined by the
  formula in \attrib{math}.  The overall units of the formula must work out
  to be the same units as those associated with the compartment, per time;
  that is, they must be either \quantity{volume/time},
  \quantity{area/time}, \quantity{length/time}, or
  \quantity{dimensionless/time}, depending on whether the compartment's
  \attrib{spatialDimensions} field has value ``\attribvalue{3}'',
  ``\attribvalue{2}'', ``\attribvalue{1}'' or ``\attribvalue{0}'',
  respectively.

  \emph{Restrictions}: No more than one \class{RateRule} can refer to a
...
\item \emph{In the case of a parameter}, a \class{RateRule} sets the rate
  ...
  \attrib{math}.  The units of the formula must work out to be the same as
  those defined for the parameter's initial value, per time (see
  Section~\ref{sec:parameters}).
...
\quantity{time}, \quantity{substance}, \quantity{volume}, \quantity{area}
and \quantity{length} are built-in unit types (see
...
in the order given in SBML.  The \attrib{math} field of an assignment rule
structure can contain any identifier in a MathML \class{ci} element except
for the following: (a) identifiers for which there exists a subsequent
assignment rule, and (b) the identifier for which the rule is defined.
These constraints are designed to eliminate algebraic loops among the
scalar rules; eliminating algebraic loops ensures that assignment rules can
be evaluated any number of times without the result of those evaluations
changing.  As an example, consider the following equations, in the order
shown:
\begin{equation*}
  \begin{array}{lll}
...
of a \attrib{KineticLaw} in a \class{Reaction} is useful in certain kinds
of structural analyses such as elementary mode analysis.  It is true that
the presence of this information in two places (i.e., the rate law and the
flag \attrib{reversible}) leaves open the possibility of a model containing
contradictory information, but the creation of such a model would indicate
an error on the part of the software generating it.  Software developers
must take care to ensure against logical contradictions in the definitions
of reactions.
...
dimensionless units.  It is recommended for maximal interoperability that
this field should simply contain an integer value (see following example).
The \attrib{math} field is optional and defaults to containing an integer
value of 1.
...
and should not appear in the list of reactants or products because it is
...
The field \attrib{math} is a MathML expression that defines the rate of the
reaction.  The expression may refer to species identifiers; in that case,
as discussed in Section~\ref{sec:mathmltokens}, the units of each species
reference are those of \quantity{concentration}.  The overall rate
expression in \attrib{math} has the units of \quantity{substance/time}.
\emph{It is important to note that this is not
  \quantity{concentration/time}}.  The units of reaction rates in SBML are
adjusted to support multi-compartmental models, and differ from the units
used in some conventional single-compartment models as well as from the way
in which classic rate equations are defined.  The typical convention for
rate laws units is \quantity{concentration}/\quantity{time}.  (When two
reactions take place in the same compartment, software tools reading and
writing SBML models may easily convert the \quantity{substance/time} units
to \quantity{concentration/time} by dividing the rate expressions by the
...
authors of BioSketchPad, BioSpice, CellDesigner, Cellerator, CellML,
...
Mark Poolman, Wayne Rindone, Sven Sahle, Takeshi Sakurada, Vijay Saraswat,
Herbert Sauro, James Schaff, Maria Schilstra, John Schwacke, Cliff Shaffer,
Bruce Shapiro, Tom Shimizu, Herbert Sauro, Hugh Spence, J\"{o}rg Stelling,
Kouichi Takahashi, Masaru Tomita, Marc Vass, John Wagner, Jonathan Webb,
J\"{o}rg Weimar, Darren Wilkinson, Olaf Wolkenhauer and Tau-Mu Yi.
