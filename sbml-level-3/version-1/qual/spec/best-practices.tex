...
\hl{The place capacity can be specified using the \token{maxLevel} attribute on a \QualitativeSpecies. However the restriction on the \Transition imposed by this capacity must be explicitly encoded into the \token{math} element of any \FunctionTerm. }
