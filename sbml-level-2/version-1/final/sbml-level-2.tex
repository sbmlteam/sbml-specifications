...
\item Formulas in Level~2 are expressed using MathML~\citep{w3c:2000b}.
  ...
  and~\ref{subsec:kinetic-law}.  The subset of MathML used in SBML includes logical
...
\item The namespace for identifiers in a model does not contain any
  ...
  same namespace as the model's identifiers.  See Section~\ref{sec:mathmltokens}.
...
Figure~\vref{fig:top-level}.  In addition to this all the
\class{listOf}\rule{0.5in}{0.5pt}\class{s} elements in SBML
are derived from \class{SBase}.  The definition of \class{SBase} is presented
...
\attrib{name} and \attrib{math}.  The \attrib{id} and \attrib{name}
...
string field.  MathML \texttt{ci} elements can refer to the function contained in a
\class{FunctionDefinition} using the value of its \attrib{id} field.  The
\attrib{math} field is a container for MathML content that defines the
function.  The content of this field can only be a MathML \texttt{lambda} element.
The function is only available for use in other MathML elements
...
following people for discussions, advice and comments: Hamid Bolouri, Benjamin Bornstein,
