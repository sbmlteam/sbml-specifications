...
this model, the global values for \token{substance},
\token{volume} and \token{time} are declared as \val{substance},
\val{litre} and \val{second}, respectively.  The units of
\token{substance} are declared to be mole units with a scale
...
millimoles per litres.  All the \token{species} elements set the
...
\token{boundaryCondition} attribute set to \val{true}.  This means their
values will be calculated using rules and not reactions.  Also, the
model uses a dummy species $E\sub{total}$ with \token{constant} set to
\val{true} to assign the total concentration of the enzyme.  This could 
...
The value of $S_1$ varies over time and it is on the boundary, so 
in SBML $S_1$ has a \token{constant} attribute with a value of
\val{false} and a \token{boundaryCondition} attribute with a
value of \val{true}.  The
value of $S_2$ is fixed and it is also on the boundary, so $S_2$ has a 
\token{constant} attribute value of \val{false} and a 
\token{boundaryCondition} attribute value of \val{true}. $S_3$ 
is fixed but not on the boundary, so the \token{constant} 
attribute is \val{true} and the \token{boundaryCondition} attribute
is \val{false}.  $S_4$ is a product whose value is
...
\token{LotkaVolterra\_transport}.  The text of the SBML
...
to the meaning of the \token{hasOnlySubstanceUnits} attribute on
