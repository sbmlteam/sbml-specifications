...
used within the \token{sbml} element.  The URI for \sbmltwothree
...
after \class{listOf}\rule{0.5in}{0.5pt}\class{ClassNames}, in part
...
The \Model structure has an optional \token{sboTerm}
...
framework.  When a given \KineticLaw element refers to a rate
law term in SBO, it therefore simultaneously also identifies a
framework.  (It is possible for a model to mix frameworks, which is
why it is important to enable individual reactions to indicate
...
structure, SBML provides a facility for defining units that can be
...
chosen base unit, whereas the \changed{attributes} \token{exponent},
...
Compatibility note: in \sbmltwoone, \Unit had an additional
...
\begin{table}[bht]
  ...
  \caption{The possible values of \token{kind} in a \primtype{UnitKind}
    ...
    quantity \changed{is a ratio whose units cancel out}, and ``\token{item}'' for
...
represented by the \CompartmentType data structure, 
...
analysis software may ignore \CompartmentType structures and
...
\end{wrapfigure}
proteins), and others.  The \SpeciesType data structure is
...
\item The value of the \token{units} \changed{attribute} is used
  as the units of the compartment identifier when it appears as a
...
\item When a \Species is to be treated in terms of concentrations,
  the units of the spatial size of the concentration value (\ie
  the denominator in the units
...
located in a specific \emph{compartment}.  The \Species data
structure is intended to represent these pools.  Its definition is
...
a \changed{compartment's definition logically \emph{should} be the
...
\item The units of the \token{math} element of a \RateRule that
  ...
  attribute and \quantity{time} is the built-in \token{time}
  units.

\item When the parameter identifier appears in other MathML
  expressions in a model, the units associated with the value
  are those defined by the parameter's \token{units} attribute.
...
abstract \Rule structure containing an \changed{element},
...
The \Reaction structure inherits from \SBase the optional
...
structure should have an ``is a'' relationship with the SBO term.
...
\changed{one} or more \Parameter structures (Section~\ref{sec:parameters})
...
numbers in SBML do not have any default units; what is required in
this context is units of time:
