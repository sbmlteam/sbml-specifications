...
for the interrelation between these entities.
...
refer to an `active' \Objective as does the extended \Reaction class which defines two attributes referring to flux capacity constraints. The \GeneProductRef class declares an attribute of type \primtype{SIdRef} which references a \GeneProduct which itself contains an attribute that can refer to a \Species while \newtxt{the \UserDefinedConstraint and \UserDefinedConstraintComponent both also declare attributes of type \primtype{SIdRef}. A more detailed description of each of properties and to what they refer can be found in the relevant class descriptions.}

\subsubsection{Type \primtypeNC{FbcType}}
\label{primtype-fbctype}

\newtxt{The \FBCPackage defines an new enumerated type \primtype{FbcType} that indicates whether the objective function should be maximized or minimized and can have either of two values \val{maximize} or \val{minimize}.}
...
represents the type of variable that can occur in either a \FluxObjective or \UserDefinedConstraintComponent. It can have one of two values, \val{linear} or \val{quadratic}.}
...
\newtxt{The optional attribute charge contains a signed double referring to the Species object’s charge (in terms of electrons, not the SI unit coulombs). Note, that unlike FBC versions one and two a \Species may, in the context of calculating charge, be interpreted as a pseudoisomer or aggregate molecule and may assume a non-integer value. However, non-integer charge values should be used with caution as their usage may have unintended side-effects, for example, the accuracy of reaction balancing operations may be reduced.}
...
%
\newtxt{The \FluxObjective class is a relatively simple container for model variables weighted by a signed numerical coefficient. It is defined as being either a `linear' or `quadratic' component.}
%
\begin{figure}[ht]
  \centering
  % Requires \usepackage{graphicx}
...
\paragraph{The \token{reaction}, \token{reaction2} and \token{coefficient} attributes}
The required \token{reaction} and optional \token{reaction2} attributes are of type \primtype{SIdRef} and are restricted
to refer only to a \Reaction while the \token{coefficient} attribute
holds a \primtype{double} referring to the coefficient that this \FluxObjective
...
represents the type of exponent contained by the \FluxObjective. For example, where $J$ represents a steady-state flux in the \token{reaction} attribute, the \primtype{FbcVariableType} allows the definition of either a \val{linear}, $J^1$ or \val{quadratic}, $J^2$ variable. In addition, a mixed \val{quadratic} component can be defined using the optional attribute \token{reaction2}. For example, with \token{reaction2} represented by $J_{R2}$ and a \token{coefficient}, $C$, a mixed two variable \val{quadratic} \FluxObjective of the form $C \times J^{1} \times J_{R2}^{1}$ can be defined.}
...
%
\newtxt{Note that the LP format (CPLEX) expression of the quadratic term is written as $\frac{2 \times 2 \times R2^2}{2}$ and it is left up to the software to make any necessary translations of these terms to the format required by specific optimization libraries.}
