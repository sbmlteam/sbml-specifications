...
% \newcommand{\otherContentTall} {\fbox{\texttt{\scalefont{1.2}{+++}}}}
% \newcommand{\otherContentNS}   {\fbox{\texttt{+++}}}
\newcommand{\otherContentTall} {\colorbox[rgb]{.88,.94,.88}{\texttt{\scalefont{1.2}{+++}}}}
\newcommand{\otherContentNS}   {\colorbox[rgb]{.88,.94,.88}{\texttt{+++}}}
...
% \newcommand{\relationElementNS}{\colorbox[rgb]{.85,.92,.85}{\texttt{RELATION\_ELEMENT}}}
% \newcommand{\relationElement}  {\relationElementNS\xspace}
\newcommand{\relationElementNS}{\fbox{\texttt{RELATION\_ELEMENT}}}
\newcommand{\relationElement}  {\relationElementNS\xspace}
...
\begin{example}
<\sbmlElement \otherContentTall metaid="\sbmlMetaNS" \otherContent >
  \otherContentTall
  <annotation>
    \otherContentTall
...
identifiers refer to generic strings of a particular type, as
follows: \sbmlElement refers to any SBML element name that can
contain an \token{annotation} element; \sbmlMeta is an XML
\primtype{ID} string; \relationElement refers to element names in
either the namespace
\uri{http://biomodels.net/biology-qualifiers/} or
\uri{http://biomodels.net/model-qualifiers/}; \resourceURI is a
URI (See Section~\ref{sec:uri-in-annotation}); and \history refers
to optional content described in
Section~\ref{sec:model-history-annotation}. `\otherContent' is a
...
the relevant standards for the enclosing elements.
`\moreOfTheSame' is a placeholder for zero or more elements of the
same form as the immediately preceding element.  The precise form
of whitespace and the XML namespace prefix definitions is not
constrained; however, the elements and attributes must be in the
namespaces shown. The rest of this section describes the format
formally in English.
...
\item \uri{http://www.ebi.ac.uk/ego/DisplayGoTerm?selected=GO:0007268}
...
\item \uri{http://www.expasy.org/cgi-bin/nicezyme.pl?3.5.4.4}
...
two symbol namespaces.  One is for model qualifiers, and this one
has the URI \uri{http://biomodels.net/model-qualifiers/} (we use
the prefix \token{bqmodel} in examples shown in this section).
The other namespace is for biological qualifiers; this has the URI
\uri{http://biomodels.net/biology-qualifiers/} (we use the prefix
\token{bqbiol}).  The list will only grow; \ie no element will be
...
\item \token{bqmodel:isDerivedFrom} The modeling object
  ...
  adaptation for usage in a different context.
...
\item \token{bqbiol:hasProperty} The subject of the referenced
...
\item \token{bqbiol:isPropertyOf} The biological entity
  represented by the model component is a property of the
  referenced resource.
...
\item \token{bqbiol:occursIn} The biological entity represented by
...
\colorbox[gray]{.9}{\begin{minipage}{3.75in}\begin{example}
\otherContent
...
profile of (restricted form of) ISO 8601.
