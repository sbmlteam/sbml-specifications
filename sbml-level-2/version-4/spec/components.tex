...
be an SBO identifier referring to a \changed{physical participant} (\ie
...
an SBO identifier referring to a \changed{physical participant} (\ie
terms derived from \sboparticipantphysical).  The relationship is
...
the value must be an SBO identifier referring to a \changed{physical
participant} (\ie terms derived from \sboparticipantphysical).
...
expression could be evaluated at the same time the \changed{assignments} are
...
As of \sbmltwofour, the \token{useValuesFromTriggerTime} attribute
...
\val{true}, which corresponds to the interpretation of event
...
delay.  If \token{useValuesFromTriggerTime}=\val{false}, it means
