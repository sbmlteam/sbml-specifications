...
\changed{\nestedContent refers to other optional RDF elements such as \token{bqbiol:isDescribedBy} that describe a clarification or another annotation about the \relationElement in which it appears.  Nested content allows one to, for example, describe protein modifications on species, or to add evidence codes for an annotation.  As with all annotations, these nested annotations only add additional information, and never change existing information.  As such, they can be ignored without changing the (broader) meaning of the model.}
The string
