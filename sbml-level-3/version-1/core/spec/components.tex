...
a component (\eg the attribute \token{units} on \Parameter) can
...
\item When a \Species is to be treated in terms of concentrations
  ...
  the concentration value (\ie the denominator in the units
...
by whether the \token{initialAmount} or
\token{initialConcentration} attribute is set on a given \Species
object, the fact that both of these \Species attributes are
...
is established by an \InitialAssignment or
...
\item When the identifier of the species appears as a numerical
  quantity in a mathematical formula expressed in MathML
  (discussed in Section~\ref{sec:ci-token}), it represents the 
  species' quantity, and the unit of measurement associated with
  this quantity is as described above.
...
\item \emph{In the case of a species reference}, an
  ...
  formula should be \token{dimensionless}, because reactant and
...
\item \emph{In the case of a species reference}, a \RateRule sets
  ...
  \SpeciesReference object to the value determined by the formula
  in \token{math}.  The unit associated with the value produced by
  the \token{math} formula should be \token{dimensionless} divided
  by \quantity{time} units (Section~\ref{sec:timeunits}).
...
\item \emph{Global conversion factor defined}.  If the \Model
  ...
  \token{conversionFactor} attribute value on \Model (see
...
\quantity{time} as set by the \Model \val{timeUnits}
