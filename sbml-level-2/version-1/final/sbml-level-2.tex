...
biochemical reactions, gene regulation, and many others. Releases
of SBML are termed \emph{levels}.  SBML Level 2 evolved out of
SBML Level 1~\citep{hucka:2001}.  SBML Level 2 is not backwards
compatible with any version of SBML Level 1.

SBML is the result of merging modeling-language features from the
...
developed with the help of the authors of these packages.  As a
result of being based on actual working simulation software, it is
a practical and functional description language.  Our goal in
creating it has been to provide an open standard that will enable
simulation software to exchange models, something that is
currently impossible because there is no standard model exchange
language. We expect SBML models to be encoded using XML, the
eXtensible Markup Language~\citep{bosak:1999,bray:1998}, and we
include here an XML Schema that defines SBML Level~2.
...
\attrib{name} fields remain but are optional and are of type
...
\item \class{Model} has an optional list of \class{Event}
structures which describe the time and form of discontinuous state
changes in the model.  See Section~\ref{sec:events}.
...
\item A model can contain no reactions (a model can be defined
through rules).  See section~\ref{sec:model}.
...
because that would force all SBML identifiers to exist in single
...
namespace, so that a identifier \emph{X} represents the same thing
...
The \class{KineticLaw}, \class{Rule} and \class{Event} types have
\class{math} subselements.  A function definition has a single
MathML \class{lambda} element. These MathML elements should be
...
A model must contain at least one \class{Species} and one
\class{Compartment} data element, but SBML does not impose
restrictions on the total number of these.  A \class{Model} data
object may optionally have lists of \class{MathDefinition},
\class{UnitDefinition}, \class{Parameter}, \class{Reaction},
\class{Event} and \class{Rule} components (they are optional
because the lists in fields \attrib{mathDefinition},
\attrib{unitDefinition}, \attrib{parameter}, \attrib{reaction},
\attrib{event} and \attrib{rule} are permitted to have zero
length).
...
\attrib{name} and \attrib{lambda}. The \attrib{id} and
...
declarations, should be made using \class{Parameter} structures.
...
A \class{Compartment} data object has a \attrib{id} field of type
...
\class{Species} has a \attrib{id} field of type \class{SId} and
...
Table~\vref{tab:unitkind}, the name ``\attrib{volume}'', or a new
...
The optional boolean field \attrib{boundaryCondition} determines
whether the amount of the species is determined by reactions over
the course of a simulation. \attrib{boundaryCondition} defaults to
a value of ``\texttt{false}'', indicating that by default, the
amount can be determined by reactions.

The optional field \attrib{charge} is an integer indicating the
charge on the species (in terms of electrons, not the SI unit
Coulombs).  This may be useful when the species involved is a
...
are two orthogonal dimensions by which rules can be described.
First, there are three different possible functional forms,
corresponding to the following three general cases (where $x$ is a
variable, $f$ is some arbitrary function, and $X$ is a vector of
variables):
...
The second dimension concerns the role of the variable $x$ in the
...
side (as in case 3 in the list above). Future releases of SBML may
...
amongst the scalar rules.

\subsubsection{Example of Rule Use}

The following is an example of the valid use of rules:
...
The following example rule set is invalid:

\begin{example}
<listOfRules>
    <speciesConcentrationRule species="x">
        <notes>
            <xhtml:p>
                this rule, x = x + 1, is invalid on its own
            </xhtml:p>
        <notes>
        <math:math>
            <math:apply>
    ...
    <speciesConcentrationRule species="y">
        <notes>
            <xhtml:p>
                this rule, y = z * 200, is invalid because it
                uses the identifer z which is defined in a later
                scalar rule
            </xhtml:p>
        <notes>
        <math:math>
    ...
    <speciesConcentrationRule species="z">
        <notes>
            <xhtml:p>
                z = y * 100
            </xhtml:p>
...
\class{Event} structure defines when the event can occur, which
...
The operation of an \class{Event} structure is divided into two
...
\class{Event} type is defined in Figure~\vref{fig:event}.  An
...
The \attrib{assignment} field consists of a non-empty list of
\class{assignment} structures.  This field is implemented as a
...
defined in the enclosing \class{Model}-type structure.  (This
species must not be defined with the \attrib{constant} field set
to \texttt{true} and the \attrib{boundaryCondition} field set to
\texttt{false}.)
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
when $P_1 > t$ then $k_2 = 1$ \\
when $P_1 \leq t$ then $k_2 = 0$ \\
\\
initially
\begin{equation*}
  \begin{array}{lllll}
    k_1 = 1 & k_2 = 0 & t = 0.25 & P_1 = 0 & P_2 = 0\\ \\[-4pt]
  \end{array}
\end{equation*}
...
specifically towards representing biochemical network models. SBML
Level~2 provides the most basic facilities that are necessary for
expressing these kinds of models in terms of compartments,
species, reactions, parameters, rules and units.
...
The constructs in CellML tend to be at a more abstract and general
level than those in SBML Level~2, and describes the structure and
underlying mathematics of cellular models in a very general way.
By contrast, SBML is closer to the internal object model used in
model analysis software. Because SBML Level~2 is being developed
in the context of interacting with a number of existing simulation
packages, it is a more concrete language than CellML and may be
better suited to its purpose of enabling interoperability with
...
Level~2, and other related documents are openly available from the
...
Software Platforms for Molecular Biology}, help in July 2002, at
...
especially the authors of BioSpice, DBSolve, E-Cell, Gepasi,
Promot/DIVA, StochSim, and Virtual Cell, and members of the
\texttt{sysbio} mailing list.  We are particularly grateful to the
following people for discussions and knowledge: Dennis Bray, Athel
Cornish-Bowden, David Fell, Carl Firth, Warren Hedley, Martin
Ginkel, Igor Goryanin, Jay Kaserger, Andreas Kremling, Nicolas Le
Nov\`{e}re, Les Loew, Daniel Lucio, Pedro Mendes, Eric Mjolsness,
David Morley, James Schaff, Cliff Shaffer, Bruce Shapiro, Tom
Shimizu, Hugh Spence, Joerg Stelling, Kouichi Takahashi, Masaru
Tomita, and John Wagner.
