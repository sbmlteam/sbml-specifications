...
\changed{Because the SBML Level~3 Multistate and Multicomponent Species (\val{multi}) package can change the mathematical interpretation of a model, the \token{multi:required} attribute must always be set to \val{true}, even if the model is completely empty.  The Graphical Layout (\val{layout})) package, by contrast, cannot ever change the mathematical interpretation of any model (empty or not), and thus its \token{required} attribute must be set to \val{false}.}

  If a package is declared optional \changed{(its \token{required} is set to \val{false})}, it means the
...
\fig{fig:model}.  All of the lists are optional. \changed{If a
given container \emph{is} present within the model, the list may
...
Finally, as mentioned above, unit definitions in SBML \thisLV may have empty \ListOfUnits substructures.  Such units are not fully defined, and a model component that references such an empty unit is taken to have \emph{no} defined unit.  A model with an empty unit definition may indicate an incomplete model or a model that needs external information.
...
Like other classes in SBML \thisLVNC, \Compartment inherits the \token{id} attribute from \SBase; however, \Compartment defines \token{id} as being required rather than optional.  The attribute otherwise behaves as described in \sec{sec:idnameattribs}.
...
\begin{blockChanged}
Like other classes in SBML \thisLVNC, \Species inherits the \token{id} attribute from \SBase; however, \Species defines \token{id} as being required rather than optional.  The attribute otherwise behaves as described in \sec{sec:idnameattribs}.
...
Like other classes in SBML \thisLVNC, \LocalParameter inherits the \token{id} attribute from \SBase; however, \LocalParameter defines \token{id} as being required rather than optional, and in addition, its type is defined to be the derived type of \primtype{LocalSId} instead of \primtype{SId}.  It otherwise behaves as described in \sec{sec:idnameattribs}.
