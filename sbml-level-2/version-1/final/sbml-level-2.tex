...
SBML Level 2 was originally created in part by studying the modeling
facilities provided by the following software systems:
...
\emph{JDesigner}~\citep{sauro:2001b}, \emph{JigCell}~\citep{vass:2003},
...
This document, a downloadable copy of the XML Schema corresponding to SBML
Level~2, and other related documents are openly available from the SBML
project web site, \url{http://www.sbml.org/}.
...
%-----------------------------------------------------------------------------
\subsection{Differences between Level 1 Version 2 and Level 2}
%-----------------------------------------------------------------------------
...
\item All data structures, including \class{Sbml} and
  \class{listOf}\rule{0.5in}{0.5pt} elements, are derived from the type
...
\item A new field, \attrib{id}, replaces the \attrib{name} field previously
  defined for most SBML structures to identify components in a model.  (See
...
\item Formulas in Level~2 are expressed using MathML~\citep{w3c:2000b}.
  ...
  addition, stoichiometry numbers are now also expressed using MathML,
...
\item SBML Level~2 makes explicit a previously unstated assumption, that
  ...
  UTF-8 encoding in their XML prefix.  (Section~\ref{sec:sbml}.)
...
\item The namespace for identifiers in a model does not contain any
  ...
  feature of MathML and are not in the same namespace as any identifiers in
  a model.  (Section~\ref{sec:mathmltokens}.)
...
\item Unlike in SBML Level~1, unit identifiers in Level~2 are in a separate
  ...
  ``\texttt{meter}'' and ``\texttt{liter}'' are not defined in SBML Level~2
  because the user community believed these were unnecessary.  Finally,
  \class{Unit} structures now have the additional attributes
  \attrib{multiplier} and \attrib{offset} to enable the definition of
  non-SI units.  (Section~\ref{sec:unitdefinitions}.)
...
\item The \class{Compartment} structure has a new field,
  ...
  built-in units for area and length.  (Section~\ref{sec:compartments}.)
...
\item The rule structures are simpler compared to SBML Level~1.
  There is no \attrib{type} attribute on \class{Rule}.  The new
  structures \class{AssignmentRule} and \class{RateRule} replace
  \class{ParameterRule}, \class{SpeciesConcentrationRule} and
  \class{CompartmentVolumeRule} from  Level~1. (Section~\ref{sec:rules}.)
...
component in an SBML model.  The definition of \class{SBase} is presented
...
usually required for most structures and is used to identify a component
within the model definition.  Other SBML structures refer to the component
using this identifier.  Section~\ref{sec:id} defines the data type
\class{SId} used for the \attrib{id} field, and
Section~\ref{sec:namespaces} describes the scoping and namespace rules for
these identifiers.
...
in a model.  The primary reason is as follows.  A species in an SBML model
must be located in a compartment, which means that if the same species
appears in multiple compartments (e.g., in the context of a transport
reaction), they must be given different identifiers.  It is currently the
case that users and software differ sharply in philosophy about how to
treat this situation: some treat these as different species, and others
treat them as the same species located in different places.  Those in the
latter group often want to use the same \attrib{name} but have different
\attrib{id} values for the differently localized ``instances'' of the
species.  The lack of restrictions on \attrib{name} values enables SBML to
...
Mathematical expressions in SBML Level~2 are written using
...
laws (Section~\ref{subsec:kinetic-law}), and stoichiometries
(Section~\ref{sec:reactions}).  The \class{KineticLaw} and \class{Rule}
structures each have a single MathML \class{math} subelement, and a
function definition has a single \class{lambda} subelement.  The XML
namespace for all of these elements is the URI
...
The subset of MathML used in SBML Level~2 is similar to that used by
...
\item \emph{trigonometric operators}:
            ...
            \class{arccsc}, \class{arccsch}, \class{arcsec},
...
\subsubsection{Use of Tokens in MathML Expressions in SBML}
...
containing structure in which \class{ci} is used:

\begin{itemize}
  
\item If \class{ci} appears in the body of a function definition, the
  referenced identifier must be either one of the declared arguments to the
  function, or the identifier of a previously defined function.
  
\item If all other situations, the referenced identifier must be the
  identifier of a species, compartment, parameter or function declared in
  the model.  There are only a few possible interpretations of using such
  an identifier in SBML:

  \begin{itemize}
    
  \item \emph{Species identifier}: When a species identifier occurs in a
    \class{ci} element, it represents the concentration of the species.
    The units of a species symbol is \quantity{substance}/\quantity{size}
    where \quantity{size} is either \quantity{volume}, \quantity{area} or
    \quantity{length} when the value of the containing compartment's
    \attrib{spatialDimensions} is 3, 2, or 1, respectively.  If the
    compartment's spatial dimension is 0, then the species symbol's units
    is \quantity{substance}.
  ...
  \item \emph{Compartment identifier}: When a compartment identifier occurs
    ...
    The units can only be one of \quantity{volume}, \quantity{area},
    \quantity{length}, or \quantity{dimensionless} when the value of the
    compartment's \attrib{spatialDimensions} is 3, 2, 1, or 0 respectively.
  ...
  \item \emph{Parameter identifier}: When a parameter identifier occurs in
    ...
    parameter.  The units are the units assigned to the parameter in its
    definition; see Section~\ref{sec:parameters}.
  ...
  The units of \emph{substance}, \emph{volume}, \emph{area} and
  \emph{length} are determined from the built-ins \texttt{substance},
  \texttt{volume}, \texttt{area} and \texttt{length}, respectively, of
...
\class{csymbol} should be set to \texttt{SBML}.  The \attrib{definitionURL}
...
two required attributes: \attrib{version} and \attrib{level}.  For SBML
Level~2 Version~1, these attributes must be set to ``\texttt{1}'' and
``\texttt{2}'', respectively.  (The \texttt{version} attribute is present
...
be given abbreviated names and so that the default units can be redefined.
This is the motivation behind the \class{UnitDefinition} data structure,
whose definition is shown in Figure~\vref{fig:unitdefinition}.
...
namespace of other identifiers in a model; thus, unit names cannot collide
with the names of species, compartments, reactions etc.

The approach to defining units in SBML is compositional; for example,
$meter\ second^{\,-2}$ is constructed by combining a \class{Unit}-type
element representing $meter$ with a \class{Unit}-type element representing
$second^{\,-2}$.  The \class{Unit} data structure has one required field,
\attrib{kind}, whose value must be taken from \class{UnitKind}, an
enumeration of units.  This means thats the unit definition feature in SBML
is not hierarchial: unit definitions cannot be composed from other unit
definitions.  SBML differs from CellML in this respect: CellML does allow
the construction of hierarchial unit definitions.  The possible values of
\class{UnitKind} are listed in Table~\vref{tab:unitkind}.
...
An instance of a \class{Unit} definition represents a transformation
starting with a base unit from \class{UnitKind}.  The formula for a single
transformation is as follows (where $u$ is the original base unit and
$u_\emph{new}$ is the new unit):
\begin{equation*}
  u_\emph{new} = (\emph{multiplier} \times 10^\emph{scale} \times u^\emph{exponent}) + \emph{offset}
\end{equation*}
...
must be an integer exponent on a power of ten multiplier used to set the
scale of the unit.  For example, a unit that has a \attrib{kind} value of
``\texttt{gram}'' and a \attrib{scale} value of ``\texttt{-3}'' signifies
$10^{-3} * gram$, or milligrams.  The default value of \texttt{scale} is
``\attribvalue{0}'' (zero), because $10^0 = 1$.

The optional \attrib{multiplier} field can be used to multiply the
\attrib{kind} unit by a real-numbered factor; this is necessary for
defining non-SI units starting with SI units.  For instance, a multiplier
of 0.3048 could be used to define ``foot'' as a measure of length in terms
of a metre.  The \attrib{multiplier} field has a default value of
``\attribvalue{1}'' (one).  Finally, the \attrib{offset} field is used to
represent the addition of a constant in the transformation the
...
The composition of $n$ \class{Unit} structures to create a more complex
unit definition involves linear chaining according to the following
formula:
\begin{equation*}
  u_\emph{new} = (m_1 \times \ldots \times m_n
  \times 10^{s_1} \times \ldots \times 10^{s_n} \times u^{e_1} \times \ldots \times u^{e_n})
\end{equation*}
...
Table~\ref{tab:builtin}, corresponding to the five types of quantities that
play roles in biochemical reactions: amount of substance, volume, area,
length and time.  SBML defines default units for these quantities, all with
a default \texttt{scale} value of zero.  The various components of a model,
...
five predefined names ``\texttt{substance}'', ``\texttt{time}'',
``\texttt{length}'', ``\texttt{area}'', and ``\texttt{volume}'' from
Table~\ref{tab:builtin}.  The latter usage signifies that the units to be
used should be the designated defaults.
...
\begin{table}[htb]
  ...
  \begin{tabular}{ll>{\ttfamily}l}
    \toprule
    \textbf{Name} & \textbf{Default Units}\\
    \midrule
    \texttt{substance}  & mole\\
    \texttt{volume} & litre\\
    \texttt{area}   & square metre\\
    \texttt{length} & metre\\
    \texttt{time}   & second\\
    \bottomrule
  \end{tabular}
...
``\texttt{substance}'', ``\texttt{length}'',
``\texttt{area}'', ``\texttt{time}'', and ``\texttt{volume}'' in a
...
The maximum value of \attrib{spatialDimensions} is 3; its default value is
also 3, meaning that by default, a compartment is a three-dimensional
volume.

A compartment also has an optional floating-point field named
...
the absence of geometry information.  The default units of \attrib{size}
depends on the value of the \attrib{spatialDimensions} attribute according
to the following rule: for spatial dimensions of 3, 2, 1 or 0,
\attrib{size} has the default units of \texttt{volume}, \texttt{area},
\texttt{length} and \texttt{dimensionless}, respectively.  (See
Tables~\vref{tab:builtin} and~\vref{tab:unitkind}.)  A missing value for
\attrib{size} for a given compartment signifies that the value is either
unknown, not required for analysis, or available from an external data
source.  Note in particular that in SBML Level~2, a missing \attrib{size}
value does \emph{not} imply that the compartment size is 1.  (This is
unlike the definition of compartment \attrib{volume} in SBML Level~1.)
...
The units of compartment size (i.e., the units of the \attrib{size} field),
may be explicitly set using the optional field \attrib{units} in
\class{Compartment}.  The named units must be either one of the base units
from Table~\vref{tab:unitkind}, the built-in defaults named
\texttt{volume}, \texttt{area}, \texttt{length}, or a new unit defined by a
unit definition in the enclosing model.
...
relationships between the compartments.  (As discussed in
Section~\ref{sec:level-3}, we expect that SBML Level~3 will introduce the
...
A final note concerning the dependency between species and compartments:
species are required to be located in a compartment in an SBML model.
Thus, a \class{Compartment} structure must be present in a model if that
model contains a \class{Species} structure.
...
\attrib{name} field of type \class{string}.  The required field
\attrib{compartment}, also of type \class{SId}, is used to identify the
compartment in which the species is located.  There is no default value for
the \attrib{compartment} field.  The field must instead refer to a
\class{Compartment} structure by the value of that structure's \attrib{id}
field.  A \class{Compartment} structure must be present in a model if that
model contains a \class{Species} structure.

The optional fields \attrib{initialAmount} and
\attrib{initialConcentration}, both of data type \class{double}, are used
to set the initial quantity of the species in the named compartment.  These
attributes are mutually exclusive and only one these attributes can have a
value on any given instance of a \class{Species} structure.  Missing
\attrib{initialAmount} or \attrib{initialConcentration} values implies that
their values are either unknown, not required for analysis or available
from an external data source.  \attrib{initialConcentration} must not have
a value if the species' compartment has a spatial dimension of 0.

The units of \attrib{initialAmount} or \attrib{initialConcentration} may be
set explicitly using the optional field \attrib{units}.  The units that can
be used depend on whether amounts or concentrations are being used for a
given species:
\begin{itemize}
  
\item \emph{Case of \attrib{initialAmount} being used}. The value assigned to
  \attrib{units} must be chosen from one of the following possibilities:
  one of the unit names from Table~\vref{tab:unitkind}, the name
  ``\attrib{substance}'', or a new unit name defined by a unit definition
  in the enclosing model.  If absent, the units default to the value set by
  the built-in ``\attrib{substance}'' of Table~\vref{tab:builtin}.
  
\item \emph{Case of \attrib{initialConcentration} being used}.  The value
  assigned to \attrib{units} must be chosen from one of the following
  possibilities: one of the unit names from Table~\vref{tab:unitkind}, or a
  new unit name defined by a unit definition in the enclosing model.  If
  absent, the units default to $substance/volume$, $substance/area$ or
  $substance/length$ units, depending on whether the species' compartment
  has spatial dimensions of 3, 2 or 1, respectively.  $substance$,
  $volume$, $area$ and $length$ are taken from Table~\vref{tab:builtin}.
\end{itemize}

The \class{Species} structure has another optional boolean field named
...
\attrib{constant} fields.  In practice, a \attrib{boundaryCondition} value
of ``\attribvalue{true}'' means that a differential equation derived from
the reaction definitions should not be generated for the species.  The
example model in section~\ref{sec:constantspecieseg} contains all four
possible combinations of the \attrib{boundaryCondition} and
\attrib{constant} attributes on \class{species} elements.
...
\class{Parameter} has an \attrib{id} field of type \class{SId} and an
optional \attrib{name} field of type \class{string}.  The symbol in the
\attrib{id} field identifies the parameter.  The optional field
...
either unknown, not required for analysis or available from an external
data source.  The units of the parameter \attrib{value} are specified by
the field \attrib{units}.  The value assigned to \attrib{units} must be
...
appearing in first column of Table~\ref{tab:builtin}); or the name of a new
...
constant.  Parameters local to a reaction should not have a
\attrib{constant} attribute value.
...
variable, $f$ is some arbitrary function, and $W$ is a vector of variables
that may include $x$):
\begin{center}
\begin{tabular}{rll}
(Algebraic rule)    & left-hand side is zero:             & $0 = f(W)$\\
(Assignment rules)  & left-hand side is a scalar:         & $x = f(W)$\\
...
of a species (to set its concentration), or a parameter name (to set its
value).
...
the other cases.  An example of the use of \class{AlgebraicRule} structures
is given in Section~\ref{sec:algeraiceg}.

\subsubsection{\class{AssignmentRule}}

The rule type \class{AssignmentRule} is used to express equations that
set the value of variables.  The left-hand sides of assignment rules
can be the identifiers of species, compartments, or parameters.  The
effects are in general terms the same, but have different effects depending
on which variable is being set:

\begin{itemize}
  
\item \emph{In the case of a species}: the rule sets the referenced
  species's concentration to the value determined by the formula in
  \attrib{math}.  We use the term `concentration' broadly here so that
  the units for the rule are in terms of $substance/size$, where the
  $size$ units are either $volume$, $area$ or $length$ when the species'
  compartment spatial dimension is 3, 2 or 1 respectively.  If the
  compartment's spatial dimension is 0 then the rule's units are
  $substance$.
  
  \emph{Restrictions}: An assignment rule and a \class{SpeciesReference}
  structure (see Section~\ref{sec:reactions}) cannot refer to the same
  species.  This means that an assignment rule cannot be defined for a
  species that is created or destroyed in a reaction.  The only exception
  is when the given species is a boundary condition; i.e., on the
  \class{Species} structure that defines the species the
  \attrib{boundaryCondition} field is set to ``\attribvalue{true}''.  No
  more than one \class{AssignmentRule} can refer to a given species in an
  SBML model definition.
  
\item \emph{In the case of a compartment}: the rule sets the referenced
  compartment's size to the size determined by the formula in
  \attrib{math}.  The units are either $volume$, $area$ or $length$,
  depending on whether the compartment's \attrib{spatialDimensions} field
  has value 3, 2 or 1, respectively.
  
  \emph{Restrictions}: No more than one \class{AssignmentRule} can refer to
  a given compartment in an SBML model definition.
  
\item \emph{In the case of a parameter}: the rule sets the referenced
  parameter's value to that determined by the formula in \attrib{math}.
  The units are the same as those used for the parameters initial value.
...
$substance$, $volume$, $area$ and $length$ are built-in unit types (see
...
the rates of change of variables.  The left-hand sides of rate rules can be
the identifiers of species, compartments, or parameters.  The effects are
in general terms the same, but have different effects depending on which
variable is being set:

\begin{itemize}
  
\item \emph{In the case of a species}: the rule sets the rate of change of the species's
  concentration to the value determined by the formula.  We use the term `concentration'
  broadly here so that the units are in terms of $substance/size/time$, where the $size$
  units are either $volume$, $area$ or $length$ when the species' compartment spatial
  dimension is 3, 2 or 1 respectively.  If the compartment's spatial dimension is 0 then
  the rule's units are $substance$. 
  
  \emph{Restrictions}: An rate rule and a \class{SpeciesReference} structure (see
  Section~\ref{sec:reactions}) cannot refer to the same species.  This means that an
  assignment rule cannot be defined for a species
  ...
  structure that defines the species the \attrib{boundaryCondition} field is
  set to ``\attribvalue{true}''.  No more than one \class{RateRule} can refer to
  a given species in an SBML model definition.
  
\item \emph{In the case of a compartment}: the rule sets the rate of change of the compartment's
  volume to the volume determined by the formula.  The units are units are either $volume/time$,
  $area/time$ or $length/time$ when the compartment's spatial dimension is 3, 2 or 1 respectively.
  
  \emph{Restrictions}: No more than one \class{RateRule} can refer to
  a given compartment in an SBML model definition.
  
\item \emph{In the case of a parameter}: the rule sets the rate of change of the parameter's
  value to that determined by the formula.  The units are the same as those used
  for the parameters initial value per time.
...
$time$, $substance$, $volume$, $area$ and $length$ are built-in unit types (see Table~\ref{tab:builtin}).
...
In SBML, no more than one assignment or rate rule can be defined for a given
identifier.  No assignment or rate rule can be defined for an identifier whose
...
is an assignment rule for that identifier.  For example, there must be
a \texttt{Parameter} structure for a given parameter if there is a rule for
that parameter.

The ordering of assignment rules is significant: they are always
evaluated in the order given in SBML.  The \attrib{math} field of an assignment
rule structure can contain any identifier in a MathML
\class{ci} element except for the following: (a) identifiers for which
there exists a subsequent assignment rule, and (b) the identifier for
...
Eliminating these algebraic loops ensures that assignment rules can be
...
assumed to be located elsewhere in the model and not shown here):
\begin{example}
<model>
    ...
    <listOfRules>
...
\class{Model}-type structure.  The stoichiometry for the products and
reactants can be specified using the \attrib{math} field of the
\class{SpeciesReference} structure. This field contains a expression in
dimensionless units.  It is recommended for maximal interoperability that this field
should simply contain an integer value (see following example).
The \attrib{math} field is optional and defaults to containing an
integer value of 1.

The following is a simple example of a species reference for species ``X0'', with
stoichiometry 2, in a list
...
These units, adopted to support multi-compartmental models, may differ from the way
conventional single compartment models are encoded and differs from the way in which
classic rate equations are composed.  The typical convention for rate laws units is $concentration$/$time$.
As in all MathML expressions in SBML the units for species symbols is $concentration$
(Section~\ref{sec:formulas} discusses the use of MathML in SBML Level~2 describes this in more detail) .
...
\subsubsection{\attrib{id} and \attrib{name}}
...
It is entirely possible for two events to be \emph{executed} simultaneously.  The order in which
these events are \emph{executed} can be significant. SBML Level 2 does not define the algorithm
for determining this order (the tie-breaking algorithm).  As a result the results of simulations
involving events may vary when simultaneous events occur during simulation.  It is anticipated
that future versions or levels of SBML will define a specific set of tie-breaking algorithms and
a mechanism for models to indicate which algorithm should be applied during simulation.
...
%-----------------------------------------------------------------------------
\subsection{Use of \class{RateRule} with \attrib{boundaryCondition} and \attrib{constant} attributes on \class{species} elements in a Model}
...
\subsection{Use of 2 dimensional compartments in a model}

The following example is a model which includes 2 dimensional compartments.
...
<sbml xmlns="http://www.sbml.org/sbml/level2" level="2" version="1">
   <model id="facilitated_ca_diffusion">
      <listOfCompartments>
         ...
         <compartment id="NuclearMembrane" outside="Cytosol" spatialDimensions="2" />
         <compartment id="Nucleus" outside="NuclearMembrane" spatialDimensions="3" />
      </listOfCompartments>
      <listOfSpecies>
         ...
         <species id="Ca_N" compartment="Nucleus" initialConcentration="0.1"/>
         <species id="Ca_EC" compartment="Extracellular" initialConcentration="1000"/>
         <species id="CaBP_N" compartment="Nucleus" initialConcentration="202.83"/>
         <species id="CaBPB_N" compartment="Nucleus" initialConcentration="47.17"/>
         <species id="Ca_C" compartment="Cytosol" initialConcentration="0.1"/>
      ...
      <listOfRules>
         <parameterRule type="scalar" id="J_Calbindin" formula="(Pcalbindin * (CaBP_C - CaBP_N))"/>
         <parameterRule type="scalar" id="J_Ca_Pump" formula="((Vmax * kP * ((Ca_C - Ca_Rest) / (Ca_C + kP)) / (Ca_Rest + kP)) * CaPump_PM)"/>
         <parameterRule type="scalar" id="Vmax" formula=" - 4000.0"/>
         <parameterRule type="scalar" id="J_CalbindinBound" formula="(Pcalbindin_bound * (CaBPB_C - CaBPB_N))"/>
         <parameterRule type="scalar" id="J_Ca_channel" formula="(J0 * Kc * (Ca_EC - Ca_C) / (Kc + Ca_C) * CaCh_PM)"/>
      </listOfRules>
      <listOfReactions>
         <reaction id="CalciumCalbindin_gt_BoundCytosol" fast="true">
            <listOfReactants>
               <speciesReference specie="CaBP_C"/>
               <speciesReference specie="Ca_C"/>
            </listOfReactants>
            <listOfProducts>
               <speciesReference specie="CaBPB_C"/>
            </listOfProducts>
            <kineticLaw formula="(((Kf_CalciumCalbindin_BoundCytosol * CaBP_C) * Ca_C) - (Kr_CalciumCalbindin_BoundCytosol * CaBPB_C))">
               <listOfParameters>
         ...
         <reaction id="CaciumCalbindin_gt_BoundNucleus" fast="true">
            <listOfReactants>
               <speciesReference specie="Ca_N"/>
               <speciesReference specie="CaBP_N"/>
            </listOfReactants>
            <listOfProducts>
               <speciesReference specie="CaBPB_N"/>
            </listOfProducts>
            <kineticLaw formula="(((Kf_CaciumCalbindin_BoundNucleus * Ca_N) * CaBP_N) - (Kr_CaciumCalbindin_BoundNucleus * CaBPB_N))">
               <listOfParameters>
                  <parameter id="Kf_CaciumCalbindin_BoundNucleus" value="20.0"/>
                  <parameter id="Kr_CaciumCalbindin_BoundNucleus" value="8.6"/>
               </listOfParameters>
            </kineticLaw>
         </reaction>
         <reaction id="CalciumBuffer_gt_BoundCytosol" fast="true">
            <listOfReactants>
               <speciesReference specie="Ca_C"/>
               <speciesReference specie="B_C"/>
            </listOfReactants>
            <listOfProducts>
               <speciesReference specie="CaB_C"/>
            </listOfProducts>
            <kineticLaw formula="(((Kf_CalciumBuffer_BoundCytosol * Ca_C) * B_C) - (Kr_CalciumBuffer_BoundCytosol * CaB_C))">
               <listOfParameters>
         ...
         <reaction id="Calcium">
            <listOfReactants />
            <listOfProducts />
            <kineticLaw formula="(Pcalcium * (Ca_C - Ca_N))">
               <listOfParameters>
                  <parameter id="Pcalcium" value="150.0"/>
               </listOfParameters>
            </kineticLaw>
         </reaction>
         <reaction id="Calbindin">
            <listOfReactants />
            <listOfProducts />
            <kineticLaw formula="J_Calbindin">
               <listOfParameters>
                  <parameter id="Pcalbindin" value="30.0"/>
               </listOfParameters>
            </kineticLaw>
         </reaction>
         <reaction id="Ca_Pump">
         ...
         <reaction id="CalbindinBound">
            <listOfReactants />
            <listOfProducts />
            <kineticLaw formula="J_CalbindinBound">
               <listOfParameters>
                  <parameter id="Pcalbindin_bound" value="30.0"/>
               </listOfParameters>
            </kineticLaw>
         </reaction>
         <reaction id="Ca_channel">
            <listOfReactants />
            <listOfProducts />
            <kineticLaw formula="J_Ca_channel">
               <listOfParameters>
...
authors of BioSketchPad, BioSpice, Cellerator, COPASI, DBSolve, E-Cell,
ESS, Gepasi, Jarnac, JDesigner, JigCell, MCell, NetBuilder, Promot/DIVA,
...
following people for discussions, advice and comments: Nicolas Allen, Adam Arkin, Hamid
Bolouri, Benjamin Bornstein, Dennis Bray, Roger Brent, Steve Burbeck, Claudine Chaouiya,
...
Kaserger, Sarah Keating, Hiroaki Kitano, Ben Kovitz, Andreas Kremling, Nick Juty, Nicolas
Le~Nov\`{e}re, Fred Livingston, Les Loew, Daniel Lucio, Joanne Matthews, Mike McCollum,
Pedro Mendes, Eric Minch, Eric Mjolsness, David Morley, Mineo Morohashi,
Poul Neilsen, Greg Peterson, Mark Poolman, Wayne Rindone, Sven Sahle, Takeshi Sakurada,
Vijay Saraswat, Herbert Sauro, James Schaff, Maria Schilstra, John Schwacke, Cliff Shaffer, Bruce Shapiro,
Tom Shimizu, Herbert Sauro, Hugh Spence, J\"{o}rg Stelling, Kouichi
Takahashi, Masaru Tomita, Marc Vass, John Wagner, Jonathan Webb, J\"{o}rg Weimar and Olaf
Wolkenhauer.
...
<xsd:schema
    ...
    <xsd:simpleType name="SId">
        <xsd:annotation>
            <xsd:documentation>
                The type SId is used throughout SBML as the
