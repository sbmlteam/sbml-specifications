...
\newcommand{\SimpleSpeciesReference}{\class{\hyperref[fig:reaction]{SimpleSpeciesReference}}\xspace}
...
\newcommand{\sbomathformula}{\emph{placeholder: sbo initial assignment term}\xspace}
...
\newcommand{\sbmltwoone}{SBML Level~2 Version~1\xspace}
\newcommand{\sbmltwotwo}{SBML Level~2 Version~2\xspace}
...
\sbmltwoone documents may need changes to their \token{annotation} elements to
comply with the new specification.  See Section~\ref{sec:annotation-use} for
more details.

\item FIXME-NOT-CHANGED-YET-FIXME SBML Level~2 Version~2 restricts the use of \token{offset}
attributes and \token{Celsius} within \token{unitdefinition}
...
\changed{SBML is designed to encode quantitative and qualitative models
of biochemical reaction networks.  The model representation scheme is
...
on models of biochemical reaction networks, SBML is also capable of
encoding quantitative models composed of first-order ordinary
...
notation and its mapping to XML Schema.  More details are
...
\item \class{Class}: Names of nonabstract classes begin with a
...
\item \token{SomeThing}, \token{otherThing}: Fields within
  ...
  XML Schema does not follow any convention and primitive XML
...
object classes such as \SBase into XML Schema complex types.  When
...
\item Appendix~\ref{apdx:mathml-subset-schema} provides the XML schema for
  the subset of MathML used in SBML; and
...
% FIXME 2006-03-19 the following definition of compartment type is still funny
...
\item \emph{Parameter}: A quantity with a symbolic name.  SBML Level~2
  provides the ability to define \emph{parameters} that are global to a model as
  well as \emph{parameters} that are local to a single reaction.
  In SBML a \emph{parameter} can either be a constant or a
  variable.
...
in XML Schema~\citep{biron:2000,fallside:2000,thompson:2000}.  A
few other primitive types are defined in SBML.  What follows is a
summary of the XML Schema types and the definitions of the
SBML-specific types.
...
The XML Schema type \primtype{string} is used to represent
...
The XML Schema type \primtype{boolean} is used to represent the
mathematical concept of binary-valued logic.  The permitted
literal value of a data field having type \primtype{boolean} are
the following: \token{true}, \token{false}, \token{1}, and
\token{0}.
...
The XML Schema type \primtype{integer} is used to represent
integer numbers.  The literal representation of an integer is a
finite-length sequence of decimal digit characters with an
optional leading sign.  There is no restriction on the absolute
size of integer values in object fields in the SBML XML namespace.


\subsubsection{Type \primtype{positiveInteger}}
\label{sec:positiveinteger}

The XML Schema type \primtype{positiveInteger} is used to
represent nonzero, nonnegative integers: 1, 2, 3, ....  The
literal representation of an integer is a finite-length sequence
of decimal digit characters, optionally preceded by a positive
sign (``\token{+}'').  There is no restriction on the absolute
size of integer values in object fields in the SBML XML namespace.
...
The XML Schema type \primtype{double} is the data type of floating
point fields in SBML objects.  It is restricted to IEEE
...
The XML Schema type \primtype{ID} is identical to the XML 1.0 type
\primtype{ID}.  This type consists of strings of characters
restricted as shown in Figure~\ref{fig:id}.
...
\begin{figure}[htb]
  ...
  \caption{The definition of the XML Schema type \primtype{ID}
    ...
    \texttt{(} and \texttt{)} are used for grouping, the
    character \texttt{*} indicates ``zero or more times'', and the
    character \texttt{|} indicates ``or''.  The
    productions \token{CombiningChar} and \token{Extender} are
    relatively long lists of Unicode characters and are omitted
    here; please consult the XML 1.0
    specification~\protect\citep{bray:2000} for the complete
    definition.}
  \label{fig:id}
\end{figure}
...
document.  All such identifiers are considered to reside in a
global namespace covering the entire XML document.
...
restrictions about the types of characters permitted and the
...
\begin{figure}[hbt]
  ...
  \caption{The definition of the type \primtype{SId}.  Please see
    Figure~\protect\ref{fig:id} for an explanation of the notation.}
  \label{fig:sid}
\end{figure}
...
\primtype{ID} type.  Using XML's \primtype{ID} would force all
SBML identifiers to exist in a single global namespace, which
would affect not only the form of local parameter definitions but
also future extensions for supporting model/submodel composition.
Further, the use of the \primtype{ID} type for SBML identifiers
would have limited utility because MathML \token{ci} elements are
not of the type \primtype{IDREF} (see Section~\ref{sec:formulas}).
Since the \primtype{IDREF}-\primtype{ID} linkage cannot be
exploited in MathML constructs, the utility of the XML
\primtype{ID} type is greatly reduced.
...
\begin{figure}[htb]
  ...
  \color{black}
  \caption{The definition of \primtype{SBOTerm}
    expressed in the variant of BNF used by the XML 1.0
    specification~\protect\citep{bray:2000}.  The
    \primtype{SBOTerm} type consists of strings beginning with
    \token{SBO:} and followed by seven decimal digits.}
  \label{fig:sboterm}
\end{figure}

Examples of valid string values of type \primtype{SBOTerm} are
``\token{SBO:0000014}'' and ``\token{SBO:00003204}''.  These
values are meant to be terms from an ontology whose vocabulary
describes entities and processes in computational models.
Section~\ref{sec:sboTerm} provides more information about the
ontology and principles for the use of these terms in SBML models.
...
The field \token{notes} in \SBase is a container for XHTML content.
It is intended to serve as a place for storing optional information
intended to be seen by humans.  Typically, the \token{notes} field will contain
user comments about the structure in which the \token{notes} field is enclosed.
Every data object derived directly or
indirectly from type \SBase can have a separate value for
\token{notes}, allowing users considerable freedom when adding comments to their
models.  Section~\ref{sec:examples} provides examples of using
\token{notes} in different models.
...
\item Within a given SBML \token{annotation} element, there can only be
      one top-level element using a given namespace.

\item No top-level element in an \token{annotation} may use an SBML XML
      ...
      (As of \sbmltwotwo, the defined SBML namespaces are the following URIs:
...
\item The ordering of top-level elements within a given \token{annotation}
      element is \emph{not} significant.  An application should
      not expect that its annotation content appears first in the
      \token{annotation} element, nor in any other particular location.
...
``\texttt{http://www.mysim.org/}'' may appear as though they are
(e.g.) Internet addresses, but there are not the same thing.  This
style of URI strings, using a domain name and other parts, is only
a simple and commonly-used way of creating a unique name string.
...
%
%The field \attrib{definitionURL} in \SBase, when present,
...
%\attrib{definitionURL} value on an SBML \Reaction object could point
...
% FIXME change this to talk about identifier scoping rather than namespaces
...
\item The identifiers (i.e., the values of the field \token{id})
  of functions, compartment types, compartments, species types,
  species, reactions, simple species references, events and
  model-level parameters reside in the same global namespace.
  This means, for example, that a reaction and a species
  definition cannot both have the same identifier.

\item Each reaction definition (see Section~\ref{sec:reactions})
  establishes a private local namespace for local parameter
  identifiers.  Within the definition of a given reaction, local
  parameter identifiers introduced in that reaction override
  (shadow) identical identifers in the global namespace.
...
namespaces for parameters internally should, in principle, be able to accept SBML
...
can perform a simple manipulation of the identifiers of local
parameter elements within reaction definitions to avoid name
collisions.  (An example approach for the latter would be the
following: when receiving an SBML-encoded model, prefix each
parameter identifier inside each reaction with a string
constructed from the reaction's identifier; when writing an
SBML-encoded model, strip off the prefix.)
...
models.  This capability will have to be built on top of SBML
Level~2's namespace organization.  A straightforward approach to
...
\item The \SBase abstract type is used as the base type for
  ...
  because they are subordinate to other structures that \emph{do}
  have identifiers.  If instead \SBase had an \token{id} field,
...
\item \emph{token}: \token{cn}, \token{ci}, \token{csymbol}, \token{sep}

\item \emph{basic content}: \token{apply}, \token{piecewise},
\token{piece}, \token{otherwise}

\item \emph{relational operators}:
            \token{eq}, \token{neq}, \token{gt}, \token{lt}, \token{geq}, \token{leq}

\item \emph{arithmetic operators}:
            \token{plus}, \token{minus}, \token{times},
            \token{divide}, \token{power}, \token{root},
            \token{abs}, \token{exp}, \token{ln}, \token{log},
            \token{floor}, \token{ceiling}, \token{factorial}

\item \emph{logical operators}:
            \token{and}, \token{or}, \token{xor}, \token{not}

\item \emph{qualifiers}:
            \token{degree}, \token{bvar}, \token{logbase}

\item \emph{trigonometric operators}:
            \token{sin}, \token{cos}, \token{tan}, \token{sec}, \token{csc}, \token{cot},
            \token{sinh}, \token{cosh}, \token{tanh}, \token{sech}, \token{csch}, \token{coth},
            \token{arcsin}, \token{arccos}, \token{arctan}, \token{arcsec}, \token{arccsc}, \token{arccot},
            \token{arcsinh}, \token{arccosh}, \token{arctanh}, \token{arcsech}, \token{arccsch}, \token{arccoth}

\item \emph{constants}:
            \token{true}, \token{false}, \token{notanumber},
            \token{pi}, \token{infinity}, \token{exponentiale}

\item \emph{annotation}:
            \token{semantics}, \token{annotation},
            \token{annotation-xml}
\end{itemize}
...
\begin{blockChanged}
\begin{itemize}\setlength{\parskip}{-0.2ex}
\item \token{style}, \token{class} and \token{id} on any
element;

\item \token{encoding} on \token{annotation},
\token{annotation-xml}, \token{csymbol} and \token{semantics} elements;

\item \token{definitionURL} on \token{annotation},
\token{annotation-xml}, \token{ci}, \token{csymbol} and \token{semantics} elements; and
...
In MathML, the content of elements such as \token{cn} (described below) can
...
The MathML ``\token{e-notation}'' requires the mantissa and exponent to
...
is \emph{not valid} in MathML~2.0.  In the SBML XML namespace,
...
URI (e.g., ``\texttt{http://www.sbml.org/sbml/level1}'') because it was
...
\begin{figure}[htb]
  ...
  \begin{classbox}{Model}
    ...
    \changed{compartmentType: CompartmentType[0..*]}     \\
    compartment: Compartment[0..*]                       \\
    \changed{speciesType: SpeciesType[0..*]}             \\
    species: Species[0..*]                               \\
    parameter: Parameter[0..*]                           \\
    \changed{initialAssignment: InitialAssignment[0..*]} \\
    rule: Rule[0..*]                                     \\
    \changed{constraint: Constraint[0..*]}               \\
    reaction: Reaction[0..*]                             \\
    event: Event[0..*]                                   \\
    \changed{sboTerm: SBOTerm \{ use="optional" \}}      \\
  \end{classbox}
...
\CompartmentType, \Compartment, \SpeciesType,
\Species,
...
the form \token{listOf}\rule{0.5in}{0.5pt}\token{s}, where the
...
model, the value must be an SBO term identifier referring to a
modeling framework defined in SBO (i.e., terms derived from
\sbomodellingframework).  The SBO term chosen should be the most
precise (narrow) one that defines the mathematical framework
assumed by the model.  An example framework might be ``continuous
deterministic'', which would mean the model was created under the
assumption it will be simulated in a tool that turns the set of
reactions into a system of coupled differential equations.
...
% FIXME 2nd sentence is confusing.  should probably talk about <ci>?
...
MathML \token{apply} elements.  \FunctionDefinition is shown in
...
\FunctionDefinition using the value of its \token{id}
field.}
...
\FunctionDefinition structure in an SBML model.  (These
...
The SBML \UnitDefinition  and
\Unit structures enable combinations of units to
be given abbreviated names, and enable built-in units to be redefined.
The definitions of \UnitDefinition  and
\Unit are shown in
...
\subsubsection{The \UnitDefinition Structure}
\label{sec:unitdefinition-structure}

An instance of a \UnitDefinition consists of an \token{id}
field of type \primtype{SId}, an optional string field \token{name}
and a list of structures of type \Unit.  As mentioned in
Section~\ref{sec:identifiers}, unit identifiers defined by the
\token{id} field are considered to be in a separate global
...
of species, compartments, reactions, etc. \changed{The \token{id}
field must not contain a value from Table~\ref{tab:unitkind}, the
table of \primtype{UnitKind} values, ensuring that the units listed
...
$meter$ with a \Unit structure representing $second^{\,-2}$.
The \Unit structure is described in the next subsection.


\subsubsection{The \Unit Structure}
\label{sec:unit-structure}

The \Unit data structure has one required field,
\token{kind}, whose value must be taken from \primtype{UnitKind}, an
enumeration of base units.  The possible values of
\primtype{UnitKind} are given in Table~\vref{tab:unitkind}.

%\changed{A \Unit data structure is a component of a
%\UnitDefinition.  A \Unit structure defines a
...
%\Model with a \UnitDefinition structure or in the
%SBML specification. To define a variant of, or simply reference,
%a unit defined in the SBML specification, the \Unit structure has a
%optional field, \token{kind}, whose value must be taken from
%\primtype{UnitKind}, an enumeration of base units. The possible
%values of \primtype{UnitKind} are given in Table~\vref{tab:unitkind}.
%To define a variant of, or simply reference, a unit defined with a
%\UnitDefinition structure, \Unit has an optional
%\primtype{SId} field, \token{definition}, which must refer to a
...
%\changed{A \Unit structure must have one and only one
%\token{kind} or \token{definition} field value.  A \Unit
%structure must not create a \UnitDefinition structure where
%the \UnitDefinition refers to itself directly or
%indirectly. (More specifically, consider the directed graph formed with vertices
%representing \UnitDefinition structures and edges
%representing \token{definition} attribute values on \Unit
%structures. The source of each edge is a vertex representing a
%\UnitDefinition structure.  The sink of each edge is the
%vertex representing a \UnitDefinition referenced by a
%\token{definition} attribute enclosed by the source
%\UnitDefinition structure. Such directed graphs must be
%acyclic.)}

%\changed{The formula for a single transformation represented by a
%\Unit structure is as follows (where $u$ is the original
...
%\end{equation}}
%The optional \token{exponent} field on \Unit represents an exponent
%on the unit.  Its default value is ``\token{1}'' (one).  For the
...
%\Unit structure also has an optional \token{scale} field; its value
%must be an integer exponent for a power of ten multiplier used to set the
%scale of the unit.  For example, a unit having a \token{kind} value of
%``\token{gram}'' and a \token{scale} value of ``\token{-3}''
%signifies $10^{-3} * gram$, or milligrams.  The default value of
%\texttt{scale} is ``\token{0}'' (zero), because $10^0 = 1$.
%
%\changed{The optional \token{multiplier} field can be used to
...
%metre.  The \token{multiplier} field has a default value of
%``\token{1}'' (one).  Finally, the \token{offset} field is
%used to represent the addition of a constant in the transformation
%of the \token{kind} unit.  For example, an \token{offset} value
%of ``\token{32.0}'' would be needed to define Fahrenheit in
%terms of degrees Celsius.  The \token{offset} field has a default
%value of ``\token{0}'' (zero).

Note that the set of acceptable values for the field \token{kind}
does not include units defined by \UnitDefinition
...
A \Unit structure represents a (possibly transformed)
reference to a base unit chosen from \primtype{UnitKind}.  The
formula for a single transformation represented by a \Unit
...
exponent on the unit.  Its default value is ``\token{1}''
(one).  For the example mentioned at the beginning of this
section, $second^{\,-2}$ would be obtained by using
\texttt{kind="second"} and \texttt{exponent="-2"}.  A \Unit
structure also has an optional \token{scale} field; its value
must be an integer exponent for a power of ten multiplier used to
set the scale of the unit.  For example, a unit having a
\token{kind} value of ``\token{gram}'' and a \token{scale}
value of ``\token{-3}'' signifies $10^{-3} * gram$, or
milligrams.  The default value of \texttt{scale} is
``\token{0}'' (zero), because $10^0 = 1$.

The optional \token{multiplier} field can be used to multiply the
...
The composition of $n$ \Unit structures within a
\UnitDefinition to create more complex units involves a
...
%a \Unit structure, $u$ refers to either a member of the
%\primtype{UnitKind} enumeration through the \token{kind} field or to
%a \UnitDefinition through the \token{definition} field.
...
%to each \UnitDefinition until a \primtype{UnitKind} unit is
%reached.
%\end{blockChanged}

The following example illustrates the definition of an abbreviation named
``\token{mmls}'' for the units $mmol\ l^{-1}\ s^{-1}$:
...
\begin{table}[htb]
  ...
  \begin{tabular}{lll>{\ttfamily}l}
    ...
    \midrule
    \token{substance} & \changed{dimensionless, kilogram}, mole, item & mole\\
    \token{volume} & \changed{dimensionless}, litre, cubic metre & litre\\
    \token{area}   & \changed{dimensionless}, square metre & square metre\\
    \token{length} & \changed{dimensionless}, metre & metre\\
    \token{time}   & \changed{dimensionless}, second & second\\
    \bottomrule
  \end{tabular}
...
the field \token{units} on \Parameter) can refer to a
...
\item The five predefined units ``\token{substance}'', ``\token{volume}'',
``\token{area}'', ``\token{length}'', and ``\token{time}'' from
Table~\ref{tab:builtin}.
\end{itemize}

Within certain limits, a model may change the built-in units by reassigning
the keywords ``\token{substance}'', ``\token{length}'', ``\token{area}'',
``\token{time}'', and ``\token{volume}'' in a \UnitDefinition.  The
...
The maximum value of \token{spatialDimensions} is ``\texttt{3}'', meaning
...
\token{size} field enables concentrations of species to be calculated in
the absence of geometry information.  Note in particular that in SBML Level~2,
a missing \token{size} value does \emph{not} imply that the compartment size is 1.
(This is unlike the definition of compartment \token{volume} in SBML
Level~1.)  The \token{size} field must not be present if the
\token{spatialDimensions} field is has a value of ``\texttt{0}''.
When the \token{spatialDimensions} field does not have a value of ``\texttt{0}'',
a missing value for \token{size} for a given compartment signifies that the value is
either unknown, \changed{determined by an \AssignmentRule or
  \InitialAssignment structure, not required for analysis,
...
``\token{false}'' indicates the compartment's size can be
...
to ``\token{false}''. The \emph{units of the species} are of
...
\token{kilogram}, \token{mole}, \token{item}, or units derived from these}. The
...
``\token{true}''. The default value of the
...
``\token{false}'', indicating that the species' concentration can be
determined by reactions, rules and initial assignments.

Another optional field defined for \Species is
\token{boundaryCondition}.  By default, when a species is a product or
...
the model.  The boolean field \token{boundaryCondition} can be used to
indicate this.  The value of the field defaults to ``\token{false}'',
indicating the species \emph{is} part of the reaction
system.  Table~\ref{tab:specieattrib} shows how to interpret the combined
values of the \token{boundaryCondition} and \token{constant} fields.  In
practice, a \token{boundaryCondition} value of ``\token{true}''
...
of the \token{boundaryCondition} and \token{constant} fields on
\token{species} elements.  Section~\ref{sec:odeeg} contains a translation into
ODEs of a model which uses \token{boundaryCondition} and \token{constant} fields.
...
during a simulation.  The field's default value is ``\token{true}'';
a value of ``\token{false}'' indicates the parameter's value can
...
have their \token{constant} field set to ``\token{false}'';
\end{blockChanged}
...
and~\ref{sec:sboTerm}).  When a value is given to this field in a
parameter definition, the value must be an SBO term identifier
referring to a parameter term defined in SBO (i.e., terms derived
from \sboparameter).  The relationship is of the form ``the SBML
parameter \emph{is a} thing defined by the SBO parameter term''.
The SBO term chosen should be the most precise (narrow) one that
captures the role of the parameter.
...
\InitialAssignment structure consists of two fields,
\token{symbol} and \token{math}, as shown in
Figure~\vref{fig:initialAssignment}.  The \token{symbol} field
has type \primtype{SId} and contains the value of an
\token{id} field defined for a \token{compartment},
\token{species} or \token{parameter} elsewhere in the model. The
\InitialAssignment structure assigns the initial value of
the constant or variable referred to by the \token{symbol} field.
...
The value calculated by an \InitialAssignment structure
...
an \InitialAssignment structure exists which has a
\token{symbol} field value equal to the symbol. This does not
mean that a structure declaring a symbol can be omitted if there
is an \InitialAssignment structure for that symbol. For
example, there must be a \texttt{Parameter} structure for a given
parameter if there is a \InitialAssignment structure for
that parameter. Any pair of \AssignmentRule and
\InitialAssignment rules cannot have the same
\token{variable} attribute value.

Unlike an \AssignmentRule structure, an
\InitialAssignment structure only applies at the start of
simulated time and can set the value of a constant. In all cases,
as would be expected, the units of the formula in the
\token{math} field are identical to the units associated with the
identifier given in the \token{symbol} field, when that variable
appears in other formulas.

The effects of an \InitialAssignment structure are in
...
\InitialAssignment sets
  ...
  \InitialAssignment's \token{symbol} field.  (See
...
\InitialAssignment structure for that identifier.  For
example, there must be a \texttt{Parameter} structure for a given
parameter if there is a \InitialAssignment structure for
that parameter.

The ordering of \InitialAssignment structures is not
significant.
...
\InitialAssignment structures with the same \token{symbol}
field value; and (b) an \InitialAssignment structure with a
\token{symbol} field value which is the same as a
\AssignmentRule \token{variable} field value. The combined
...
occurrence of a symbol in a assignment statement \token{math}
field.  The directed arcs in this graph start from the statement
assigning the symbol and end at the statement that contains the
symbol in their math fields. Such a graph must be acyclic.)
Examples of valid and invalid set of assignment statements are
given in Section~\ref{sec:ruleconstraints}.

\subsubsection{The \token{sboTerm} Field}

The \InitialAssignment structure has an optional \token{sboTerm}
field of type \primtype{SBOTerm} (see
Sections~\ref{sec:sboterm-type} and~\ref{sec:sboTerm}).  When a
value is given to this field in an initial assignment definition,
the value must be a valid SBO term identifier.  The
\InitialAssignment structure should have a `is a' relationship
with the SBO term. When a value is given to this field the value
must be an SBO term identifier referring to a mathematical formula
term defined in SBO (i.e., terms derived from \sbomathformula).
The SBO term chosen should be the most precise (narrow) term that
matches the form of the \InitialAssignment.
...
\primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this field in a
rule definition (which could be an assignment rule, algebraic
rule, or rate rule), the value must be a valid SBO term
identifier.  The rule should have a `is a' relationship with the
SBO term.  When a value is given to this field the value must be
an SBO term identifier referring to a mathematical formula term
defined in SBO (i.e., terms derived from \sbomathformula).  The
SBO term chosen should be the most precise (narrow) term that
matches the form of the \Rule.
...
\item \emph{In the case of a species}, an \AssignmentRule sets the
  ...
  \token{boundaryCondition} field is set to ``\token{true}''.
...
rate, that is, the value of a \token{variable} field in a rule
cannot be \token{id} value of a \Reaction structure.}

\changed{ The value calculated by an \AssignmentRule
...
associated with a symbol by the structure declaring the symbol if
an \AssignmentRule structure exists which has a
\token{variable} field value equal to the symbol. This does not
...
\AssignmentRule and \InitialAssignment structures
cannot have the same \token{variable} attribute value. }

\changed{The combined set of \InitialAssignment,
\AssignmentRule and \KineticLaw structures form a
set of assignment statements that should be considered as a whole.
...
``\token{false}''. (Invalid simulation results do not make a
...
type \primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this field in a
constraint definition, the value must be a valid SBO term
identifier.  The \Constraint structure should have a `is a'
relationship with the SBO term.  When a value is given to this
field the value must be an SBO term identifier referring to a
mathematical formula term defined in SBO (i.e., terms derived from
\sbomathformula).  The SBO term chosen should be the most precise
(narrow) term that matches the form of the constraint.
...
unspecified in a model, it defaults to a value of ``\token{true}''.
...
The optional boolean field \token{fast} is another boolean field in the
\Reaction data structure; a value of ``\token{true}''
...
reaction as fast when in fact it is not.)  The \token{fast} field
...
type \primtype{SBOTerm} (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this field in a
reaction definition, the value must be a valid SBO term identifier
referring to a modeling framework (\ie terms derived from
\sbomodellingframework).  The \Reaction structure should have a
`is a' relationship with the SBO term.  The SBO term chosen should
be the most precise (narrow) term that defines the mathematical
framework used in the reaction.
...
The optional identifier stored in the \token{id} field gives the
reference an identifier for use in the reaction's kinetic rate
expression (Section~\ref{subsec:kinetic-law}).  The identifier
must be a text string conforming to the syntax permitted by the
\primtype{SId} data type described in Section~\ref{sec:sid}.  The
\token{id} value (whether it is in a \SpeciesReference or
\ModifierSpeciesReference object) exists in the global namespace
of the model, as described in Section~\ref{sec:idnameattribs}.
\SimpleSpeciesReference also has an optional \token{name} field,
of type \primtype{string}.  The \token{name} and \token{id} fields
must be used as described in Section~\ref{sec:idnameattribs}.
...
Sections~\ref{sec:sboterm-type} and~\ref{sec:sboTerm}).  When a
value is given to this field in a \SpeciesReference or
\ModifierSpeciesReference object, the value must be a valid SBO
term identifier.  The kind of term referenced depends on whether
the object is a \SpeciesReference or a \ModifierSpeciesReference:
if the former, then the SBO term must be a product or reactant
(\ie \sboproduct or \sboreactant, respectively); if the latter,
then the SBO term must be a modifier (\ie \sbomodifier).  The
\SpeciesReference and \ModifierSpeciesReference structures should
have an ``is a'' relationship to the term identified by the SBO
term identifier.  The SBO terms chosen should be the most precise
(narrow) one that defines the role of the species in the reaction.
...
the stoichiometry values given on the \SpeciesReference structures in the list
of products minus the sum of stoichiometry values given on the
\SpeciesReference structures in the list of reactants.
...
those listed in the \token{reactant}, \token{product} and
\token{modifier} fields of the \Reaction structure.
...
multiplied variant of \token{dimensionless}, \token{moles}, \token{kilogram} or
\token{item}. In the case of \token{timeUnits}, the value chosen
must be a scaled and/or multiplied variant of \token{seconds} or
\token{dimensionless}}. If these fields are not set in a given
\KineticLaw instance, the units are taken from the defaults
defined by the built-in ``\token{substance}'' and ``\token{time}''
...
\subsubsection{The \token{trigger} Field}
...
\subsubsection{The \token{delay} Field}
...
\subsubsection{The \token{timeUnits} Field}
...
\subsubsection{The \token{eventAssignment} Field}
...
the event is \emph{executed}. The \token{Assignment} structure is
shown in Figure~\ref{fig:event}. The \token{variable} field is of
...
``\token{false}''. The \token{math} field contains a MathML
...
\subsubsection{Example \Event structure}
...
\begin{blockChanged}
%-----------------------------------------------------------------------------
\section{The \token{sboTerm} Field and the Systems Biology Ontology}
\label{sec:sboTerm}
%-----------------------------------------------------------------------------

Identifiers on SBML components allows them to be cross-referenced
within a model, and names on SBML components allows them to be
given more meaningful labels suitable for display to humans.  The
specific identifiers and labels used in a model necessarily must
be unrestricted by SBML, so that software and users are free to
pick whatever they need.  However, this freedom makes it more
...
This is the purpose of the optional \token{sboTerm} field provided
in many SBML object classes.  The SBML object classes
\InitialAssignment, \SimpleSpeciesReference, \Parameter,
\KineticLaw, \Rule, \Model, \Constraint and \Reaction all have
optional \token{sboTerm} fields.  In this section, we discuss the
\token{sboTerm} field, the motivations and theory behind its
introduction, guidelines for its use, and provide examples fo
illustration.
...
identifiers that are not tool-specific.  An example situation
where this is useful is the desire by many software developers to
provide users with meaningful names for reaction rate equations.
Software tools with model editing interfaces frequently provide
these names in menus or lists of choices for users.  However,
without a standardized set of names or identifiers shared between
multiple software tools, a given software package cannot reliably
interpret the names or identifiers of reactions in models written
by other tools.

The first solution that might come to mind is to stipulate that
certain common reactions always have the same name, but this is
simply impossible to do: not only do humans sometimes disagree on
the names themselves, but it would not allow for correction of
errors or updates to the list of predefined names except by
issuing new revisions of the SBML specification---to say nothing
of many other limitations with this approach.  Moreover, the
parameters and variables that appear in rate expressions also need
to be identified in a way that software tools can interpret
mechanically.
...
frameworks (e.g., ``continuous deterministic'', ``discrete
stochastic'', etc.).  The relationship implied by an
\token{sboTerm} value on an SBML model component is ``is a'': the
thing defined by that SBML model component ``is a'' instance of
the thing defined in SBO having the identifier given by the
\token{sboTerm} field value.  By adding SBO term references on the
components of a model, a software tool can provide additional
details using an independent, shared vocabulary that can enable
\emph{other} software tools to recognize precisely what the
component is meant to be.  Those tools can then act on that
information.  For example, if the SBO identifier
\token{SBO:0000012} is assigned to the concept of a unimolecular
reversible mass-action reaction, and a given reaction in a model
has an \token{sboTerm} field with this value, then regardless of
the identifier and name given to the reaction itself, a software
tool could use this information to display ``reversible
mass-action reaction'' to the user in the tool's user interface.
...
application could then use internal, optimized versions of the
rate laws indexed by identifiers such as \texttt{SBO:010001}
appearing in SBML models.


\subsection{Using \token{sboTerm}}
...
the entity represented by the SBML object in question.  An example
of the type of relationship intended is: \emph{the KineticLaw in
  reaction R1 is a first-order irreversible mass action rate law}.

Note the careful use of the words ``defines'' and ``entity
represented by the SBML object'' in the paragraph above.  As
already mentioned, the relationship between the SBML object and
the URI is:

\begin{quote}
   The ``thing'' represented by this SBML object \emph{is an}
   instance of the ``thing'' represented by the referenced
   SBO term.
\end{quote}

It is critical to keep in mind that the presence of an
\token{sboTerm} value on an object \emph{must not change the
  fundamental mathematical meaning} of the model.  An SBML model
must stand on its own and not require the additional information
added by SBO terms for a correct mathematical interpretation.
There are two nuances to this.  First, it would be too limiting to
require that all software tools be able to understand the SBO
vocabularies in addition to understanding \sbmltwotwo.  Supporting
SBO is not only additional work for the software developer; for
some kinds of applications, it may not make sense.  If the SBO
terms on a model are optional, it follows that the SBML model
\emph{must} remain unambiguous and fully interpretable without
them, because an application reading the model may ignore the
terms.  Second, we believe allowing the use of \token{sboTerm} to
alter the mathematical meaning of a model would give software
authors too much leeway to shoehorn incompatible concepts into
SBML, ultimately reducing the interoperability of the models.
...
annotation in SBML, the SBO references are separated out, both to
simplify their use for software tools and because they assert a
slightly stronger and more focused connection in a more regimented
fashion.  SBO term references are intended to allow a modeler to
make a statement of the form ``this object is identical to meaning
and intention to the definition of X in the SBO vocabulary'' and
do so in a way that a \emph{software tool can interpret
  unambiguously}.
...
\item It supports a large fraction of what SBML users have wanted
...
similar in purpose to SBO.  Again, for maximal software
interoperability, the best-practice recommendation in SBML is to
use the \primtype{sboTerm} fields in preference to using
application-specific annotation schemes.  Software applications
should attempt to translate their private terms to and from SBO
terms when writing and reading SBML, respectively.
...
\subsubsection{About the Systems Biology Ontology}
...
the ontology independently of SBML.  The terms in the ontology are
designed with SBML components in mind, and are classified into
subsets that can be related one-to-one with SBML components such
as reaction rate expressions, parameters, and a few others.

The SBO development approach follows conventional ontology
development approaches in bioinformatics.  One of the principles
being followed is that identifiers are never reassigned or removed
from SBO.  New terms in SBO can render old terms obsolete, but the
old terms are not removed.  Therefore, a model is never in danger
of ending up with SBO identifiers that cannot be dereferenced.
...
already.  The guideline for these situations is that the SBML
model must be self-contained and stand on its own.  Therefore, in
cases where they differ, the definitions in the model take
precedence over the definitions referenced by the CV.  In other
words, the model (and its MathML) is authoritative.
...
No.  Since the SBO will likely stabilize and change infrequently
once a core set of terms is defined, applications can simply cache
the controlled vocabulary and not make network accesses to the
master SBO copy unless something forces them to (\eg detecting a
reference in a model to an SBO term that the application does not
recognize).  In addition, applications could have user preference
settings indicating how often the CV definitions should be
refreshed, (similar to how modern applications provide a setting
...
uses a restricted form of Dublin Core~\citep{DCMI:2003} and
Biomodels qualifier elements (see
...
\token{annotation} element to include XML that is compliant with
...
aware that a valid \token{annotation} element may contain an
\token{rdf:RDF} element which is not compliant with the format
...
\begin{table}[bh]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    \token{dc} & \url{http://purl.org/dc/elements/1.1/} & \citep{powell:2003}\\
    \token{rdf} & \url{http://www.w3.org/1999/02/22-rdf-syntax-ns\#} & \citep{w3c:2004b} \\
    \token{dcterms} & \url{http://purl.org/dc/terms/} & \citep{kokkelink:2002}\\
    & & \citep{DCMIUB:2005} \\
    \token{vcard} & \url{http://www.w3.org/2001/vcard-rdf/3.0#"} & \citep{iannella:2001} \\
    \token {bmqual} & \url{http://biomodels.net/qualifiers} \\
    \bottomrule
  \end{tabular}
  \vspace*{-0.95ex}
...
can contain an \token{annotation} element; \texttt{SBML\_META\_ID}
is a XML \primtype{ID} string; \texttt{RELATION\_ELEMENT} refers
to a element names in the \url{http://biomodels.net/qualifiers}
namespace; and \texttt{URI} is a URI.  \texttt{[MODEL\_HISTORY]}
refers to an optional section described in
...
The \token{rdf:Description} element must contain only an optional
...
history section can only be present within an SBML \Model
...
Biomodels qualifier relation elements must only contain a single
...
\token{rdf:resource} attribute values.  The Dublin core relation
...
The relation elements with a given tag enclosed in the same SBML
element each represent an alternative annotation to the SBML
element. For example two \class{bmqual:hasPart} elements within a
...
qualifier namespace (\url{http://biomodels.net/qualifiers}) is
documented at \url{http://sbml.org/wiki/Biomodels_Qualifiers}.
This list will only grow i.e no element will be removed from the
list. The following is the list of elements at the time of
writing:
...
\item \token{bmqual:is} The object represented by the SBML
component is the subject of the referenced resource

\item \token{bmqual:hasPart} The object represented by the SBML
component includes the subject of the referenced resource, either
physically or logically

\item \token{bmqual:isPartOf} The object represented by the SBML
component is a physical or logical part of the subject of the
referenced resource

\item \token{bmqual:isVersionOf} The object represented by the
SBML component is a version or an instance of the subject of the
referenced resource

\item \token{bmqual:hasVersion} The subject of the referenced
resource is a version or an instance of the object represented by
the SBML component

\item \token{bmqual:isDescribedBy} The object represented by the
...
\item \token{bmqual:isHomologTo} The object represented by the
SBML component is a homolog to the referenced resource.
\end{itemize}

In all cases above the 'object' of the relation is the biological
or biochemical object represented by the enclosing SBML element.
The resources referenced by the \class{rdf:li} elements contained
within the relation element are the 'object' of the relation.
...
The described format, when enclosed in an SBML \Model
...
<reaction id="calciumBinding" metaid="jb007">
  <annotation>
    <rdf:RDF
      xmlns:dc="http://purl.org/dc/elements/1.1/"
      xmlns:bmqual="http://biomodels.net/qualifiers/"
      xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns\#"
    >
...
\texttt{isVersionOf}. The first example is the relationship
...
It is possible to mix the URI of annotations in a given set. The
...
It is possible mix different qualified sets in the same annotation
...
\subsection{Example Involving Combinations of
  \token{boundaryCondition} and \token{constant} Values on \Species
  with \RateRule structures}
...
``\token{false}''.  The values of $S_2$ and $S_3$ are fixed so in
SBML they have a \token{constant} field values of
``\token{true}''.  $S_3$ only occurs as a modifier so the value of
...
\token{annotation} element and associated restrictions}

An \token{annotation} element cannot contain two or more top level
elements in the same XML namespace.  An \token{annotation} element
cannot contain a top level element in the SBML namespace.  The
order of top level elements within an \token{annotation} element
is not significant.

\item \emph{sboTerm}

In Version 2 \SimpleSpeciesReference,
\InitialAssignment, \KineticLaw, \Parameter,
\Reaction, \Model, \Rule and
\Constraint structures have an addition field
\token{sboTerm}.  These fields can only contain valid Systems
...
In Version 2 the \Model structure has an addition list of
\CompartmentType structures.  Each of these structures
represents a type of compartment and consists of just
\token{name} and \token{id} fields.  The \token{id} field can
optionally be referenced from an \Compartment structure
using a new \token{compartmentType} field.

\item \emph{Species Type}

In Version 2 the \Model structure has an addition list of
\SpeciesType structures.  A \SpeciesType structure
represents a type of chemical entity independent of location and
consists of just \token{name} and \token{id} fields.  The
\token{id} field can optionally be referenced from an
\Species structure using a new \token{speciesType} field.

\item \emph{\token{charge} field deprecated}

In Version 2 the \Species structure \token{charge} field
is deprecated.

\item \emph{Constraint}

In Version 2 the \Model structure has an addition list of
\Constraint structures.  The \token{math} field of
\Constraint contains a boolean expression which is is
function of the model state which returns whether the state is
valid.  Unlike the \Rule structures \Constraint
should not be used to compute the dynamical behavior of the model.

\item \emph{\token{Id} on \SimpleSpeciesReference}

Version 2 adds \token{id} and \token{name} fields to the
...
\KineticLaw structure of the \Reaction.  It is not
possible to explicitly assign a value to the symbol using
\InitialAssignment, \EventAssignment,
\AssignmentRule or \RateRule structures.

\item \emph{Initial Assignment Structures}

In Version 2 the \Model structure has an additional list of
\InitialAssignment structures.  This list of structures is
...
\InitialAssignment structures.

\item \emph{The order of \AssignmentRule structures is not
significant}

In Version 2 the order of \AssignmentRule is not
significant however the set of assignment rules formed from
\KineticLaw, \InitialAssignment and
\AssignmentRule structures as a whole must not contain
algebraic loops.
...
permitted on MathML \token{annotation} and
\\ \token{annotation-xml} elements, not only on \token{csymbol} as
stated on that page.
...
to the \Unit structure in this specification (SBML Level 2
Version 2).
...
\item page 46 Example 5.8: The \token{definitionURL} attribute
value for the \token{csymbol} delay should be
...
\sbmlrule{900} \item The \token{id} field on a
\token{functionDefinition} element, a \token{species} element, a
\token{speciesType} element, a \token{modifierSpeciesReference}
element, a \token{speciesReference} element, a \token{compartment}
element, a \\ \token{compartmentType} element, a \token{parameter}
element outside any \token{kineticLaw} element, a \token{reaction}
element or an \token{events} element should contain a value unique
among the set of all these attributes.  See
Section~\ref{sec:identifiers}.

\sbmlrule{901}
\item The \token{id} attributes on
\token{unitDefinition} elements should contain a unique value
amongst the set of all such attributes.  See
section~\ref{sec:unitdefinition-structure}.

\sbmlrule{902}
\item The \token{id} attributes on
\token{parameter} elements inside \token{kineticLaw} elements
should contain values unique to each element among the set of
these attributes within the same \token{kineticLaw} element.  See
Section~\ref{sec:identifiers}.

\sbmlrule{903}
\item The \token{variable} attribute value
must be unique amongst the set of \token{assignmentRule} and
\token{rateRule} elements. See Sections~\ref{sec:assignmentrule}
and~\ref{sec:raterule}.

\sbmlrule{904}
\item The \token{variable} attribute value
must be unique amongst the set of \token{eventAssignment} elements
within a each \token{event} element.  See
...
\token{functionDefinition} element, a \token{ci} element that is a
leaf operand (argument) of a MathML operator must contain the
value of the \token{id} attribute of a \token{species},
\token{compartment}, \token{parameter} element or \token{reaction}
element.(see section~\ref{sec:ci-token})

\sbmlrule{9999} \item In a MathML element inside a
\token{functionDefinition} element, a \token{ci} element that is a
leaf operand (argument) of a MathML operator must contain the
value the content of one of the top-level \token{bvar} elements.
(see section~\ref{sec:ci-token}).

\sbmlrule{9999} \item The value of an \token{id} attribute of a
local parameter can only be used in \token{ci} elements within the
MathML contained in the same \token{kineticLaw} element that
contains the \token{parameter} element. (see
sections~\ref{sec:ci-token} and \ref{subsec:kinetic-law}).

\sbmlrule{9999} \item A \token{ci} element that represents a math
function i.e. the first subelement of an \token{apply} element can
only contain the values of \token{id} attributes of previous
\token{functionDefinition} elements.  The \token{ci} element
cannot contain the value of the \token{id} field of any enclosing
\token{functionDefinition} element. (Function definitions are not
recursive or mutually recursive). (see
section~\ref{sec:ci-token}).

\sbmlrule{9999}
\item The \token{lambda} elements can only be used as the outer
most element contained within the MathML element contained within
a \token{functionDefinition} element. (see
...
\item the second argument of a \token{piece} operator; and

\item the arguments of the logical operators (\token{and},
\token{or}, \token{xor}, \token{not});
...
\token{piece} and \token{otherwise} operators within the same
\token{piecewise} operator should all return values of the same
type. The arguments of the \token{eq} and \token{neq} operators
should return the same type.  The relational operators
(\token{eq}, \token{neq}, \token{gt}, \token{lt}, \token{geq},
\token{leq}), the logical operators and the boolean constants
(\token{false}, \token{true}) always return boolean values. All
...
%\end{enumerate} \subsection*{\Unit} \begin{enumerate}
%
%\sbmlrule{9999}
%\item All \token{unit} elements must have one and only value for
%the \token{kind} and \token{definition} attributes. \token{unit}
%elements with no \token{kind} or \token{definition} attribute
%values are invalid.  \token{unit} elements with both \token{kind}
%and \token{definition} attribute values are invalid. See
%section~\ref{sec:unit-structure}.
%
%\sbmlrule{9999}
%\item The \token{definition} attribute on a \token{unit} element,
%when present, must contain the value of an \token{id} attribute
%on a \token{unitDefinition} element.  See
%section~\ref{sec:unit-structure}.

\end{enumerate} \subsection*{\UnitDefinition} \begin{enumerate}

%\sbmlrule{9999}
%\item The directed graph formed from unit definitions must be
%acyclic. A directed graph is formed from unit definitions by
%creating a vertex for each \token{unitDefinition} element.  Each
%edge represents a \token{unit} element with a \token{definition}
%attribute value. The edge's source is the vertex representing the
%\token{unitDefinition} element containing the \token{unit}
%element. The edge's sink is the vertex representing the unit
%definition referenced by the \token{definition} attribute value.
%See section~\ref{sec:unit-structure}.

\sbmlrule{1201}
\item The \token{id} attributes on
\token{unitDefinition} elements should not contain the values of
the Unit kind enumeration (i.e. \texttt{metre}, \texttt{litre}
etc). See section~\ref{sec:unitdefinition-structure}

\sbmlrule{1202} \item If the \token{id} attribute of a
\token{unitDefinition} element contains \texttt{substance} then
the element must contain a single \token{unit} element containing
a \token{kind} attribute value of \token{dimensionless},
\token{mole} or \token{item} and \texttt{exponent} attribute value
of 1 (can default to 1).  See section~\ref{sec:built-in-units}.

\sbmlrule{1203} \item If the \token{id} attribute of a
\token{unitDefinition} element contains \texttt{length} then the
element must contain a single \token{unit} element containing
either:
\begin{itemize}

\item a \token{kind} attribute value of \texttt{metre} and
\token{exponent} attribute value of 1 (can default to 1); or

\item a \token{kind} attribute value of \texttt{dimensionless}
with arbitrary \token{exponent} attribute value.

\end{itemize}

See section~\ref{sec:built-in-units}.

\sbmlrule{1204} \item If the \token{id} attribute of a
\token{unitDefinition} element contains \texttt{area} then the
element must contain a single \token{unit} element containing
either:
\begin{itemize}

\item a \token{kind} attribute value of \texttt{metre} and
\token{exponent} attribute value of 2; or
...
\sbmlrule{1205} \item If the \token{id} attribute of a
\token{unitDefinition} element contains \texttt{volume} the
element must contain a single \token{unit} element containing a
\token{kind} attribute value of \texttt{dimensionless},
\texttt{litre} or \texttt{metre}. See
section~\ref{sec:built-in-units}.

\sbmlrule{1206} \item If the \token{id} attribute of a
\token{unitDefinition} element contains \texttt{volume} and the
the enclosed \token{unit} element \token{kind} attribute contains
\texttt{litre} then the \token{exponent} attribute must be 1 (can
default to 1). See section~\ref{sec:built-in-units}.

\sbmlrule{1207} \item If the \token{id} attribute of a
\token{unitDefinition} element contains \texttt{volume} and the
the enclosed \token{unit} element \token{kind} attribute contains
\texttt{metre} then the \token{exponent} attribute must be 3. See
section~\ref{sec:built-in-units}.

\sbmlrule{1208} \item If the \token{id} attribute of a
\token{unitDefinition} element contains \texttt{time} then the
element must contain a single \token{unit} element containing
either:
\begin{itemize}

\item a \token{kind} attribute value of \texttt{second} and
\token{exponent} attribute value of 1 (can default to 1); or
...
The value of the \token{spatialDimensions} attribute of a
\token{compartment} element is known as the `spatial dimensions'
in this section.

\begin{enumerate}

\sbmlrule{1300}
\item The \token{size} attribute must not be set
if \token{spatialDimensions} is zero. See
section~\ref{sec:compartment-units}.

%\item The \token{units} attribute must not be set if
%\token{spatialDimensions} is zero.  (this has not been confirmed
%yet - watch this space).  ''I required units to be
%'''dimensionless''' if it's set. --[[Ben Kovitz]]''

\sbmlrule{1301}
\item A \token{compartment} element with
attribute \token{spatialDimensions} set to 0 must have the
\token{units} attribute set to \texttt{dimensionless}, or the
\token{id} attribute value of a \token{unitDefinition} element
that defines a variant of \texttt{dimensionless}. See
section~\ref{sec:compartment-units}.

\sbmlrule{1302} \item The \token{constant} field of a
\token{compartment} element must default to or be set to
\texttt{true} if the value of the \token{spatialDimensions} field
is 0. See section~\ref{sec:compartment-constant}.

\sbmlrule{1303}
\item The value of the \token{outside} attribute of a
\token{compartment} element must contain the value of a
\token{id} field of another \token{compartment}. See
section~\ref{sec:compartment-outside}.

\sbmlrule{1304}
\item The graph of links between compartments implied by the
\token{outside} attributes must be acyclic.  See
section~\ref{sec:compartment-outside}.

\sbmlrule{1305}
\item A \token{compartment} element with
attribute \token{spatialDimensions} set to 1 must have the
\token{units} attribute set to \texttt{dimensionless}, \texttt{length},
\texttt{metre}, or the \token{id} attribute value of a
\token{unitDefinition} element that defines a variant of
\texttt{metre} with an exponent value of 1 or a variant of \texttt{dimensionless}. See
section~\ref{sec:compartment-units}.

\sbmlrule{1306}
\item A \token{compartment} element with
attribute \token{spatialDimensions} set to 2 must have the
\token{units} attribute set to \texttt{dimensionless}, \texttt{area} or the \token{id}
attribute value of a \token{unitDefinition} element that defines a
variant of \texttt{metre} with an exponent value of 2 or a variant of \texttt{dimensionless}. See
section~\ref{sec:compartment-units}.

\sbmlrule{1307}
\item A \token{compartment} element with
attribute \token{spatialDimensions} set to 3 must have the
\token{units} attribute set to \texttt{dimensionless}, \texttt{volume} or the
\token{id} attribute value of a \token{unitDefinition} element
...
\sbmlrule{9999} \item The \token{compartmentType} attribute, when
present, on a \token{compartment} element should contain the same
value as an \token{id} attribute on a \token{compartmentType}
element. See Section~\ref{sec:compartment-compartment-type}.

\end{enumerate} \subsection*{\Species} \begin{enumerate}

\sbmlrule{1400}
\item The \token{compartment} attribute
on a \token{species} element must contain the value of an \token{id}
attribute on a
\\ \token{compartment} element.  This \token{id} value is known as
the `\emph{compartment}' in this section.  The value of the
\token{spatialDimensions} attribute of the compartment is known
...
\sbmlrule{1401}
\item The \token{spatialSizeUnits} attribute must not be present
if the \token{hasOnlySubstanceUnits} attribute is true.  See
Section~\ref{sec:species-units}.
...
\sbmlrule{1404}
\item The \token{initialConcentration} attribute must not be
...
\token{species} element is 1 then the \token{spatialSizeUnits}
attribute must a value of `\texttt{dimensionless}',
`\texttt{length}', `texttt{metre}' or the \token{id} value of a
\token{unitDefinition} element that defines a variant of
...
\token{species} element is 2 then the \token{spatialSizeUnits}
attribute must a value of `\texttt{dimensionless}',`\texttt{area}'
or the \token{id} value of a \token{unitDefinition} element that
...
\token{species} element is 3 then the \token{spatialSizeUnits}
attribute must a value of `\texttt{dimensionless}',
`\texttt{volume}', `texttt{litre}' or the \token{id} value of a
\token{unitDefinition} element that defines a variant of
`texttt{metre}' with an exponent value of 3 or a variant of
\texttt{dimensionless}. See Section~\ref{sec:species-units}.

\sbmlrule{1408} \item The \token{substanceUnits} attribute must
contain either \texttt{dimensionless}, \texttt{kilogram},
\texttt{substance}, \texttt{item}, \texttt{moles} or the values of
\token{id} attributes of \token{unitDefinition} elements that
define variants (i.e. have only arbitrary \token{scale},
\token{multiplier} and \token{offset} values) of \texttt{item},
\texttt{moles}, \texttt{kilogram} or \texttt{dimensionless}. See
Section~\ref{sec:species-units}.

\sbmlrule{9999}
\item The \token{initialConcentration} and \token{initialAmount}
attributes must not be present on the same element at the same
time.  They are mutually exclusive.  See
Section~\ref{sec:initialAmount}.

\sbmlrule{9999} \item Consider a \token{species} element that has
a \token{boundaryCondition} attribute of false and there exists a
\token{species} attribute on a \token{speciesReference} element
which contains the value of the \token{species} element's
\token{id} attribute. (The species concentration is determined by
a reaction.) In this case there must not exist a \token{variable}
attribute of a rule element that contains the \token{id}
attribute value on the \token{species} element. (A species cannot
be determined by rules and reactions at the same time). See
Section~\ref{sec:species-constant}.

\sbmlrule{9999}
\item The \token{speciesType} attribute, when present, on a
\token{species} element should contain the same value as an
\token{id} attribute on a \token{speciesType} element. See
Section~\ref{sec:species-species-type}.

\sbmlrule{9999} \item In any \Species structure, that has a
\token{speciesType} field value, the values of a pair of \token{speciesType}
and \token{compartment} fields must be unique in the set of
all \Species structures. See
Section~\ref{sec:species-species-type}.

\end{enumerate} \subsection*{\Parameter} \begin{enumerate}

\sbmlrule{1500}
\item The \token{units} field must
contain one of the following: a value from the unit kind
enumeration (eg \texttt{metre}), a value from the built-in units
table e.g. \texttt{substance} or the value of a \token{id}
attribute of a \token{unitDefinition} element.  See
Section~\ref{sec:parameter-units}.

\end{enumerate} \subsection*{\token{Initial Assignment}} \begin{enumerate}

\sbmlrule{9999}
\item The \token{symbol} attribute of a
\token{initialAssignment} element must contain the value of a
\token{id} attribute of a \token{compartment}, \token{species},
or \token{parameter} element.  See
Sections~\ref{sec:initialAssignment}.

\sbmlrule{9999} \item Two or more \token{symbol} attributes of
\token{initialAssignment} elements must not contain the same
value.  See Sections~\ref{sec:initialAssignment}.

\sbmlrule{9999} \item The \token{symbol} attribute of a
\token{initialAssignment} element and the \token{variable}
attribute of an\\ \token{assignmentRule} must not contain the same
value.  See sections~\ref{sec:initialAssignment}.

\end{enumerate} \subsection*{\Reaction} \begin{enumerate}

\sbmlrule{1600}
\item A reaction must contain at least one
\token{speciesReference} element (either in the products or
reactants lists).  See Section~\ref{subsec:speciesreference}.

\sbmlrule{9999}
\item Consider a \token{ci} element which
contains the value of an \token{id} field of a \token{species}
element where the \token{ci} element is contained within any
MathML element within a \token{reaction} element (i.e. part of
stoichiometry math and kinetic law elements). For all such
\token{ci} elements its content must also be contained in a
\token{species} attribute (on a \token{speciesReference} and
\token{modifierSpeciesReference} element) within the given
\token{reaction} element. (All species used in MathML in a
...
\end{enumerate} \subsection*{\SimpleSpeciesReference} \begin{enumerate}

\sbmlrule{1601}
\item The value of a \token{species}
attribute of a \token{speciesReference} and
\token{modifierSpeciesReference} element must contain the value of
a \token{id} attribute of a \token{species} element.  See
section~\ref{subsec:speciesreference}
and\ref{subsec:modifierreference}.

\textbf{\SpeciesReference}

\sbmlrule{1602}
\item When a \token{species} element has a
\token{constant} attribute value of true and a
\token{boundaryCondition} attribute of false then there must be
no \token{speciesReference} element's \token{species} attribute
that contains the value of the \token{species} element's
\token{id} attribute. Section~\ref{sec:species-constant}.

\sbmlrule{1603}
\item The \token{stoichiometry} attribute
and \token{stoichiometryMath} element are mutually exclusive. See
section~\ref{subsec:speciesreference}.

\end{enumerate} \subsection*{\KineticLaw} \begin{enumerate}

\sbmlrule{1604} \item The \token{substanceUnits} attribute must
contain either \texttt{dimensionless}, \texttt{kilogram},
\texttt{substance}, \texttt{item}, \texttt{moles} or the values of
\token{id} attributes of \token{unitDefinition} elements that
define variants of \\
...
\sbmlrule{1605} \item The \token{timeUnits} attribute must
contain either \texttt{dimensionless}, \texttt{time},
\texttt{second} or the values of \token{id} attributes of
\token{unitDefinition} elements that define variants of
\texttt{second} or \texttt{dimensionless}.  See
section~\ref{subsec:kinetic-law}.

\end{enumerate} \subsection*{\AssignmentRule and \RateRule} \begin{enumerate}

\sbmlrule{1700} \item The \token{variable} attribute of a
\token{assignmentRule} element must only contain the value of a
\token{id} attribute of a \token{compartment}, \token{species},
or \token{parameter} element.  See
Sections~\ref{sec:assignmentrule}.

\sbmlrule{1701} \item The \token{variable} attribute of a
\token{rateRule} element must only contain the value of a
\token{id} attribute of a \token{species}, \token{compartment},
or \token{parameter} element.  See Sections~\ref{sec:raterule}.

\sbmlrule{1702}
\item The \token{constant} attribute of
a \token{compartment}, \token{species}, or \token{parameter}
element must be false when referred to by the \token{variable}
attribute of an \token{assignmentRule} element. See
Sections~\ref{sec:assignmentrule}.
...
a \token{compartment}, \token{species}, or \token{parameter}
element must be false when referred to by the \token{variable}
attribute of an \token{rateRule} element. See
Sections~\ref{sec:raterule}.

\sbmlrule{9999} The combined set of \token{initialAssignment},
\token{assignmentRule} and \token{kineticLaw} elements form a set
...
(content of a \token{ci} element) in their \token{math}
subelements. Such a graph must be acyclic.
\token{initialAssignment} and \token{assignmentRule} elements
assign the symbol contained in their \token{variable} field.
\token{kineticLaw} elements assign the symbol contained in the
\token{id} field of the enclosing \token{reaction} element. See
Section~\ref{sec:ruleconstraints}.

\end{enumerate} \subsection*{\AlgebraicRule} \begin{enumerate}
...
\end{enumerate} \subsection*{\Event} \begin{enumerate}

\sbmlrule{1800}
\item The \token{timeUnits} attribute
must contain either \texttt{time}, \texttt{second} or the values
of \token{id} attributes of \\ \token{unitDefinition} elements
...
\token{trigger} field of an \token{event} structure;

\end{enumerate} \subsection*{\\EventAssignment}
\begin{enumerate}

\sbmlrule{1802}
\item The \token{variable} attribute of
an \token{e\EventAssignment} must contain the value of a \token{id}
attribute of a \\ \token{compartment}, \token{species}, or
\token{parameter} element.  See Section~\ref{sec:eventassignment}.

\sbmlrule{1803}
\item The \token{constant} attribute of
the \token{compartment}, \token{species}, or \token{parameter}
element must be false.  See Section~\ref{sec:eventassignment}.

\sbmlrule{9999}
\item The value of a \token{variable} attribute on
a \EventAssignment structure cannot be the same as the
value of a \token{variable} attribute on a \AssignmentRule
...
allows: the \token{initialConcentration} attribute must not be
present if the \token{hasOnlySubstanceUnits} attribute is true.
\end{blockChanged}
\clearpage
