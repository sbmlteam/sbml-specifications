...
\packageVersion{Version 0.15 (Draft)}
\packageVersionDate{March, 2015}
...
\mathml block containing the standard SBML function definition, \changed{because that element is required in SBML Level~3 Version~1 (in SBML Level~3 Version~2, this requirement has been dropped).  This also}
ensures a degree
...
\class{functionDefinition} is not used, \changed{but is required by \sbmlthreecore.  To ensure the continued validity of the model, the following rules must be followed:}
...
\changed{Here, the species with an initial amount of 3.22 is described as having a standard deviation of 0.3, a value that might be written as \val{3.22 $\pm$ 0.3}.  This is probably the simplest way to use the package to introduce facts about the uncertainty of the measurements of the values present in the model.}

\changed{It is also possible to include additional information about the species, should more be known:}
