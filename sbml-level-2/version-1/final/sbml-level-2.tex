...
documents and software are also available from the SBML project web site,
...
The Systems Biology Markup Language was originally created in collaboration
...
\item If all other situations, the referenced identifier must be the
  ...
  \item \emph{Species identifier}: When a species identifier occurs in a
    ...
    broadly).  The units of concentration associated with a given species
    symbol depends on how the species is declared in the \class{Species}
    structure (see Section~\ref{sec:species}).  If the \class{Species}
    structure has an \attrib{initialConcentration} value, then the units of
    concentration associated with the species are exactly those defined in
    the \class{Species} structure.  If instead the \class{Species}
    structure has an \attrib{initialAmount} value, then the units of
    concentration associated with the species are \quantity{species
      substance}/\quantity{compartment volume} units, where the units of
    \quantity{species substance} are those defined in the \class{Species}
    structure, and the \quantity{compartment volume} units are those given
    on the \class{Compartment} structure where the species is defined to be
    located.
...
five predefined names ``\unit{substance}'', ``\unit{volume}'',
...
Within certain limits, a model may change the default units by reassigning
the keywords ``\unit{substance}'', ``\unit{length}'', ``\unit{area}'',
``\unit{time}'', and ``\unit{volume}'' in a \class{UnitDefinition}.  The
second column in Table~\ref{tab:builtin} lists the set of units that can be
used in redefining a given default unit.  Note that, in order for unit
consistency to be maintained, only the \attrib{multiplier} and
\attrib{scale} fields on \class{Unit} structures can sensibly be used for
redefining the default units.

The following example illustrates how to change the default units of volume
...
is defined in SBML.  Sections~\ref{sec:mathmltokens}, \ref{sec:species} and
...
It is worth point out that, although compartments are optional in the
...
Table~\vref{tab:unitkind}, or the built-in defaults ``\unit{volume}'',
...
following guidelines: for spatial dimensions of 3, 2, 1 or 0, the
...
The units of the compartment size are used in all contexts where a
compartment identifier may appear as a numerical quantity in a mathematical
formula expressed in MathML (discussed in Section~\ref{sec:mathmltokens}).
The units are also used in \class{AssignmentRule} structures determining
the compartment's size.  In the case of \class{RateRule} structures that
set the rate of change of a compartment's size
(Section~\ref{sec:raterule}), the units on the rate expression are the
units set by the \attrib{units} field divided by the default
...
As with other major structures in SBML, \class{Species} has an optional
...
compartment.  These fields are mutually exclusive and \emph{only one} can
...
The units of a species' initial quantity may be set explicitly using the
optional field \attrib{units}.  This sets the units associated with the
values in the species' \attrib{initialAmount} or
\attrib{initialConcentration} field, depending on which one is being used.
The \attrib{units} field value also sets the units associated of the
species symbol when it appears in MathML expressions and in
\class{AssignmentRule} and \class{RateRule} structures that determine the
value of the species concentration.

The value that can be assigned to the \attrib{units} field depends on
whether amounts or concentrations are being used for a given species; it
also depends on the number of spatial dimensions of the compartment in
which the species is located.  The following are the principles that must
be followed:
\begin{itemize}
...
\item \emph{Case of \attrib{initialAmount} being used}. The value assigned
  ...
  \unit{mole} or \unit{item} units.  If absent, the units default to the
  value set by the built-in ``\attrib{substance}'' shown in
...
\item \emph{Case of \attrib{initialConcentration} being used}.  The value
  ...
  new unit name defined by a unit definition in the enclosing model.  The
  chosen units must be scaled variants of \unit{mole}/\quantity{size} or
  \unit{item}/\quantity{size} units, where \quantity{size} must be units of
  volume if the compartment's \attrib{spatialDimensions} is
  ``\attribvalue{3}'', units of area if the compartment's
  \attrib{spatialDimensions} is ``\attribvalue{2}'', or units of length if
  the compartment's \attrib{spatialDimensions} is ``\attribvalue{1}''.  As
  noted above, \attrib{initialConcentration} cannot be used if the
  compartment where the species is located has a \attrib{spatialDimensions}
  value of ``\attribvalue{0}''.  If absent, the units default to units of
  \quantity{substance}/\quantity{size} where \quantity{substance} is the
  built-in unit ``\unit{substance}'' (see Table~\ref{tab:builtin}) and
  \quantity{size} is the units assigned to the compartment containing the
  species.
\end{itemize}
...
\item \emph{In the case of a species}, an \class{AssignmentRule} sets the
  ...
  in \attrib{math}.  The overall units of the formula must match the units
  assigned to the species' quantity in its \class{Species} structure
  declaration according to the following principles.  If the
  \class{Species} structure has a value for \attrib{initialConcentration},
  then the units of the \class{AssignmentRule} formula must be the same as
  those set by the \class{Species} structure.  If the \class{Species}
  structure has a value for \attrib{initialAmount}, then the units of the
  formula must be units of \quantity{amount}/\quantity{size}, where the
  \quantity{amount} units are those associated with the species' amount
  (see Section~\ref{sec:species}) and the \quantity{size} units are those
  defined for the size of the compartment in which the species is located
...
\item \emph{In the case of a compartment}, an \class{AssignmentRule} sets
  ...
  Section~\ref{sec:compartments} for an explanation of how the units of the
  compartment's size are determined.)
  
  \emph{Restrictions}: No more than one \class{AssignmentRule} or
  \class{RateRule} can refer to a given compartment in an SBML model
  definition.
...
\item \emph{In the case of a parameter}, an \class{AssignmentRule} sets the
  ...
  \attrib{math}.  The overall units of the formula must match the units
  defined for the parameter identified by the value of the
  \class{AssignmentRule}'s \attrib{variable} field.  (See
  Section~\ref{sec:parameters} for an explanation of how the units of the
...
\item \emph{In the case of a species}, a \class{RateRule} sets the rate of
  ...
  depends on the species according to the following principles.  If the
  \class{Species} structure has a value for \attrib{initialConcentration},
  then the units of \quantity{concentration} must be the same as those set
  in the \class{Species} structure.  If the \class{Species} structure has a
  value for \attrib{initialAmount}, then the units of
  \quantity{concentration} are \quantity{amount}/\quantity{size}, where the
  \quantity{amount} units are those associated with the species' amount
  (see Section~\ref{sec:species}) and the \quantity{size} units are those
  defined for the size of the compartment in which the species is located
  (see Section~\ref{sec:compartments}).
...
\item \emph{In the case of a compartment}, a \class{RateRule} sets the rate
  ...
  field.  (See Section~\ref{sec:compartments} for an explanation of how the
