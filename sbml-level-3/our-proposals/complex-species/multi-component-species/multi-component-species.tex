...
All the \class{SpeciesTypeInstance} and \class{SpecificBond} structures in these graphs that
are mapped from equivalent structures in the reactant \class{SpeciesReference} structures are discarded.
The \class{SpecificBond} structures that are mapped from \class{GenericBond} structures
modified.  The \class{BindingSiteReference} that matches that contained in the \class{GenericBond}
structure is discarded.  A mapping, the \emph{generic bond assignment}, is created from the
\class{GenericBond} structures to the remaining \class{BindingSiteReference} structures.
...
only those components not made explicit by the corresponding
 reactant \class{SpeciesReference} structures.

The product graphs are created starting from the species graphs in the \class{SpeciesReference}
structures.  The \class{GenericBond} structures are replaced by \class{SpecificBond} structures
where one \class{BindingSiteReference} is taken from the product \class{GenericBond} and the other
\class{BindingSiteReference} is mapped from the reactant \class{GenericBond} with the same \attrib{id}
