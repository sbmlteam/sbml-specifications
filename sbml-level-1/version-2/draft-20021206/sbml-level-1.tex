% not all places where I added ``optiona'' are right in the text
% update discussion of tracking the xml schema standard
% update the discussion of Level 2 future
% update schema at the end
...
\date{\vspace*{-1ex}SBML Level 1, \changed{Version 2 (Final)}\\[3pt]
  \changed{5 May 2003}\vspace*{-10pt}}
...
\item The values of attributes on \class{speciesReference} are required to
  be positive numbers; also, the intended use of \class{speciesReference}
  has been clarified.  (Section~\ref{subsec:speciesreference}.)
  
\item The example in Section~\ref{subsection:ruleseg} of the Level 1
  Version 1 specification did not conform to the restrictions on rules and
  has been replaced by a different example.
...
\begin{table}[tbh]
  ...
  \begin{center}
    \begin{tabular}{lllcl}
      ...
      \texttt{(}\emph{expression}\texttt{)} & expression grouping & operand & 8 & n/a\\
      \emph{f}\texttt{(}\emph{...}\texttt{)} & function call & prefix & 8 & left\\
      \texttt{-} & negation & unary & 6 & right\\
      \verb|^| & power & binary & 5 & left \\
      \texttt{*} & multiplication & binary & 4 & left\\
      \texttt{/} & division & binary & 4 & left\\
      \texttt{+} & addition & binary & 3 & left\\
      \texttt{-} & subtraction & binary & 3 & left\\
      \bottomrule
    \end{tabular}
    \vspace*{-3pt}
  \end{center}
  \caption{A table of the expression operators available in SBML.  In the
    ...
    purpose.  \changed{In addition to the tokens shown here, the space, tab
      and comma characters may be used as separators.  Commas are allowed only
      to separate tokens acting as arguments to function calls.}}
  \label{tab:operators}
\end{table}
...
stream or document.  The UML definition of the \class{Model} structure is
shown in Figure~\vref{fig:model}.  Only one component of type \class{Model}
...
\changed{\class{Model} serves as a container for \class{UnitDefinition},
  ...
  and \class{Reaction} components.  All of these components except
  compartments are optional (that is, the lists in each of the respective
  fields are permitted to have zero length).}  An instance of a
...
headings of the form \class{listOf}\rule{0.5in}{0.5pt}\class{s}, where the
...
\changed{The \class{Unit} structure has one
  ...
  unit.}
Its default value is ``\attribvalue{1}'' (one).  In the example just
mentioned, $second^{\,-2}$ is obtained by using \texttt{kind="second"} and
\texttt{exponent="-2"}.
\changed{Finally, a \class{Unit} structure also has an
optional \attrib{scale} field; its value must be an integer used to set the
scale of the unit.}
For example, a unit that has a \attrib{kind} value of
...
\begin{blockChanged}
There is one required field for a \class{Compartment} data object,
...
the value set by the built-in ``\attrib{volume}''.
\end{blockChanged}
...
\begin{blockChanged}
The \class{Species} structure has two required fields: \attrib{name}
and \attrib{initialAmount}.  The attribute \attrib{name} is required to
give each species a unique name in a model.  The attribute
\attrib{initialAmount}, of type \class{double}, is used to define the
...
may be set explicitly using the optional field \attrib{units}.  The
value of \attrib{units} must be one of the following: a predefined unit
...
by a unit definition in the enclosing \class{Model}.  If absent, the
units default to the value set by the built-in ``\texttt{substance}''.

The field \attrib{compartment} is a string that names the compartment
...
a model must provide a value for \attrib{compartment}.
\end{blockChanged}
...
\begin{blockChanged}
  ...
  must accept \emph{both} spellings on input for all elements and
  attributes where the term occurs (i.e., \class{species} as well as
  \class{specie}, \attrib{speciesReference} as well as
  \attrib{specieReference}, and \class{speciesConcentrationRule} as well as
  \class{specieConcentrationRule}).  However, they should write out SBML
  Level~1 Version~2 models using only the \emph{species} spelling variant.
  Beginning with SBML Level~2, the \emph{specie} spelling will be removed
  entirely and only \emph{species} will be used.
\end{blockChanged}
...
of the three names ``\attribvalue{substance}'', ``\attribvalue{time}'', or
...
``\class{rate}'', the rule sets the rate of change of the \changed{specie}'
...
All the \changed{\class{species}} elements set the initial amount of every
...
\changed{Cliff Shaffer}, Bruce Shapiro, Tom Shimizu, Hugh Spence, J\"{o}rg
Stelling, Kouichi Takahashi, Masaru Tomita, John Wagner, \changed{Jonathan
  Webb}, \changed{Darren Wilkinson}, \changed{Marc Vass}, and
\changed{Tau-Mu Yi}.
