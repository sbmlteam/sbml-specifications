...
\markboth{\textbf{SBML Level 2 Working Draft}}{\textbf{SBML Level 2 Working Draft}}
...
symbols.  We use a MathML mechanism to refer to two built-in entities: a symbol representing time and a delay function. See Section~\ref{sec:mathmltokens}.
...
point in time, and the form of, a set of discontinuous state
...
In SBML Level 2 there are two URLs in the set of predefined SBML symbols:
\begin{itemize}
\item \texttt{http://www.sbml.org/symbols/time}, which represents the
...
\item \texttt{http://www.sbml.org/symbols/delay}, which represents the function $delay(x, d)$.  The result of this function is the value of $x$ at $d$ time units before the current time.  The units of the $d$ parameter are determined from the built-in \texttt{time} of Table~\vref{tab:builtin}.
...
Section~\ref{sec:delayeg} contains an SBML model which uses delay to represent gene expression.
...
A \class{Model} data
object may optionally have lists of \class{Species}, \class{Compartment}, \class{MathDefinition},
\class{UnitDefinition}, \class{Parameter}, \class{Reaction},
\class{Event} and \class{Rule} components (they are optional
because the lists in fields \attrib{species}, \attrib{compartment}, \attrib{mathDefinition}, \attrib{unitDefinition}, \attrib{parameter}, \attrib{reaction}, \attrib{event} and \attrib{rule} are permitted to have zero
length).
...
vary during the simulation.  A \texttt{false} value indicates that
the parameter's value can be determined by rules and events.  The
...
\class{AssignmentRule}.  \class{AlgebraicRule} represents case 1 above whereas \class{AssignmentRule} represents cases 2 and 3 above.
...
%-----------------------------------------------------------------------------
\subsection{Use of $delay$ function in a Model}
\label{sec:delayeg}
%-----------------------------------------------------------------------------
This section contains a simple model which uses the built-in $delay$ function described in Section~\ref{sec:mathmltokens}.  This model represents a gene that suppresses its own expression and consists of the single rule:
\begin{equation*}
