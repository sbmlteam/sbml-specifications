...
\item \emph{In the case of a species reference}, a \RateRule sets
  ...
  reactant or product to the value determined by the formula in\question{MH reworded}
  \token{math}.  The unit associated with the value produced by
...
\item \emph{No conversion factors defined}.  If the \Species
  ...
  does not define a value for its \token{conversionFactor} attribute,
...
value of a \Reaction \token{id} attribute.  It should be noted
...
associated with the \token{id} attribute of a \Reaction when used
in MathML formulas. \question{Added statement about unit}
