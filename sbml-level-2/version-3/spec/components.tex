...
\begin{figure}[htb]
  ...
  \vspace*{-1.1ex}
  \caption{\changed{The definition of \Model and the many helper
...
\changed{models in XML} easier.  More importantly, the fact that
...
The \token{math} \changed{element} is a container for MathML
content that defines the function.  The content of this
\changed{element} can only be a MathML \token{lambda} element
\changed{or a MathML \token{semantics} element containing a
...
the model.  (These restrictions prevent \changed{definition of
...
in the model.  \changed{This restriction also applies to the
...
and~\ref{sec:sboTerm}).  \changed{When a value is given to this
  attribute in a \FunctionDefinition instance, it must be an
}\changed{SBO identifier referring to a mathematical expression
...
As with other major \changed{class of objects} in SBML, \CompartmentType has a
...
conversion factors into kinetic \changed{rate formulas.  Such conversions
...
\changed{As an aside, we note that treating species in terms of
...
\changed{instance}, and within individual reaction definitions (as
...
using the relevant \changed{attribute} of a component in a model, then
...
\changed{The classes derived from \Rule inherit \token{math} and
...
The \SimpleSpeciesReference \changed{object class} has a \changed{required attribute},
\token{species}, of type \primtype{SId}.  As with the other
\changed{attributes, it is inherited by \SpeciesReference and
...
\changed{The} SBO terms chosen should be the most precise (narrow) one that
...
For maximum interoperability, \changed{\SpeciesReference's \token{stoichiometry} attribute}
...
namespaces for parameter identifiers, \changed{and} within a
...
reaction kinetics in the model \changed{assume} the same mathematical
...
reaction).  \changed{For a species $S_i$ involved in $m$
...
\begin{linenomath}
  ...
  \end{equation}
\end{linenomath}
\changed{with the units of $k_1$ being $\quantity{liter}^{\,2}
...
\changed{An} event only fires when its \Trigger expression makes the
...
\changed{nonnegative} number (otherwise, a nonsensical situation could arise
...
(the tie-breaking algorithm).  As a result, the \changed{outcomes} of
