...
not involving substances.  Consequently, we avoid using \val{mole}
...
the unit is \token{dimensionless}.  In other words, it is defined
as
\begin{linenomath}
  \begin{equation*}
...
As mentioned above, the SBML unit system does not provide a means
for specifying offsets directly.
...
An example may help make this more clear.  We know that one meter
...
both sides.  If readers ignored unit labels altogether or were
unable to process them, they would see
\begin{linenomath}
  \begin{equation*}
    1 = 1000
  \end{equation*}
...
definition of the unit.  That is, an explicit factor needs to be
inserted, such as
\begin{linenomath}
  \begin{equation}\label{eq:factor}
    1 \, \{m\} = \frac{1 \, \{m\}}{1000 \, \{mm\}} \cdot 1000 \, \{mm\}
  \end{equation}
\end{linenomath}
even though a unit definition for millimeters in SBML would take
the following form:
...
$1/1000$.  The reason, again, is that the numerical interpretation
of an SBML model is made without regards to the units, which are
in effect a kind of annotation attached to quantities in the
model.  While the result is that information is duplicated between
the definition of \token{millimeter} above and the explicit factor
inserted into Equation~\ref{eq:factor}, the machinery provided by
\UnitDefinition is still necessary in order to allow units to be
properly defined.  The result is still useful and powerful: it
permits software tools to check the consistency of a model,
perform unit conversions, label numbers in the outputs of
...
\item \emph{No conversion factors defined}.  If the \Species
  ...
  does not define a value for its \token{conversionFactor}
