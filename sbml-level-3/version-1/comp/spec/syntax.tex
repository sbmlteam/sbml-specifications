...
single unit of time in the parent model multiplied by the time conversion
factor should equal a single unit of time in the \Submodel.  All references
to time in the \Submodel should be modified accordingly before being used 
mathematically in conjunction with the parent \Model.  The entire list of 
such core SBML Level~3 Version~1 conversions is listed in \tab{sbml-conversions},
and includes the \token{time} and 
\token{delay} MathML Csymbols, and \Delay, \RateRule, and \KineticLaw elements.
...
In SBML Level~3 Version~1 core, this only
...
SBML Level~3 Version~1 core, this means \Compartment, \Parameter, 
...
\item \emph{Type must be maitained}.  Replacements must be defined such
  ...
  its base class, but not the reverse.  No such elements exist in Level~3
  Version~1 core, but the possibility exists for this to happen in a package definition.

  The single exception to this rule for core elements is that \Parameter
