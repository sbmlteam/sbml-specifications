...
contain a single \token{rdf:Bag} element which in turn must only
...
attributes. The BioModels.net relation elements simply define the
relationship.
...
Several relation elements with a given tag name, enclosed in the
same SBML element, represent alternative annotations for that SBML
element.  For example, two \token{bqbiol:hasPart} elements within
a \Species object represent two different sets of references to
the parts making up the entity represented by the species.  (The
...
sequence of dates recording dates of creation and subsequent
modifications.  The syntax for these elements is outlined below.
...
The \token{dc:creator} element describes the person(s) who created
...
order: \token{vCard:N}, \token{vCard:EMAIL} and \token{vCard:ORG}.
The \token{vCard:N}, \token{dcterms:created}, and
\token{dcterms:modified} elements must have the attribute
\token{rdf:parseType} set to the literal value \val{Resource}.
