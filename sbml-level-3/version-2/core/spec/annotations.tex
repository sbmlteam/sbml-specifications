...
\begin{table}[bh]
  \relverdiff{\vspace*{-4ex}}{\vspace*{-3ex}}
...
contain one or more \token{rdf:li} elements.  The \token{rdf:li}
elements must only have a \token{rdf:resource} attribute
containing a URI referring to an information resource (see
\sec{sec:uri-in-annotation}).  \relchange{The relation element may also contain nested content (indicated by \nestedContent in the diagram above) with annotations \emph{about} the contents of the \token{rdf:Bag}}.
...
\begin{blockRelChange}
Note that \token{dcterms:creator} has been added to \token{http://purl.org/dc/terms/} relatively recently, but the same term (with the same meaning) exists in the \token{http://purl.org/dc/elements/1.1/} namespace.  It is legal to continue using the old namespace (called \val{dc} in previous versions of the SBML specifications), but since all the terms defined there are now also defined in \token{http://purl.org/dc/terms/}, we recommend using the latter.\end{blockRelChange}
