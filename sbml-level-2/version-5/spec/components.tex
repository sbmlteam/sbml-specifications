...
<sbml xmlns="http://www.sbml.org/sbml/level2/version5" level="2" version="\changed{5}">
  ...
</sbml>
\end{example}
...
concentration will change even as its \quantity{amount} remains constant,
...
As described in Section~\ref{sec:compartments}, a zero-dimensional
\Compartment (i.e. one with a \token{spatialDimensions} value of
\val{0}) has no value, and its value may not change.  It follows
that no \Compartment identifier of this type may appear as the
\token{variable} of a \RateRule or an \AssignmentRule, nor may it
appear in the \class{Math} of an \AlgebraicRule.
...
occur during simulation.  \changed{SBML Level 3 defines a specific set of tie-breaking
