...
In this case, the three parameters (\val{kon}, \val{koff},
\val{kcat}) all have unique identifiers and they could also have
just as easily been declared global parameters in the model.
Local parameters frequently become more useful in larger models,
where it may become tedious to assign unique identifiers for all
...
\begin{center}
  \ce{\species{E} + \species{S} <=>[\kon][\koff] \species{ES} ->[\kcat] \species{E} + \species{P}}
\end{center}
In this new version of the model, we deliberately define species
with different units from the unit of reaction extent in the
model, then use conversion factors to reconcile the differences.
Specifically, we set the overall unit of extent on the model to be
\emph{mole} and the unit of time to be \emph{second} by assigning
appropriate values to the attributes \token{extentUnits} and
\token{timeUnits}, respectively, on the \Model object definition.
In the \Species object definitions, we then set the initial
amounts and units as follows:
\begin{linenomath}
  \begin{align*}
  ...
  \end{align*}
\end{linenomath}
Note how species \species{E}, \species{S} and \species{ES} are
given in units of \emph{millimoles}, but \species{P} is given in
\emph{grams}; all are different from the unit of extent
(\emph{mole}) declared on the \Model object.  To construct
mathematical expressions that equate the rates of changes of the
species' quantities to the reaction fluxes, we have to adjust the
reaction rate expressions.  In what follows, we illustrate one
approach to doing so, and in
...
\newcommand{\veq}  {v_{\token{veq}}}
\newcommand{\vcat} {v_{\token{vcat}}}
\newcommand{\Vcomp}{V_\text{comp}}
\newcommand{\convE} {\ensuremath{c_\species{E}}}
\newcommand{\convS} {\ensuremath{c_\species{S}}}
\newcommand{\convP} {\ensuremath{c_\species{P}}}
\newcommand{\convES}{\ensuremath{c_{\species{ES}}}}

The unit associated with the rate of each reaction in the model is
\emph{mole}/\emph{second} by virtue of the \token{extentUnits} and
\token{timeUnits} attribute values on the \Model object
definition.  The reaction rates (\ie the formulas in the
\token{math} elements of \KineticLaw objects) are the same as in
the original model,
\begin{linenomath}
  \begin{align}
    \label{eq:v1}
  ...
  \end{align}
\end{linenomath}
where $V\sub{comp}$ stands for the size of compartment \val{comp}
in the SBML model.  In Equations~\ref{eq:v1} and~\ref{eq:v2}, the
values of \species{E}, \species{S} and \species{ES} are all in
\emph{millimoles}, which results in a mismatch with the
\emph{mole}/\emph{second} unit of the reaction rates.  To
compensate, we change the values of the constants \kon, \koff, and
\kcat as follows:
\begin{linenomath}
  \begin{align*}
    k_{\text{on (new)}}  & = k_{\text{on (original)}} \cdot (\frac{1\:mole}{1000\:millimoles})^{2}
    & = 1\;\emph{litre} \;\emph{mole} \;\emph{millimole}^{\,-2}\emph{second}^{\,-1}\\
    k_{\text{off (new)}} & = k_{\text{off (original)}} \cdot \frac{1\:mole}{1000\:millimoles}
    & = 0.0002\;\emph{mole} \;\emph{millimole}^{\,-1}\emph{second}^{\,-1}\\
    k_{\text{cat (new)}} & = k_{\text{cat (original)}} \cdot \frac{1\:mole}{1000\:millimoles}
    & = 0.0001\;\emph{mole} \;\emph{millimole}^{\,-1}\emph{second}^{\,-1}\\
  \end{align*}
\end{linenomath}
These are admittedly unconventional units for mass-action kinetic
rate constants.  They are unlikely to correspond to values found
in textbooks or databases.  The logic of this approach is that in
an actual experimental situation, with the units of the species as
given in the model (presumably representing how the species are
being measured), the kinetic rate constants are likely to be
measured in terms of the units above.  However, if that is not the
case, then the approach of Section~\ref{sec:eg:conversionfactor2}
may be more appropriate.

The guidelines
described in Section~\ref{sec:about-kinetic-laws}, can now
be applied to produce the rate-of-change equations for
each species in the model.  Let \csg stand for the value of the \Model
object's \token{conversionFactor} attribute.  Thus the
rate-of-change equation for \species{S} is:
\begin{linenomath}
\begin{equation*}
  \dfrac{d n\sub{S}}{d t} = - \csg \cdot \Vcomp \cdot
   (k_{\text{on (new)}} \cdot \concent{E} \cdot \concent{S} 
   - k_{\text{off (new)}} \cdot \concent{ES})
\end{equation*}
\end{linenomath}

For species \species{P}, we need a separate value for the \Species
attribute \token{conversionFactor}, to convert between the units
of \emph{gram} and \emph{mole}.  By virtue of being defined on the
\Species object for \species{P}, this conversion factor value will
override the global value defined on the \Model object.  Letting
this conversion factor be represented by \convP, the equation for
the rate-of-change of \species{P} is:
\begin{linenomath}
\begin{equation*}
  \dfrac{d n\sub{P}}{d t} = \convP \cdot \Vcomp \cdot k_{\text{cat (new)}} \cdot \concent{ES}
\end{equation*}
\end{linenomath}
 

The following is the SBML encoding of the present model:
...
\begin{linenomath}
  \begin{align*}
    \kon  & = 1\:000\:000 \;\emph{litre} \;\emph{mole}^{\,-1} \emph{second}^{\,-1}\\
    \koff & = 0.2 \;\emph{second}^{\,-1}\\
    \kcat & = 0.1 \;\emph{second}^{\,-1}
  \end{align*}
\end{linenomath}
...
\emph{millimole}/\emph{litre}, while the unit of reaction extent
...
\begin{linenomath}
  ...
  \end{equation*}
\end{linenomath}
Applying this approach the reaction rate equations that must now 
be encoded in the \Reaction object definitions are:
\begin{linenomath}
  \begin{align*}
  ...
  \end{align*}
\end{linenomath}
where again $V\sub{comp}$ stands for the size of compartment \val{comp}
in the SBML model.  The rate-of-change equations for the each species are 
constructed using the the guidelines
described in Section~\ref{sec:about-kinetic-laws} and in this example the 
...
\begin{linenomath}
  \begin{align*}
		\dfrac{d n\sub{S}}{d t} & = - \csg \cdot \Vcomp \cdot
			...
			- \koff \cdot \concent{ES} \cdot \convES)\\
       \dfrac{d n\sub{P}}{d t} & = \convP \cdot \Vcomp \cdot \kcat \cdot \concent{ES} \cdot \convES
  \end{align*}
\end{linenomath}
where again \csg stands for the value of the \Model
object's \token{conversionFactor} attribute and \convP is the \token{conversionFactor} attribute of
\Species \species{P}.
