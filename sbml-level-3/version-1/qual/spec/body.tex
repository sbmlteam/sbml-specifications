...
\Q{Since a qualitativeSpecies cannot take part in a reaction what meaning does boundaryCondition have ??}

\Q{Does constant mean it cannot be changed by a transition ?}
...
The default result is described by a \token{resultLevel} or a \token{resultSymbol}. Both are optional, but one of them must be defined.
...
\Q{Can both be defined ?}

\Q{Again, should the symbol refer to a symbolicValue from the qualitativeSpecies}
