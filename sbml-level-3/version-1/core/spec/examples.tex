...
This example also considers the enzymatic reaction:
\begin{center}
  \ce{E + S <=>[k_{\text{on}}][k_{\text{off}}] ES ->[k_{\text{cat}}] E + P}
\end{center}
However on this occasion the units declared for the species amounts and
the model extent are inconsistent and it will be necessary to use the 
\token{conversionFactor} to correctly construct ODEs for the species. 
The initial amounts are specified as
\begin{linenomath}
  \begin{equation*}
    \begin{array}{@{}lll@{}}
...
The unit of species \val{E}, \val{S} and \val{ES} is \emph{millimoles} and 
the unit for \val{P} is \emph{grams}. Using the guidelines for constructing
rate-of-change equations for the species detailed in Section~\ref{sec:about-kinetic-laws}
the equation for species \val{S} is
\begin{linenomath}
\begin{equation*}
  \dfrac{d n\sub{S}}{d t} = - \csg \cdot comp \cdot (k\sub{on} \cdot [E] \cdot [S] - k\sub{off} \cdot [ES])
\end{equation*}
\end{linenomath}
using the global \Model \token{conversionFactor} which defines the conversion between
\emph{millimole} and \emph{mole}.
Species \val{P} has unit \emph{gram} and thus must use a \token{conversionFactor} that
converts between \emph{gram} and \emph{mole}.  In order to override the global 
\token{conversionFactor} the definition of \Species \val{P} must define a 
\token{conversionFactor} attribute.  Thus the equation for species \val{P} is
\begin{linenomath}
\begin{equation*}
  \dfrac{d n\sub{P}}{d t} = c\sub{P} \cdot comp \cdot k\sub{cat} \cdot [ES]
\end{equation*}
\end{linenomath}
