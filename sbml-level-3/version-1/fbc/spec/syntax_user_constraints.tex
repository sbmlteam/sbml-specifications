...
\begin{quote}
 \newtxt{\textbf{This section is based on discussions that took place on the \FBC PWG mailing list and HARMONY 2018 meeting in Oxford. These extensions and modifications are proposed as \FBC version 3.}}\\ Note that where text (or UML) defined in previous FBC specifications has been changed it has been colored \newtxt{red}.
\end{quote}
...
\subsection{The extended \class{Model} class (updates specification section \ref{model-class})}
...
\subsection{The extended \class{Species} class (updates specification section \ref{species-class})}
...
\newtxt{The optional attribute charge contains a signed double referring to the Species object’s charge (in terms of electrons, not the SI unit coulombs). Note unlike FBC versions one and two a \Species may, for the purposes of charge, be interpreted as a pseudoisomer or aggregate molecule and may assume a non-integer value. However, non-integer charges should be limited to these cases only.}
...
The \FBC \KeyValuePair class is derived from \SBase and inherits the attribute \token{metaid},  \token{sboTerm} as well as the subcomponents needed for \Notes. It's sole purpose is to define a key--value pair with an extended key definition.

The \KeyValuePair defines a single mandatory attribute the \token{key} as well as two optional attributes: \token{value} and \token{uri}.
