...
This example illustrates the use of the \Event \Trigger object
syntax, particularly the \token{persistent} attribute, and the
\Event \Delay object.  The event has a trigger expression that
tests the value of species \val{a} in the model, and if the value
comes within the range $0.999 \leq a \leq 1.001$, the event
triggers and reassigns the value of \val{a} after a delay of 3
seconds.
