...
\begin{figure}[htb]
  \centering
  \vspace*{-2ex}
  \includegraphics[scale=0.77]{figs/model-uml}
  \caption{The definition of \Model.}
  \label{fig:model}
\end{figure}
...
\begin{figure}[htb]
  ...
  \end{tikzpicture}
  \caption{The definition of class \FunctionDefinition.  The
    contents of the \class{Lambda} class is a single \mathml
...
\begin{figure}[htb]
  ...
  \end{tikzpicture}
  \caption{The definition of classes \UnitDefinition and \Unit.  A
    sequence of zero or more instances of \UnitDefinition objects
    can be located in an instance of \ListOfUnitDefinitions in
    \Model, as shown in Figure~\protect\ref{fig:model}.  Note how
    \class{ListOfUnits} has no attributes (beyond those it
    inherits from class \SBase); it merely acts as a container for
    one or more instances of \Unit objects.}
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \draw[open triangle 60-] (a) -- (b);
  \end{tikzpicture}
  \caption{The definition of class \CompartmentType.  A sequence
    of zero or more instances of \CompartmentType objects can be
    located in an instance of \ListOfCompartmentTypes in \Model,
    as shown in Figure~\protect\ref{fig:model}.}
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \draw[open triangle 60-] (a) -- (b);
  \end{tikzpicture}
  \caption{The definition of class \SpeciesType.  A sequence of
    zero or more instances of \SpeciesType objects can be located
    in an instance of \ListOfSpeciesType in \Model, as shown in
    Figure~\protect\ref{fig:model}.}
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \draw[open triangle 60-] (a) -- (b);
  \end{tikzpicture}
  \caption{The definition of class \Compartment.  A sequence of
    zero or more instances of \Compartment objects can be located
    in an instance of \ListOfCompartments in \Model, as shown in
    Figure~\protect\ref{fig:model}.}
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \draw[open triangle 60-] (a) -- (b);
  \end{tikzpicture}
  \caption{The definition of class \Species.  A sequence of
    zero or more instances of \Species objects can be located
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \draw[open triangle 60-] (a) -- (b);
  \end{tikzpicture}
  \caption{The definition of class \Parameter.  A sequence of
    zero or more instances of \Parameter objects can be located
    in an instance of \ListOfParameters in \Model, as shown in
    Figure~\protect\ref{fig:model}.}
  \label{fig:parameter}
\end{figure}
...
\begin{figure}[htb]
  \centering
  \small
  \begin{tikzpicture}[level distance=1.2in]
    \node[left=1.15in,above=0.35in] (a) { \emptyClassbox{\textsl{SBase}} };
    \node[left=0.75in] (b) { \emptyClassbox{Constraint} }
      [diamond-,edge from parent fork right,grow=right,sibling distance=0.75in,shorten >=-6pt]
      child {node[right=0.15in] (d) {
          \begin{classbox}{Message}
            xmlns: string \{ "http://www.w3.org/1999/xhtml" \} \\
            \{ \emph{Any content permitted in the XHTML XML namespace} \} \\
          \end{classbox}
        }}
      child {node[right=0.15in] (c) {
          \begin{classbox}{Math}
            xmlns: string \{ "http://www.w3.org/1998/Math/MathML" \} \\
  ...
  \caption{The definition of class \Constraint.  The contents of
    ...
    of \class{Message}; this element is simply a wrapper (in the
    XML form, within \token{<message> \ldots{} </message> tags})
    for XHTML content.  The same guidelines for XHTML content as
    explained in Section~\ref{sec:notes} for notes on \SBase also
    apply to the XHTML within messages in a \Constraint. A sequence of
