...
\texttt{$-5+100\%$} & 5 points left of the right edge of the current bounding box.\\ \hline
\texttt{$50\%$} & 50\% from the left edge of the current bounding box. \\ \hline
\texttt{$2$} & 2 points from the left edge of the bounding box. \\ 
\hline
\end{tabular}
\end{center}
\smallskip
...
defines the RGBA value of the color. The string is formatted as \val{\#} followed by the six or eight digits \val{0-9 a-f A-F}. If only the first six digits for the RGB value 
...
The \RenderPackage extends the \class{GraphicalObject} object from the \LayoutPackage 
with the addition of the \\ \token{objectRole} attributes.
...
In addition, the \ColorDefinition object has the required attributes \token{id} 
and \token{value} and the optional attribute \token{name}.
...
A \ColorDefinition has an optional attribute \token{name} of type
\primtype{string}. This \token{name} attribute can be used to give the object a 
more user friendly identifier.
...
In addition, the \GradientBase object has a required \token{id} attribute and two optional attributes: 
\token{name} and \token{spreadMethod}.
...
A \GradientBase has an optional attribute \token{name} of type
\primtype{string}. This \token{name} attribute can be used to give the object a 
more user friendly identifier.
...
and \token{r} do not actually specify a circle, but an ellipse.
...
Currently there has been no requirement for 3D transformation and this class is only 
used as a base class for \TransformationTwoD. We leave the complete specification 
of this class for a future version of this document.


The \Transformation object derives from the \SBase class and thus
inherits any attributes and elements that are present on this class.
In addition, the \Transformation object has two optional attributes: \token{name} 
and \token{transform}.

\paragraph{The \fixttspace\token{name} attribute}

A \Transformation has an optional attribute \token{name} of type
\primtype{string}. This \token{name} attribute can be used to give the object a 
user friendly identifier.

\paragraph{The \fixttspace\token{transform} attribute}

A \Transformation has an optional attribute \token{transform} of type \doubleArray. This specifies an affine transformation matrix in three dimensions in which case the array must consist of exactly 12 values. However, we currently limit the \RenderPackage to the discussion of 2D transformations.
...
For example,  \texttt{"5,10"} would mean to draw 5 points, make a 10 point gap, draw 5 points etc. If the pattern is to start with a gap, the first number has to be 0.
...
class.
A \RenderCurve contains at most one \ListOfElements and at most one \class{ListOfCurveSegments} from the \LayoutPackage.
...
This attribute is an \primtype{xsi:type}. For a \RenderCubicBezier object this attribute will always have the value \\
\val{RenderCubicBezier}.
...
two control points at (50\%, 90\%).  The equivalent curve defined using the \class{ListOfLineSegments} from the \LayoutPackage is also included (assuming a $100$ $\times$ $100$ square object).
...
\begin{figure}[!htp]
  \centering
  \includegraphics[width=0.95\textwidth]{images/render-group-uml}\\
...
\token{y}, \token{height}, and \token{width} as well as the optional attributes 
...
\paragraph{The \fixttspace\token{width} and \fixttspace\token{height} attributes}
...
means that text actually has to be rendered with the top towards lower y-values.
...
The actual text to be rendered will be written as inline text characters of the \Text element.
Note that it is valid to have a \Text element with empty characters.
...
\VTextAnchor which specifies the vertical alignment of the text (see \apdx{apdx:text-anchor}).
...
\primtype{SId} which allows a unique identifier to be provided for this \LineEnding so that it may be referenced by other objects. The \token{startHead} and \token{endHead} attributes on a \RenderCurve expect to point to the \token{id} of a \LineEnding.
