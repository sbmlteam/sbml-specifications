...
\begin{minipage}{5in}
  ...
  for representing computational models of biochemical cellular processes
...
\changed{Each SBML Level-plus-version specification inevitably requires
minor editorial changes as the community of users discovers errors and
ambiguities.  As a practical reality, these discoveries occur over time.
Such problems are publicly announced as \emph{errata} to the
specification.  Errata result in new \emph{issues} of the specification to
clarify and disambiguate the problems.  Each issue is numbered.
The first issue of the specification is numbered 1.  The
...
in which the errata is introduced.  Errata do not change the
fundamental semantics or syntax of SBML, but rather clarify and
disambiguate the specification and correct errors.  (New syntax and
semantics are only introduced in SBML Versions and Levels.) A complete list
of the errata for SBML Level~2 Version~2 created since Issue~1 is made
...
resource referenced by the URI.  A typical use of \attrib{definitionURL} is
...
\changed{In the XML encoding of SBML, the} XML namespace URI for SBML Level~2 is
...
\class{Parameter}, \class{Rule}, \class{Constraint}, \class{Reaction} and
...
definitions within \class{Reaction} structures should \emph{not} have
their \attrib{constant} field set to ``\attribvalue{true}'';
\end{blockChanged}
...
\emph{valid} state.  The contents of the \attrib{message} field
may optionally be displayed to the user by a software tool when
the tool detects that the function given by the \attrib{math}
returns a value of ``\attribvalue{false}''.  Software tools are
not required to display the message, but it is recommended as a
matter of best practice.

\class{Constraint} structures \emph{cannot and should not} be
used to compute the dynamical behavior of a model.  They simply
provide a mechanism for expressing the validity of a model's
state during numerical analysis.
