...
Figure~\ref{fig:sbml}.  This class contains \changed{three} required
attributes, for the \changed{XML namespace (\token{xmlns}) and the} SBML \token{level} and \token{version}, and
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}[level distance=0.85in]
    \node { \emptyClassbox{\textsl{SBase}} }
      ...
      child {node (a) {
          \begin{classbox}{Sbml}
            xmlns: string \{ "http://www.sbml.org/sbml/level2/\changed{version5}" \} \\
            level: positiveInteger \{ use="required" fixed="2" \}   \\
            version: positiveInteger \{ use="required" fixed="\changed{5}" \} \\
          \end{classbox}
        }}
      child {node (b) {
          \emptyClassbox{Model}
...
an SBML Level~2 \changed{Version~5} document:
...
used within the \token{sbml} element.  The URI for SBML Level~2 \changed{Version~5}
is \uri{http://www.sbml.org/sbml/level2/\changed{version5}}.  All
...
the same model.  This restriction also applies to the
  \token{csymbol} for \quantity{time} \changed{and to the \token{csymbol} for \quantity{delay}}.  Functions must be written
...
Versions~2--\changed{\sbmlversion}.  Modelers and software authors are
...
in more detail below.  Another change in Version~3--\changed{\sbmlversion} is the
...
\begin{linenomath}
\begin{equation}
  \unew = (\tokenNC{multiplier} \cdot 10^\tokenNC{scale} \, \ubase)^\tokenNC{exponent}
...
\intertext{then,}
  \ybase \, \uone^{\changed{x_1}} \utwo^{\changed{x_2}} \ldots \un^{\changed{x_n}}
    &= \ynew \, \unew \left(
...
interoperability.  SBML Level~2 Versions 2--\changed{\sbmlversion} acknowledge this reality by reducing
...
<functionDefinition id="Fahrenheit_to_kelvin">
  ...
  SBML Level~2 Versions 2--\changed{\sbmlversion}.  It can be used for other temperature units
...
involved and metre is chosen as the \token{kind}, the
\token{exponent} must be \val{3} to get cubic metres; if area is
involved and metre is again used for \token{kind}, the
\token{exponent} must be \val{2}.  Conversely, if
...
In \sbmltwothree and beyond, a compartment type only has an
...
class \SBase (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this
attribute in a \CompartmentType instance, the value should
be an SBO identifier referring to a material entity (\ie
...
class \SBase (see Sections~\ref{sec:sboterm-type}
and~\ref{sec:sboTerm}).  When a value is given to this
attribute in a \SpeciesType instance, the value should
be an SBO identifier referring to a material entity (\ie
terms derived from \sbomaterialentity).  The relationship is
of the form ``the species type \emph{is a} X'', where X is the
SBO term.  The term chosen should be the most precise (narrow) one
that captures the role of the species type in the model.
...
\item The \token{math} element of an \AssignmentRule or
  \InitialAssignment referring to this compartment \changed{should} have
...
\item In \RateRule objects that set the rate of change of the
  ...
  the rule's \token{math} element \changed{should} be identical to
...
answer is that such a compartment's identifier \changed{must} not appear
...
\begin{table}[htb]
  ...
  \end{edtable}
  \vspace*{-0.5ex}
  \caption{How to interpret the value of the \Species \token{hasOnlySubstanceUnits}
  attribute.}
...
whether and how the \changed{amount} of that species can vary during a
...
\emph{not} \changed{affected} by the set of reactions even when that
...
The \token{constant} attribute indicates whether the species' \changed{amount}
...
\val{false}, indicating that the species' \changed{amount} can be changed,
...
species' \token{constant} attribute is \val{true}, then its \changed{amount}
...
definitions within \Reaction objects \changed{must} \emph{not} have
...
\paragraph{The model must not change the value of a zero-dimensional \Compartment}

As described in Section~\ref{sec:size}, a zero-dimensional \Compartment object
(i.e. one with a \token{spatialDimensions} attribute value of \val{0}) has no
size.  A \Compartment identifier of this type may not appear as
the \token{variable} of a \RateRule or an \AssignmentRule, nor may it appear
in the \class{Math} of an \AlgebraicRule.
...
\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.785]{figs/reaction-uml-v2}
...
is set to \val{true}.  \sbmltwothree and \thisV therefore stipulate that if a
...
in a \SpeciesReference object.  \changed{The \token{stoichiometry}
attribute is of type double}.  The \token{stoichiometryMath} element is implemented as
...
units should match the \quantity{substance} units of the species.
...
expression to be defined on a per-reaction basis.  These
...
identifier as \changed{any other global model entity (\Compartment, \CompartmentType, \Event, \FunctionDefinition, \Model, \ModifierSpeciesReference, \Parameter, \Reaction, \Species, \SpeciesReference, or \SpeciesType)
will override the global value, or will provide a value for an identifier that otherwise had no mathematical meaning}.  Modelers and software
...
What happens if the definition of the SBML predefined unit
...
What happens if instead the definition of the SBML predefined unit
...
And finally, what happens if the definition of the SBML predefined
...
\paragraph{The optional \token{useValuesFromTriggerTime} attribute}
...
the \token{useValuesFromTriggerTime} attribute on \Event allows a
model to indicate the time at which the event's assignments are
intended to be evaluated.  The default value is \val{true}, which
...
attribute value is \val{true} (the default), the expression must
...
occur during simulation.  \changed{All triggered simultaneous events must fire, and the order in which they are executed is not defined.  They may be executed randomly, alphabetically, arbitrarily, or in any other order determined by the simulation software.}
