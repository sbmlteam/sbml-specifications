...
\relchange{Unless otherwise specified, all SBML documents must follow the XML specification:  anything allowed in XML is allowed here, and anything disallowed in XML is disallowed here.
As such, all well-formed SBML documents must begin with an \emph{XML
...
and requires a document encoding of UTF-8.}
...
ellipses \relchange{(\val{...})} are used to indicate content elided from this example):
...
\item SBML Level~3 requires that every package defines the
  ...
  \token{required} attribute indicates whether \relchange{constructs in that package may change the mathematical interpretation of constructs defined in SBML Level~3 Core.  A \token{required} value of \val{true} indicates that the package may do so.  The value of the attribute is set by the Level~3 package specification, and does \emph{not} depend on the actual presence or absence of particular package constructs in the SBML document: in other words, if the package defines \emph{any} construct that can change the model's meaning, the value of the \token{required} attribute \emph{must} always be set \val{true} in an SBML document that uses the package.  For example, if the package declares new object classes with mathematical meanings and with identifiers that may be used in core \Math elements of any kind, that package's \token{required} attribute must always be set to \val{true} for all SBML documents declaring the use of that package's namespace, regardless of whether any objects in those particular SBML document are actually affected.} 
  The following is a \relchange{complete} example:
  \begin{example}
<sbml xmlns="http://www.sbml.org/sbml/level3/\thisvlc/core" level="3" version="\sbmlversionnum"
      xmlns:multi="http://www.sbml.org/sbml/level3/\thisvlc/multi/\thisvlc"
      xmlns:layout="http://www.sbml.org/sbml/level3/\thisvlc/layout/\thisvlc"
...
\relchange{For example:  because the SBML Level~3 Hierarchical Model Composition (\val{comp}) package can change the mathematical interpretation of a model, the \token{comp:required} attribute must always be set to \val{true}, even if the model is completely empty.  The Graphical Layout (\val{layout})) package, by contrast, cannot ever change the mathematical interpretation of any model (empty or not), and thus its \token{required} attribute must be set to \val{false}.}

  If a package is declared optional \relchange{(its \token{required} flag is set to \val{false})}, it means the
...
Some ways in which packages which are marked \token{required} = \val{true} may actually change (instead of merely augment) the time-course dynamics of a package include:
\begin{itemize}
  ...
  \itemvertwo{An element may be defined within a package to have mathematical meaning, and its \token{SId} may be used to determine one or more mathematical functions in core.}
\end{itemize}
However, no package may ever give mathematical meaning to the \token{SId} of an element that was defined as not having mathematical meaning: no \token{SId} of this type may ever appear in any mathematical context.
\end{blockRelChange}
...
6 within the model, the list must not be empty; that is, it must have length one or more}{and, if present, may
...
{\FunctionDefinition inherits the \token{id} attribute from \SBase, but defines \token{id} as being required rather than optional.  The \token{id} attribute otherwise obeys the behavior described in \sec{sec:idnameattribs}.  A function's identifier can be used in \token{math} elements as the first target of an \token{apply} element, but has no value associated with it, and may not be the target of an \InitialAssignment, \EventAssignment, or \Rule object in the model.}
...
definition.  \vertwo{If the \FunctionDefinition has no \token{math} child, no restriction is placed, in SBML Level~3 Core, on the number of arguments the function may take when used in MathML elsewhere in a model.  However, without further information (such as might be supplied by an SBML Level~3 package), the call will be mathematically meaningless.}
...
\relverdiff{\paragraph{The \token{id} and \token{name} attributes}}{\paragraph{The \token{id} attribute}}
...
that case is ignored.  \sec{sec:before-t0} provides
...
\relchange{Finally, it is worth clarifying that while the \token{constant} and \token{boundaryCondition} attributes restrict whether and how the species amount changes, the same is not true of a species' concentration.  In SBML, the concentration of a species is a quantity that depends on the size of the compartment in which it is located.  A compartment's size may change, and therefore, so can the concentration of a species even if the \emph{amount} of the species remains unchanged.  A species' concentration may therefore vary even if the \Species object's \token{constant} attribute is set to \val{true} in a model.}
...
\begin{figure}[htb]
  \centering
  \relverdiff{\includegraphics[scale=0.8]{figs/rule-uml-l3v1r2}}
...
to apply also at $t \leq 0$.  \sec{sec:before-t0}
...
\relchange{Because these rules indicate what symbols may \emph{not} be determined by an \AlgebraicRule, they can thus also be used to discover what symbol \emph{is} being determined by an \AlgebraicRule.  If three symbols appear in the \token{math} element of an \AlgebraicRule, the first of which is flagged \token{constant}=\val{true}, and the second of which is a \Reaction identifier, one may conclude that the \AlgebraicRule must be used to determine the value of the third symbol.  This is, in fact, a principle use (outside of validation) of the \token{constant} attribute: its use in allowing software to properly identify the dependent variable in an \AlgebraicRule.}
...
\vertwo{Note that a \LocalParameter must not take the \token{id} of a \Species referenced by any of its parent \Reaction's \SpeciesReference elements: the principle by which a \LocalParameter shadows any global \token{SId} would mean that the \SpeciesReference would now be referring to a \LocalParameter, which is not, of course, a \Species.}

\vertwo{The effective scope of a \LocalParameter \token{id} in \sbmlthree Core is therefore restricted to the \token{math} of the \KineticLaw, since no other construct within a \Reaction references an \token{SId}.  However, \sbmlthree packages may use this fact to introduce new \token{SId} references within a \Reaction that could refer to a \LocalParameter.  In some cases, this could mean that a \KineticLaw is present in the \Reaction with no child \token{math}, but with a \ListOfLocalParameters to provide a new package constuct with locally-scoped parameters to reference.}
...
In \sec{sec:bp:reactions}, we present some recommendations for
...
\begin{figure}[htb]
  ...
  \small
  \vspace*{-1ex}
  \relverdiff{\includegraphics[scale=0.77]{figs/event-uml-l3v1r2}}
...
\token{variable} attribute value.  More precisely\relverdiff{:}{, for classes of objects defined by SBML Level~3 Core:}
\begin{itemize}
