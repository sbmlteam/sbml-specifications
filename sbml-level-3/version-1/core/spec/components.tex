...
The possibility of defining an optional \Delay within \Event and the possibility of multiple events being triggered simultaneously means
there are two times to consider when interpreting an event: the
moment at which the event \emph{triggered}, and the moment at which
its assignments are \emph{executed}.  (If a \Delay is present, these moments are separated by simulation time.  If multiple events are triggered simultaneously, these moments are separated by the sequential execution of the event assignments.)  Similarly, it is also
possible to distinguish between the moment at which the mathematical
expression of an \EventAssignment object is evaluated, and the
moment at which this value is assigned to the entity referenced by
...
the moment the event is triggered, or at the moment the event
...
model to indicate the moment at which the event's assignments
...
event's assignments are to be computed at the moment the event is
executed.  The attribute has no default value.
...
executing one event, all remaining simultaneous events whose
trigger conditions did not transition from \val{true} to
\val{false} must have their \Priority expression reevaluated, and
the highest-priority remaining event selected for the next
